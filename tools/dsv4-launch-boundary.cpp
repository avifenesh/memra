...
struct Rank {
    cudaStream_t stream{};
    cudaEvent_t base{}, before{}, start{}, reply{};
...
void init_rank(int rank, cudaStream_t stream) {
    ...
    r.stream=stream;
    for(auto event:{&r.base,&r.before,&r.start,&r.reply}) ck(cudaEventCreate(event),"create event");
}
...
extern "C" int __wrap_memra_dsv4_replay_capture_end(void* graph,void** executable,void* raw_stream) {
    ...
    entries.push_back(e);
    std::printf("BOUNDARY_CLONE {\"rank\":%d,\"original_nodes\":%zu,\"kernels\":%u,\"ar\":%u,\"added_event_nodes\":1,\"original_edges_preserved\":true}\n",rank,count,kernels,ars);
    return rc;
}
...
extern "C" void memra_launch_boundary_finish(unsigned position) {
    ...
    float start[2],before[2],queued[2];
    ...
    std::printf("BOUNDARY_ROW {\"position\":%u,\"host_before_ns\":[%llu,%llu],\"host_after_ns\":[%llu,%llu],\"host_launch_ms\":[%.9f,%.9f],\"calibration_roundtrip_ms\":%.9f,\"start_from_local_base_ms\":[%.9f,%.9f],\"prelaunch_from_local_base_ms\":[%.9f,%.9f],\"prelaunch_to_graph_start_ms\":[%.9f,%.9f],\"rank1_minus_rank0_start_lower_ms\":%.9f,\"rank1_minus_rank0_start_upper_ms\":%.9f,\"rank1_minus_rank0_start_midpoint_ms\":%.9f,\"event_resolution_us_approx\":0.5,\"first_ar_wait\":null}\n",position,
        ...
        double(ranks[0].host_after-ranks[0].host_before)/1e6,double(ranks[1].host_after-ranks[1].host_before)/1e6,roundtrip,start[0],start[1],before[0],before[1],queued[0],queued[1],lower,lower+roundtrip,lower+roundtrip/2);
...
extern "C" void memra_launch_boundary_cleanup() {
    ...
    for(int rank=0;rank<2;++rank) {
        ...
        for(auto event:{ranks[rank].base,ranks[rank].before,ranks[rank].start,ranks[rank].reply}) ck(cudaEventDestroy(event),"cleanup event");
    }
