...
#   MEMRA_KV_HOST_MB=$HOST_MB (1024)         the pool. At most three ~161 MB entries are ever
...
#   MEMRA_PREFIX_CACHE_MB=$CACHE_MB (256)    device budget: ONE entry resident, so every new
#                                            seed evicts the previous one into the host tier.
#   r1 beta  P_D        seeds E_D on device
#   r2 acme  P_A        device evicts E_D -> demote (beta row: E_D); seeds E_A
#   r3 acme  P_B        device evicts E_A -> demote (acme row: E_A); seeds E_B
#   r4 acme  P_C        device evicts E_B -> demote (acme row: E_A E_B, at its share); seeds E_C
#   r5 acme  P_E        device evicts E_C -> acme's THIRD demotion hits the cap with the pool
#                       two-thirds empty:
...
#   r6 beta  P_D + EXT  beta's E_D promotes in BOTH arms (cached_tokens == r1 prompt_tokens):
#                       acme's reclaim never touched beta's row. The promote evicts E_E from the
#                       device: base evaporates it, fix reclaims E_B and demotes it.
#   r7 acme  P_C + EXT  fix: E_C promotes (cached_tokens == r4 prompt_tokens), the reclaimed
...
# env:   MEMRA_TENANTGATE_CACHE_MB (256), MEMRA_TENANTGATE_HOST_MB (1024), MEMRA_TENANTGATE_PCT (38),
#        MEMRA_GATE_PORT (18129), MEMRA_GPU_LOCK (/tmp/memra-5090.lock or /tmp/memra-gpu.lock).
# Exit 0 = every assertion for the named arm held. Evidence: <evidence_dir>/r{1..7}.json,
...
SERVER_PID=""
CACHE_MB=${MEMRA_TENANTGATE_CACHE_MB:-256}
...
boot() { # $1 log
    ...
    env CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} \
        ...
        "MEMRA_KV_HOST_TENANT_PCT=$PCT" "$BIN" >"$1" 2>&1 &
    SERVER_PID=$!
...
req "$KEY_BETA" "$P_D$EXT" "$EV/r6.json"
req "$KEY_ACME" "$P_C$EXT" "$EV/r7.json"
...
chk "beta's entry promotes at r6 in this arm: acme's cap did not touch beta's row" python3 -c "
import json, sys
r1 = json.load(open('$EV/r1.json'))['usage']['prompt_tokens']
r6 = json.load(open('$EV/r6.json'))['usage']['prompt_tokens_details']['cached_tokens']
sys.exit(0 if r6 == r1 and r1 > 0 else 1)"
chk "the r6 promote line names beta" present "[prefix-host] promote: .*t:beta" "$LOG"
...
if [ "$ARM" = base ]; then
    ...
    chk "base: r7 is cold (the evaporated entry is gone)" jqpy "$EV/r7.json" "r['usage']['prompt_tokens_details']['cached_tokens'] == 0"
...
sys.exit(0 if ok else 1)"
    chk "fix: r7 promotes the reclaimed admission (cached_tokens == r4 prompt_tokens)" python3 -c "
import json, sys
r4 = json.load(open('$EV/r4.json'))['usage']['prompt_tokens']
r7 = json.load(open('$EV/r7.json'))['usage']['prompt_tokens_details']['cached_tokens']
sys.exit(0 if r7 == r4 and r4 > 0 else 1)"
