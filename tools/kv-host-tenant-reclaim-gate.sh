...
#   r7 beta  P_D + EXT  beta's E_D promotes in BOTH arms (cached_tokens == capture_len(r1 prompt_tokens),
#                       the grid-aligned length every entry publishes at since memra#602 day 19, the
#                       spec session's `insert (spec-boundary)` included: 64 for an 83-token prompt):
...
#   r8 acme  P_F + EXT  fix: E_F promotes (cached_tokens == capture_len(r6 prompt_tokens)), the reclaimed
...
# THE CAPTURE LAW (memra#602; day 19 moved the spec session's capture onto the grid, so it holds for
# every entry this gate sees): the server boots with spec at its default, every leader is a spec
# session, and its `insert (spec-boundary)` has capture_len(prompt_tokens) tokens, the largest
# multiple of 32 not exceeding the prompt end that leaves at least PRIME_MIN_T=16 prompt tokens
# behind it, never under the 64-token entry floor (64 for the 83-token P_A: `insert (spec-boundary):
# 64 tokens`, `hit: 64 of 96`, measured on the target card 2026-09-21). A promote therefore restores
# that many, and the rest cold-primes from the grid. Same arithmetic as the twin gate's helper.
...
chk "beta's entry promotes at r7 in this arm: acme's cap did not touch beta's row (cached_tokens == capture_len(r1))" python3 -c "
import json, sys
...
sys.exit(0 if ok else 1)"
    chk "fix: r8 promotes the reclaimed admission (cached_tokens == capture_len(r6))" python3 -c "
import json, sys
