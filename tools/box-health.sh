#!/usr/bin/env bash
# tools/box-health.sh — the box-window boot receipt: is this machine fit to be MEASURED on?
#
# Born as `HEALTH.sh` in lane/glm5-tp-transport (2026-09-01); promoted to a first-class fleet
# tool in lane/glm5-extract2 because nothing in it is glm5, TP, or even memra — every check is
# a host/driver/fabric fact that silently degrades ANY workload. Documented in docs/TESTING.md
# ("Box health before measurement").
#
# CONTRACT
#   usage:  bash tools/box-health.sh [OUTDIR]
#   OUTDIR: default ./health-$(hostname)-<utc-stamp>; every check's raw output is banked there
#           alongside health.log, so the directory IS the receipt.
#   exit 0: every hard check passed — the box is fit to measure on. Bank OUTDIR beside the
#           window's receipts.
#   exit 1: at least one HARD-FAIL. Do NOT open the window on this box; fix or re-place first.
#           A window that measures on a degraded box banks the degradation as a result.
#   WARNs never change the exit code: they are facts to RECORD beside every number from this
#           box (IOMMU translated mode, ACS ReqRedir on a VM) rather than things to fix.
#
# DEPENDENCIES, and what happens without them: `nvidia-smi` (required), `nvcc` (required for
#   section 8, the only check that catches the driver's SysMem-staging default — its absence is
#   a HARD-FAIL, never a skip), `lspci`/`lscpu`/`dmesg` (soft — missing ones WARN).
#   `tools/peer-read-probe.cu` must sit beside this script; section 8 builds it with
#   `nvcc -O2 -arch=${MEMRA_CUDA_ARCH:-sm_120}`.
#
# NO INTERACTIVE STATE: this script reads only files, /proc and nvidia-smi. It needs no
#   credential, no keyring and no shell export, so a scheduled runner can call it (the
#   scheduled-jobs-read-files law).
#
...
log "=== box-health.sh $(date -u +%Y-%m-%dT%H:%M:%SZ) host=$(hostname) out=$OUT ==="
...
if [ ! -r "$PROBE_SRC" ]; then
  fail "peer-read-probe.cu missing beside tools/box-health.sh — the kernel peer-read check cannot run"
  ...
  if nvcc -O2 -arch="$ARCH" -o "$PROBE_BIN" "$PROBE_SRC" >"$OUT/peer-read-probe.build.log" 2>&1; then
    ...
    # No `set +e`/`set -e` dance here: this script never enables errexit (it runs
    # `set -u -o pipefail`, line 43), so restoring `set -e` afterwards would TURN IT ON for
    # sections 9-10 and let any non-zero command abort the run BEFORE the summary block that
    # computes the exit code — a hard-failing box would then exit non-zero with no HARD-FAIL
    # recorded, no summary line, and no "DO NOT OPEN THE WINDOW". The probe's exit code is
    # captured from PIPESTATUS and dispatched below; that is the whole error handling it needs.
...
# ---------------------------------------------------------------------------------------
log ""
log "=== box-health.sh summary: hard_fails=$HARD_FAILS warns=$SOFT_WARNS receipts=$OUT ==="
