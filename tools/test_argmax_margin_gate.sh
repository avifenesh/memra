#!/usr/bin/env bash
# CPU-only shell wrapper tests adapted from b9749ae4's test_argmax_margin_gate.py.
# Runs a copied gate with a synthetic probe in a temporary tree; never a GPU binary.
set -euo pipefail
exec python3 -B - "$0" <<'PY'
#!/usr/bin/env python3
...
class GateTest(unittest.TestCase):
    def setUp(self):
        ...
        shutil.copyfile(pathlib.Path(sys.argv[1]).with_name(self.gate.name), self.gate)
    ...
    def test_default_flip_budget_is_not_loosened_for_hf(self):
        rows = "10 1 0.01 2 0.01 0.02 NO\n11 2 0.01 3 0.01 0.02 NO"
        for model in (self.gguf, self.hf):
            with self.subTest(model=model.name):
                r = self.run_gate(rows, model)
                self.assertNotEqual(r.returncode, 0)
                self.assertIn("TOO-MANY-FLIPS 2 > 1", r.stdout)
