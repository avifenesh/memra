...
#
# WHAT. One command, the ONE entry point (tools/ci-portable.sh only forwards here), the same
# text in .github/workflows/ci.yml (job portable-suites), tools/local-ci.sh (the CPU chain)
# and tools/test_portable_suites.sh (the teeth: a planted
...
#   * --offline --locked: the resolution is the committed Cargo.lock and a lockfile that would
#     need to change is a refusal, not an offline re-resolution (--locked folded from #590's
#     tools/ci-portable.sh on day 14); the caller warms the registry (`cargo fetch --locked`
#     in ci.yml; the rig's cache locally)
...
echo "portable-suites: cargo test -p memra-tier -p memra-kv -p memra-cli --offline --locked --no-fail-fast"
...
MEMRA_PORTABLE_SKIP_BUDGET=0 python3 tools/skip-census.py run \
    ...
    -- cargo test -p memra-tier -p memra-kv -p memra-cli --offline --locked --no-fail-fast
