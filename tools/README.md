...
- `box-health.sh`: the box-window boot receipt — *is this machine fit to be MEASURED on?* Run it
  ...
  ReqRedir, P-state normalization) plus section 8's kernel peer-read probe. `exit 0` = fit to
  measure, `exit 1` = do not open the window; OUTDIR is the receipt. Promoted from
