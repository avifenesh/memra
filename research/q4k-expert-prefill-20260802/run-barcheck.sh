#!/bin/bash
# q4k-expert-prefill: Ornith-35B vs llama same-session bar check, post AUTO-KQUANT flip.
# Interleaved per rep (rep loop outside): memra board (pp2048 pp-only + gen512), llama-bench
# board (-p 512,2048 -n 128), then per-class memra spec-K=2 (run-spec, NGEN=256) and llama
# rates (-p <class-len> -n 256). e2e convention = the 9B-cell rule (decode-rate term,
# generous-to-llama): memra = prime_wall + 256/spec_rate; llama = p_len/pp + 256/tg.
# usage: run-barcheck.sh [nreps]
set -u
N=${1:-3}
W=/home/avifenesh/projects/wt-q4k-expert-prefill
R=$W/research/q4k-expert-prefill-20260802
PDIR=$W/research/e2e/prompts
OUT=$R/barcheck.jsonl
...
row() { # arm metric value rep
  printf '{"ts":"%s","git":"%s","cell":"o35b-barcheck","arm":"%s","metric":"%s","value":%s,"rep":%s,"profile":"%s","temp_c":%s}\n' \
...
memra_board() { # rep
  local rep=$1 log="$R/bar-memra-board-rep$1.log"
  wait_idle
  ...
  local log2="$R/bar-memra-gen512-rep$1.log"
  wait_idle
...
llama_board() { # rep
  local rep=$1 log="$R/bar-llama-board-rep$1.log"
  wait_idle
...
if m:
    for r in json.loads(m.group(0)):
        if r['n_prompt'] > 0 and r['n_gen'] == 0:
            ...
            continue
        print(json.dumps({"cell":"o35b-barcheck","arm":"llama","metric":met,"value":r['avg_ts'],"rep":int(rep),"temp_c":int(temp)}))
EOF
  tail -2 "$OUT" | sed 's/^/  [llama] /'
}

memra_class() { # class rep
  local cls=$1 rep=$2 log="$R/bar-memra-$1-rep$2.log"
  wait_idle
  ...
  # [generate] N tok in Xs = Y tok/s ... ; [generate_spec K=2] 256 tok in Xs = Y tok/s (...; this run's prime Zs)
...
llama_class() { # rep  (one llama-bench call, all three class lengths)
  local rep=$1 log="$R/bar-llama-class-rep$1.log"
  wait_idle
  flock /tmp/gpu5090.lock timeout 1800 "$LLAMA" -m "$O35B" -ngl 999 -fa 1 -ctk q8_0 -ctv q5_1 \
...
if m:
    for r in json.loads(m.group(0)):
        if r['n_prompt'] > 0 and r['n_gen'] == 0:
            ...
            continue
        print(json.dumps({"cell":"o35b-barcheck","arm":"llama-class","metric":met,"value":r['avg_ts'],"rep":int(rep),"temp_c":int(temp)}))
EOF
  tail -4 "$OUT" | sed 's/^/  [llama-class] /'
}

echo "=== O35B BAR CHECK x$N $TS git=$GIT_SHA profile=$PROFILE ===" | tee -a "$R/sweep-console.log"
{
  ...
  echo "BARCHECK-DONE $(date -u +%FT%TZ)"
