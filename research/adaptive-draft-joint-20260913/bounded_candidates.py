"""Separate discovery-quality and bounded-probe cost experiment."""
...
OUT = ROOT/'raw/bounded-candidates'
...
def main():
    ...
    def run(prompt, mode, repeat, phase):
        ...
        if mode == 'bounded': config['MEMRA_GEMMA_CANDIDATE_PROBE'] = str(trace)
        ...
        with (OUT/'telemetry.csv').open('w') as f:
            ...
            try:
                ...
                for repeat in range(6):
                    ...
                    for prompt in order:
                        for mode in (['off','bounded'] if (cost.index(prompt)+repeat)%2 == 0 else ['bounded','off']): run(prompt, mode, repeat, 'cost')
            finally:
                telemetry.terminate()
                telemetry.wait(timeout=10)
    (ROOT/'BOUNDED_DONE').write_text('192 runs completed')
