"""Candidate-source ablation with fixed head, budget, depth and confidence."""
...
def main():
    ...
    prompts = sorted((OLD/'raw/row-oracle/prompts').glob('*.txt'))
    ...
    (OUT/'manifest.json').write_text(json.dumps({'source_commit': (ROOT/'SOURCE_COMMIT').read_text().strip(),
        'binary_sha256': sha(BIN/'gemma-gate'), 'prompts': {p.name: sha(p) for p in prompts}}, indent=2))
    known = {}
    def run(prompt, mode, repeat, phase):
        ...
        config['MEMRA_GEMMA_CANDIDATE_SUFFIX'] = '1' if mode == 'suffix' else '0'
        if phase == 'collection': config['MEMRA_GEMMA_DRAFT_TRACE'] = str(OUT/(name+'-draft.jsonl'))
        ...
        if not (OUT/'admission.json').exists():
            red_env = dict(env)
            red_env.pop('MEMRA_GEMMA_CANDIDATE_PROBE')
            red_env['MEMRA_GEMMA_CANDIDATE_SUFFIX'] = '1'
            with (OUT/'admission.log').open('xb') as f:
                red = subprocess.run(argv, env=red_env, stdout=f, stderr=subprocess.STDOUT, timeout=300)
            assert red.returncode != 0 and 'candidate suffix discovery requires the bounded probe' in (OUT/'admission.log').read_text()
        ...
        try:
            ...
            if phase == 'collection': row['draft_trace_sha256'] = sha(OUT/(name+'-draft.jsonl'))
            row['status'] = 'pass'
        ...
        with (OUT/'telemetry.csv').open('w') as f:
            ...
            try:
                for i, prompt in enumerate(prompts):
                    for mode in (['committed','suffix'] if i%2 == 0 else ['suffix','committed']): run(prompt, mode, 0, 'collection')
                cost = [p for p in prompts if p.name.startswith('calibration-code-')]
                for repeat in range(6):
                    order = list(cost)
                    random.Random(20260913+repeat).shuffle(order)
                    for prompt in order:
                        for mode in (['committed','suffix'] if (cost.index(prompt)+repeat)%2 == 0 else ['suffix','committed']): run(prompt, mode, repeat, 'cost')
            finally:
                telemetry.terminate()
                telemetry.wait(timeout=10)
    (ROOT/'SOURCE_ABLATION_DONE').write_text('192 runs completed')
