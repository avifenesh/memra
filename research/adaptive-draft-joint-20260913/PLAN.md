...
6. **Owner order, 2026-09-13: isolated mechanisms first.** Finish and bank separate head-only, confidence-only and depth-only experiments before any combinations. See `ISOLATED.md` for pre-registered controls and the distinction between existing component baselines and the proposed learned policies. The previous unequal-width diagnostic does not satisfy this requirement. Only after separate verdicts: pairwise combinations, then static/adaptive head x fixed/calibrated threshold x fixed/adaptive depth (8 cells), then head-aware joint control. Freeze head capacity and target numerical program; explicitly register each permitted head update boundary.
