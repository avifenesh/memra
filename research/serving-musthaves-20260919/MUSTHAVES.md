...
|---|---|---|---|---|
| 9.1 | **Eager-only routes prime the whole prompt in one synchronous call — CORRECTED 2026-09-19.** `prefill_tick_take`: `take = if eager_mono { q } else { q.min(budget) }`. For **gemma4** this is structural: `gemma4_prime` is a separate prefill graph that refuses `cache.pos != 0` because `prime_layers` runs the serial residual and `GemmaResidual` is not in it. For **dsv4** the prime is monolithic at 673 KiB/token (#456/#457). For **glm5_next** the first pass overstated it: `prime_cache_hyper` chunks through `hyper_prime_ranges` (gates `glm5_chunked_prime_gpu`, `glm5_prime_capacity`), and both serve routes already drive a `PrimeWalker` (`Glm5PrimeWalker`, `Glm5PlainPrimeWalker`) through `PrimeService::advance`; the gap there is that the yield door `MEMRA_PRIME_YIELD` is default OFF with PP-1 fixture receipts only (PP-2/TP-2 cells queued) — the 2026-09-05 incident ran v0.126.0, before the walker (v0.137.0). A strict phase barrier still runs all prefill before any decode in a tick on the single CUDA thread. | `worker.rs:220-224`, `:22845-22856`, `:22920-22924`, `:25193`, `:13294-13330`; `hybrid_forward.rs:5761-5774`, `:23516-23530`; `glm_spec/prime.rs:499`, `:613`; `docs/FLAGS.md:426` | 3.1: gemma4/dsv4 by mechanism, glm5 by an unqualified default; why the flag alone (#521) is not enough on gemma4/dsv4 and not yet proven on the served glm5 shapes | **#534** (corrected in its first comment) |
