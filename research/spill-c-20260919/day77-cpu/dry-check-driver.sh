#!/usr/bin/env bash
# DAY77: a control-flow dry run of day77-box.sh in a sandbox: the /root paths redirected under a temp dir, a tiny
...
for b in run-gen-i15 run-gen-i17; do touch \$OUT/bins/\$b; chmod +x \$OUT/bins/\$b; done
STUB
...
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D77_BUILDS="i15=1 i17=2" bash "$D/day77-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== run $run calls"
...
find "$D/root/spill-receipts/c-day77" -name "*.nsys-rep" -o -name "*.sqlite" | sed "s#$D#D#g" | sort
sed "s#$D#D#g" "$D/root/spill-receipts/c-day77/profiles.sha256"
