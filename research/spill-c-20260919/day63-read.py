#!/usr/bin/env python3
"""Day 63 reader for cell `i13` (research/spill-c-20260919/DAY63.md section 3, registered before the cell's script):
integrity, I13 against I12, the door against REF, and I13S's stage split per window token.

usage: day63-read.py <cell-dir> [--rig NAME]
"""
import hashlib
...
def main():
    ...
    print(f"DAY63 host demand sequence sha256 {sorted(door_traces)[0][:16] if door_traces else '-'}"
          f" lines={sorted({r['trace_lines'] for r in runs.values() if r['arm'] in DOORS})}")
    integrity = "ok" if not fails else "FAIL"
    print(f"DAY63 I13 CHECKS rig={rig} runs={len(runs)} integrity={integrity}"
    ...
    for key, label in (("gen_s", "gen-only decode"), ("window_s", "steady window")):
        ...
        print(f"DAY63 {label} medians (N=10 each): {medians}")
        noise, diff = compare("i13", "i12", key)
        if all(d < -noise for d in diff.values()):
            ...
            name = "flat"
        verdicts[key] = name
        print(f"DAY63 STEP i13_vs_i12 {label}: pooled={diff[None]:+.4f} o1={diff['o1']:+.4f}"
              f" o2={diff['o2']:+.4f} noise={noise:.4f} -> {name}")
    door = "i12" if verdicts.get("gen_s") == "regresses" else "i13"
    out = {}
    for key, label in (("gen_s", "gen-only decode"), ("window_s", "steady window")):
        ...
        if all(d < -noise for d in diff.values()):
            ...
            name = "matches"
        out[key] = name
        print(f"DAY63 DOOR {door}_vs_ref {label}: pooled={diff[None]:+.4f} o1={diff['o1']:+.4f}"
              f" o2={diff['o2']:+.4f} noise={noise:.4f} -> {name}")
    # I13S's split per window token (window minus warm over the 32 window tokens), medians over its runs.
    split = [r["segments"] for r in runs.values() if r["arm"] == "i13s" and "warm" in r["segments"]]
    ...
    for index, name in enumerate(names):
        ...
        print(f"DAY63 SPLIT {name} per window token (N={len(split)} runs): " + " ".join(cells))
    if integrity != "ok":
        print(f"DAY63 VERDICT rig={rig} integrity=FAIL -> void")
        return 1
    print(f"DAY63 VERDICT rig={rig} integrity=ok i13={verdicts['gen_s']} door={door} vs_ref={out['gen_s']}"
          f" (window: i13={verdicts['window_s']} vs_ref={out['window_s']})")
    return 0
