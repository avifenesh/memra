#!/usr/bin/env bash
# DAY72: a control-flow dry run of day72-box.sh in a sandbox: the /root paths redirected under a temp dir, a tiny
...
for b in run-gen-i15; do touch \$OUT/bins/\$b; chmod +x \$OUT/bins/\$b; done
STUB
...
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D72_BUILDS="i15=1" bash "$D/day72-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== run $run calls"
...
find "$D/root/spill-receipts/c-day72" -name "*.nsys-rep" -o -name "*.sqlite" | sed "s#$D#D#g" | sort
sed "s#$D#D#g" "$D/root/spill-receipts/c-day72/profiles.sha256"
