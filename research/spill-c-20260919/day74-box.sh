#!/usr/bin/env bash
# Day 74: the cell `induce` (DAY74.md sections 1 and 1a, OWED C12) on any Ryzen 9 9950X machine, then the 285K class,
...
# id or price here.
# usage: D74_BUILDS="p71=<sha>" [D74_RIG=<name>] day74-box.sh   (paths below; the lead stages the artifact and the two
# worktrees; the inducer needs root only for the optional compaction trigger, and reads not_run below 8 GiB)
set -uo pipefail
...
R=/root/spill-receipts/c-day74-$RIG
...
echo "cpu cap: each run pinned by the cell to its arm's 12 CPUs (DAY74 section 1); the runner unpinned" | tee -a "$R/provenance.log"
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        induce) python3 "$L/day74-read.py" "$R/$cell" --rig "$RIG" ;;
    esac > "$R/$cell/reading.log" 2>&1
    echo "$cell reader rc=$?" | tee -a "$R/box-driver.log"
    touch "$R/$cell.done"
}
cell_run induce day74-cell.sh 5400
