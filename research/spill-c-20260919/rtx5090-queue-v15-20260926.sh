#!/usr/bin/env bash
# Lane C RTX 5090 queue v15 (2026-09-26): DAY80's cell regtime on the RTX 5090 (regpool needs 98 GiB MemFree, which
# this 60 GB host cannot give), on run-gen-p80 built locally from 57086efc8, after the pid given (the local GPU check).
...
# usage: bash rtx5090-queue-v15-20260926.sh <pid to wait for, or 0>
set -uo pipefail
...
log "queue v15 start at $(git -C "$T" rev-parse --short HEAD)"
W=${1:-0}
[ "$W" != 0 ] && while kill -0 "$W" 2>/dev/null; do sleep 30; done
[ -x "$BINS/run-gen-p80" ] || { log "missing run-gen-p80, stopping"; exit 1; }
R=$L/rtx5090-day80; name=regtime
mkdir -p "$R/builds"; printf '*.log -whitespace\n*.txt -whitespace\n*.snap -whitespace\n*.json -whitespace\n*.tsv -whitespace\n' > "$R/.gitattributes"
sha256sum "$BINS/run-gen-p80" > "$R/builds/run-gen-p80.sha256"
...
systemd-run --user --scope -q -p CPUQuota=1200% env PATH="$PATH" D40_POLL_S=5 D40_LOG_EVERY=24 D40_WAITS=4320 \
    D40_LOCK_TRIES=720 D40_MIN_AVAIL_GB=40 D40_RIG=rtx5090 D40_R="$R" D40_TREE=$T D40_CELL_SCRIPT="$L/day80-cell.sh" \
...
mkdir -p "$R/$name"
/usr/bin/python3 "$L/day80-regtime-read.py" "$R/$name" --rig rtx5090 > "$R/$name/reading.log" 2>&1
log "$name reader rc=$?"
P=$T/target/c-profiles-day80; mkdir -p "$P"
