#!/usr/bin/env bash
# Day 68: the cell `eclock` (DAY68.md section 1, OWED C12) on the Ryzen 9 9950X class (preferably BOX15's
# machine), one RTX PRO 6000 Blackwell Workstation Edition. In order: provenance (tree, the artifact's SHA-256 against
# the registered value, the card, CPU and memory), the build (day63-box-build.sh in a separate build worktree, label
# p68), then the cell through the day-40 runner under /tmp/memra-gpu.lock (bounded waits, never signals a
# holder), the collector's --validate and the registered reader, both teed. The cell pins each run to DAY65's ONE
# CPUs, which is the run's CPU budget; the runner is not pinned. A rerun skips a cell with a .done marker. No host,
# id or price here.
# usage: D68_BUILDS="p68=<sha>" day68-box.sh   (paths below; the lead stages the artifact and the two
# worktrees)
set -uo pipefail
...
: "${D68_BUILDS:?set D68_BUILDS to the label=sha list of DAY68 section 1a}"
mkdir -p "$R"
...
[ -x "$R/bins/run-gen-p68" ] || { echo "missing run-gen-p68, stopping" | tee -a "$R/box-driver.log"; exit 1; }

echo "cpu cap: each run pinned by the cell to its arm's 12 CPUs (DAY68 section 1); the runner unpinned" | tee -a "$R/provenance.log"
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        eclock) python3 "$L/day68-read.py" "$R/$cell" --rig pro-single ;;
    esac > "$R/$cell/reading.log" 2>&1
    echo "$cell reader rc=$?" | tee -a "$R/box-driver.log"
    touch "$R/$cell.done"
}
cell_run eclock day68-cell.sh 5400
