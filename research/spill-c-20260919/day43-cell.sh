#!/usr/bin/env bash
# Day 43 cell `resid` (lane/spill-c-20260919, research/spill-c-20260919/DAY43.md section 1, pre-registered before
# any I6 code): improvement I6, the host tier at the machine's scale. One collector lock hold. Arms: off (no door,
# the I6 binary), base (the day-40 binary run-gen, default budget), i6d (run-gen-i6, default budget), i6g (run-gen-i6,
# --expert-bank-host-bytes=8589934592); every door arm with --expert-bank-stages. Order 1 (off, base, i6d, i6g) x 5,
# order 2 (i6g, i6d, base, off) x 5; the day-18 overlap environment.
...
# usage: day43-cell.sh resid <lockfd>
set -uo pipefail
cell=$1; fd=$2
...
case $cell in
resid)
    sha256sum "$D40_BINS/run-gen" "$D40_BINS/run-gen-i6" | tee "$EV/binary.sha256"
    stat -c '%n %s %Y' "$D40_ART" | tee "$EV/artifact.stat"
    [ -f "$D40_ART.sha256" ] && cp "$D40_ART.sha256" "$EV/artifact.sha256"
    arm() { # $1 off|base|i6d|i6g  $2 label
        local bin=$D40_BINS/run-gen-i6 door=()
        case $1 in
            base) bin=$D40_BINS/run-gen; door=(--experts-via-tier --expert-bank-stages) ;;
            i6d) door=(--experts-via-tier --expert-bank-stages) ;;
            i6g) door=(--experts-via-tier --expert-bank-stages --expert-bank-host-bytes=8589934592) ;;
        esac
        run_gen "$2" env MEMRA_MOE_RESIDENT=0 MEMRA_NGEN=32 MEMRA_MOE_SLOTS=9986 "$bin" "$D40_ART" 55 88 13 "${door[@]}"
    }
    for i in 1 2 3 4 5; do arm off "o1-off-r$i"; arm base "o1-base-r$i"; arm i6d "o1-i6d-r$i"; arm i6g "o1-i6g-r$i"; done
    for i in 1 2 3 4 5; do arm i6g "o2-i6g-r$i"; arm i6d "o2-i6d-r$i"; arm base "o2-base-r$i"; arm off "o2-off-r$i"; done
    echo "resid cell done: $(cat "$EV"/*.exit | sort | uniq -c | tr '\n' ' ')"
    ;;
