...
| The same cell on the day-31 tree (`6d940a97c`, code `33b1285e0`: DAY30's owed items 1a to 1d, the span staging back to the set on every post-take refusal and charged to the governor's pinned ledger; A day 31) | 85.4 / 85.4 (stall, per order); 115.6 / 115.5 (e2e median, 50 runs per order); `demote_in median=6.2 promote_in median=10.7` | 76.9 / 76.9 (stall; `on_minus_off=-8.4 unc=0.1` / `-8.5 unc=0.2 -> isolated`); 127.4 / 127.3 (e2e; `+11.8 unc=0.9` / `+11.8 unc=0.8 -> isolated`); `owner in-completion N=100 median=2.02 min=1.96 max=2.66` (day 30: 1.96, max 3.03), pre-submit steady `N=80 median=0.63 min=0.60 max=0.66` (day 30: 0.62, max 1.10), demote 1 of a boot 29.71 (the one staging fill, `96 fresh pinned buffer(s), 156893184 bytes charged to the governor's pinned ledger`), demotes 2 and 3 1.25 / 1.23, helper `79.8` (79.4 to 108.8), wall `in - completion` 86.7, `settle modes={'tick-top poll': 100}`, tenant top gaps 90.4 and 18.9 | A day 31, the same shape (day 26's script byte for byte), one collector hold, twenty interleaved boots, N=5 boots per arm per order, both orders, 20 of 20 replays PASS, 35 to 50 C, 33.5 to 355.6 W; the same sitting's gates ALL GREEN in every arm; same box, cross-sitting against day 30, not a same-window pair | `spill-a-20260919/pro-single-day31/box/double-park/ev`, `box/reading-day30-doublepark.log`, `box/reading-day28.log`, `box/reading-day25.log`, `box/counts.log`; `DAY31.md` sections 3 to 5 |
| The same cell twice in ONE hold on the day-32 trees (A day 32, Move 2 owed item 1's H2D half: the promote's recurrent planes ride its ticket as H2D spans after a staging fill on the hash helper; pre-H2D binary `71d21057c`, H2D binary `5ae139734`, code `4d1335cd6`) | pre-H2D: 85.1 / 85.3 (stall, per order), 115.2 / 115.3 (e2e); H2D: 85.1 / 85.2, 115.3 / 115.5; `promote_in median=10.6` / `10.7` | pre-H2D: 76.8 / 76.9 (stall; `-8.2` / `-8.4 -> isolated`), 127.3 / 127.5 (e2e; `+12.2` / `+12.1`, `DAY28 VERDICT clauses_failed=0 -> ALL PASS`), promote owner segment `N=90 median=4.58` steady; H2D: 76.0 / 76.1 (stall; `-9.1` / `-9.2 -> isolated`), 137.8 / 137.7 (e2e; `+22.5` / `+22.2`, `DAY28 CLAUSE 1b ... rule <=+20.0 -> FAIL`, `clauses_failed=2`), promote owner segment `N=90 median=0.70 min=0.65 max=1.22` (`DAY32 B2 ... -> PASS`), promote in-ms 20.70 to 31.00 (the fill 6.40 ms, landed at the first poll), `owner in-completion N=100 median=2.00`, pre-submit steady `N=80 median=0.59` | A day 32, day 26's script byte for byte, run on each binary in one collector hold (13:52Z to 14:31Z), twenty interleaved boots each, N=5 boots per arm per order, both orders, 20 of 20 replays PASS each, 33 to 51 C, 34.0 to 362.6 W; the same sitting's gates ALL GREEN in every arm; the pre-H2D and H2D rows ARE a same-hold pair | `spill-a-20260919/pro-single-day32/box/double-park/ev`, `box/pre/double-park/ev`, `box/reading-day32-b2.log`, `box/reading-day28.log`, `box/pre/reading-day28.log`, `box/counts.log`; `DAY32.md` sections 3 to 6 |
...
| A day 31 on this card: no cost cell (gates only) | n/a | n/a | the day-31 tree's own figures, recorded not compared (`counts.sh`, the demote ledger lines per cell dir): the identity default-ON boot helper `hashed in 30.3ms` per 53.7 MB (50 payloads), `landed after 14 poll(s)`, the parked hit `13 re-park(s)`, `owner in-completion 58.34ms` (the verify arm: `pre-submit 49.10`); fault-default `N=16 pre-submit median=2.10`, helper `median=28.00`, owner in-completion `median=11.43`; fault-plain `N=16 pre-submit median=1.98`, helper `28.80`, owner in-completion `10.27`; one staging fill per boot, `48 fresh pinned buffer(s), 52690944 bytes charged` (19 of 19 logs); the slot-class split `conv 24 (2359296 B), ssm 24 (50331648 B), hidden 1 (0 B or 16384 B), logits 1 (993280 B)` (gate traffic, not a cost cell) | `spill-a-20260919/rtx5090-day31/counts.log`, `rtx5090-day31/identity-default-on/ev/host-on-server.log`; `DAY31.md` sections 4 and 5 |
| A day 32 on this card: no cost cell (gates only) | n/a | n/a | the day-32 tree's own figures, recorded not compared (`counts.sh`): promote owner segment in the fault gate's default and plain arms `N=8 median=0.43` / `0.41`, identity default-ON `0.50`, the helper's fill `3.9ms` per 52.7 MB (48 planes), 13 of 13 H2D receipts `; 48 f32 spans landed under the ticket and taken back before the retire`, the `promote-span-refusal` cell `refusal handed back 48 span(s); the next H2D receipt landed 48`, `byte-unequal request(s): none`; identity default ON, fault default and plain (160 ok), hit OFF/ON ALL GREEN | `spill-a-20260919/rtx5090-day32/`; `DAY32.md` section 4 |
...
Move 1 owed items 2 and 2a, and names the pre-submit segment (Move 2 owed item 1; its D2H half landed since on A day 30, on integ47 (`origin/lane/spill-integ47-20260923` at `160929a92`), on `main` since #656 (`5f1b0eda4`), `DAY30 A2 pre-submit steady N=80 median=0.62 min=0.58 max=1.10 boots_on=10 demotes_per_boot=[11] rule N>=80 median<=1.5 max<=3.0 -> PASS`, `DAY28 VERDICT clauses_failed=0 -> ALL PASS`) as the demote's
...
DAY 39: the target card has its demote-class tenant-stall cell on the option (a) tree (`DAY39.md`, `pro-single-day39/`).
Day 35's five arms on the 27B in A day 16's shape, three binaries in one collector hold on one RTX PRO 6000 Blackwell
(B0 `091a931c0` before option (a), B1 `9717e8d57` option (a) plus 2a, B2 `160929a92` plus A day 30's D2H spans), programs
p1-b0 p2-b1 p3-b2 p4-b2 p5-b1 p6-b0, pre-registered and pushed at `89c2cd521` before the first boot; 36 boots in 56.0
minutes, 60 of 60 receipts admissible, 33 to 59 C, SM clock median 2422 MHz in every program. Demote ON minus OFF per
tree: `+32.1 unc=1.4 -> isolated` (b0), `+1.2 unc=1.7 -> under_resolution` (b1), `-40.6 unc=1.0 -> isolated` (b2). By the
worst-tick rule b1 minus b0 `moved` for the demote-on arm (`-31.8/0.8`, `-31.1/1.2`) and the demote DiD (`-31.2/2.1`,
`-30.6/2.0`), every control `under_resolution`; per tick the move sits on tick 2 (`DAY39 TICK HYPOTHESIS contrast=b1-b0
... -> consistent with H`). b2 minus b1 `moved` for the demote-on arm (`-41.2/0.7`, `-41.8/1.2`) on tick 1, and for the
promote-on arm (`-5.0/0.8`, `-5.1/0.8`). On b2 every copy-complete line carries `items=128 (32 KV, 96 f32 spans)`; its
