...
| The demote's tick AFTER the bundle hash left it (A day 28, ruling 39, option (a); the day-26 double-park cell byte-for-byte on `45f824a75`), target card, same window: the owner thread's `in - completion` per demote; the request's e2e; the tenant's stall and gaps | n/a (OFF hashes nothing; demote `in` 6.2; e2e 115.4; stall 85.2; gaps 98.6 and 16.6) | `owner in-completion median=7.40 min=7.17 max=45.01` (N=100; 74.8 on day 27; the max is the first two demotes of a boot, the pinned first touch); the helper hashes the 157.9 MB in 73.2 ms (73.0 to 73.4) off the tick; wall `in - completion` 86.5 (the publication waits for the helper and a tick top, 6 polls median); e2e 132.3 / 132.2, `on_minus_off=+16.9 / +16.9 isolated` (+91.3 on day 27); stall 81.8 / 81.9, `-3.4 unc=0.1 isolated` (unchanged); gaps 95.3 (the intruder's prime, both arms) and 19.0 (92.4 on day 27) | N=5 boots per arm per order, both orders, one hold, 20 of 20 replays PASS, 33 to 52 C, 32 to 360 W; `DAY28 VERDICT clauses_failed=0 -> ALL PASS` | A `pro-single-day28/box/double-park/ev`, `box/reading-day28.log`; `DAY28.md` |
| The same on the RTX 5090 class (this card's own figure, never compared to the row above) | | not measured on day 28 (the card carried the lead's battery and lane C; only the hit gate and the fault gate ran there, `rtx5090-day28/`); the 9B's heap hash is about 12 ms at that host's 4.5 GB/s | | `DAY28.md` section 6 |
...
|---|---|---|---|---|---|
| A day 28, the tree with the bundle hash off the tick (`45f824a75`), both cards, both arms: the RED row | RTX PRO 6000, 27B; RTX 5090 Laptop, 9B | target card `identity-default-off`, `identity-plain-off` `ALL GREEN (teeth=0)`; `failure-off` `ALL GREEN`; `twin-off` rc 0; `hitgate-off` `ALL GREEN (qwen)` (61 ok) | target card `identity-plain-on` `ALL GREEN (teeth=0)`, `twin-on` rc 0, `hitgate-on` `ALL GREEN (qwen)` (68 ok, census equal to day 24); **`identity-default-on` `4 FAILURE(S)`, `failure-on` `1 FAILURE(S)` (the `digest` cell), `contract-fault` `23 FAILURE(S)`** (the four promote cells; the demote cells' `the next demote publishes`; `d2d-capture`, `d2d-restore`, `hash-helper-gone`, `hash-never-lands` 12/14/14/14 ok, 0 fail); 5090 `hit-on` `ALL GREEN (qwen)` (68 ok, day-24 census), `fault-plain` `ALL GREEN` (121 ok), **`fault-default` `23 FAILURE(S)`**, `hit-off` not run (the gate's `flock -w 300` timed out behind another lane's gate; re-run in `DAY28.md` section 6). One cause: a hit inside the `Hashing` window is a miss and the default arm's boundary insert puts the next request there (`DAY28.md` section 5; owed item 2a) | none on this tree for the default arm; the plain arm `45f824a75` | A `pro-single-day28/box/gates/`, `rtx5090-day28/`; `DAY28.md` |
...
6. **The promote-side census question and the day-16 write-combined contradiction** named in the arithmetic.
   ...
   **CLOSED, A day 28.** The code census (A day 27, `DAY27.md` section 1): the demote's two receipt hashes cover the KV
   planes only (about 1.9 MB on the 27B, the card's pinned kind); the 74.8 ms was `bind_tier_image`'s one SHA-256 pass
   over the whole image, about 98 percent of it the recurrent f32 state in pageable heap (`HostF32::Heap`), on the owner
   thread (H3's reading of the leases stands: the door's leases are not the printed write-combined arm's bytes for the
   bulk of the hash). A day 28 (ruling 39) moved that pass to one long-lived helper thread per tier context (the same
   program over the same bytes, the entry `Hashing` until the digests land): the target card's owner-thread
   `in - completion` reads `median=7.40 min=7.17 max=45.01` (N=100) from 74.8, the helper 73.2 ms per 157.9 MB off
   the tick; the remaining owner-thread segment is the pre-submit one (the 32 pinned lease allocations and the 100 f32
   D2H copies into pageable `Vec`s), about 6 ms at steady state and 39 to 45 ms at first touch (Move 2 owed item 1).
   The promote-side half of this item stays as stated (the H2D's 1.9 MB receipt hash, `promote_completion` 19.6).
   The door's default-arm gates are red on that tree for a different reason (the C table's day-28 row; a hit inside
   the `Hashing` window is a miss), owed as Move 1 item 2a in `OWNER-THREAD-OFFLOAD.md`.
