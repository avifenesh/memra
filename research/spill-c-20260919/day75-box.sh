#!/usr/bin/env bash
# Day 75: the target-card sitting of DAY75.md section 2 (cell i16, OWED C11), one RTX PRO 6000 Blackwell
# Workstation Edition on the 285K class. In order: provenance (tree, the artifact's SHA-256 against the registered
# value, the card, CPU and memory), the builds (day63-box-build.sh in a separate build worktree, labels i15 and i16),
# then the cell i16 (day75-cell.sh), one collector hold under
...
# usage: D75_BUILDS="i15=<sha> i16=<sha>" day75-box.sh   (paths below; the lead stages the artifact and the two worktrees)
set -uo pipefail
...
: "${D75_BUILDS:?set D75_BUILDS to the label=sha list of DAY75 section 2a}"
mkdir -p "$R"
...
for b in run-gen-i15 run-gen-i16; do
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        i16) python3 "$L/day75-read.py" "$R/$cell" --rig pro-single ;;
...
cell_run i16 day75-cell.sh 7200
