#!/usr/bin/env bash
# DAY66: a control-flow dry run of day66-box.sh in a sandbox: the /root paths redirected under a temp dir, a tiny
...
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D66_BUILDS="c60=1 i15=2" bash "$D/day66-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== run $run calls"
