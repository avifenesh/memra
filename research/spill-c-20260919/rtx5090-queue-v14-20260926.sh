#!/usr/bin/env bash
# Lane C RTX 5090 queue v14 (2026-09-26): DAY79's cell i18 on the RTX 5090. First run-gen-i18 built from its commit
# by c-local-build.sh in the build worktree under target/ (inside the CPU cap, into target/c-bins, beside v9's
# run-gen-i17). Behind /tmp/memra-5090.lock with v9's idle wait; the
...
# usage: bash rtx5090-queue-v14-20260926.sh
set -uo pipefail
...
log "queue v14 start at $(git -C "$T" rev-parse --short HEAD)"
systemd-run --user --scope -q -p CPUQuota=1200% -p MemoryMax=20G bash "$L/c-local-build.sh" "$T/target/c-build-wt" "$BINS" \
    i18=c7294b912:memra-engine/run-gen
rc=$?; log "build rc=$rc"; [ "$rc" -eq 0 ] || exit 1
for b in run-gen-i17 run-gen-i18; do [ -x "$BINS/$b" ] || { log "missing $b, stopping"; exit 1; }; done
R=$L/rtx5090-day79; name=i18
mkdir -p "$R/builds"; printf '*.log -whitespace\n*.txt -whitespace\n*.snap -whitespace\n*.json -whitespace\n*.tsv -whitespace\n' > "$R/.gitattributes"
cp "$BINS/logs/build-i17.log" "$BINS/logs/build-i18.log" "$R/builds/"
...
systemd-run --user --scope -q -p CPUQuota=1200% env PATH="$PATH" D40_POLL_S=5 D40_LOG_EVERY=24 D40_WAITS=4320 \
    D40_LOCK_TRIES=720 D40_MIN_AVAIL_GB=40 D40_RIG=rtx5090 D40_R="$R" D40_TREE=$T D40_CELL_SCRIPT="$L/day79-cell.sh" \
...
mkdir -p "$R/$name"
/usr/bin/python3 "$L/day79-read.py" "$R/$name" --rig rtx5090 > "$R/$name/reading.log" 2>&1
log "$name reader rc=$?"
P=$T/target/c-profiles-day79; mkdir -p "$P"
