#!/usr/bin/env bash
# DAY80: a control-flow dry run of day80-cell.sh `regtime` then `regpool` in a sandbox: a sandbox tree holding the cell's sampler and a
# stub lock proof, a stub run-gen-p80 (the counters check prints a counted line; a run prints nothing and exits 0),
...
PATH=$D/bin:$PATH D74_DRY_F=1 D40_R=$D/R D40_BINS=$D/bins D40_TREE=$D/tree D40_LOCK=$D/none D40_ART=$D/art.gguf \
    bash "$L/day80-cell.sh" regtime 0 > cell-regtime.log 2>&1
echo "regtime cell rc=$? exits: $(cat "$D/R/regtime/ev"/*.exit | sort | uniq -c | tr '\n' ' ')"
PATH=$D/bin:$PATH D74_DRY_F=1 D40_R=$D/R D40_BINS=$D/bins D40_TREE=$D/tree D40_LOCK=$D/none D40_ART=$D/art.gguf \
    bash "$L/day80-cell.sh" regpool 0 > cell.log 2>&1
echo "regpool cell rc=$?"
EV=$D/R/regpool/ev
...
echo "== census runs: $(find "$EV" -name "c[12]-*-r1.exit" | wc -l), census attempts: $(find "$EV" -name "*.pages.log" | wc -l) (the stub is a script, named bash, so the census finds no run-gen-p80 and writes nothing)"; echo "== inducer"; cat "$EV/inducer.txt"; echo "== rewarm rows: $(wc -l < "$EV/rewarm.tsv")"; head -2 "$EV/rewarm.tsv"; echo "== compactor rows (labels, ready, loop, stopped)"; grep -P "\t(o[12]-|ready|loop|stopped)" "$EV/compactor.tsv" | cut -f2- | cut -c1-60
