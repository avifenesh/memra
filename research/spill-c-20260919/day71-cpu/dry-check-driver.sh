#!/usr/bin/env bash
# DAY71: a control-flow dry run of day71-box.sh in a sandbox, once per machine label (pro-single, then
# pro-single-b, each into its own receipts root): the /root paths redirected under a temp dir, a tiny
...
for b in run-gen-p71; do touch \$OUT/bins/\$b; chmod +x \$OUT/bins/\$b; done
STUB
...
cd "$D"
for RIG in pro-single pro-single-b; do
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D71_BUILDS="p71=1" D71_RIG=$RIG bash "$D/day71-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== $RIG run $run calls"
    sed "s#$D#D#g" calls.log
    echo "== $RIG run $run driver lines"
