#!/usr/bin/env bash
# DAY71: a control-flow dry run of day71-cell.sh `core` in a sandbox: a sandbox tree holding the cell's sampler and a
...
set -euo pipefail
L=$(cd "$(dirname "$0")/.." && pwd)
D=$(mktemp -d /tmp/c71cell.XXXXXX)
trap 'rm -rf "$D"' EXIT
mkdir -p "$D/tree/research/spill-c-20260919" "$D/tree/tools" "$D/bins" "$D/bin" "$D/R"
cp "$L/day71-sampler.py" "$D/tree/research/spill-c-20260919/"
...
PATH=$D/bin:$PATH D40_R=$D/R D40_BINS=$D/bins D40_TREE=$D/tree D40_LOCK=$D/none D40_ART=$D/art.gguf \
    bash "$L/day71-cell.sh" core 0 > cell.log 2>&1
echo "cell rc=$?"
EV=$D/R/core/ev
...
echo "== dmon rows"; head -3 "$EV/pcie.log"
echo "== host state files"; for f in "$EV"/host-*.txt; do echo "$(basename "$f") $(wc -l < "$f") lines, $(head -1 "$f")"; done
echo "== cell tail"; tail -2 cell.log
sleep 1
echo "== leftover processes (none expected)"; ps -eo pid,args | grep -E "[d]ay71-sampler.py $D|[b]in/nvidia-smi dmon" || echo none
