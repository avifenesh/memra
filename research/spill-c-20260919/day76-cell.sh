#!/usr/bin/env bash
# Day 76 cell `chunk` (lane/spill-c-20260919, research/spill-c-20260919/DAY76.md section 1, registered before this
# script): OWED C12, whether the door's one large pinned allocation draws the compaction. One binary p76; every run
# under induce-b's fragmentation (DAY74 section 4: all but 2 GiB of free memory in order-0 holes, the inducer's
# huge-page loop from the gate, the artifact reread before every run), DAY73's sampler and the phase probes. Arms:
# refi (REF), di (the door), dci (the door with --expert-bank-pool-chunk-bytes=268435456). Order 1 (refi, di, dci)
# x 4, order 2 reversed x 4. At each arm's first run the process's smaps is saved at the gate.
# usage: day76-cell.sh chunk <lockfd>
set -uo pipefail
cell=$1; fd=$2
...
case $cell in
chunk)
    sha256sum "$D40_BINS/run-gen-p76" | tee "$EV/binary.sha256"
    ...
    taskset -c "$one" "$D40_BINS/run-gen-p76" --cpu-probe-counters-check > "$EV/counters-check.txt" 2>&1
    ...
    arm() { # $1 refi|di|dci  $2 label
        ...
        case $kind in
            ...
            dci) door+=(--expert-bank-pool-chunk-bytes=268435456) ;;
        esac
        ...
        if true; then
            ...
            ( for _ in $(seq 1 12000); do
                  if grep -q "phase=gate" "$EV/$label.log" 2>/dev/null; then
                      kill -USR1 "$cpid"
                      # DAY76: at each arm's first run, the run-gen process's smaps at the gate.
                      if [ "${label##*-r}" = 1 ] && [ "${label%%-*}" = o1 ]; then
        ...
        run_gen "$label" taskset -c "$one" env MEMRA_MOE_RESIDENT=0 MEMRA_NGEN=32 MEMRA_MOE_SLOTS=9986 "${pre[@]}" "$D40_BINS/run-gen-p76" "$D40_ART" 55 88 13 "${door[@]}" --cpu-probe --cpu-probe-phases "${counters[@]}"
        if [ -n "$cpid" ]; then
            kill "$w" 2>/dev/null; kill -TERM "$cpid" 2>/dev/null; wait "$cpid" 2>/dev/null
        fi
    }
    for i in 1 2 3 4; do arm refi "o1-refi-r$i"; arm di "o1-di-r$i"; arm dci "o1-dci-r$i"; done
    for i in 1 2 3 4; do arm dci "o2-dci-r$i"; arm di "o2-di-r$i"; arm refi "o2-refi-r$i"; done
    kill "$sampler" 2>/dev/null
    host_state after
    echo "chunk cell done: $(cat "$EV"/*.exit | sort | uniq -c | tr '\n' ' ')"
    ;;
