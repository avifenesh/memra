#!/usr/bin/env bash
# Day 81 cell `i19` (lane/spill-c-20260919, research/spill-c-20260919/DAY81.md section 2, registered before this
# script): I19 against I18 and the door against REF. One collector lock hold. Arms: ref (run-gen-i18,
# MEMRA_MOE_PREFETCH=1), i18 (run-gen-i18 with the door), i19 (run-gen-i19 with the door), i19c (i19 with
# --moe-dispatch-clock); order 1 (ref, i18, i19, i19c) x 5, order 2 reversed x 5. Then ref and i19 under Nsight
# Systems (--trace=cuda, no sampling, no context switches), order ref, i19, i19, ref, each exported to SQLite. The
...
# usage: day81-cell.sh i19 <lockfd>
set -uo pipefail
cell=$1; fd=$2
...
case $cell in
i19)
    sha256sum "$D40_BINS/run-gen-i18" "$D40_BINS/run-gen-i19" | tee "$EV/binary.sha256"
    stat -c '%n %s %Y' "$D40_ART" | tee "$EV/artifact.stat"
    [ -f "$D40_ART.sha256" ] && cp "$D40_ART.sha256" "$EV/artifact.sha256"
    arm() { # $1 ref|i18|i19|i19c  $2 label
        local pre=() door=(--experts-via-tier --expert-bank-host-bytes=17179869184) bin=run-gen-i19
        case $1 in
            ref) pre=(MEMRA_MOE_PREFETCH=1); door=(); bin=run-gen-i18 ;;
            i18) bin=run-gen-i18 ;;
            i19c) door+=(--moe-dispatch-clock) ;;
        esac
        run_gen "$2" env MEMRA_MOE_RESIDENT=0 MEMRA_NGEN=32 MEMRA_MOE_SLOTS=9986 "${pre[@]}" "$D40_BINS/$bin" "$D40_ART" 55 88 13 "${door[@]}"
    }
    for i in 1 2 3 4 5; do arm ref "o1-ref-r$i"; arm i18 "o1-i18-r$i"; arm i19 "o1-i19-r$i"; arm i19c "o1-i19c-r$i"; done
    for i in 1 2 3 4 5; do arm i19c "o2-i19c-r$i"; arm i19 "o2-i19-r$i"; arm i18 "o2-i18-r$i"; arm ref "o2-ref-r$i"; done
    ...
    if [ -z "$NSYS" ]; then
        ...
        prof() { # $1 ref|i19  $2 label
            local pre=() door=() bin=run-gen-i19
            case $1 in
                ref) pre=(MEMRA_MOE_PREFETCH=1); bin=run-gen-i18 ;;
                i19) door=(--experts-via-tier --expert-bank-host-bytes=17179869184) ;;
            esac
        ...
        prof ref p-ref-r1; prof i19 p-on-r1; prof i19 p-on-r2; prof ref p-ref-r2
    fi
    echo "i19 cell done: $(cat "$EV"/*.exit | sort | uniq -c | tr '\n' ' ')"
    ;;
