#!/usr/bin/env bash
# DAY74 section 4: a control-flow dry run of day74b-box.sh in a sandbox, with the default machine label: the /root paths redirected under a temp dir, a tiny
...
sed -e "s#/root#$D/root#g" -e "s#df27a780435b7b45c2597536112ea3cb091f8544c3d0c3318d9f4258b31f7adf#$SA#" \
    "$L/day74b-box.sh" > "$D/day74b-box.sh"
...
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D74_BUILDS="p71=1" bash "$D/day74b-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== run $run calls"
