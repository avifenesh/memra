#!/usr/bin/env bash
# DAY74: a control-flow dry run of day74-cell.sh `induce` in a sandbox: a sandbox tree holding the cell's sampler and a
...
PATH=$D/bin:$PATH D74_DRY_F=1 D40_R=$D/R D40_BINS=$D/bins D40_TREE=$D/tree D40_LOCK=$D/none D40_ART=$D/art.gguf \
    bash "$L/day74-cell.sh" induce 0 > cell.log 2>&1
echo "cell rc=$?"
EV=$D/R/induce/ev
...
echo "== inducer"; cat "$EV/inducer.txt"; echo "== compactor rows (labels, ready, loop, stopped)"; grep -P "\t(o[12]-|ready|loop|stopped)" "$EV/compactor.tsv" | cut -f2- | cut -c1-60
