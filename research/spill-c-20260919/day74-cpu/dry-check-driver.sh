#!/usr/bin/env bash
# DAY74: a control-flow dry run of day74-box.sh in a sandbox, with the default machine label: the /root paths redirected under a temp dir, a tiny
...
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D74_BUILDS="p71=1" bash "$D/day74-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== run $run calls"
