#!/usr/bin/env bash
# WP-C RTX 5090 queue v7: DAY63's cell i13 on the local card, behind queue v6 (it waits
# for v6 to exit; the local run-gen-c60, -i12 and -i13 are built), the cell through the day-40 runner under the 1200% cap
...
log "queue v7 start: waits for v6 (pid $V6)"
while kill -0 "$V6" 2>/dev/null; do sleep 30; done
log "v6 exited"
for b in run-gen-c60 run-gen-i12 run-gen-i13; do [ -x "/tmp/c40-bins/$b" ] || { log "missing $b, stopping"; exit 1; }; done
...
mkdir -p "$L/rtx5090-day63/builds" && cp /tmp/c61-build/build-*.log "$L/rtx5090-day63/builds/"
