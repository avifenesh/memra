#!/usr/bin/env python3
"""Day 80 reader for cell `regpool` (research/spill-c-20260919/DAY80.md section 1, registered before the cell's
scripts): DAY78's reader with the registered pool's arm (`dri`) in the pageable arm's place, two census runs, and
DAY80's verdict (`registered_clears`, `registered_does_not`, `not_reproduced`, `undecided`).

usage: day80-regpool-read.py <cell-dir> [--rig NAME]
"""
...
def main():
    ...
    print(f"DAY80 INDUCER rig={rig} {inducer.splitlines()[0]}")
    if "induce=1" not in inducer:
        print(f"DAY80 REGPOOL VERDICT rig={rig} -> not_run")
        return 0
    ...
    for label, r in runs.items():
        ...
        if r["arm"] in ("di", "dri"):
            ...
            if not m or (r["arm"] == "dri") != m.group(0).endswith(" registered"):
    ...
    print(f"DAY80 REGPOOL CHECKS rig={rig} runs={len(runs)} state={unit} integrity={integrity}"
          + ("" if not fails else " failed=" + "; ".join(fails)))
    if integrity != "ok":
        print(f"DAY80 REGPOOL VERDICT rig={rig} integrity=FAIL -> void")
        return 1
    if pools:
        print(f"DAY80 POOL door arms: bytes={sorted(set(pools.values()))}")
    ...
    for label in sorted(runs, key=lambda k: marks[k]["start"]):
        r = runs[label]
        print(f"DAY80 R {label}: gen={r['gen_s']:.3f} window={r['window_s']:.3f} window_{unit}="
    ...
    for arm in ARMS:
        ...
        print(f"DAY80 ARM rig={rig} {arm}: fail_heavy={count[arm][0]} of {len(rs)} slow={count[arm][1]} of {len(rs)}"
    ...
    elif count["dri"][0] == 0 and count["dri"][1] == 0:
        verdict = "registered_clears"
    elif count["dri"][0] >= 3:
        verdict = "registered_does_not"
    else:
        verdict = "undecided"
    ...
    for label in sorted(census):
        ...
        print(f"DAY80 CENSUS {label}: {access}")
        for f in big:
            print(f"DAY80 CENSUS {label} map: {f[3]} {f[4]} {f[5][:48]} {f[6]} {f[7]} {f[9]} {f[10]} {f[11]} {f[12]}"
                  f" {f[14]} {f[15][:60]} {f[16]} {f[17][:80]}")
        print(f"DAY80 CENSUS {label}: mappings with frames changed between the passes: {len(moved)}"
              + (f" ({', '.join(moved[:4])})" if moved else ""))
    print(f"DAY80 REGPOOL VERDICT rig={rig} integrity=ok -> {verdict} (fail_heavy: di {count['di'][0]} of 8,"
          f" dri {count['dri'][0]} of 8, refi {count['refi'][0]} of 8; slow: dri {count['dri'][1]} of 8)")
    return 0
