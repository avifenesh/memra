#!/usr/bin/env bash
# Day 70: the cell `sched` (DAY70.md section 1, OWED C12) on the Ryzen 9 9950X class (preferably BOX15's
...
# id or price here.
# usage: D70_BUILDS="p68=<sha>" day70-box.sh   (paths below; the lead stages the artifact and the two
# worktrees)
set -uo pipefail
...
: "${D70_BUILDS:?set D70_BUILDS to the label=sha list of DAY70 section 1a}"
mkdir -p "$R"
...
echo "cpu cap: each run pinned by the cell to its arm's 12 CPUs (DAY70 section 1); the runner unpinned" | tee -a "$R/provenance.log"
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        sched) python3 "$L/day70-read.py" "$R/$cell" --rig pro-single ;;
    esac > "$R/$cell/reading.log" 2>&1
    echo "$cell reader rc=$?" | tee -a "$R/box-driver.log"
    touch "$R/$cell.done"
}
cell_run sched day70-cell.sh 5400
