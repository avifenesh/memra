#!/usr/bin/env bash
# Day 60 cell `gap` (lane/spill-c-20260919, research/spill-c-20260919/DAY60.md section 1, registered before the
# instrument's code): the door's gap to REF, attributed. One collector lock hold, one binary (run-gen-c60) for every
# arm. Arms: ref (MEMRA_MOE_PREFETCH=1), refc (ref with --moe-dispatch-clock), on (--experts-via-tier
# --expert-bank-host-bytes=17179869184), onc (on with --moe-dispatch-clock). Order 1 (ref, refc, on, onc) x 5,
...
# usage: day60-cell.sh gap <lockfd>
set -uo pipefail
cell=$1; fd=$2
...
gap)
    ...
    arm() { # $1 ref|refc|on|onc  $2 label
        ...
        run_gen "$2" env MEMRA_MOE_RESIDENT=0 MEMRA_NGEN=32 MEMRA_MOE_SLOTS=9986 "${pre[@]}" "$D40_BINS/run-gen-c60" "$D40_ART" 55 88 13 "${door[@]}"
    }
    ...
    echo "gap cell done: $(cat "$EV"/*.exit | sort | uniq -c | tr '\n' ' ')"
    ;;
