#!/usr/bin/env bash
# DAY61: a control-flow dry run of day61-box.sh in a sandbox: the /root paths redirected under a temp dir, a tiny
...
for f in day59-pf.py day60-gap.py day61-read.py; do
...
cat > "$S/day61-box-build.sh" <<STUB
#!/usr/bin/env bash
echo "STUB day61-box-build.sh \$*" >> $D/calls.log
OUT=\$2; mkdir -p \$OUT/bins
for b in run-gen-c60 run-spec-c60 memra-server-c60 run-gen-i11 run-gen-i12; do touch \$OUT/bins/\$b; chmod +x \$OUT/bins/\$b; done
STUB
...
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D61_BUILDS="c60=1 i11=2 i12=3" bash "$D/day61-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== run $run calls"
