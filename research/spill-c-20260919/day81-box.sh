#!/usr/bin/env bash
# Day 81: the target-card sitting of DAY81.md section 2 (cell i19, OWED C11), one RTX PRO 6000 Blackwell
# Workstation Edition on the 285K class. In order: provenance (tree, the artifact's SHA-256 against the registered
# value, the card, CPU and memory), the builds (day63-box-build.sh in a separate build worktree, labels i18 and i19),
# then the cell i19 (day81-cell.sh), one collector hold under
...
# usage: D81_BUILDS="i18=<sha> i19=<sha>" day81-box.sh   (paths below; the lead stages the artifact and the two worktrees)
set -uo pipefail
...
: "${D81_BUILDS:?set D81_BUILDS to the label=sha list of DAY81 section 2a}"
mkdir -p "$R"
...
for b in run-gen-i18 run-gen-i19; do
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        i19) python3 "$L/day81-read.py" "$R/$cell" --rig pro-single ;;
...
cell_run i19 day81-cell.sh 7200
