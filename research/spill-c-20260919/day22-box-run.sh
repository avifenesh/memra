#!/usr/bin/env bash
# Day 22 target-card runner (one RTX PRO 6000 Blackwell, collector rig `pro-single`, lock
# /tmp/memra-gpu.lock): waits for the tree's server build receipt, then runs the host-tier gate cells one
# at a time through tools/tier-battery.py --external-lock, bounded lock retries (75 x 120 s, another lane
# may hold the card, the holder is never signalled). The cell list is an argument so a partial re-run
# names its cells. No host, id or price here.
# usage: day22-box-run.sh <tree> <receipts_root> <model.gguf> [cell ...]
set -uo pipefail
TREE=$1; R=$2; MODEL=$3; shift 3
CELLS=("$@")
[ ${#CELLS[@]} -eq 0 ] && CELLS=(fault-default fault-plain
    failure-default-off failure-default-on failure-plain-off failure-plain-on
    failure-default-pct100-off failure-default-pct100-on
...
for cell in "${CELLS[@]}"; do
    for attempt in $(seq 0 75); do
        ...
        python3 tools/tier-battery.py --rig pro-single --timeout 3600 --out "$out" --external-lock \
            --execute bash research/spill-c-20260919/day22-cell.sh "$cell" "$MODEL" "$BIN" "$R/cells" 256 @COLLECTOR_LOCK_FD@ \
            > "$out-driver.log" 2>&1
        rc=$?
        echo $rc > "$out.exit"
