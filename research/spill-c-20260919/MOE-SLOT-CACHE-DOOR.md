...
| 63 | I13: the governor reserves and releases without temporaries, one body per `BankLease`, the retire side in one pending lookup, the SLRU's maps on a deterministic Fx hasher (3569 to 2940 ns per host-hit prefetch cycle on the local CPU); card cell `i13` pending | `contracts.rs`, `governor.rs`, `residency.rs`, `expert_dispatch.rs`, `slru.rs` | `9bbab60ed` to `c9379c051` |
