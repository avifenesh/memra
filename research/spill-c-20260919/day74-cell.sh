#!/usr/bin/env bash
# Day 74 cell `induce` (lane/spill-c-20260919, research/spill-c-20260919/DAY74.md sections 1 and 1a, registered
# before this script): OWED C12, REF and the door (I15) from the binary p71 on DAY65's ONE pin with DAY73's probes and
# sampler, each with and without the inducer (day74-compactor.py: host memory fragmented before the run, huge pages
# asked for and compaction triggered from the run's gate line to its end), the inducer pinned to the sampler's CPU.
# Order 1 (ref, refi, i15, i15i) x 3, order 2 reversed x 3. The day-18 overlap environment.
...
# usage: day74-cell.sh induce <lockfd>
set -uo pipefail
cell=$1; fd=$2
...
induce)
    ...
    sampler=$!
    # The inducer's size (DAY74 section 1a): F = min(64, 2 x (MemFree - 48 GiB)), the induced arms only when F >= 8.
    free_gib=$(awk '/MemFree/ {printf "%d", $2 / 1048576}' /proc/meminfo)
    F=$(( 2 * (free_gib - 48) )); [ "$F" -gt 64 ] && F=64
    induce=1; [ "$F" -lt 8 ] && induce=0
    ...
    : > "$EV/compactor.tsv"
    arm() { # $1 ref|refi|i15|i15i  $2 label
        local pre=() door=(--experts-via-tier --expert-bank-host-bytes=17179869184) kind=$1 label=$2 cpid='' w='' before
        case $kind in ref|refi) pre=(MEMRA_MOE_PREFETCH=1); door=() ;; esac
        if [ "$kind" = refi ] || [ "$kind" = i15i ]; then
            ...
            ( for _ in $(seq 1 12000); do grep -q "phase=gate" "$EV/$label.log" 2>/dev/null && { kill -USR1 "$cpid"; exit 0; }; sleep 0.05; done ) &
            w=$!
        fi
        run_gen "$label" taskset -c "$one" env MEMRA_MOE_RESIDENT=0 MEMRA_NGEN=32 MEMRA_MOE_SLOTS=9986 "${pre[@]}" "$D40_BINS/run-gen-p71" "$D40_ART" 55 88 13 "${door[@]}" --cpu-probe --cpu-probe-phases "${counters[@]}"
        if [ -n "$cpid" ]; then
            kill "$w" 2>/dev/null; kill -TERM "$cpid" 2>/dev/null; wait "$cpid" 2>/dev/null
        fi
    }
    for i in 1 2 3; do arm ref "o1-ref-r$i"; arm refi "o1-refi-r$i"; arm i15 "o1-i15-r$i"; arm i15i "o1-i15i-r$i"; done
    for i in 1 2 3; do arm i15i "o2-i15i-r$i"; arm i15 "o2-i15-r$i"; arm refi "o2-refi-r$i"; arm ref "o2-ref-r$i"; done
    kill "$sampler" 2>/dev/null
    host_state after
    echo "induce cell done: $(cat "$EV"/*.exit | sort | uniq -c | tr '\n' ' ')"
    ;;
