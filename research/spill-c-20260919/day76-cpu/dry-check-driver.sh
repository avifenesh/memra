#!/usr/bin/env bash
# DAY76: a control-flow dry run of day76-box.sh in a sandbox, with the default machine label: the /root paths redirected under a temp dir, a tiny
...
for b in run-gen-p76; do touch \$OUT/bins/\$b; chmod +x \$OUT/bins/\$b; done
STUB
...
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D76_BUILDS="p76=1" bash "$D/day76-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== run $run calls"
