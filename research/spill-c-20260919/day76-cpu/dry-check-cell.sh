#!/usr/bin/env bash
# DAY76: a control-flow dry run of day76-cell.sh `chunk` in a sandbox: a sandbox tree holding the cell's sampler and a
# stub lock proof, a stub run-gen-p76 (the counters check prints a counted line; a run prints nothing and exits 0),
...
set -euo pipefail
L=$(cd "$(dirname "$0")/.." && pwd)
D=$(mktemp -d /tmp/c76cell.XXXXXX)
trap 'rm -rf "$D"' EXIT
mkdir -p "$D/tree/research/spill-c-20260919" "$D/tree/tools" "$D/bins" "$D/bin" "$D/R"
cp "$L/day73-sampler.py" "$L/day74-compactor.py" "$D/tree/research/spill-c-20260919/"
...
PATH=$D/bin:$PATH D74_DRY_F=1 D40_R=$D/R D40_BINS=$D/bins D40_TREE=$D/tree D40_LOCK=$D/none D40_ART=$D/art.gguf \
    bash "$L/day76-cell.sh" chunk 0 > cell.log 2>&1
echo "cell rc=$?"
EV=$D/R/chunk/ev
...
echo "== smaps files: $(ls "$EV"/*.smaps 2>/dev/null | wc -l)"; echo "== inducer"; cat "$EV/inducer.txt"; echo "== rewarm rows: $(wc -l < "$EV/rewarm.tsv")"; head -2 "$EV/rewarm.tsv"; echo "== compactor rows (labels, ready, loop, stopped)"; grep -P "\t(o[12]-|ready|loop|stopped)" "$EV/compactor.tsv" | cut -f2- | cut -c1-60
