#!/usr/bin/env bash
# DAY63: a control-flow dry run of day63-box.sh in a sandbox: the /root paths redirected under a temp dir, a tiny
...
for b in run-gen-c60 run-gen-i12 run-gen-i13; do touch \$OUT/bins/\$b; chmod +x \$OUT/bins/\$b; done
STUB
...
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D63_BUILDS="c60=1 i12=2 i13=3" bash "$D/day63-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== run $run calls"
