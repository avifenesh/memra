#!/usr/bin/env bash
# Day 76: the cell `chunk` (DAY76.md section 1, OWED C12) on any Ryzen 9 9950X machine, then the 285K class,
# one RTX PRO 6000 Blackwell Workstation Edition; D76_RIG names the machine in the reading (default pro-single). It can
...
# CPU and memory), the build (day63-box-build.sh in a separate build worktree, label p76), then the cell through the
...
# id or price here.
# usage: D76_BUILDS="p76=<sha>" [D76_RIG=<name>] day76-box.sh   (paths below; the lead stages the artifact and the two
...
RIG=${D76_RIG:-pro-single}
R=/root/spill-receipts/c-day76-$RIG
...
: "${D76_BUILDS:?set D76_BUILDS to the label=sha list of DAY76 section 1a}"
mkdir -p "$R"
...
[ -x "$R/bins/run-gen-p76" ] || { echo "missing run-gen-p76, stopping" | tee -a "$R/box-driver.log"; exit 1; }

echo "cpu cap: each run pinned by the cell to its arm's 12 CPUs (DAY76 section 1); the runner unpinned" | tee -a "$R/provenance.log"
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        chunk) python3 "$L/day76-read.py" "$R/$cell" --rig "$RIG" ;;
    esac > "$R/$cell/reading.log" 2>&1
    echo "$cell reader rc=$?" | tee -a "$R/box-driver.log"
    touch "$R/$cell.done"
}
cell_run chunk day76-cell.sh 5400
