#!/usr/bin/env bash
# DAY73: a control-flow dry run of day73-box.sh in a sandbox, with the default machine label: the /root paths redirected under a temp dir, a tiny
...
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D73_BUILDS="p71=1" bash "$D/day73-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== run $run calls"
