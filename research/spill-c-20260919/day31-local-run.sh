#!/usr/bin/env bash
# Day 31 local runner for the pair cell (the local RTX 5090 Laptop GPU, collector rig `rtx5090`, lock
# /tmp/memra-5090.lock): waits, bounded, for the card to carry no compute app and at least 20000 MiB free (the
# day-26 rule; another session's server may hold the card for long windows), then runs day31-wc-cell.sh ONCE
# through tools/tier-battery.py --external-lock (one lock hold, four boots). Bounded: 15 waits of 120 s in total
...
# usage: day31-local-run.sh <receipts_root> [server_bin]
set -uo pipefail
R=$1
...
if [ "$waits" -gt 15 ]; then
    ...
    } | tee "$R/pair/NOT-RUN.txt"
    exit 3
fi
echo "$(date -u +%FT%TZ) $cell rc=$rc" | tee -a "$R/pair/progress.log"
exit "$rc"
