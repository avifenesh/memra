#!/usr/bin/env python3
"""Day 77 reader for cell `i17` (research/spill-c-20260919/DAY77.md section 2, registered before the cell's script):
integrity (one host demand sequence within each door arm, and I17's equal to I15's), DAY64 section 5's admissibility
clause, I17 against I15
(`DAY61.md` section 2's improves / regresses / flat, gen-only primary, the window beside it), the door against REF,
I17C's dispatch-clock brackets, and Part B (REF and I17 under Nsight Systems, `day72-read.py`'s window reading),
deciding nothing. Then the verdict line.

usage: day77-read.py <cell-dir> [--rig NAME]
"""
import hashlib
...
def main():
    ...
    for label, r in runs.items():
        ...
        if (r["arm"] == "i17c") != bool(r["clocks"]):
            fails.append(f"{label} dispatch-clock lines {'missing' if r['arm'] == 'i17c' else 'without the flag'}")
    if len({r.get("tokens") for r in runs.values()}) != 1:
        fails.append("tapes differ across arms")
    # DAY77 section 2: I17's host demand sequence must be I15's (the same program).
    if {r["trace"] for r in runs.values() if r["arm"] in DOORS} != {r["trace"] for r in runs.values() if r["arm"] == "i15"}:
        fails.append("I17's host demand sequence differs from I15's")
    for arm in DOORS:
        ...
        print(f"DAY77 host demand sequence {arm} sha256 {sorted(traces)[0][:16] if traces else '-'}"
              f" lines={sorted({r['trace_lines'] for r in runs.values() if r['arm'] == arm})}")
    integrity = "ok" if not fails else "FAIL"
    print(f"DAY77 I17 CHECKS rig={rig} runs={len(runs)} integrity={integrity}"
          + ("" if not fails else " failed=" + "; ".join(fails)))
    if integrity != "ok":
        print(f"DAY77 VERDICT rig={rig} integrity=FAIL -> void")
        return 1
    ...
    print(f"DAY77 ADMISSIBILITY rig={rig} ceiling={ADMISSIBLE_IQR} max_iqr_gen={worst['gen_s']:.4f}"
    ...
    for key, label in (("gen_s", "gen-only decode"), ("window_s", "steady window")):
        print(f"DAY77 {label} medians (N=10 each): " + " ".join(f"{a}={med(values(a, key)):.3f}" for a in ARMS))
        noise, diff = compare("i17", "i15", key)
        if all(d < -noise for d in diff.values()):
            ...
            name = "flat"
        step[key] = name
        print(f"DAY77 STEP i17_vs_i15 {label}: pooled={diff[None]:+.4f} o1={diff['o1']:+.4f} o2={diff['o2']:+.4f}"
              f" noise={noise:.4f} -> {name}")
    door = "i15" if step["gen_s"] == "regresses" else "i17"
    out = {}
    for key, label in (("gen_s", "gen-only decode"), ("window_s", "steady window")):
        ...
        if all(d < -noise for d in diff.values()):
            ...
            name = "matches"
        out[key] = name
        print(f"DAY77 DOOR {door}_vs_ref {label}: pooled={diff[None]:+.4f} o1={diff['o1']:+.4f} o2={diff['o2']:+.4f}"
              f" noise={noise:.4f} -> {name}")
    # I17C's brackets per window token (window minus warm over the 32 window tokens), medians over its runs.
    ...
    per = {k: med([(r["clocks"]["window"].get(k, 0) - r["clocks"]["warm"].get(k, 0)) / N / 1e6
                   for r in runs.values() if r["arm"] == "i17c" and "window" in r["clocks"] and "warm" in r["clocks"]])
           for k in keys}
    print("DAY77 I17C brackets per window token (ms, medians): " + " ".join(f"{k}={v:.4f}" for k, v in per.items()))
    got = {"ref": [], "on": []}
    for label in ("p-ref-r1", "p-on-r1", "p-on-r2", "p-ref-r2"):
        res, why = d72.profiled(ev, label)
        if res is None:
            print(f"DAY77 B {label}: not read ({why})")
            continue
        got[label.split("-")[1]].append(res)
        print(f"DAY77 B {label}: window_ms={res['window_ms']:.1f} gpu_busy={res['gpu_busy']:.4f}"
    ...
    if len(got["ref"]) == 2 and len(got["on"]) == 2:
        ...
        print("DAY77 B i17_minus_ref per window token (ms, medians of two; deciding nothing): "
              + " ".join(f"{k}={v:+.4f}" for k, v in diff.items()))
    tail = (f"i17={step['gen_s']} door={door} vs_ref={out['gen_s']}"
            f" (window: i17={step['window_s']} vs_ref={out['window_s']})")
    if not admissible:
        print(f"DAY77 VERDICT rig={rig} integrity=ok -> void (inadmissible) [as read: {tail}]")
    else:
        print(f"DAY77 VERDICT rig={rig} integrity=ok {tail}")
    return 0
