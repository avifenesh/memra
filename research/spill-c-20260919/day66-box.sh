#!/usr/bin/env bash
# Day 66: the cell `freq` (DAY66.md section 1, OWED C12) on the Ryzen 9 9950X class (preferably BOX15's
...
# id or price here.
# usage: D66_BUILDS="c60=<sha> i15=<sha>" day66-box.sh   (paths below; the lead stages the artifact and the two
# worktrees)
set -uo pipefail
...
: "${D66_BUILDS:?set D66_BUILDS to the label=sha list of DAY66 section 1}"
mkdir -p "$R"
...
echo "cpu cap: each run pinned by the cell to its arm's 12 CPUs (DAY66 section 1); the runner unpinned" | tee -a "$R/provenance.log"
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        freq) python3 "$L/day66-read.py" "$R/$cell" --rig pro-single ;;
    esac > "$R/$cell/reading.log" 2>&1
    echo "$cell reader rc=$?" | tee -a "$R/box-driver.log"
    touch "$R/$cell.done"
}
cell_run freq day66-cell.sh 5400
