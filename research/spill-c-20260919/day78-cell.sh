#!/usr/bin/env bash
# Day 78 cell `pages` (lane/spill-c-20260919, research/spill-c-20260919/DAY78.md section 1, registered before this
# script): OWED C12, which of the door's pages compaction takes and cannot move. One binary p78; every run under
# induce-b's fragmentation (DAY74 section 4), DAY73's sampler and the phase probes. Arms: refi (REF), di (the door),
# dpi (the door with --expert-bank-pool-pageable). Order 1 (refi, di, dpi) x 4, order 2 reversed x 4: the timed runs.
# Then six census runs (c1: refi, di, dpi; c2: dpi, di, refi), each with day78-pages.py started at its gate on the
# run's own run-gen process (found through the process tree, never by name alone), pinned to the sampler's CPU.
# usage: day78-cell.sh pages <lockfd>
set -uo pipefail
cell=$1; fd=$2
...
case $cell in
pages)
    sha256sum "$D40_BINS/run-gen-p78" | tee "$EV/binary.sha256"
    ...
    taskset -c "$one" "$D40_BINS/run-gen-p78" --cpu-probe-counters-check > "$EV/counters-check.txt" 2>&1
    ...
    : > "$EV/compactor.tsv"
    : > "$EV/rewarm.tsv"
    # The run-gen process below this cell's shell: a run-gen-p78 whose ancestors include $$.
    own_run_gen() {
        local p q
    ...
    arm() { # $1 refi|di|dpi  $2 label
        ...
        case $kind in
            ...
            dpi) door+=(--expert-bank-pool-pageable) ;;
        esac
        ...
        run_gen "$label" taskset -c "$one" env MEMRA_MOE_RESIDENT=0 MEMRA_NGEN=32 MEMRA_MOE_SLOTS=9986 "${pre[@]}" "$D40_BINS/run-gen-p78" "$D40_ART" 55 88 13 "${door[@]}" --cpu-probe --cpu-probe-phases "${counters[@]}"
        if [ -n "$cpid" ]; then
            kill "$w" 2>/dev/null; kill -TERM "$cpid" 2>/dev/null; wait "$cpid" 2>/dev/null
        fi
    }
    for i in 1 2 3 4; do arm refi "o1-refi-r$i"; arm di "o1-di-r$i"; arm dpi "o1-dpi-r$i"; done
    for i in 1 2 3 4; do arm dpi "o2-dpi-r$i"; arm di "o2-di-r$i"; arm refi "o2-refi-r$i"; done
    for k in refi di dpi; do arm "$k" "c1-$k-r1"; done
    for k in dpi di refi; do arm "$k" "c2-$k-r1"; done
    kill "$sampler" 2>/dev/null
    host_state after
    echo "pages cell done: $(cat "$EV"/*.exit | sort | uniq -c | tr '\n' ' ')"
    ;;
