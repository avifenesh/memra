#!/usr/bin/env bash
# DAY65: a control-flow dry run of day65-box.sh in a sandbox: the /root paths redirected under a temp dir, a tiny
...
for b in run-gen-c60 run-gen-i15; do touch \$OUT/bins/\$b; chmod +x \$OUT/bins/\$b; done
STUB
...
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D65_BUILDS="c60=1 i15=2" bash "$D/day65-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== run $run calls"
