#!/usr/bin/env bash
# DAY79: a control-flow dry run of day79-box.sh in a sandbox: the /root paths redirected under a temp dir, a tiny
...
for b in run-gen-i17 run-gen-i18; do touch \$OUT/bins/\$b; chmod +x \$OUT/bins/\$b; done
STUB
...
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D79_BUILDS="i17=1 i18=2" bash "$D/day79-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== run $run calls"
...
find "$D/root/spill-receipts/c-day79" -name "*.nsys-rep" -o -name "*.sqlite" | sed "s#$D#D#g" | sort
sed "s#$D#D#g" "$D/root/spill-receipts/c-day79/profiles.sha256"
