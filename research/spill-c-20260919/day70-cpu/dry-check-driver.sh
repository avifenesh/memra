#!/usr/bin/env bash
# DAY70: a control-flow dry run of day70-box.sh in a sandbox: the /root paths redirected under a temp dir, a tiny
...
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D70_BUILDS="p68=1" bash "$D/day70-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== run $run calls"
