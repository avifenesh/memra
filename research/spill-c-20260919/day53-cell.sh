#!/usr/bin/env bash
# day53-cell.sh <cell> <model.gguf> <server_bin> <out_root> [cache_mb]
# The day-24 cell driver (DAY53.md section 1a): the host-tier failure and identity gates on the verify digest v3
# binary, and the v3 GPU unit cell under this driver's own `flock -n` on the canonical lock. Every gate that takes
# the lock itself is left to do so; a busy lock is retried 90 x 120 s and the holder is never inspected or
# signalled. Records nvidia-smi compute apps and driver free before and after, the binary digest, the tree SHA, the
# verdict line and the exit code. Every cell is executed-not-qualified development evidence.
# cells: failure-{default,plain}-{off,on}  identity-{default,plain}-{off,on}  unit-server
set -uo pipefail
CELL=$1; MODEL9=$2; BIN=$3; ROOT=$4; CACHE_MB=${5:-64}
...
case "$CELL" in
    ...
    unit-server) GATE="cargo test -p memra-server --lib -- --ignored --test-threads=1 verify_digest_v3_"; MODEL= ;;
    *) echo "unknown cell $CELL" >&2; exit 2 ;;
esac
ENVS=()
[[ $CELL == failure-* || $CELL == identity-* ]] && ENVS+=("MEMRA_HOSTGATE_CACHE_MB=$CACHE_MB")
