#!/usr/bin/env python3
"""Day 50 reader for cell `prefetch` (research/spill-c-20260919/DAY50.md section 1, registered before any I4 code).
Integrity, clause (i) I4 beats I7 in the window, clause (ii) at least half of I4's GPU misses are prefetch hits.

usage: day50-prefetch.py <cell-dir> [--rig NAME]
"""
...
def main():
    ...
    print(f"DAY50 PREFETCH CHECKS rig={rig} runs={len(runs)} integrity={integrity}" +
    ...
    for arm in ("i7", "i4"):
        ...
        print(f"DAY50 ARM {arm} window_door_ms_per_token={(med(values(arm, 'window_s')) - off_w) * 1000 / N_TOKENS:.2f}"
    ...
    noise = max(iqr(values("i4", "window_s")), iqr(values("i7", "window_s")))
    diffs = {o: med(values("i4", "window_s", o)) - med(values("i7", "window_s", o)) for o in ("o1", "o2")}
    c1 = all(d < -noise for d in diffs.values())
    print(f"DAY50 CLAUSE (i) i4_minus_i7 window o1={diffs['o1']:+.3f} o2={diffs['o2']:+.3f} noise={noise:.3f}"
          f" rule < -noise both orders -> {'PASS' if c1 else 'FAIL'}")
    ir = [r for r in runs.values() if r["arm"] == "i4"]
    share = med([window_delta(r, "prefetch_hits") / window_delta(r, "gpu_misses")
                 for r in ir if window_delta(r, "gpu_misses")])
    c2 = share >= 0.5
    print(f"DAY50 CLAUSE (ii) i4 prefetch_hits/gpu_misses per window token median={share:.3f} rule >=0.5 -> "
          f"{'PASS' if c2 else 'FAIL'}")
    print(f"DAY50 PREFETCH rig={rig} integrity={integrity} clause_i={'PASS' if c1 else 'FAIL'}"
