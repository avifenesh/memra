#!/usr/bin/env python3
"""Day 73 sampler for cell `compact` (research/spill-c-20260919/DAY73.md section 1): DAY71's sampler (as of its
section 3) with two more row kinds, `B` and `P`; log only,
...
- `B` (DAY73) every `/proc/buddyinfo` line: `node zone:count,count,...` (free blocks per order).
Every 1 s:
- `P` (DAY73) each `run-gen` process: `VmRSS`, `VmLck`, `VmPin` of its `status` and `AnonHugePages`, `Shared_Hugetlb`,
  `Locked`, `Swap` of its `smaps_rollup` (kB), as `key=value` pairs;
- `T` every host thread whose user or system time moved since the last pass (DAY70's rows).

usage: day73-sampler.py <out.tsv>
"""
import glob
import os
import sys
...
def main():
    ...
    while True:
        ...
        for line in (read("/proc/buddyinfo") or "").splitlines():
            f = line.replace(",", " ").split()
            if len(f) > 4 and f[0] == "Node":
                rows.append(f"{t}\tB\t{f[1]} {f[3]}\t{','.join(f[4:])}")
        ...
        if now >= next_threads:
            ...
            for pid, comm in run_gens():
                ...
                for line in (read(f"/proc/{pid}/status") or "").splitlines():
                    k, _, v = line.partition(":")
                    if k in ("VmRSS", "VmLck", "VmPin"):
                        vals.append(f"{k}={v.split()[0] if v.split() else '?'}")
                for line in (read(f"/proc/{pid}/smaps_rollup") or "").splitlines():
                    k, _, v = line.partition(":")
                    if k in ("AnonHugePages", "Shared_Hugetlb", "Locked", "Swap"):
                        vals.append(f"{k}={v.split()[0] if v.split() else '?'}")
                rows.append(f"{t}\tP\t{pid}\t{','.join(vals)}")
        out.write("\n".join(rows) + "\n")
