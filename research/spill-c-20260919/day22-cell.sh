#!/usr/bin/env bash
# day22-cell.sh <cell> <model.gguf> <server_bin> <out_root> [cache_mb] [lock_fd]
# The day-21 cell driver plus the failure gate's whole-budget arm. One host-tier gate under one named
# environment. Without a lock fd the gate takes the canonical GPU lock itself (`flock -n`, MEMRA_GPU_LOCK,
# default the 5090 lock); a busy lock is retried 15 x 120 s and the holder is never signalled. With a lock
# fd (the collector's `--external-lock`, the fd replacing @COLLECTOR_LOCK_FD@) the gate inherits the
# collector's hold. Records nvidia-smi compute apps before and after, the binary digest, the tree SHA, the
# verdict line and the exit code. Every cell is executed-not-qualified development evidence.
# cells: failure-{default,plain}-{off,on}  failure-{default,plain}-pct100-{off,on}
#        fault-{default,plain}  identity-{default,plain}-{off,on}
# `-pct100-` sets MEMRA_KV_HOST_TENANT_PCT=100 (the server disarms the tenant share cap; the pool-full cell
# then reaches the insert-path `skip demote: entry X MB > host budget B MB` line after the D2H copy, and the
# gate asserts that arm; day 21 asserted it by pattern against a banked log only).
set -uo pipefail
...
[[ $CELL == *-pct100-* ]] && ENVS+=("MEMRA_KV_HOST_TENANT_PCT=100")
