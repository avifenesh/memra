#!/usr/bin/env bash
# DAY64: a control-flow dry run of day64-box.sh in a sandbox: the /root paths redirected under a temp dir, a tiny
...
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D64_BUILDS="c60=1 i13=2 i14=3 i15=4" bash "$D/day64-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== run $run calls"
