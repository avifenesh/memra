#!/usr/bin/env bash
# DAY64 section 5: a control-flow dry run of day64b-box.sh in a sandbox: the /root paths redirected under a temp dir, a tiny
...
sed -e "s#/root#$D/root#g" -e "s#df27a780435b7b45c2597536112ea3cb091f8544c3d0c3318d9f4258b31f7adf#$SA#" \
    "$L/day64b-box.sh" > "$D/day64b-box.sh"
...
for b in run-gen-c60 run-gen-i13 run-gen-i14 run-gen-i15; do touch \$OUT/bins/\$b; chmod +x \$OUT/bins/\$b; done
STUB
...
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D64_BUILDS="c60=1 i13=2 i14=3 i15=4" bash "$D/day64b-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== run $run calls"
