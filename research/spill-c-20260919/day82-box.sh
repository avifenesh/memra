#!/usr/bin/env bash
# Day 82: the target-card sitting of DAY82.md section 2 (cell i20, OWED C11), one RTX PRO 6000 Blackwell
# Workstation Edition on the 285K class. In order: provenance (tree, the artifact's SHA-256 against the registered
# value, the card, CPU and memory), the builds (day63-box-build.sh in a separate build worktree, labels i15, i18 and i20),
# then the cell i20 (day82-cell.sh), one collector hold under
...
# After the reader, the profiler's reports and SQLite exports move to $R/profiles-hash-only/ with their SHA-256 in
# $R/profiles.sha256 (too large to mirror; the window rows the reader cut from them stay in the cell's ev/).
# usage: D82_BUILDS="i15=<sha> i18=<sha> i20=<sha>" day82-box.sh   (paths below; the lead stages the artifact and the two worktrees)
set -uo pipefail
...
: "${D82_BUILDS:?set D82_BUILDS to the label=sha list of DAY82 section 2a}"
mkdir -p "$R"
...
for b in run-gen-i15 run-gen-i18 run-gen-i20; do
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        i20) python3 "$L/day82-read.py" "$R/$cell" --rig pro-single ;;
    esac > "$R/$cell/reading.log" 2>&1
    echo "$cell reader rc=$?" | tee -a "$R/box-driver.log"
    mkdir -p "$R/profiles-hash-only"
    for f in "$out"/ev/*.nsys-rep "$out"/ev/*.sqlite; do [ -f "$f" ] && mv "$f" "$R/profiles-hash-only/"; done
    (cd "$R/profiles-hash-only" && sha256sum -- * 2>/dev/null) > "$R/profiles.sha256"
    touch "$R/$cell.done"
}
cell_run i20 day82-cell.sh 7200
