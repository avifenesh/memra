#!/usr/bin/env bash
# Day 67 cell `probe` (lane/spill-c-20260919, research/spill-c-20260919/DAY67.md section 1, registered before this
# script): OWED C12, REF and the door (I15) from one binary (label p67, which adds run-gen --cpu-probe) on DAY65's ONE
# pin, --cpu-probe on every run (after all timing); the 250 ms placement and clock sampler of DAY66 and, every 1 s, the
# main thread's /proc/<pid>/sched migration and switch counts (no smaps_rollup). Order 1 (ref, i15) x 10, order 2
# (i15, ref) x 10; the day-18 overlap environment.
...
# usage: day67-cell.sh probe <lockfd>
set -uo pipefail
cell=$1; fd=$2
...
case $cell in
probe)
    sha256sum "$D40_BINS/run-gen-p67" | tee "$EV/binary.sha256"
    ...
    arm() { # $1 ref|i15  $2 label
        ...
        run_gen "$2" taskset -c "$one" env MEMRA_MOE_RESIDENT=0 MEMRA_NGEN=32 MEMRA_MOE_SLOTS=9986 "${pre[@]}" "$D40_BINS/run-gen-p67" "$D40_ART" 55 88 13 "${door[@]}" --cpu-probe
    }
    ...
    echo "probe cell done: $(cat "$EV"/*.exit | sort | uniq -c | tr '\n' ' ')"
    ;;
