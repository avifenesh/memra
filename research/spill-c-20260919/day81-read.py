#!/usr/bin/env python3
"""Day 81 reader for cell `i19` (research/spill-c-20260919/DAY81.md section 2, registered before the cell's script):
integrity (one host demand sequence within each door arm; I19's printed beside I18's, not compared, since the
prefetch order changes it), DAY64 section 5's admissibility clause, I19 against I18 (`DAY61.md` section 2's improves / regresses / flat, gen-only primary, the window beside it), the door against REF,
I19C's dispatch-clock brackets, and Part B (REF and I19 under Nsight Systems, `day72-read.py`'s window reading),
deciding nothing. Then the verdict line.

usage: day81-read.py <cell-dir> [--rig NAME]
"""
import hashlib
...
def main():
    ...
    for label, r in runs.items():
        ...
        if (r["arm"] == "i19c") != bool(r["clocks"]):
            fails.append(f"{label} dispatch-clock lines {'missing' if r['arm'] == 'i19c' else 'without the flag'}")
    ...
    for arm in DOORS:
        ...
        print(f"DAY81 host demand sequence {arm} sha256 {sorted(traces)[0][:16] if traces else '-'}"
              f" lines={sorted({r['trace_lines'] for r in runs.values() if r['arm'] == arm})}")
    integrity = "ok" if not fails else "FAIL"
    print(f"DAY81 I19 CHECKS rig={rig} runs={len(runs)} integrity={integrity}"
          + ("" if not fails else " failed=" + "; ".join(fails)))
    if integrity != "ok":
        print(f"DAY81 VERDICT rig={rig} integrity=FAIL -> void")
        return 1
    ...
    print(f"DAY81 ADMISSIBILITY rig={rig} ceiling={ADMISSIBLE_IQR} max_iqr_gen={worst['gen_s']:.4f}"
    ...
    for key, label in (("gen_s", "gen-only decode"), ("window_s", "steady window")):
        print(f"DAY81 {label} medians (N=10 each): " + " ".join(f"{a}={med(values(a, key)):.3f}" for a in ARMS))
        noise, diff = compare("i19", "i18", key)
        if all(d < -noise for d in diff.values()):
            ...
            name = "flat"
        step[key] = name
        print(f"DAY81 STEP i19_vs_i18 {label}: pooled={diff[None]:+.4f} o1={diff['o1']:+.4f} o2={diff['o2']:+.4f}"
              f" noise={noise:.4f} -> {name}")
    door = "i18" if step["gen_s"] == "regresses" else "i19"
    out = {}
    for key, label in (("gen_s", "gen-only decode"), ("window_s", "steady window")):
        ...
        if all(d < -noise for d in diff.values()):
            ...
            name = "matches"
        out[key] = name
        print(f"DAY81 DOOR {door}_vs_ref {label}: pooled={diff[None]:+.4f} o1={diff['o1']:+.4f} o2={diff['o2']:+.4f}"
              f" noise={noise:.4f} -> {name}")
    # I19C's brackets per window token (window minus warm over the 32 window tokens), medians over its runs.
    ...
    per = {k: med([(r["clocks"]["window"].get(k, 0) - r["clocks"]["warm"].get(k, 0)) / N / 1e6
                   for r in runs.values() if r["arm"] == "i19c" and "window" in r["clocks"] and "warm" in r["clocks"]])
           for k in keys}
    print("DAY81 I19C brackets per window token (ms, medians): " + " ".join(f"{k}={v:.4f}" for k, v in per.items()))
    got = {"ref": [], "on": []}
    for label in ("p-ref-r1", "p-on-r1", "p-on-r2", "p-ref-r2"):
        res, why = d72.profiled(ev, label)
        if res is None:
            print(f"DAY81 B {label}: not read ({why})")
            continue
        got[label.split("-")[1]].append(res)
        print(f"DAY81 B {label}: window_ms={res['window_ms']:.1f} gpu_busy={res['gpu_busy']:.4f}"
    ...
    if len(got["ref"]) == 2 and len(got["on"]) == 2:
        ...
        print("DAY81 B i19_minus_ref per window token (ms, medians of two; deciding nothing): "
              + " ".join(f"{k}={v:+.4f}" for k, v in diff.items()))
    tail = (f"i19={step['gen_s']} door={door} vs_ref={out['gen_s']}"
            f" (window: i19={step['window_s']} vs_ref={out['window_s']})")
    if not admissible:
        print(f"DAY81 VERDICT rig={rig} integrity=ok -> void (inadmissible) [as read: {tail}]")
    else:
        print(f"DAY81 VERDICT rig={rig} integrity=ok {tail}")
    return 0
