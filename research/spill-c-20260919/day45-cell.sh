#!/usr/bin/env bash
# Day 45 cell `fill` (lane/spill-c-20260919, research/spill-c-20260919/DAY45.md section 1, pre-registered before any
# fill code): improvement I3, the host fill. One collector lock hold. Arms: off (run-gen-fill, no door), i9g
# (run-gen-i9, the day-44 binary, --expert-bank-host-bytes=17179869184), fill (run-gen-fill, the same budget); every
# door arm with --expert-bank-stages. Order 1 (off, i9g, fill) x 5, order 2 (fill, i9g, off) x 5; the day-18 overlap
# environment.
...
# usage: day45-cell.sh fill <lockfd>
set -uo pipefail
cell=$1; fd=$2
...
case $cell in
fill)
    sha256sum "$D40_BINS/run-gen-i9" "$D40_BINS/run-gen-fill" | tee "$EV/binary.sha256"
    stat -c '%n %s %Y' "$D40_ART" | tee "$EV/artifact.stat"
    [ -f "$D40_ART.sha256" ] && cp "$D40_ART.sha256" "$EV/artifact.sha256"
    arm() { # $1 off|i9g|fill  $2 label
        local bin=$D40_BINS/run-gen-fill door=()
        case $1 in
            i9g) bin=$D40_BINS/run-gen-i9; door=(--experts-via-tier --expert-bank-stages --expert-bank-host-bytes=17179869184) ;;
            fill) door=(--experts-via-tier --expert-bank-stages --expert-bank-host-bytes=17179869184) ;;
        esac
        run_gen "$2" env MEMRA_MOE_RESIDENT=0 MEMRA_NGEN=32 MEMRA_MOE_SLOTS=9986 "$bin" "$D40_ART" 55 88 13 "${door[@]}"
    }
    for i in 1 2 3 4 5; do arm off "o1-off-r$i"; arm i9g "o1-i9g-r$i"; arm fill "o1-fill-r$i"; done
    for i in 1 2 3 4 5; do arm fill "o2-fill-r$i"; arm i9g "o2-i9g-r$i"; arm off "o2-off-r$i"; done
    echo "fill cell done: $(cat "$EV"/*.exit | sort | uniq -c | tr '\n' ' ')"
    ;;
