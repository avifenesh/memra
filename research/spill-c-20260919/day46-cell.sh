#!/usr/bin/env bash
# Day 46 cell `nodrain` (lane/spill-c-20260919, research/spill-c-20260919/DAY46.md section 1, pre-registered before
# any I1 code): improvement I1, leases retired on events, no drain. One collector lock hold. Arms: off (run-gen-i1, no
# door), fill (run-gen-fill, the day-45 binary), i1 (run-gen-i1); both door arms with --expert-bank-stages and
# --expert-bank-host-bytes=17179869184. Order 1 (off, fill, i1) x 5, order 2 (i1, fill, off) x 5; the day-18 overlap
# environment.
...
# usage: day46-cell.sh nodrain <lockfd>
set -uo pipefail
cell=$1; fd=$2
...
case $cell in
nodrain)
    sha256sum "$D40_BINS/run-gen-fill" "$D40_BINS/run-gen-i1" | tee "$EV/binary.sha256"
    stat -c '%n %s %Y' "$D40_ART" | tee "$EV/artifact.stat"
    [ -f "$D40_ART.sha256" ] && cp "$D40_ART.sha256" "$EV/artifact.sha256"
    arm() { # $1 off|fill|i1  $2 label
        local bin=$D40_BINS/run-gen-i1 door=()
        case $1 in
            fill) bin=$D40_BINS/run-gen-fill; door=(--experts-via-tier --expert-bank-stages --expert-bank-host-bytes=17179869184) ;;
            i1) door=(--experts-via-tier --expert-bank-stages --expert-bank-host-bytes=17179869184) ;;
        esac
        run_gen "$2" env MEMRA_MOE_RESIDENT=0 MEMRA_NGEN=32 MEMRA_MOE_SLOTS=9986 "$bin" "$D40_ART" 55 88 13 "${door[@]}"
    }
    for i in 1 2 3 4 5; do arm off "o1-off-r$i"; arm fill "o1-fill-r$i"; arm i1 "o1-i1-r$i"; done
    for i in 1 2 3 4 5; do arm i1 "o2-i1-r$i"; arm fill "o2-fill-r$i"; arm off "o2-off-r$i"; done
    echo "nodrain cell done: $(cat "$EV"/*.exit | sort | uniq -c | tr '\n' ' ')"
    ;;
