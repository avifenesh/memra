#!/usr/bin/env python3
"""Day 82 reader for cell `i20` (research/spill-c-20260919/DAY82.md sections 3 and 2a, registered before the cell's
script): integrity (one host demand sequence across the door arms I15, I18, I20, the same program), DAY64 section 5's
admissibility clause, I20 against I18 (`DAY61.md` section 2's improves / regresses / flat, gen-only primary, the window
beside it), I20 against I15 beside it (deciding nothing), the door against REF,
I20C's dispatch-clock brackets, and Part B (REF and I20 under Nsight Systems, `day72-read.py`'s window reading),
deciding nothing. Then the verdict line.

usage: day82-read.py <cell-dir> [--rig NAME]
"""
import hashlib
...
ARMS = ("ref", "i15", "i18", "i20", "i20c")
DOORS = ("i15", "i18", "i20", "i20c")
...
def main():
    ...
    for label, r in runs.items():
        ...
        if (r["arm"] == "i20c") != bool(r["clocks"]):
            fails.append(f"{label} dispatch-clock lines {'missing' if r['arm'] == 'i20c' else 'without the flag'}")
    if len({r.get("tokens") for r in runs.values()}) != 1:
        fails.append("tapes differ across arms")
    # DAY82 section 2a: I15's, I18's and I20's host demand sequences equal (the same program).
    ...
    for arm in DOORS:
        ...
        print(f"DAY82 host demand sequence {arm} sha256 {sorted(traces)[0][:16] if traces else '-'}"
              f" lines={sorted({r['trace_lines'] for r in runs.values() if r['arm'] == arm})}")
    integrity = "ok" if not fails else "FAIL"
    print(f"DAY82 I20 CHECKS rig={rig} runs={len(runs)} integrity={integrity}"
          + ("" if not fails else " failed=" + "; ".join(fails)))
    if integrity != "ok":
        print(f"DAY82 VERDICT rig={rig} integrity=FAIL -> void")
        return 1
    ...
    print(f"DAY82 ADMISSIBILITY rig={rig} ceiling={ADMISSIBLE_IQR} max_iqr_gen={worst['gen_s']:.4f}"
    ...
    for key, label in (("gen_s", "gen-only decode"), ("window_s", "steady window")):
        print(f"DAY82 {label} medians (N=10 each): " + " ".join(f"{a}={med(values(a, key)):.3f}" for a in ARMS))
        noise, diff = compare("i20", "i18", key)
        if all(d < -noise for d in diff.values()):
            ...
            name = "flat"
        step[key] = name
        print(f"DAY82 STEP i20_vs_i18 {label}: pooled={diff[None]:+.4f} o1={diff['o1']:+.4f} o2={diff['o2']:+.4f}"
              f" noise={noise:.4f} -> {name}")
    # DAY82 section 2a: the cumulative cut since I15, printed beside, deciding nothing.
    for key, label in (("gen_s", "gen-only decode"), ("window_s", "steady window")):
        noise, diff = compare("i20", "i15", key)
        name = ("improves" if all(d < -noise for d in diff.values())
                else "regresses" if diff["o1"] > noise and diff["o2"] > noise else "flat")
        print(f"DAY82 BESIDE i20_vs_i15 {label}: pooled={diff[None]:+.4f} o1={diff['o1']:+.4f} o2={diff['o2']:+.4f}"
              f" noise={noise:.4f} -> {name} (deciding nothing)")
    door = "i18" if step["gen_s"] == "regresses" else "i20"
    out = {}
    for key, label in (("gen_s", "gen-only decode"), ("window_s", "steady window")):
        ...
        if all(d < -noise for d in diff.values()):
            ...
            name = "matches"
        out[key] = name
        print(f"DAY82 DOOR {door}_vs_ref {label}: pooled={diff[None]:+.4f} o1={diff['o1']:+.4f} o2={diff['o2']:+.4f}"
              f" noise={noise:.4f} -> {name}")
    # I20C's brackets per window token (window minus warm over the 32 window tokens), medians over its runs.
    ...
    per = {k: med([(r["clocks"]["window"].get(k, 0) - r["clocks"]["warm"].get(k, 0)) / N / 1e6
                   for r in runs.values() if r["arm"] == "i20c" and "window" in r["clocks"] and "warm" in r["clocks"]])
           for k in keys}
    print("DAY82 I20C brackets per window token (ms, medians): " + " ".join(f"{k}={v:.4f}" for k, v in per.items()))
    got = {"ref": [], "on": []}
    for label in ("p-ref-r1", "p-on-r1", "p-on-r2", "p-ref-r2"):
        res, why = d72.profiled(ev, label)
        if res is None:
            print(f"DAY82 B {label}: not read ({why})")
            continue
        got[label.split("-")[1]].append(res)
        print(f"DAY82 B {label}: window_ms={res['window_ms']:.1f} gpu_busy={res['gpu_busy']:.4f}"
    ...
    if len(got["ref"]) == 2 and len(got["on"]) == 2:
        ...
        print("DAY82 B i20_minus_ref per window token (ms, medians of two; deciding nothing): "
              + " ".join(f"{k}={v:+.4f}" for k, v in diff.items()))
    tail = (f"i20={step['gen_s']} door={door} vs_ref={out['gen_s']}"
            f" (window: i20={step['window_s']} vs_ref={out['window_s']})")
    if not admissible:
        print(f"DAY82 VERDICT rig={rig} integrity=ok -> void (inadmissible) [as read: {tail}]")
    else:
        print(f"DAY82 VERDICT rig={rig} integrity=ok {tail}")
    return 0
