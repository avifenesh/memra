#!/usr/bin/env bash
# Day 78: the cell `pages` (DAY78.md section 1, OWED C12) on any Ryzen 9 9950X machine, then the 285K class,
# one RTX PRO 6000 Blackwell Workstation Edition; D78_RIG names the machine in the reading (default pro-single). It can
...
# CPU and memory), the build (day63-box-build.sh in a separate build worktree, label p78), then the cell through the
...
# id or price here.
# usage: D78_BUILDS="p78=<sha>" [D78_RIG=<name>] day78-box.sh   (paths below; the lead stages the artifact and the two
...
RIG=${D78_RIG:-pro-single}
R=/root/spill-receipts/c-day78-$RIG
...
: "${D78_BUILDS:?set D78_BUILDS to the label=sha list of DAY78 section 1a}"
mkdir -p "$R"
...
[ -x "$R/bins/run-gen-p78" ] || { echo "missing run-gen-p78, stopping" | tee -a "$R/box-driver.log"; exit 1; }

echo "cpu cap: each run pinned by the cell to its arm's 12 CPUs (DAY78 section 1); the runner unpinned" | tee -a "$R/provenance.log"
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        pages) python3 "$L/day78-read.py" "$R/$cell" --rig "$RIG" ;;
    esac > "$R/$cell/reading.log" 2>&1
    echo "$cell reader rc=$?" | tee -a "$R/box-driver.log"
    touch "$R/$cell.done"
}
cell_run pages day78-cell.sh 7200
