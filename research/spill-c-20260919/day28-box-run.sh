#!/usr/bin/env bash
# Day 28 target-card runner (one RTX PRO 6000 Blackwell, collector rig `pro-single`, lock /tmp/memra-gpu.lock):
# waits for the tree's server build receipt, then runs day28-stall-cell.sh ONCE through
# tools/tier-battery.py --external-lock (one lock hold, ten boots), bounded lock retries (60 x 120 s; lane A
# shares the card today, the holder is never inspected or signalled). No host, id or price here.
# usage: day28-box-run.sh <tree> <receipts_root> <model.gguf>
set -uo pipefail
TREE=$1; R=$2; MODEL=$3
...
for attempt in $(seq 0 60); do
    ...
    python3 tools/tier-battery.py --rig pro-single --timeout 5400 --out "$out" --external-lock \
        --execute bash research/spill-c-20260919/day28-stall-cell.sh @COLLECTOR_LOCK_FD@ "$TREE" "$R" "$MODEL" "$BIN" \
        > "$out-driver.log" 2>&1
    rc=$?
    echo $rc > "$out.exit"
