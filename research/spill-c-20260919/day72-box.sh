#!/usr/bin/env bash
# Day 72: the target-card sitting of DAY72.md section 1 (cell gap15, OWED C11), one RTX PRO 6000 Blackwell
# Workstation Edition on the 285K class. In order: provenance (tree, the artifact's SHA-256 against the registered
# value, the card, CPU and memory), the build (day63-box-build.sh in a separate build worktree, label i15), then the
# cell gap15 (day72-cell.sh), one collector hold under
...
# usage: D72_BUILDS="i15=<sha>" day72-box.sh   (paths below; the lead stages the artifact and the two worktrees)
set -uo pipefail
...
: "${D72_BUILDS:?set D72_BUILDS to the label=sha list of DAY72 section 1a}"
mkdir -p "$R"
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        gap15) python3 "$L/day72-read.py" "$R/$cell" --rig pro-single ;;
...
cell_run gap15 day72-cell.sh 7200
