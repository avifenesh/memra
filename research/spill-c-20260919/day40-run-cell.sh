#!/usr/bin/env bash
# Day 40 cell runner: one cell through the canonical collector with bounded lock retries (up to 30 x 120 s;
# another lane may hold the card), never signals a holder; every attempt keeps its own driver log. Before each
# attempt it waits, bounded (15 x 120 s), for no compute app on the card and at least D40_MIN_AVAIL_GB of host
# MemAvailable (the door's cell pins the loader's 15 GB of expert slabs beside the page-cached artifact).
...
wait_free() {
  ...
  echo "$(date -u +%FT%TZ) $cell: card or host never freed in 15 waits" >> "$D40_R/waits.log"
  return 1
}
