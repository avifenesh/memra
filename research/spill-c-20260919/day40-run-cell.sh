...
# Before each attempt it waits, bounded (D40_WAITS x D40_POLL_S s, defaults 15 and 120), for no compute app on the card and at
...
# 2026-09-24 (the RTX 5090 shared with another lane's back-to-back boots): the poll interval and how often a wait is
# logged are settable; the defaults are the day-40 runner's (120 s, every wait).
poll_s=${D40_POLL_S:-120}
log_every=${D40_LOG_EVERY:-1}
...
for attempt in $(seq 0 "$lock_tries"); do
  ...
  if grep -q "canonical rig lock\|BlockingIOError\|Resource temporarily unavailable" "$out-driver.log" && [ ! -f "$out/CELL.jsonl" ]; then
    echo "$(date -u +%FT%TZ) $cell attempt $attempt: lock busy, sleeping ${poll_s}s" >> "$D40_R/lock-retries.log"
