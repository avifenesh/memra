#!/usr/bin/env bash
# Day 75 cell `i16` (lane/spill-c-20260919, research/spill-c-20260919/DAY75.md section 2, registered before this
# script): I16 against I15 and the door against REF. One collector lock hold. Arms: ref (run-gen-i15,
# MEMRA_MOE_PREFETCH=1), i15 (run-gen-i15 with the door), i16 (run-gen-i16 with the door), i16c (i16 with
# --moe-dispatch-clock); order 1 (ref, i15, i16, i16c) x 5, order 2 reversed x 5. Then ref and i16 under Nsight
# Systems (--trace=cuda, no sampling, no context switches), order ref, i16, i16, ref, each exported to SQLite. The
...
# usage: day75-cell.sh i16 <lockfd>
set -uo pipefail
cell=$1; fd=$2
...
case $cell in
i16)
    sha256sum "$D40_BINS/run-gen-i15" "$D40_BINS/run-gen-i16" | tee "$EV/binary.sha256"
    stat -c '%n %s %Y' "$D40_ART" | tee "$EV/artifact.stat"
    [ -f "$D40_ART.sha256" ] && cp "$D40_ART.sha256" "$EV/artifact.sha256"
    arm() { # $1 ref|i15|i16|i16c  $2 label
        local pre=() door=(--experts-via-tier --expert-bank-host-bytes=17179869184) bin=run-gen-i16
        case $1 in
            ref) pre=(MEMRA_MOE_PREFETCH=1); door=(); bin=run-gen-i15 ;;
            i15) bin=run-gen-i15 ;;
            i16c) door+=(--moe-dispatch-clock) ;;
        esac
        run_gen "$2" env MEMRA_MOE_RESIDENT=0 MEMRA_NGEN=32 MEMRA_MOE_SLOTS=9986 "${pre[@]}" "$D40_BINS/$bin" "$D40_ART" 55 88 13 "${door[@]}"
    }
    for i in 1 2 3 4 5; do arm ref "o1-ref-r$i"; arm i15 "o1-i15-r$i"; arm i16 "o1-i16-r$i"; arm i16c "o1-i16c-r$i"; done
    for i in 1 2 3 4 5; do arm i16c "o2-i16c-r$i"; arm i16 "o2-i16-r$i"; arm i15 "o2-i15-r$i"; arm ref "o2-ref-r$i"; done
    ...
    if [ -z "$NSYS" ]; then
        ...
        prof() { # $1 ref|i16  $2 label
            local pre=() door=() bin=run-gen-i16
            case $1 in
                ref) pre=(MEMRA_MOE_PREFETCH=1); bin=run-gen-i15 ;;
                i16) door=(--experts-via-tier --expert-bank-host-bytes=17179869184) ;;
            esac
        ...
        prof ref p-ref-r1; prof i16 p-on-r1; prof i16 p-on-r2; prof ref p-ref-r2
    fi
    echo "i16 cell done: $(cat "$EV"/*.exit | sort | uniq -c | tr '\n' ' ')"
    ;;
