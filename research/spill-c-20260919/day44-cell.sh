#!/usr/bin/env bash
# Day 44 cell `mapped` (lane/spill-c-20260919, research/spill-c-20260919/DAY44.md sections 1 and 1a, pre-registered
# before any I9 code): improvement I9, no pinned duplicate of the bank under the door. One collector lock hold. Arms:
# off (run-gen-i9, no door), i6 (run-gen-i6, the day-43 binary, default budget), i9 (run-gen-i9, default budget);
# every door arm with --expert-bank-stages. Order 1 (off, i6, i9) x 5, order 2 (i9, i6, off) x 5; the day-18 overlap
# environment.
...
# usage: day44-cell.sh mapped <lockfd>
set -uo pipefail
cell=$1; fd=$2
...
case $cell in
mapped)
    sha256sum "$D40_BINS/run-gen-i6" "$D40_BINS/run-gen-i9" | tee "$EV/binary.sha256"
    stat -c '%n %s %Y' "$D40_ART" | tee "$EV/artifact.stat"
    [ -f "$D40_ART.sha256" ] && cp "$D40_ART.sha256" "$EV/artifact.sha256"
    arm() { # $1 off|i6|i9  $2 label
        local bin=$D40_BINS/run-gen-i9 door=()
        case $1 in
            i6) bin=$D40_BINS/run-gen-i6; door=(--experts-via-tier --expert-bank-stages) ;;
            i9) door=(--experts-via-tier --expert-bank-stages) ;;
        esac
        run_gen "$2" env MEMRA_MOE_RESIDENT=0 MEMRA_NGEN=32 MEMRA_MOE_SLOTS=9986 "$bin" "$D40_ART" 55 88 13 "${door[@]}"
    }
    for i in 1 2 3 4 5; do arm off "o1-off-r$i"; arm i6 "o1-i6-r$i"; arm i9 "o1-i9-r$i"; done
    for i in 1 2 3 4 5; do arm i9 "o2-i9-r$i"; arm i6 "o2-i6-r$i"; arm off "o2-off-r$i"; done
    echo "mapped cell done: $(cat "$EV"/*.exit | sort | uniq -c | tr '\n' ' ')"
    ;;
