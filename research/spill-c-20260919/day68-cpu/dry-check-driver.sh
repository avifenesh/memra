#!/usr/bin/env bash
# DAY68: a control-flow dry run of day68-box.sh in a sandbox: the /root paths redirected under a temp dir, a tiny
...
for b in run-gen-p68; do touch \$OUT/bins/\$b; chmod +x \$OUT/bins/\$b; done
STUB
...
for run in 1 2; do
    : > calls.log
    PATH=$D/bin:$PATH D68_BUILDS="p68=1" bash "$D/day68-box.sh" > "run$run.log" 2>&1 || echo "run $run rc=$?"
    echo "== run $run calls"
