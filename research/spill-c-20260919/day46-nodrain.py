#!/usr/bin/env python3
"""Day 46 reader for cell `nodrain` (research/spill-c-20260919/DAY46.md section 1, registered before any I1 code).
Integrity, clause (i) I1 beats FILL in the window, clause (ii) nothing drains.

usage: day46-nodrain.py <cell-dir> [--rig NAME]
"""
...
def main():
    ...
    print(f"DAY46 NODRAIN CHECKS rig={rig} runs={len(runs)} integrity={integrity}" +
    ...
    for arm in ("fill", "i1"):
        ...
        per = {k: med([window_delta(r, k) for r in rs]) for k in
               ("gpu_misses", "host_hits", "demand_ns", "drain_ns", "sync2_ns", "wait_ns", "retire_ns", "finish_ns",
                "enqueue_ns", "miss_total_ns")}
        print(f"DAY46 ARM {arm} window_door_ms_per_token={(med(values(arm, 'window_s')) - off_w) * 1000 / N_TOKENS:.2f}"
    ...
    noise = max(iqr(values("i1", "window_s")), iqr(values("fill", "window_s")))
    diffs = {o: med(values("i1", "window_s", o)) - med(values("fill", "window_s", o)) for o in ("o1", "o2")}
    c1 = all(d < -noise for d in diffs.values())
    print(f"DAY46 CLAUSE (i) i1_minus_fill window o1={diffs['o1']:+.3f} o2={diffs['o2']:+.3f} noise={noise:.3f}"
          f" rule < -noise both orders -> {'PASS' if c1 else 'FAIL'}")
    ir = [r for r in runs.values() if r["arm"] == "i1"]
    drain = med([window_delta(r, "drain_ns") + window_delta(r, "sync2_ns") for r in ir]) / 1e6
    wait = med([window_delta(r, "wait_ns") for r in ir]) / 1e6
    c2 = drain < 0.05 and wait < 1.0
    print(f"DAY46 CLAUSE (ii) i1 drain+sync2={drain:.3f} wait={wait:.3f} ms per window token rule <0.05 and <1.0 -> "
          f"{'PASS' if c2 else 'FAIL'}")
    print(f"DAY46 NODRAIN rig={rig} integrity={integrity} clause_i={'PASS' if c1 else 'FAIL'}"
