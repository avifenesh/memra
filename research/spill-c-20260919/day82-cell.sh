#!/usr/bin/env bash
# Day 82 cell `i20` (lane/spill-c-20260919, research/spill-c-20260919/DAY82.md section 2, registered before this
# script, and section 2a's fifth arm): I20 against I18, I20 against I15 beside it, and the door against REF. One
# collector lock hold. Arms: ref (run-gen-i18, MEMRA_MOE_PREFETCH=1), i15 (run-gen-i15 with the door), i18
# (run-gen-i18 with the door), i20 (run-gen-i20 with the door), i20c (i20 with --moe-dispatch-clock); order 1 (ref,
# i15, i18, i20, i20c) x 5, order 2 reversed x 5. Then ref and i20 under Nsight
# Systems (--trace=cuda, no sampling, no context switches), order ref, i20, i20, ref, each exported to SQLite. The
...
# usage: day82-cell.sh i20 <lockfd>
set -uo pipefail
cell=$1; fd=$2
...
case $cell in
i20)
    sha256sum "$D40_BINS/run-gen-i15" "$D40_BINS/run-gen-i18" "$D40_BINS/run-gen-i20" | tee "$EV/binary.sha256"
    stat -c '%n %s %Y' "$D40_ART" | tee "$EV/artifact.stat"
    [ -f "$D40_ART.sha256" ] && cp "$D40_ART.sha256" "$EV/artifact.sha256"
    arm() { # $1 ref|i18|i20|i20c  $2 label
        local pre=() door=(--experts-via-tier --expert-bank-host-bytes=17179869184) bin=run-gen-i20
        case $1 in
            ref) pre=(MEMRA_MOE_PREFETCH=1); door=(); bin=run-gen-i18 ;;
            i15) bin=run-gen-i15 ;;
            i18) bin=run-gen-i18 ;;
            i20c) door+=(--moe-dispatch-clock) ;;
        esac
        run_gen "$2" env MEMRA_MOE_RESIDENT=0 MEMRA_NGEN=32 MEMRA_MOE_SLOTS=9986 "${pre[@]}" "$D40_BINS/$bin" "$D40_ART" 55 88 13 "${door[@]}"
    }
    for i in 1 2 3 4 5; do arm ref "o1-ref-r$i"; arm i15 "o1-i15-r$i"; arm i18 "o1-i18-r$i"; arm i20 "o1-i20-r$i"; arm i20c "o1-i20c-r$i"; done
    for i in 1 2 3 4 5; do arm i20c "o2-i20c-r$i"; arm i20 "o2-i20-r$i"; arm i18 "o2-i18-r$i"; arm i15 "o2-i15-r$i"; arm ref "o2-ref-r$i"; done
    ...
    if [ -z "$NSYS" ]; then
        ...
        prof() { # $1 ref|i20  $2 label
            local pre=() door=() bin=run-gen-i20
            case $1 in
                ref) pre=(MEMRA_MOE_PREFETCH=1); bin=run-gen-i18 ;;
                i20) door=(--experts-via-tier --expert-bank-host-bytes=17179869184) ;;
            esac
        ...
        prof ref p-ref-r1; prof i20 p-on-r1; prof i20 p-on-r2; prof ref p-ref-r2
    fi
    echo "i20 cell done: $(cat "$EV"/*.exit | sort | uniq -c | tr '\n' ' ')"
    ;;
