#!/usr/bin/env bash
# Day 80: the cells `regtime` then `regpool` (DAY80.md section 1, OWED C12) on any Ryzen 9 9950X machine, then the 285K class,
# one RTX PRO 6000 Blackwell Workstation Edition; D80_RIG names the machine in the reading (default pro-single). It can
...
# CPU and memory), the build (day63-box-build.sh in a separate build worktree, label p80), then the cell through the
...
# id or price here.
# usage: D80_BUILDS="p80=<sha>" [D80_RIG=<name>] day80-box.sh   (paths below; the lead stages the artifact and the two
...
RIG=${D80_RIG:-pro-single}
R=/root/spill-receipts/c-day80-$RIG
...
: "${D80_BUILDS:?set D80_BUILDS to the label=sha list of DAY80 section 1a}"
mkdir -p "$R"
...
[ -x "$R/bins/run-gen-p80" ] || { echo "missing run-gen-p80, stopping" | tee -a "$R/box-driver.log"; exit 1; }

echo "cpu cap: each run pinned by the cell to its arm's 12 CPUs (DAY80 section 1); the runner unpinned" | tee -a "$R/provenance.log"
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        regtime) python3 "$L/day80-regtime-read.py" "$R/$cell" --rig "$RIG" ;;
        regpool) python3 "$L/day80-regpool-read.py" "$R/$cell" --rig "$RIG" ;;
    esac > "$R/$cell/reading.log" 2>&1
    echo "$cell reader rc=$?" | tee -a "$R/box-driver.log"
    touch "$R/$cell.done"
}
# regtime first (no fragmentation), then regpool (it needs the page-cache eviction's MemFree at its own start).
cell_run regtime day80-cell.sh 5400
cell_run regpool day80-cell.sh 7200
