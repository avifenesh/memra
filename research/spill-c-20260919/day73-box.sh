#!/usr/bin/env bash
# Day 73: the cell `compact` (DAY73.md section 1, OWED C12) on any Ryzen 9 9950X machine, one RTX PRO 6000 Blackwell
# Workstation Edition; D73_RIG names the machine in the reading (default pro-single). It can follow DAY71's default
# half in the same sitting (its own receipts root). In order: provenance (tree, the artifact's SHA-256 against the registered value, the card,
...
# id or price here.
# usage: D73_BUILDS="p71=<sha>" [D73_RIG=<name>] day73-box.sh   (paths below; the lead stages the artifact and the two
# worktrees; strace is used when the box has it, else Part B reads not_read)
set -uo pipefail
...
RIG=${D73_RIG:-pro-single}
R=/root/spill-receipts/c-day73-$RIG
...
: "${D73_BUILDS:?set D73_BUILDS to the label=sha list of DAY73 section 1a}"
mkdir -p "$R"
...
echo "cpu cap: each run pinned by the cell to its arm's 12 CPUs (DAY73 section 1); the runner unpinned" | tee -a "$R/provenance.log"
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        compact) python3 "$L/day73-read.py" "$R/$cell" --rig "$RIG" ;;
    esac > "$R/$cell/reading.log" 2>&1
    echo "$cell reader rc=$?" | tee -a "$R/box-driver.log"
    touch "$R/$cell.done"
}
cell_run compact day73-cell.sh 5400
