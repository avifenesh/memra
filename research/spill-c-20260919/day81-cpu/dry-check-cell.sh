#!/usr/bin/env bash
# DAY81: a control-flow dry run of day81-cell.sh `i19` in a sandbox: a sandbox tree with a stub lock proof, a stub
...
echo a > "$D/art.gguf"
for b in run-gen-i18 run-gen-i19; do printf '#!/usr/bin/env bash\necho "%s $*" >> %s/calls.log\nexit 0\n' "$b" "$D" > "$D/bins/$b"; chmod +x "$D/bins/$b"; done
cat > "$D/bin/nsys" <<STUB
#!/usr/bin/env bash
...
PATH=$D/bin:$PATH D40_R=$D/R D40_BINS=$D/bins D40_TREE=$D/tree D40_LOCK=$D/none D40_ART=$D/art.gguf \
    bash "$L/day81-cell.sh" i19 0 > cell.log 2>&1
echo "cell rc=$?"
EV=$D/R/i19/ev
echo "== nsys.txt"; cat "$EV/nsys.txt"
echo "== calls: $(wc -l < calls.log) lines; per binary: $(grep -oE "^run-gen-i1[89]" calls.log | sort | uniq -c | tr "\n" " ")"; grep -E "^nsys (profile|export)" calls.log | sed "s#$D#D#g" | cut -c1-170
