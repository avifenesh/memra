#!/usr/bin/env bash
# Day 64: the target-card sitting (DAY64.md section 3), one RTX PRO 6000 Blackwell Workstation Edition. In order:
# provenance (tree, the artifact's SHA-256 against the registered value, the card, CPU and memory), the builds
# (day63-box-build.sh in a separate build worktree), then the cell i15 (day64-cell.sh), one collector hold under
...
# usage: D64_BUILDS="c60=<sha> i13=<sha> i14=<sha> i15=<sha>" day64-box.sh   (paths below; the lead stages the artifact and the
# two worktrees)
set -uo pipefail
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        i15) python3 "$L/day64-read.py" "$R/$cell" --rig pro-single ;;
    esac > "$R/$cell/reading.log" 2>&1
    echo "$cell reader rc=$?" | tee -a "$R/box-driver.log"
    touch "$R/$cell.done"
}
cell_run i15 day64-cell.sh 7200
