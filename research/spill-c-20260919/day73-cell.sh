#!/usr/bin/env bash
# Day 73 cell `compact` (lane/spill-c-20260919, research/spill-c-20260919/DAY73.md section 1, registered before this
# script): OWED C12, REF and the door (I15) from the binary p71 on DAY65's ONE pin, --cpu-probe, --cpu-probe-phases
# and (when DAY71's counters check passes) --cpu-probe-counters on every run; one sampler (day73-sampler.py: DAY71's
# rows plus buddyinfo every 250 ms and the processes' pinned, locked and huge-page memory every 1 s) pinned to a CPU
# outside the pin and its hardware-thread siblings. Part A: order 1 (ref, i15) x 5, order 2 (i15, ref) x 5. Part B:
# ref, i15, i15, ref under `strace -f -c -S calls`, untimed. The day-18 overlap environment.
...
# usage: day73-cell.sh compact <lockfd>
set -uo pipefail
cell=$1; fd=$2
...
compact)
    ...
    sampler=$!
    arm() { # $1 ref|i15  $2 label  [$3.. a wrapper's words]
        local pre=() door=(--experts-via-tier --expert-bank-host-bytes=17179869184) kind=$1 label=$2
        shift 2
        if [ "$kind" = ref ]; then pre=(MEMRA_MOE_PREFETCH=1); door=(); fi
    ...
    for i in $(seq 1 5); do arm ref "o1-ref-r$i"; arm i15 "o1-i15-r$i"; done
    for i in $(seq 1 5); do arm i15 "o2-i15-r$i"; arm ref "o2-ref-r$i"; done
    # Part B: the system calls of every thread, counted; untimed.
    if command -v strace > /dev/null; then
        strace -V 2>&1 | head -1 | tee "$EV/strace.txt"
        for spec in ref:s-ref-r1 i15:s-i15-r1 i15:s-i15-r2 ref:s-ref-r2; do
            arm "${spec%%:*}" "${spec#*:}" strace -f -c -S calls -o "$EV/${spec#*:}.strace"
        done
    else
        echo "strace=none" | tee "$EV/strace.txt"
    fi
    kill "$sampler" 2>/dev/null
    host_state after
    echo "compact cell done: $(cat "$EV"/*.exit | sort | uniq -c | tr '\n' ' ')"
    ;;
