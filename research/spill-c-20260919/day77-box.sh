#!/usr/bin/env bash
# Day 77: the target-card sitting of DAY77.md section 2 (cell i17, OWED C11), one RTX PRO 6000 Blackwell
# Workstation Edition on the 285K class. In order: provenance (tree, the artifact's SHA-256 against the registered
# value, the card, CPU and memory), the builds (day63-box-build.sh in a separate build worktree, labels i15 and i17),
# then the cell i17 (day77-cell.sh), one collector hold under
...
# usage: D77_BUILDS="i15=<sha> i17=<sha>" day77-box.sh   (paths below; the lead stages the artifact and the two worktrees)
set -uo pipefail
...
: "${D77_BUILDS:?set D77_BUILDS to the label=sha list of DAY77 section 2a}"
mkdir -p "$R"
...
for b in run-gen-i15 run-gen-i17; do
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        i17) python3 "$L/day77-read.py" "$R/$cell" --rig pro-single ;;
...
cell_run i17 day77-cell.sh 7200
