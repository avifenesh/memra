#!/usr/bin/env bash
# Day 15 review cell runner: one command through the canonical collector (pro-single, /tmp/memra-gpu.lock),
# bounded lock retries (up to 75 x 120 s = 150 min, lane B's A/B holds the card for up to 90 min today),
# never kills a holder; every attempt keeps its own driver log.
