#!/usr/bin/env bash
# Day 79: the target-card sitting of DAY79.md section 2 (cell i18, OWED C11), one RTX PRO 6000 Blackwell
# Workstation Edition on the 285K class. In order: provenance (tree, the artifact's SHA-256 against the registered
# value, the card, CPU and memory), the builds (day63-box-build.sh in a separate build worktree, labels i17 and i18),
# then the cell i18 (day79-cell.sh), one collector hold under
...
# usage: D79_BUILDS="i17=<sha> i18=<sha>" day79-box.sh   (paths below; the lead stages the artifact and the two worktrees)
set -uo pipefail
...
: "${D79_BUILDS:?set D79_BUILDS to the label=sha list of DAY79 section 2a}"
mkdir -p "$R"
...
for b in run-gen-i17 run-gen-i18; do
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        i18) python3 "$L/day79-read.py" "$R/$cell" --rig pro-single ;;
...
cell_run i18 day79-cell.sh 7200
