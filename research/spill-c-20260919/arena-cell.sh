#!/usr/bin/env bash
# Arena pair cell (lane/spill-c-20260919 day 17, research/spill-c-20260919/DAY17.md "The arena cell"):
# the startup pinned arena (MEMRA_GLM5_TP_KV_HOST=1) against the pageable host tier for the SAME two
# entries, the contracts door OFF in BOTH arms (the door refuses to boot with the arena), N=5 per arm per
# order, BOTH orders (page,arena then arena,page), under ONE collector lock hold (one thermal window, the
# collector's 250 ms telemetry). Four server boots. Per boot: r1 P_A seeds E_A; r2 P_B seeds E_B and
# evicts E_A (demote 1); r3..r7 alternate P_A/P_B: each is a host hit that promotes and whose insert evicts
# the other entry (demote inside the promote window). Demotes r2..r6 (N=5) and promotes r3..r7 (N=5) are
# the observations; the server log is the receipt. MEMRA_KV_HOST_VERIFY unset (the production shape).
# Derived statement for statement from the day-16 wc-cell.sh; only the arm variable differs.
# usage: arena-cell.sh <lockfd> [cell-name]
set -uo pipefail
fd=$1
cell=${2:-arena-pair}
...
PORT=${MEMRA_GATE_PORT:-18121}
. tools/port-guard.sh
SERVER_PID=""
boot() { # $1 extra-env-string  $2 log
    memra_port_guard arena-cell "$PORT" MEMRA_GATE_PORT || return 1
...
arm() { # $1 page|arena  $2 label
    local a=$1 label=$2
    local log="$EV/$label-server.log" extra=""
    [ "$a" = arena ] && extra="MEMRA_GLM5_TP_KV_HOST=1"
    mark "$label boot"
...
: > "$EV/marks.tsv"
rc=0
arm page  o1-page  || rc=1
arm arena o1-arena || rc=1
arm arena o2-arena || rc=1
arm page  o2-page  || rc=1
echo "arena-pair cell rc=$rc"
exit $rc
