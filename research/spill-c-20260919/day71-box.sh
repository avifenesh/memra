#!/usr/bin/env bash
# Day 71: the cell `core` (DAY71.md section 1, OWED C12) on a Ryzen 9 9950X machine, one RTX PRO 6000 Blackwell
# Workstation Edition, run once on BOX15's machine (D71_RIG=pro-single, the default) and once on a second 9950X machine
# (D71_RIG=pro-single-b). In order: provenance (tree, the artifact's SHA-256 against the registered value, the card,
...
# id or price here.
# usage: D71_BUILDS="p71=<sha>" [D71_RIG=pro-single-b] day71-box.sh   (paths below; the lead stages the artifact
# and the two worktrees)
set -uo pipefail
...
RIG=${D71_RIG:-pro-single}
R=/root/spill-receipts/c-day71-$RIG
...
: "${D71_BUILDS:?set D71_BUILDS to the label=sha list of DAY71 section 1a}"
mkdir -p "$R"
...
echo "cpu cap: each run pinned by the cell to its arm's 12 CPUs (DAY71 section 1); the runner unpinned" | tee -a "$R/provenance.log"
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        core) python3 "$L/day71-read.py" "$R/$cell" --rig "$RIG" ;;
    esac > "$R/$cell/reading.log" 2>&1
    echo "$cell reader rc=$?" | tee -a "$R/box-driver.log"
    touch "$R/$cell.done"
}
cell_run core day71-cell.sh 5400
