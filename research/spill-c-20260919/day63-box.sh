#!/usr/bin/env bash
# Day 63: the target-card sitting (DAY63.md section 3), one RTX PRO 6000 Blackwell Workstation Edition. In order:
# provenance (tree, the artifact's SHA-256 against the registered value, the card, CPU and memory), the builds
# (day63-box-build.sh in a separate build worktree), then the cell i13 (day63-cell.sh), one collector hold under
...
# usage: D63_BUILDS="c60=<sha> i12=<sha> i13=<sha>" day63-box.sh   (paths below; the lead stages the artifact and the
# two worktrees)
set -uo pipefail
...
: "${D63_BUILDS:?set D63_BUILDS to the label=sha list of DAY63 section 3}"
mkdir -p "$R"
...
for b in run-gen-c60 run-gen-i12 run-gen-i13; do
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        i13) python3 "$L/day63-read.py" "$R/$cell" --rig pro-single ;;
    esac > "$R/$cell/reading.log" 2>&1
    echo "$cell reader rc=$?" | tee -a "$R/box-driver.log"
    touch "$R/$cell.done"
}
cell_run i13 day63-cell.sh 5400
