#!/usr/bin/env bash
# Day 64 rerun: the target-card sitting of DAY64.md section 5 (cell i15b, the same arms as section 3's i15), one RTX PRO 6000 Blackwell Workstation Edition. In order:
# provenance (tree, the artifact's SHA-256 against the registered value, the card, CPU and memory), the builds
# (day63-box-build.sh in a separate build worktree), then the cell i15b (day64-cell.sh), one collector hold under
...
# usage: D64_BUILDS="c60=<sha> i13=<sha> i14=<sha> i15=<sha>" day64b-box.sh   (paths below; the lead stages the artifact and the
# two worktrees)
set -uo pipefail
...
R=/root/spill-receipts/c-day64b
...
: "${D64_BUILDS:?set D64_BUILDS to the label=sha list of DAY64 section 3a}"
mkdir -p "$R"
...
for b in run-gen-c60 run-gen-i13 run-gen-i14 run-gen-i15; do
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        i15b) python3 "$L/day64-read.py" "$R/$cell" --rig pro-single --admissibility ;;
    esac > "$R/$cell/reading.log" 2>&1
    echo "$cell reader rc=$?" | tee -a "$R/box-driver.log"
    touch "$R/$cell.done"
}
cell_run i15b day64-cell.sh 7200
