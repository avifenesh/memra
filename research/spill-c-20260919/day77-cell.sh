#!/usr/bin/env bash
# Day 77 cell `i17` (lane/spill-c-20260919, research/spill-c-20260919/DAY77.md section 2, registered before this
# script): I17 against I15 and the door against REF. One collector lock hold. Arms: ref (run-gen-i15,
# MEMRA_MOE_PREFETCH=1), i15 (run-gen-i15 with the door), i17 (run-gen-i17 with the door), i17c (i17 with
# --moe-dispatch-clock); order 1 (ref, i15, i17, i17c) x 5, order 2 reversed x 5. Then ref and i17 under Nsight
# Systems (--trace=cuda, no sampling, no context switches), order ref, i17, i17, ref, each exported to SQLite. The
...
# usage: day77-cell.sh i17 <lockfd>
set -uo pipefail
cell=$1; fd=$2
...
case $cell in
i17)
    sha256sum "$D40_BINS/run-gen-i15" "$D40_BINS/run-gen-i17" | tee "$EV/binary.sha256"
    stat -c '%n %s %Y' "$D40_ART" | tee "$EV/artifact.stat"
    [ -f "$D40_ART.sha256" ] && cp "$D40_ART.sha256" "$EV/artifact.sha256"
    arm() { # $1 ref|i15|i17|i17c  $2 label
        local pre=() door=(--experts-via-tier --expert-bank-host-bytes=17179869184) bin=run-gen-i17
        case $1 in
            ref) pre=(MEMRA_MOE_PREFETCH=1); door=(); bin=run-gen-i15 ;;
            i15) bin=run-gen-i15 ;;
            i17c) door+=(--moe-dispatch-clock) ;;
        esac
        run_gen "$2" env MEMRA_MOE_RESIDENT=0 MEMRA_NGEN=32 MEMRA_MOE_SLOTS=9986 "${pre[@]}" "$D40_BINS/$bin" "$D40_ART" 55 88 13 "${door[@]}"
    }
    for i in 1 2 3 4 5; do arm ref "o1-ref-r$i"; arm i15 "o1-i15-r$i"; arm i17 "o1-i17-r$i"; arm i17c "o1-i17c-r$i"; done
    for i in 1 2 3 4 5; do arm i17c "o2-i17c-r$i"; arm i17 "o2-i17-r$i"; arm i15 "o2-i15-r$i"; arm ref "o2-ref-r$i"; done
    ...
    if [ -z "$NSYS" ]; then
        ...
        prof() { # $1 ref|i17  $2 label
            local pre=() door=() bin=run-gen-i17
            case $1 in
                ref) pre=(MEMRA_MOE_PREFETCH=1); bin=run-gen-i15 ;;
                i17) door=(--experts-via-tier --expert-bank-host-bytes=17179869184) ;;
            esac
        ...
        prof ref p-ref-r1; prof i17 p-on-r1; prof i17 p-on-r2; prof ref p-ref-r2
    fi
    echo "i17 cell done: $(cat "$EV"/*.exit | sort | uniq -c | tr '\n' ' ')"
    ;;
