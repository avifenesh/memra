#!/usr/bin/env bash
# Day 67: the cell `probe` (DAY67.md section 1, OWED C12) on the Ryzen 9 9950X class (preferably BOX15's
...
# p67), then the cell through the day-40 runner under /tmp/memra-gpu.lock (bounded waits, never signals a
...
# id or price here.
# usage: D67_BUILDS="p67=<sha>" day67-box.sh   (paths below; the lead stages the artifact and the two
# worktrees)
set -uo pipefail
...
: "${D67_BUILDS:?set D67_BUILDS to the label=sha list of DAY67 section 1a}"
mkdir -p "$R"
...
[ -x "$R/bins/run-gen-p67" ] || { echo "missing run-gen-p67, stopping" | tee -a "$R/box-driver.log"; exit 1; }

echo "cpu cap: each run pinned by the cell to its arm's 12 CPUs (DAY67 section 1); the runner unpinned" | tee -a "$R/provenance.log"
...
cell_run() { # $1 cell  $2 script  $3 timeout
    ...
    mkdir -p "$R/$cell"
    case $cell in
        probe) python3 "$L/day67-read.py" "$R/$cell" --rig pro-single ;;
    esac > "$R/$cell/reading.log" 2>&1
    echo "$cell reader rc=$?" | tee -a "$R/box-driver.log"
    touch "$R/$cell.done"
}
cell_run probe day67-cell.sh 5400
