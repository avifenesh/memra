#!/usr/bin/env bash
# Lane C RTX 5090 queue v13 (2026-09-26): DAY77's cell i17 on the RTX 5090. First run-gen-i17 built from its commit
...
# usage: bash rtx5090-queue-v13-20260926.sh
set -uo pipefail
...
log "queue v13 start at $(git -C "$T" rev-parse --short HEAD)"
systemd-run --user --scope -q -p CPUQuota=1200% -p MemoryMax=20G bash "$L/c-local-build.sh" "$T/target/c-build-wt" "$BINS" \
    i17=d4ab19f1d:memra-engine/run-gen
rc=$?; log "build rc=$rc"; [ "$rc" -eq 0 ] || exit 1
for b in run-gen-i15 run-gen-i17; do [ -x "$BINS/$b" ] || { log "missing $b, stopping"; exit 1; }; done
R=$L/rtx5090-day77; name=i17
mkdir -p "$R/builds"; printf '*.log -whitespace\n*.txt -whitespace\n*.snap -whitespace\n*.json -whitespace\n*.tsv -whitespace\n' > "$R/.gitattributes"
cp "$BINS/logs/build-i15.log" "$BINS/logs/build-i17.log" "$R/builds/"
...
systemd-run --user --scope -q -p CPUQuota=1200% env PATH="$PATH" D40_POLL_S=5 D40_LOG_EVERY=24 D40_WAITS=4320 \
    D40_LOCK_TRIES=720 D40_MIN_AVAIL_GB=40 D40_RIG=rtx5090 D40_R="$R" D40_TREE=$T D40_CELL_SCRIPT="$L/day77-cell.sh" \
...
mkdir -p "$R/$name"
/usr/bin/python3 "$L/day77-read.py" "$R/$name" --rig rtx5090 > "$R/$name/reading.log" 2>&1
log "$name reader rc=$?"
P=$T/target/c-profiles-day77; mkdir -p "$P"
