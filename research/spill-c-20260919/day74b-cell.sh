#!/usr/bin/env bash
# Day 74 cell `induce-b` (lane/spill-c-20260919, research/spill-c-20260919/DAY74.md section 4, registered before this
# script; section 1's cell `induce` as amended there: all but 2 GiB of free memory fragmented, the artifact reread
# through the page cache before every run): OWED C12, REF and the door (I15) from the binary p71 on DAY65's ONE pin with DAY73's probes and
...
# usage: day74b-cell.sh induce-b <lockfd>
set -uo pipefail
cell=$1; fd=$2
...
induce-b)
    ...
    host_state after
    echo "induce-b cell done: $(cat "$EV"/*.exit | sort | uniq -c | tr '\n' ' ')"
    ;;
