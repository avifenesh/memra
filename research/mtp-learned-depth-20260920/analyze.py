...
def analyze(ledger, receipts, family, expected_sets=8):
    ...
    initial_prompt, hardware, binary, workload = None, None, None, None
    ...
    for group in groups:
        ...
        if hardware is None:
            hardware, binary, workload = ident['gpu'], ident['binary_sha256'], ident['workload_sha256']
        assert (hardware, binary, workload) == (ident['gpu'], ident['binary_sha256'], ident['workload_sha256'])
        records = group['records']
        assert len(records) == 4 and {r['arm'] for r in records} == set(arms)
        ...
        for record in records:
            ...
            if arm == 'native':
                ...
                if arm == 'fixed':
                    assert all(int(r['draft_depth']) == fixed for r in rounds if r['eligible_for_learning'] == 'true')
    ...
    comparisons = {}
    for control in ['fixed', 'native', 'measured']:
        pairs = []
        for group in groups:
    ...
    return {'family': family, 'planned_sets': 8, 'observed_sets': len(groups), 'complete_planned_matrix': len(groups) == 8, 'audited_runs': len(all_records), 'audited_turns': expected_rows,
            'initial_prompt_tokens': len(initial_prompt.splitlines()), 'hardware': hardware,
            'binary_sha256': binary, 'workload_sha256': workload, 'pooled_e2e_tok_s': pooled,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--expected-sets', type=int, choices=range(1,9), default=8)
