    ...
    for cycle, order in cycles:
        ...
        for arm in order:
            ...
            cmd = [str(a.binary.resolve()), str(a.target.resolve()), str(a.draft.resolve()) if a.draft else 'embedded',
                   str(a.workload.resolve()), str(run.resolve()), arm, str(seed),
            ...
            metrics = metrics_from_log(log_text)[arm]
        ...
        audit_control_ids(a.out, seed, ['measured', 'fixed', 'learned'] if a.gate else ['measured'], turns=16 if a.family == 'gemma' else 8)
