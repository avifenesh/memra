# WP-F resumable state (2026-09-26 about 12:25Z; OWED 26 correction awaiting its 5090 cells; PRO sitting prepared)
...
- Done since 290fbbc1e: OWED 26 routed to F and registered (`M1-PREREG.md` G): G1 visibility in
  ...
  serving-shape check FAILED (a free buffer still waited 30 s; 0.65 tok/s), correction committed with
  two red-arm cells; G3 census `owed26/CENSUS.md`. PRO sitting prepared: `pro-sitting/SITTING.md`
...
- Queue (detached, idle-gated on `/tmp/memra-5090.lock`, lanes B and C hold the card for long
  ...
  7864223232 --step-rounds handoff-1g=3-10`; steps owed26-cells (correction red/green), owed26-serve,
  f17-spec-staged, f17-spec-mapped, handoff-1g rounds 3 to 10, bounded, g2, f17, handoff-8g.
  `touch ~/spill-f-5090/PAUSE` holds it between cells; a failing step stops it.
- Next when owed26-serve passes: report NEED TARGET CARD with `pro-sitting/run-sitting.sh`.
- Mirror each finished step into `rtx5090/`, `owed17/5090/`, `owed18/5090/`, `owed26/5090/`; pool
  ...
  `m1-handoff-pairs.py`.
- Scratch to remove at the end: `~/spill-f-5090/` (includes the `red-wt` worktree: remove it with
  `git worktree remove`), `/data/cache/spill-f-b2/`, `/data/cache/spill-f-5090-proof/`,
