...
| # | Item | Source | Status |
|---|---|---|---|
| 1 | M1 proof: the spill path's writable storage is physical local NVMe, proven in the running kernel from the mount to the PCI function, fail-closed | owner order 2026-09-24; lead prompt 2026-09-25; `NVME-DECISION.md` (parked NO-GO 2026-09-20); lead `HANDOVER-20260920.md` item 6 | **closed** 2026-09-25 on BOX27: `M1-PROOF verdict=PASS class=nvme-local-direct reasons=0` (`box27/RESULTS.md` A) |
...
| 3 | B0 device envelope on the proven path (fio: positioned-read threads vs io_uring vs libaio at the worker's request sizes and depths; sequential write with fsync) | `M1-PREREG.md` B0; CLAUDE.md "Hy3 spilling" (measure the storage-to-compute pipeline; 70% headroom rule in `spill-a-20260919/CELLS.md`) | **closed** 2026-09-25: 88 fio invocations; psync threads equal libaio at every point; io_uring refused by the container's seccomp (`box27/RESULTS.md` B0) |
| 4 | B1 KV ObjectStore storage cells (`storage-bench` roundtrip and restore, buffered vs O_DIRECT read vs O_DIRECT read+write, six sizes, cold host cache) | `spill-a-20260919/CELLS.md` M1-row/bulk cells; `M1-PREREG.md` B1 | **closed** 2026-09-25: 360/360 scored under the amended read gate; buffered wins every size and phase; the store read path is the ceiling (`box27/RESULTS.md` B1) |
| 5 | B2 KV host-tier handoff export/import on the proven path (fsync+rename export, cold import, restored completions byte-identical to cold) | `M1-PREREG.md` B2; `MEMRA_KV_HOST_HANDOFF` row in `docs/FLAGS.md` | **closed** 2026-09-26: 1 GiB 5/5 and 8 GiB 5/5 after amendments 2 and 3 (`box27/RESULTS.md` B2) |
| 6 | B3 expert-bank spill cells: `MEMRA_SPILL_IO` mmap (random and normal advice), pread, worker depth 2, worker depth 16 (the measured baseline), direct, three cache regimes, round-robin balanced N=10 per arm | CLAUDE.md "Hy3 spilling" (compare O_DIRECT, io_uring and mapped access only against the measured worker baseline; stages measured together); `M1-PREREG.md` B3 | bounded regime **scored** (worker16 beats every challenger); cold unscored (host-level foreign I/O, second-window rerun owed on this box); warm unscored registered, read-gate rescoring post-hoc: mmap arms 1.19x (`box27/RESULTS.md` B3) |
| 7 | Direct arm is 100% mmap fallback on the pinned artifact: 0 of 31,488 expert slices pass `direct_extent_aligned` (every tensor base offset is misaligned at `general.alignment=32`). Owed engine improvement: aligned over-read in the direct worker, byte identity against the mmap oracle | new finding, `m1-prereg/direct-alignment-census-qwen36-35b.json`; `per-expert-quant/README.md` ("aligned over-read or an aligned sidecar") | **closed** 2026-09-25: CPU gates plus the target-card pinned-pool test 12/12; direct16 zero fallbacks and over-read exactly 4096 x reads on every B3 visit |
...
| 10 | Host/storage 250 ms sampler: `/proc/diskstats` for the proven leaf set, `/proc/meminfo`, `/proc/stat`, target-process `/proc/<pid>/io`; runs inside the collector-executed worker so the lock covers it | `spill-a-20260919/CELLS.md` "No telemetry means no score"; `M1-PREREG.md` B | **landed CPU-verified** 2026-09-25 (`owed10/RESULTS.md`) |
| 11 | Cache-regime helper: file-scoped `POSIX_FADV_DONTNEED` plus `mincore` verification (cold), full buffered read plus `mincore` (warm), and the bounded page-cache regime (lane-owned mlocked balloon with a MemAvailable floor) | `spill-a-20260919/IO-BASELINE.md` cold/warm method; `M1-PREREG.md` B3 regime (iii) | **landed CPU-verified** 2026-09-25 (`owed11/RESULTS.md`) |
| 12 | M1 cell runner (round-robin arm order, fresh process per visit, tee raw logs before parsing, identity check against the proof receipt before and after every visit) | `M1-PREREG.md` B | **landed CPU-verified** 2026-09-25 against a stub (`owed12/RESULTS.md`) |
| 13 | B2 handoff driver: registered (`M1-PREREG.md` B2 amendment) and built: `kv-handoff-gate` seam, export write/fsync split, driver, prompt generator and manifest | `M1-PREREG.md` B2 | **landed CPU-verified** 2026-09-25 (`owed13/RESULTS.md`) |
...
| 15 | B4 serving-shape cell (memra-server, c=1 and c=4, TTFT/E2E/TPOT/ITL p50/p95/p99, request and token throughput) for the worker baseline and any B3 candidate | CLAUDE.md "Active accelerator owners" serving-claim fields; `M1-PREREG.md` B4 | running on BOX27 (worker16 registered descriptive row plus the two post-hoc warm challengers, `M1-PREREG.md` B4 arms) |
| 16 | io_uring decision input: B0's ring vs positioned-read threads at the worker operating point; implementation of memra's ring stays with the lead's assignment of `spill-a-20260919/IO-URING-PROPOSAL.md` | IO-URING-PROPOSAL.md (deferred until the bounded-pread baseline is measured); `M1-PREREG.md` B5 | **closed** as refused-here 2026-09-25: io_uring cannot run in the rented container (seccomp EPERM at io_queue_init); psync threads equal libaio, so no async-engine headroom showed at matched depth |
