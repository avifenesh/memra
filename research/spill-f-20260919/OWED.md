...
| 6 | B3 expert-bank spill cells: `MEMRA_SPILL_IO` mmap (random and normal advice), pread, worker depth 2, worker depth 16 (the measured baseline), direct, three cache regimes, round-robin balanced N=10 per arm | CLAUDE.md "Hy3 spilling" (compare O_DIRECT, io_uring and mapped access only against the measured worker baseline; stages measured together); `M1-PREREG.md` B3 | **closed** 2026-09-26 on BOX27: cold (window 2) and bounded scored under the registered gate; warm unscored registered, rescored post-hoc; every arm passed correctness in every regime (`box27/RESULTS.md`) |
...
| 15 | B4 serving-shape cell (memra-server, c=1 and c=4, TTFT/E2E/TPOT/ITL p50/p95/p99, request and token throughput) for the worker baseline and any B3 candidate | CLAUDE.md "Active accelerator owners" serving-claim fields; `M1-PREREG.md` B4 | **closed** 2026-09-26: 60 visits, all scored; worker16 registered descriptive row; post-hoc mapped challengers 1.17x and 1.18x |
