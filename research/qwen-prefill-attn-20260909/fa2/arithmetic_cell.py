...
root = p / "fa2-src"
fatbin, kernel = "prototype-controls.fatbin", "arithmetic_isolation"
...
for arm in ["control0", "control1", "control2", "control3"]:
    ...
    try:
        ...
        for key in ["FA2_PROBE_ACTIVE", "FA2_PROBE_KERNEL", "FA2_PROBE_DUMP", "FA2_PROBE_FATBIN"]:
            env.pop(key, None)
        env.update(FA2_PROBE_KERNEL="fa2_gqa32_rotate_" + arm, FA2_PROBE_FATBIN=str(root / fatbin))
        ...
        (root / "arithmetic-run.json").write_text(json.dumps(receipt, indent=2) + "\n")
        if code:
