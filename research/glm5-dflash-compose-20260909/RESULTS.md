...
plain K=0 for correct vendor-default sampling on this measured shape. All three
runtime doors were removed on 2026-09-09 (#413, #394 and #393); this record does
not restore them. This is a small diagnostic win at the archived heads, not a
corrected-sampling deployment qualification.

## Source and protocol

Historical measurement base: `9396a817e` (v0.137.0). The cell composed these
archived component heads, in order:

| PR | Exact measured component head |
|---|---|
...
That historical composed source contained the three default-OFF mechanisms.
The merged runtime removed them on 2026-09-09: fused E4M3 verify in #413,
MLA split-KV in #394, and causal PMIN in #393. The re-derive heads recorded in
`docs/FLAGS.md`'s removed-doors ledger are `7c3ddf0d`, `c776611e` and `f197eb41`.
This PR banks only the compose research directory and its index entry; it changes
no engine code, live flags, or kernel catalog. The measured source history remains
in the private source bundle, with `instrumentation.patch` and the original
scripts preserved here. No boundary exception or archived receipt edit is made.
...
## Historical posture recommendation and TP2 implications

The cell recommends PP1 p32k **plain K=0**. The three composed source doors
were subsequently removed; the runtime does not retain an OFF arm to enable.
...
Those checks describe the original measurement revision, not a fresh run on the
current engine. The research-only integration preserves the raw receipts and
requires fresh hosted text gates. No local CI or GPU work was run for the trim.
The three component PRs already landed with their runtime doors removed; this
record neither reopens them nor qualifies a new deployment.
