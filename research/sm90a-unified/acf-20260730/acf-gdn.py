# ACF search: gdn harness on H100, ptxas 13.3 apply-controls (same recipe as acf-fa3.py).
import pathlib
    ...
    try:
        ...
        r = subprocess.run(
            [os.path.expanduser("~/cuda-13.3.1/bin/nvcc"),
             ...
             "-o", exe, os.path.expanduser("~/bw24-unified/tools/bench_gdn_k5.cu")],
        ...
        if "FAIL" in out or "MISMATCH" in out or "BAD" in out:
            return INVALID
        m = re.findall(r"v2: ([0-9.]+) us/launch", out)
        if len(m) != 1:
            return INVALID
...
if __name__ == "__main__":
    search = Search(
        ...
        dump_results=pathlib.Path("~/acf-gdn-results.csv").expanduser(),
        disable_progress_bar=True,
    )
    res = search.start(num_workers=5)
    best = res.get_best_result()
    print("BEST score:", best["score_1"])
    save_compiler_config(str(pathlib.Path("~/acf-gdn-best.acf").expanduser()), best["params"])
    print("saved ~/acf-gdn-best.acf")
