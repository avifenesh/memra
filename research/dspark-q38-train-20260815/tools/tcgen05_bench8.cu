...
#define NVAL 256
#endif
constexpr int M = 128, N = NVAL, KB = 64;  // values; 32B/row
...
__device__ static inline uint32_t idesc_nvf4(int m, int n) {
  uint32_t d = 0;
  d |= 1u << 7;                       // atype e2m1
  d |= 1u << 10;                      // btype e2m1
...
extern "C" __global__ void t1(const uint8_t* A, const uint8_t* B, float* D, int iters) {
  __shared__ __align__(128) uint8_t sA[M * KB / 2];
  __shared__ __align__(128) uint8_t sB[N * KB / 2];
  ...
  int tid = threadIdx.x;
  for (int i = tid; i < M * KB / 2; i += blockDim.x) sA[i] = A[i % 4096];
  for (int i = tid; i < N * KB / 2; i += blockDim.x) sB[i] = B[i % 4096];
  for (int i = tid; i < 512; i += blockDim.x) sSF[i] = 0x38;   // ue4m3 identity
  if (tid == 0)
  ...
  if (tid == 0) {
    ...
    if (N > 128) asm volatile("tcgen05.cp.cta_group::1.32x128b.warpx4 [%0], %1;" :: "r"(sfb_tmem + 4), "l"(sfd));
