...
set -ux
exec >> /home/ubuntu/regen3.log 2>&1
export PATH=/scratch/venvs/train/bin:$HOME/.local/bin:$PATH
PYE=/scratch/venvs/eval/bin/python
SF=/scratch/repos/SpecForge
OUT=/scratch/corpus

[ -s $OUT/pb-30k.jsonl ] || cp $OUT/perfectblend-30k.jsonl/perfectblend_train.jsonl $OUT/pb-30k.jsonl
wc -l $OUT/pb-30k.jsonl
...
$REGEN --max-tokens 8192  --reasoning save    --input-file-path $OUT/pb-30k.jsonl --output-file-path $OUT/regen/pb-think.jsonl     --server-address $SRV || true
$REGEN --max-tokens 8192  --reasoning disable --input-file-path $OUT/pb-30k.jsonl --output-file-path $OUT/regen/pb-nothink.jsonl   --server-address $SRV || true
