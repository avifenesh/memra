...
class BuildRecordTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for name in (self.builder.PRODUCER, self.builder.RUNNER, self.builder.GEMMA_RUNNER):
...
if sys.argv[1] == 'test':
    name = (sys.argv[sys.argv.index('--test') + 1] if '--test' in sys.argv
            else sys.argv[sys.argv.index('-p') + 1].replace('-', '_'))
...
'''
    ...
    def test_owned_success_binds_source_build_inputs_and_all_executables(self):
        ...
        self.assertIn(self.builder.GEMMA_RUNNER, record['source']['files'])
        self.assertEqual(set(record['tests']), set(self.builder.TEST_TARGETS))
    ...
    def test_incomplete_engine_lib_build_emits_no_success_record(self):
        with self.assertRaisesRegex(RuntimeError, 'incomplete/unsuccessful native test build'):
            self.build(omit_test='memra_engine')
        self.assertFalse(self.receipt.exists())

    def test_hidden_gemma_controller_edit_refuses_before_gpu(self):
        self.build()
        name = self.builder.GEMMA_RUNNER
        self.git('update-index', '--assume-unchanged', name)
        with (self.root / name).open('a') as stream:
            stream.write('\n# hidden changed controller\n')
        self.before_gpu()
