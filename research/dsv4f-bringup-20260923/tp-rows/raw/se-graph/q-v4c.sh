#!/usr/bin/env bash
# q-v4c.sh (2x RTX PRO 6000 Server Edition pair): memra #710 served A/B of the TP/EP B-row graphs, lane
# lane/dsv4-tp-rows-graph-20260926 ($1) against main cf6b82db4 (the flip: TP/EP one lane).
# Gd = lane default (TP/EP, four lanes since 47df550b2, B-row graphs), G2 = two lanes, Pp = lane
# with MEMRA_DSV4_TOPOLOGY=pp, Mn = main. Cells c1/c2/c4/sampled c2/2k c2, one boot per row,
# order Gd Pp G2 Mn x3, 45-minute timeout per row (#722).
while ! grep -q V4B_DONE /root/rcpt/q-v4b.summary 2>/dev/null; do sleep 30; done
set -u
SHA=$1
S=/root/rcpt/q-v4c.summary; R=/root/rcpt/rows-graph-served-se; mkdir -p $R
...
arm_env() {
  case $1 in
    ...
    G2) echo "MEMRA_DSV4_SESSIONS=2" ;;
    Pp) echo "MEMRA_DSV4_TOPOLOGY=pp" ;;
  esac
}
i=0
for arm in Gd Pp G2 Mn Gd Pp G2 Mn Gd Pp G2 Mn; do
