#!/usr/bin/env bash
# q-pair20.sh (2x RTX PRO 6000 WS pod): memra #710 served A/B of the TP/EP B-row graphs, lane
# lane/dsv4-tp-rows-graph-20260926 ($1) against main cf6b82db4 (the flip: TP/EP one lane).
# Gd = lane default (TP/EP, two lanes, B-row graphs), G4 = lane with four lanes, Pp = lane
# with MEMRA_DSV4_TOPOLOGY=pp, Mn = main. Cells c1/c2/c4/sampled c2/2k c2, one boot per row,
# order Gd Pp G4 Mn x3, 45-minute timeout per row (#722).
set -u
SHA=$1
S=/root/rcpt/q-pair20.summary; R=/root/rcpt/rows-graph-served; mkdir -p $R
...
[[ -d /root/lane/target-main10 ]] || cp -a /root/lane/target-rg /root/lane/target-main10
bash /root/box/build.sh /root/lane/t-main10 /root/lane/target-main10 main10
...
i=0
for arm in Gd Pp G4 Mn Gd Pp G4 Mn Gd Pp G4 Mn; do
