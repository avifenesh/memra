#!/usr/bin/env bash
# q-pair9.sh (2x RTX PRO 6000 WS pod), after q-pair7.sh: request-turnover stress for the #699 fault
...
while ! grep -q PAIR7_DONE /root/rcpt/q-pair7.summary 2>/dev/null; do sleep 30; done
set -u
S=/root/rcpt/q-pair9.summary; R=/root/rcpt/turnover; mkdir -p $R
...
for a in pfix:017f2bdae pctl:289ea34e2; do
  ...
  [[ -d /root/lane/target-$name ]] || cp -a /root/lane/target-brow /root/lane/target-$name
