...
root = sys.argv[1]; BASE = sys.argv[2] if len(sys.argv) > 2 else "s1"
rows = sorted((r for r in glob.glob(os.path.join(root, "r*-*")) if os.path.isdir(r)
...
for lab in labels:
    ...
    for arm in sorted(med):
        if arm != BASE and BASE in med:
            print(f"\nAggregate {arm} vs {BASE}: {100*(med[arm][0]/med[BASE][0]-1):+.1f}%; per-request decode {100*(med[arm][1]/med[BASE][1]-1):+.1f}%.")
