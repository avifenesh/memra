#!/usr/bin/env bash
# Day 22 memra#427 cell S, local RTX 5090: the b16 batched-MMVQ tier as the seam. Four arms of the unchanged
# reproducer `qwen-a4-continuation-gate` on the same artifact and prompt as day 21: S0 baseline (9296: 16, 48);
# S1 MEMRA_NO_BATCHED=1 (9296: 16, 48); S2 MEMRA_NO_BATCHED=1 MEMRA_PRIME_CHUNK=32 (9296: 16, 48);
# S3 MEMRA_NO_BATCHED=1 (9297: 17) as the inert-at-17 control. Predictions are in DAY22.md section 1.2, written
# before this ran. MEMRA_NO_BATCHED and MEMRA_PRIME_CHUNK are documented (FLAGS.md); nothing new is read.
# Pass/fail digests, not timed. Bounded lock retries, never kills a holder.
# usage: run-day22-seam.sh <cell-name> <collector-timeout-s>
set -uo pipefail
...
arm S0-baseline 9296 "16 48"
arm S1-nobatched 9296 "16 48" MEMRA_NO_BATCHED=1
arm S2-nobatched-chunk32 9296 "16 48" MEMRA_NO_BATCHED=1 MEMRA_PRIME_CHUNK=32
arm S3-nobatched-t17 9297 "17" MEMRA_NO_BATCHED=1'
for attempt in 0 1 2 3 4 5; do
  ...
  run python3 tools/tier-battery.py --rig rtx5090 --timeout "$tmo" --out "$out" \
    --execute bash -c "$body" seam "$GATE" "$MODEL" "$PROMPT" "$out/cell" > "$out-driver.log" 2>&1
  rc=$?
  echo $rc > "$out.exit"
