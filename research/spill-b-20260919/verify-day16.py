#!/usr/bin/env python3
"""Offline replay of the day-16 prefix-policy A/B receipts on the second card class (memra#523 item 2).
...
pre-registration source ref `9466b8912`, the card (the local RTX 5090 Laptop GPU), the shape gates and the
pre-registered scaled shares, the interleaved AB/BA schedule, every run's primary and secondary recomputed from
its rows, digest identity across runs and against the cache-off boot, the pre-registered rule re-applied, the
per-request `cached_tokens` compared with `day16-predict.py`, and the admission-reclaim prefix evictions counted
from each run's server log (the confound DAY16.md records).

Day 16's cell is the smoke (`ab-smoke`, one pair per order) and it FAILED its precondition: the replay must
re-derive `-> DIGEST-FAIL`, the mismatching requests and the collector's `failed` status, and it says so. A
passing full cell (five pairs per order, every digest identical) would be replayed to its `WINNER=`/`INCONCLUSIVE`.

usage: verify-day16.py [--raw research/spill-b-20260919/rtx5090-day16] [--cell ab-smoke]
"""
import argparse
...
SOURCE = "9466b89122a5815dacef0dd54db30fb1476ed533"
ARMS = ("slru", "lru")
BOOT_WORD = {"slru": "byte-SLRU", "lru": "plain-LRU"}
SHARES = {"cohort_share_of_budget": 0.739, "turn1_share_of_budget": 0.450, "last_turn_share_of_budget": 0.496}
...
def predicted():
    ...
    return mod.predict(mod.SHAPES["day16"])
...
def replay_run(run_dir, plan, cold_digest):
    ...
    return run, [r["text_sha256"] for r in rows], {"events": len(reclaim), "entries": sum(reclaim), "policy_evicts": policy_evicts, "mismatches": mism}


def main():
    ap = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument("--raw", type=Path, default=HERE / "rtx5090-day16")
    ap.add_argument("--cell", default="ab-smoke")
    ...
    require(source == SOURCE, f"the binary was built from {source[:9]}, not the pre-registration ref {SOURCE[:9]}")
    ...
    require((cell_dir / "gate-source.txt").read_text().strip() == SOURCE, "the harness did not run from the pre-registration tree")
    plan = load_json(cell / "plan.json")
    plan_args, plan = plan["args"], plan["requests"]
    require((plan_args["budget_mib"], plan_args["cohort_tokens"], plan_args["turns"], plan_args["start_tokens"], plan_args["grow_tokens"], plan_args["return_every"], plan_args["ctx"], plan_args["max_tokens"], plan_args["gpu_lock"])
            == (1024, "1250,1350,1450,1550", 12, 11000, 150, 3, 16384, 8, LOCK), f"the plan is not the pre-registered scaled shape: {plan_args}")
    shape = summary["shape"]
    ...
    # Identity ACROSS runs of the same arm (the policy never moved a byte between runs), separately from identity to cold.
    for arm in ARMS:
    ...
    print("DAY16 REPLAY OK" + ("" if identical else ": receipts consistent, precondition FAILED on this card, no verdict"))
    ...
    for run, extra in zip(runs, extras):
        print(f"  {run['pair_id']} {run['arm']}: policy evict lines {extra['policy_evicts']}, admission reclaim events {extra['events']} evicting {extra['entries']} prefix entries; digest mismatches vs cold {extra['mismatches']}")
    ...
    print(f"prediction (day16-predict.py): slru {pred['slru']['computed']} lru {pred['lru']['computed']}; per-request cached_tokens matched slru {matched['slru']}/{total} lru {matched['lru']}/{total}")
