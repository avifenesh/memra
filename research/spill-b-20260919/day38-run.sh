#!/usr/bin/env bash
# DAY38 boots (1.2 to 1.5): one boot per spec through run-day26-cell.sh (it takes the rig lock), after a bounded idle
...
BIN=${BIN:-$WT/target/day38/green/memra-server}
RED_BIN=${RED_BIN:-$WT/target/day38/red/memra-server}
...
for spec in "$@"; do
  ...
  use_red=0
  args="--lengths $LENGTHS --shapes X,A"
  case $arm in
    off) ;;
    on) aenv+=(MEMRA_KV_PARK_COMPACT=1) ;;
    on-fault-batch) aenv+=(MEMRA_KV_PARK_COMPACT=1 MEMRA_STEP_OOM_FAULT=1); args="--lengths 6144 --shapes X --turn4" ;;
    on-fault-nobatch) aenv+=(MEMRA_KV_PARK_COMPACT=1 MEMRA_STEP_OOM_FAULT=1 MEMRA_SERVE_BATCH=0); args="--lengths 6144 --shapes X --turn4" ;;
    on-fault-nobatch-red) aenv+=(MEMRA_KV_PARK_COMPACT=1 MEMRA_STEP_OOM_FAULT=1 MEMRA_SERVE_BATCH=0); args="--lengths 6144 --shapes X --turn4"; use_red=1 ;;
    vmm-off) aenv+=(MEMRA_KV_ALLOCATOR=vmm); args="--lengths 6144 --shapes X" ;;
    vmm-on) aenv+=(MEMRA_KV_ALLOCATOR=vmm MEMRA_KV_PARK_COMPACT=1); args="--lengths 6144 --shapes X" ;;
