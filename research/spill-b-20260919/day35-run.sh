#!/usr/bin/env bash
# WP-B day 35 (DAY35.md 1.3): run named boots of the memra#680 seed-term cell, one at a time, each under its own
...
#        shape = G2 | L64 | off (DAY35.md 1.3); binaries target/day35/<red|green>/memra-server (the file must be
...
for spec in "$@"; do
  ...
  BIN=$WT/target/day35/$role/memra-server
  ...
  case $shape in
    G2) v=8192; burst=64 ;; L64) v=32768; burst=64 ;; off) v=; burst=0 ;;
