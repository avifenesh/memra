#!/usr/bin/env bash
# Day 23 (lead ruling 26), local RTX 5090: the standard `run-gen` argmax gate and the `run-spec` K=1..8
...
# Pass/fail, not timed, through the collector (--rig rtx5090, /tmp/memra-5090.lock). Bounded lock retries, never
# kills a holder. Verdict lines are grepped out of each arm's full log, which is kept.
# usage: run-day23-genspec.sh <cell-name> <collector-timeout-s>
set -uo pipefail
...
P16=${P16:-$WT/research/spill-b-20260919/rtx5090-day23/prompts/p16-ids.txt}
P4112=${P4112:-$WT/research/spill-b-20260919/rtx5090-day23/prompts/p4112-ids.txt}
