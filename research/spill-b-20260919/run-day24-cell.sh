#!/usr/bin/env bash
# Day 24 memra#476 cell (DAY24.md 1.1): booked versus real on the local RTX 5090, one fixed request sequence,
# the same script before and after the fix. Shadow mode only (MEMRA_ADMIT_PREDICT_SHADOW=1): nothing is refused.
# usage: run-day24-cell.sh <cell-name> [bin]     (cell dir: rtx5090-day24/<cell-name>/)
set -uo pipefail
cell=${1:?cell}
WT=${WT:-$HOME/projects/wt-spill-b}; R=$WT/research/spill-b-20260919/rtx5090-day24
BIN=${2:-$WT/target/release/memra-server}
MODEL=${MODEL:-/data/ai-ml/hf-models/qwen35-9b-nvfp4-gguf/Qwen3.5-9B-NVFP4-MTP-GGUF.gguf}
PORT=${PORT:-18476}; BASE=http://127.0.0.1:$PORT
...
exec 9>"$LOCK"
echo "$(date -u +%FT%TZ) waiting for $LOCK (flock -w 1800)" | tee "$C/lock.txt"
flock -w 1800 9 || { echo "$(date -u +%FT%TZ) lock not acquired within 1800 s; cell not run" | tee -a "$C/lock.txt"; exit 3; }
...
MEMRA_COMPAT=openai MEMRA_MODELS="q9=$MODEL" MEMRA_ADDR=127.0.0.1:$PORT MEMRA_ADMIT_PREDICT_SHADOW=1 MEMRA_TTFT_TRACE=1 \
...
run python3 "$WT/research/spill-b-20260919/day24-client.py" --base "$BASE" --out "$C" --model q9 > "$C/client.log" 2>&1
rc=$?; echo $rc > "$C.exit"
...
python3 "$WT/research/spill-b-20260919/day24-parse.py" "$C" > "$C/REPORT.txt" 2>&1
