...
cd "$WT" || exit 1
EV=$R/hitgate-fix/qwen; rm -rf "$EV"; mkdir -p "$EV"
nvidia-smi --query-gpu=name,memory.used,temperature.gpu,power.draw --format=csv,noheader > $R/card-before-hitgate.csv
sha256sum "$BIN" > $R/hitgate-fix/binary.sha256
if [ -f "$HITQD" ]; then export MEMRA_GATE_MTP_DRAFT="$HITQD"; echo "drafter: $HITQD" > $R/hitgate-fix/drafter.txt; else echo "drafter absent: $HITQD (assertion no-ops, as local-ci's WARNING branch)" > $R/hitgate-fix/drafter.txt; fi
MEMRA_GPU_LOCK=/tmp/memra-5090.lock run bash tools/spec-on-cache-hit-gate.sh qwen "$HITQ" "$BIN" "$EV" > $R/hitgate-fix/gate.log 2>&1
rc=$?; echo $rc > $R/hitgate-fix.exit; echo "hitgate-fix rc=$rc" >> $R/chain.log
tail -3 $R/hitgate-fix/gate.log
