...
# binary and target/day49 from 6102fb63a); the gate's arm i twice, then the serving shape (burst 8 x 6,144, no warm,
...
[ -s target/day49/SHA256SUMS ] || { export WT; TARGET=$WT/target WRAP="$W" bash research/spill-b-20260919/build-arms.sh \
  "$WT/target/day49" "$(git rev-parse 6102fb63a)" tip > "$D/build.out" 2>&1 || { echo "$(date -u +%FT%TZ) arm build failed" >> "$D/run.log"; exit 2; }; }
