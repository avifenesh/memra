#!/usr/bin/env bash
# Day 19: the #379 gate (tools/spec-on-cache-hit-gate.sh qwen) on the fix binary, local RTX 5090. The gate
...
BIN=${BIN:-$WT/target/bins/fix/memra-server}
...
cd "$WT" || exit 1
EV=$R/hitgate-fix-2/qwen; rm -rf "$EV"; mkdir -p "$EV"
nvidia-smi --query-gpu=name,memory.used,temperature.gpu,power.draw --format=csv,noheader > $R/card-before-hitgate.csv
sha256sum "$BIN" > $R/hitgate-fix-2/binary.sha256
if [ -f "$HITQD" ]; then export MEMRA_GATE_MTP_DRAFT="$HITQD"; echo "drafter: $HITQD" > $R/hitgate-fix-2/drafter.txt; else echo "drafter absent: $HITQD (assertion no-ops, as local-ci's WARNING branch)" > $R/hitgate-fix-2/drafter.txt; fi
MEMRA_GPU_LOCK=/tmp/memra-5090.lock run bash tools/spec-on-cache-hit-gate.sh qwen "$HITQ" "$BIN" "$EV" > $R/hitgate-fix-2/gate.log 2>&1
rc=$?; echo $rc > $R/hitgate-fix-2.exit; echo "hitgate-fix-2 rc=$rc" >> $R/chain.log
tail -3 $R/hitgate-fix-2/gate.log
