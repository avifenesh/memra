#!/usr/bin/env bash
# Day 13 gate cell driver: one arm (main|fix) through the collector with the inherited canonical lock.
...
for attempt in 0 1 2 3 4 5; do
  ...
  [ $attempt -gt 0 ] && out=$R/gate-$arm-retry$attempt
  python3 tools/tier-battery.py --rig pro-single --timeout 2400 --out "$out" --external-lock \
    --execute python3 tools/prefix-evict-reclaim-gate.py --external-lock @COLLECTOR_LOCK_FD@ \
      --model /root/artifacts/Qwen3.8-27B-NVFP4-Q5K-mtp.gguf \
      --bin $R/bins/$arm/memra-server \
      --out "$out/cell" > "$out-driver.log" 2>&1
  rc=$?
  echo $rc > "$out.exit"
