#!/usr/bin/env bash
# Bounded wait for the local card to free (memory.used < 4000 MiB for two consecutive 30 s polls, up to 90 min), then the
...
ok=0; for i in $(seq 180); do
  ...
  if [ "${u:-99999}" -lt 4000 ]; then ok=$((ok+1)); else ok=0; fi
