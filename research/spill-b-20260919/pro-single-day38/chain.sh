...
# checkpoint's context, after the day-37 sitting on the same box. Builds green (the lane tip at the sitting) and red
# (green plus day38-red.patch), then the same boots as the local chain with the target card's lengths, the reader,
# and a receipts manifest. Never a signal to anything this lane did not start.
set -uo pipefail
...
if [ ! -x "$R/bins/green/memra-server" ]; then
  ( nice -n 10 cargo build --release -p memra-server --bin memra-server ) > "$R/bins/green/build.log" 2>&1 || { log "green build failed"; exit 1; }
  cp target/release/memra-server "$R/bins/green/memra-server"; git rev-parse HEAD > "$R/bins/green/source.commit"
fi
if [ ! -x "$R/bins/red/memra-server" ]; then
  W=/root/wt-b38-red
  git worktree add --detach "$W" HEAD >> "$R/fetch.log" 2>&1 || { log "red worktree failed"; exit 1; }
  ( cd "$W" && git apply research/spill-b-20260919/day38-red.patch && CARGO_TARGET_DIR=/root/target-b38-red nice -n 10 \
      cargo build --release -p memra-server --bin memra-server ) > "$R/bins/red/build.log" 2>&1 || { log "red build failed"; exit 1; }
  cp /root/target-b38-red/release/memra-server "$R/bins/red/memra-server"
  { git -C "$W" rev-parse HEAD; sha256sum "$W/research/spill-b-20260919/day38-red.patch"; } > "$R/bins/red/source.commit"
  git worktree remove --force "$W" >> "$R/fetch.log" 2>&1; rm -rf /root/target-b38-red
fi
