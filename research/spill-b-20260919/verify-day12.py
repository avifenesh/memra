#!/usr/bin/env python3
"""Offline replay of day-12 reclaim-cycle receipts on two cards under lead ruling 6.
...
continuation surfaces, every cycle's roundtrip arithmetic and probe accounting, the series class
(lead ruling 5, day 11) and the series verdict (lead ruling 6, day 12): `g1_reclaim_qualified=true`
with a nonzero residual only for a series of N >= 5 cycles whose class is
`one-time-driver-mapping-metadata`, with criteria (a) to (c) in every cycle, a bit-identical
restored prefix in every cycle and baseline drift 0; the label is then
`ACTIVE-32K G1 PASS (classified one-time-driver-mapping-metadata, N cycles)` and must have been
PRINTED by the gate. Every other shape keeps its day-11 wording. Criteria (a) to (d) are unchanged
and tightening (e) stays in force for every other shape.
"""
import argparse
import csv
import gzip
...
NOT_APPLICABLE_POOLED = "not-applicable-pooled"
...
fields = day9.fields
observe = day11.observe
classify_cycles = day11.classify_cycles
...
# ---- pure series verdict (mirrors kv_tier_gate/reclaim_contract.rs::series_verdict) ----

def label_for(context, n):
    return f"ACTIVE-{context // 1024}K G1 PASS (classified {ONE_TIME}, {n} cycles)"


def series_verdict(cycles, granule, restored_identical, context):
    """Returns (class, g1_reclaim_qualified string, label or None). Lead ruling 6, exactly."""
    klass = classify_cycles(cycles, granule)
    if granule == 0:
        return klass, NOT_APPLICABLE_POOLED, None
    n = len(cycles)
    obs = [observe(c["free_before"], c["free_after_demote"], c["free_after_restore"], c["released"], granule)
           for c in cycles]
    identical = len(restored_identical) == n and all(restored_identical)
    drift_zero = all(c["free_before"] == cycles[0]["free_before"] for c in cycles)
    every_bounded = all(o[1] for o in obs)
    every_exact = all(o[1] and o[2] == 0 for o in obs)
    exact_series = n > 0 and every_exact and drift_zero and identical
    classified_series = (klass == ONE_TIME and n >= SERIES_MIN_CYCLES and every_bounded and drift_zero
                         and identical)
    qualified = "true" if (exact_series or classified_series) else "false"
    return klass, qualified, (label_for(context, n) if classified_series else None)


# ---- receipt replay ----

def collector(folder, rig):
    """Journal, lock, telemetry, power envelope, co-tenancy, command shape. Returns
    (identity, envelope, RECLAIM-CYCLES console line, full console)."""
    spec = RIGS[rig]
    ...
    require(len(series_lines) == 1, "console series line")
    return identity, envelope, series_lines[0], log


def console_label(log, context, label):
    """The label exists only when the gate printed it as the final status line, once, and the
    console carries no other G1 PASS claim. Without a label the console must carry none."""
    last = log.rstrip("\n").splitlines()[-1]
    stripped = log.replace("not G1 PASS", "")
    if label is None:
        require("G1 PASS" not in stripped, "console claims G1 PASS without a series verdict")
        require(last.startswith("ACTIVE_") and last.endswith(f"not G1 PASS committed={context} generated=128"),
                "final status line")
        return None
    require(last == f"{label} committed={context} generated=128", "label is not the final status line")
    require(stripped.count("G1 PASS") == 1, "a second G1 PASS claim on the console")
    return label


def continuation(folder, identity, rig, status):
    """Frozen-bundle comparison. PRO: must match the target-card bundle. 5090 laptop: same
    artifact/plan/prompt as the rented RTX 5090 bundle; decoded surfaces compared and REPORTED."""
    r = folder / "receipt"
    context = identity["context"]
    done = fields(r / "ACTIVE.txt")
    require(done["committed"] == context and done["active_engaged"] == "true", "unfinished context")
    require((r / "ACTIVE.txt").read_text().splitlines()[0] == status, "ACTIVE.txt status differs from console")
...
def cycle(receipt, k, prefix_bytes):
    ...
    The per-cycle G1 line is unchanged: bounded and residual 0, never a classified residual."""
    d = receipt / f"cycle-{k}"
    ...
    require(flags["g1_reclaim_qualified"] == str(bounded and residual == 0).lower(),
            "per-cycle G1 line promoted a residual")
    if residual == 0:
    ...
    return {"cycle": k, "free_before_bytes": m["free_before_bytes"],
            ...
            "restored_prefix_bit_identical": restored == prefix_bytes,
            "restored_prefix_state_manifest_sha256": hashlib.sha256(restored).hexdigest()}


def series(receipt, rows, context):
    """reclaim-cycles.tsv/.txt agree with the cycle directories, the pure class and the pure
    series verdict of ruling 6. Returns (class, g1_reclaim_qualified, label or None)."""
    ...
    cycles = [{"free_before": r["free_before_bytes"], "free_after_demote": r["free_after_demote_bytes"],
              ...
              for r in rows]
    identical = [r["restored_prefix_bit_identical"] for r in rows]
    klass, qualified, label = series_verdict(cycles, granule, identical, context)
    ...
    require(int(summary["series_min_cycles"]) == SERIES_MIN_CYCLES, "series minimum")
    ...
    require(summary["all_cycles_reclaim_observed"] == str(all(r["reclaim_observed"] for r in rows)).lower(),
            "series bounded flag")
    require(summary["all_cycles_restored_bit_identical"] == str(all(identical)).lower(), "series identity flag")
    require(summary["g1_reclaim_qualified"] == qualified, "series G1 flag does not follow ruling 6")
    require(summary["series_label"] == (label or "not-printed"), "series label does not follow ruling 6")
    # Ruling 6 boundaries restated on the receipt itself: a residual qualifies only as the
    # classified one-time series of at least five cycles with drift 0 and identical restores.
    if qualified == "true" and any(residuals):
        require(klass == ONE_TIME and CYCLES >= SERIES_MIN_CYCLES and label is not None
                and all(identical) and summary["free_before_drift_last_bytes"] == "0"
                and all(r["reclaim_observed"] for r in rows), "nonzero residual promoted outside ruling 6")
    else:
        require(label is None, "label without qualification")
    return klass, qualified, label


def verdict(context, rows, klass, qualified, label, baseline_status):
    """The label verbatim when the gate produced it; every other shape keeps the day-11 wording."""
    tag = f"ACTIVE-{context // 1024}K"
    residuals = [r["residual_bytes"] for r in rows]
    bounded = all(r["reclaim_observed"] for r in rows)
    if label is not None:
        require(qualified == "true" and klass == ONE_TIME and label == label_for(context, len(rows)),
                "label without the ruling-6 verdict")
        return label
    require((qualified == "true") == (bounded and klass == NONE and not any(residuals)), "verdict inputs disagree")
    if qualified == "true" and baseline_status.startswith("matches"):
...
def main():
    ...
    ap.add_argument("--seal", action="store_true", help="write day12-raw-manifest.json from the raw bytes")
    ...
    for rig, spec in RIGS.items():
        ...
        for folder in sorted(p for p in raw.iterdir() if p.is_dir() and p.name.startswith("cycles-")):
            ...
            if (folder / "collector.exit").read_text().strip() == "2":
                result[f"{rig}/{label_name}"] = day11.refused(folder, rig)
                continue
            identity, envelope, console, log = collector(folder, rig)
            ...
            klass, qualified, label = series(receipt, rows, context)
            printed = console_label(log, context, label)
            status = log.rstrip("\n").splitlines()[-1].rsplit(" committed=", 1)[0]
            baseline_status = continuation(folder, identity, rig, status)
            result[f"{rig}/{label_name}"] = {
                ...
                "continuation": baseline_status, "console_series_line": console, "printed_status_line": status,
                "cycles": rows, "residual_series_class": klass, "g1_reclaim_qualified": qualified,
                "printed_label": printed,
                "verdict": verdict(context, rows, klass, qualified, label, baseline_status)}
    if args.require_complete:
        expected = {f"{rig}/{name}" for rig in RIGS for name in CELLS}
