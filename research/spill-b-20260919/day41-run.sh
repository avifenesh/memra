#!/usr/bin/env bash
# DAY41 boots (1.3, addendum A): one boot per spec through run-day26-cell.sh (it takes the rig lock), after a bounded idle
# wait (at most 7200 s: lock free, no compute app, >= 24 GB host memory). Never a signal to anything this lane did not
# start. usage: day41-run.sh <receipt root> <name>:<arm>:<route>:<shape> ...
#   arm = keep | rewind | offprev      route = plain | spec      shape = RX | FX | RX6 (RX at 6,144 only)
...
BIN=${BIN:-$WT/target/day41/tip/memra-server}
PREV_BIN=${PREV_BIN:-$WT/target/day41/offprev/memra-server}
...
for spec in "$@"; do
  ...
  uenv=(-u MEMRA_RESUME_GRID_REWIND -u MEMRA_SERVE_SPEC -u MEMRA_PREFIX_CACHE_MB)
  aenv=()
  B=$BIN
  case $arm in
    keep) ;;
    rewind) aenv+=(MEMRA_RESUME_GRID_REWIND=1) ;;
    offprev) B=$PREV_BIN ;;
