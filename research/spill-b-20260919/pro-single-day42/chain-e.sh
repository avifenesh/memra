#!/usr/bin/env bash
# WP-B DAY42 target-card half, addenda C to F shape (DAY42.md 1.4 to 1.5, addenda A to F: the worker-level demote queue): one RTX PRO 6000 Blackwell Workstation Edition, the 27B
# at the checkpoint's context, the host tier at 32,768 MB, burst 64, warm 24 x 8,192 tokens at max_tokens 16, the burst at max_ctx 133,120 and max_tokens 64 (addendum D). Builds S42's memra-server (build-arms.sh), runs the
# eight boots (day42-run.sh), the reader, a manifest. Refuses to start without ss or lsof. Never a signal to anything this
# lane did not start. Receipts under /root/spill-receipts/b-day42e.
set -uo pipefail
R=/root/spill-receipts/b-day42e; mkdir -p "$R/bins" "$R/boots"
WT=/root/wt-b
...
S42=${S42:-0cf59870a73a72f47c168b242250683ffcb4a14d}
...
export RIG_LOCK=/tmp/memra-gpu.lock BIN="$R/bins/tip/memra-server" MODEL MODEL_KEY=q38 BOOT_CTX='' NO_SCOPE=1 HOST_MB=32768 BURST=64 \
  WARM=24 WARM_TOKENS=8192 WARM_MAX_TOKENS=16 BURST_MAX_CTX=133120
