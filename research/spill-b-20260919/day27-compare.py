...
for t in tags:
    ...
    comparable = ra["sha"] is not None and rb["sha"] is not None
    if comparable and ra["sha"] == rb["sha"]:
        same_sha += 1
    elif comparable:
        diff_sha += 1
    if ra != rb:
        note = "  <- digest differs" if comparable and ra["sha"] != rb["sha"] else ("  (digest equal)" if comparable else "  (no digest on one side)")
        print(f"{t}: A P={ra['P']} G={ra['G']} cached={ra['cached']} finish={ra['finish']} sha={ra['sha'] and ra['sha'][:16]} | "
              f"B P={rb['P']} G={rb['G']} cached={rb['cached']} finish={rb['finish']} sha={rb['sha'] and rb['sha'][:16]}{note}")
both_sha = sum(1 for t in tags if t in a and t in b and a[t]["sha"] is not None and b[t]["sha"] is not None)
same_pg = sum(1 for t in tags if t in a and t in b and (a[t]["P"], a[t]["G"], a[t]["chars"]) == (b[t]["P"], b[t]["G"], b[t]["chars"]))
print(f"tags={len(tags)} digest_equal={same_sha} digest_differs={diff_sha} missing={missing} rows_with_both_digests={both_sha} "
      f"(a day-26 tape records content_chars only, no sha256) P_G_chars_equal={same_pg}")
