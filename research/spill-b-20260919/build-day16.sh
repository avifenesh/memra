#!/usr/bin/env bash
# Day 16 native build receipt on the local RTX 5090: memra-server release at the pre-registration
# ref 9466b8912 (the two-arm binary and the A/B harness exist only there), built in a DETACHED
# worktree with its own target dir, never on the lane. No GPU cell, no lock.
# usage: build-day16.sh [worktree] [receipt-dir]
set -uo pipefail
wt=${1:-$HOME/projects/wt-spill-b-ab}
out=${2:-$HOME/projects/wt-spill-b/research/spill-b-20260919/rtx5090-day16/build-tip}
