#!/usr/bin/env python3
"""WP-B day 41 client (DAY41.md 1.3): the grid-checkpoint rewind arm's pricing workload, one boot = one arm.
    ...
    for L in lengths:
        for G in gens:
            for rep in range(a.n):
                ...
                prompts = []
                for turn in (1, 2, 3):
                    meta = dict(shape="RX", L=L, G=G, turn=turn, cold=False, rep=rep)
