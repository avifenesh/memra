...
set -uo pipefail
# RUN names a whole-order rerun (DAY31.md 1.8); ORDER_TIMEOUT is the collector hold per order in seconds.
R=/root/spill-receipts/b-day31${RUN:+-$RUN}; mkdir -p $R/bins
cd /root/wt-b || exit 1
export PATH=/root/.cargo/bin:/usr/local/cuda/bin:$PATH
log() { echo "$(date -u +%FT%TZ) $*" >> $R/chain.log; }
log "chain start HEAD=$(git rev-parse HEAD) RUN=${RUN:-} ORDER_TIMEOUT=${ORDER_TIMEOUT:-16200}"
...
for order in O1 O2; do
  ...
  env R=$R RIG_LOCK=/tmp/memra-gpu.lock BIN=$R/bins/memra-server WT=/root/wt-b NO_SCOPE=1 BURST=64 \
    ...
    python3 tools/tier-battery.py --rig pro-single --timeout "${ORDER_TIMEOUT:-16200}" --out $R/collector-$order --external-lock --execute \
