#!/usr/bin/env bash
# DAY37 local chain on the revised binary (addenda C and D): A2 again (grow-32768-r3), the gate set per arm again
# (gates-r3-*, twin on the 27B), then the serving boots of addendum B. Same rules as chain.sh.
set -uo pipefail
WT=$HOME/projects/wt-spill-b
R=$WT/research/spill-b-20260919/rtx5090-day37
cd "$WT" || exit 1
log() { echo "$(date -u +%FT%TZ) $*" >> "$R/chain-r3.log"; }
log "chain-r3 start HEAD=$(git rev-parse HEAD) server=$(sha256sum target/day37/r3/memra-server | cut -c1-16) gate=$(sha256sum target/day37/r3/kv-tier-gate | cut -c1-16) main=$(sha256sum target/day37/main/memra-server | cut -c1-16)"
if [ ! -e "$R/grow-32768-r3/collector.exit" ]; then
  GATE_BIN=$WT/target/day37/r3/kv-tier-gate bash research/spill-b-20260919/rtx5090-day37/run-grow.sh rtx5090 grow-32768-r3 >> "$R/chain-r3.log" 2>&1
  log "A2 r3 rc=$? $(head -1 "$R/grow-32768-r3/receipt/GROW.txt" 2>/dev/null)"
fi
export BIN=$WT/target/day37/r3/memra-server
for arm in pooled vmm; do
  out=$R/gates-r3-$arm
  if [ -e "$out/battery.log" ] && command grep -q "gates done arm=$arm" "$out/battery.log"; then log "gates r3 $arm: already done"; continue; fi
  for attempt in $(seq 0 90); do
    ...
    rc=$?
    if [ $rc = 0 ] || [ -e "$out/battery.log" ]; then log "gates r3 $arm: collector exit=$rc attempt=$attempt"; break; fi
    if ! command grep -q 'Resource temporarily unavailable' "$out/collector-$attempt.log"; then log "gates r3 $arm: collector failed rc=$rc before any cell"; break; fi
    sleep 120
  done
done
export LANE_BIN=$WT/target/day37/r3/memra-server
...
bash $B "$R" "${args[@]}"
python3 research/spill-b-20260919/day37-read.py rtx5090 "$R" gates-r3 > "$R/read-r3.log" 2>&1
log "chain-r3 done"
