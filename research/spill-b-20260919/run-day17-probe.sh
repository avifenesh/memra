#!/usr/bin/env bash
# Day 17 probe cell driver, local RTX 5090: research/spill-b-20260919/day17-probe.py through the collector
# with the inherited canonical lock (`--rig rtx5090`, `/tmp/memra-5090.lock`), the lane-tip release binary.
# usage: run-day17-probe.sh <cell-name> <collector-timeout-s> [probe args...]
set -uo pipefail
...
for attempt in 0 1 2 3 4 5; do
  ...
  run python3 tools/tier-battery.py --rig rtx5090 --timeout "$tmo" --out "$out" --external-lock \
    --execute python3 research/spill-b-20260919/day17-probe.py --external-lock @COLLECTOR_LOCK_FD@ \
