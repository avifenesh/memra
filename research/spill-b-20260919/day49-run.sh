#!/usr/bin/env bash
# DAY49 serving-shape boots (1.3): one boot per spec through run-day26-cell.sh (it takes the rig lock), after a bounded idle
# wait (at most 7200 s: lock free, no compute app, >= 24 GB host memory). Never a signal to anything this lane did not
# start. usage: day49-run.sh <receipt root> <name>:<arm> ...   arm = off (MEMRA_STEP_OOM_FAULT=1) | on (the same plus
#   MEMRA_BATCH_OOM_RECOVER=1). day45-client.py's warm, burst and probe; env CLIENT_EXTRA passes the burst shape.
...
BIN=${BIN:-$WT/target/day49/tip/memra-server}
...
for spec in "$@"; do
  ...
  uenv=(-u MEMRA_BATCH_OOM_RECOVER -u MEMRA_STEP_OOM_FAULT -u MEMRA_ADMIT_W_RELEASE -u MEMRA_ADMIT_PREDICT_SHADOW -u MEMRA_ADMIT_BY_MEMORY)
  aenv=(MEMRA_STEP_OOM_FAULT=1)
  B=$BIN
  case $arm in
    off) ;;
    on) aenv+=(MEMRA_BATCH_OOM_RECOVER=1) ;;
