#!/usr/bin/env bash
# vm-host-556c - #556 affected-case native check (teeth arm per the reviewed recipe research/spill-d-20260919/DAY8-CELLS.md:93, tenant cap disabled for the global-pool cell): host-tier demote/promote/restore identity plus graceful SIGTERM drain
...
EV=$W/host-556-${TESTED:0:10}-cache80-teeth-pct100
...
mkdir -p "$EV/lease"
{
    ...
    echo "gate_env=MEMRA_HOSTGATE_CACHE_MB=80 MEMRA_KV_HOST_TENANT_PCT=100 MEMRA_HOSTGATE_TEETH=1"
...
for arm in teeth; do
    ...
    (cd "$SRC" && memra-gpu-run --gpus "$GPU" --receipt "$EV/lease/$arm" -- \
        env MEMRA_GPU_LOCK=/tmp/memra-gpu.lock MEMRA_HOSTGATE_CACHE_MB=80 MEMRA_KV_HOST_TENANT_PCT=100 MEMRA_HOSTGATE_TEETH=$teeth \
