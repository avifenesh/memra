#!/usr/bin/env bash
# vm-host-556 - #556 affected-case native check: host-tier demote/promote/restore identity plus graceful SIGTERM drain
# Runs on the VM as root after vm-queue2. Evidence dir is never reused; a failed run stays on disk.
set -uo pipefail
...
EV=$W/host-556-${TESTED:0:10}
log() { echo "[$(date -u +%FT%TZ)] $*"; }

log "waiting for vm-queue2 (#654 A/B)"
while pgrep -f "vm-queue2.sh" >/dev/null; do sleep 30; done
...
for arm in default teeth; do
    ...
    (cd "$SRC" && memra-gpu-run --gpus "$GPU" --receipt "$EV/lease/$arm" -- \
        env MEMRA_GPU_LOCK=/tmp/memra-gpu.lock MEMRA_HOSTGATE_TEETH=$teeth \
