#!/usr/bin/env bash
# agentworld: bar cells — plain-vs-llama AND best-vs-best in one interleaved session
...
row() { # arm class metric value rep
  printf '{"ts":"%s","git":"%s","llama_build":"%s","cell":"aw-bar","arm":"%s","class":"%s","metric":"%s","value":%s,"rep":%s,"profile":"%s","temp_c":%s}\n' \
...
echo "=== AGENTWORLD BAR CELL $TS git=$GIT_SHA profile=$PROFILE llama=[$LLAMA_VER] K=($K_p1,$K_p2,$K_p3) ===" | tee -a "$R/aw-cell-console.log"
