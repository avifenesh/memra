...
# ARM env is baked by the caller: DIRECT=1 (naked direct) or DIRECT=0 (workspace form).
cd ~/memra
...
DIRECT=${DIRECT:-1}
EXTRA=""
[ "$DIRECT" = "0" ] && EXTRA="MEMRA_F16G_DIRECT=0"
for X in 16 32 64; do
  env MEMRA_MOE_F16G=2 MEMRA_F16G_SK_CROSS=$X $EXTRA \
