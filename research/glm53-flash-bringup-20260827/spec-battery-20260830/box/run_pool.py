#!/usr/bin/env python3
"""spec-battery pool driver (glm5 spec arc, box stage). stdlib only.

Subcommands:
  sample  --out DIR                       fresh-boot output sample (greedy 64, decode d00)
  cell    --out DIR --pool decode|l3|both --mode greedy|vendor --max-tokens N
          non-streaming rows; saves per-prompt tape (.txt = reasoning\\x00content), .json
          (full response), meta.json (usage/spec aggregation inputs)
  timed   --out DIR                       stage-4 per-boot cell: streamed greedy decode pool
          (TTFT + decode tok/s) + deep TTFT rows (l3 WARM+A4630) + ONE vendor sampled row
...
def cmd_cell(a):
    os.makedirs(a.out, exist_ok=True)
    meta = {"pool": a.pool, "mode": a.mode, "max_tokens": a.max_tokens, "rows": []}
    for tag, kind, text in load_pool(a.pool):
        ...
        print(f"[cell {a.mode}] {tag} wall={wall:.1f}s spec={row.get('spec')} err={err}")
...
def cmd_timed(a):
    ...
    for tag, kind, text in load_pool("l3"):
        ...
        out["deep_ttft"].append({"tag": tag, "ttft_s": ttft, "prompt_tokens":
                                 (usage or {}).get("prompt_tokens"), "err": err})
...
def main():
    ...
    a = p.parse_args()
    sys.exit({"sample": cmd_sample, "cell": cmd_cell, "timed": cmd_timed,
              "twin": cmd_twin, "compare": cmd_compare, "agg": cmd_agg}[a.cmd](a))
