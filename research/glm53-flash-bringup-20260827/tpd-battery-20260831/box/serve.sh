#!/usr/bin/env bash
# tpd-battery scoped serve (window: tpd-battery agent, cards 0/1/2, port 18400).
# Used for exactly TWO things this window: the c0 trace boot (untimed, tap armed) and
# the CELL-2 SERVED PP-3 CALIBRATION ROW (the tp2-battery method: engine twins cannot
# price PP arms, so the served baseline is re-measured on THIS build and must reproduce
# the banked 35.41/70.4-class rows before any engine-twin ratio is read as a serving
# projection). The served worker REFUSES MEMRA_GLM5_TP by design (v1), so no TP or diet
# door is ever set on a served boot — engage() forbids their announces.
...
BIN=/root/memra-tpd/target/release/memra-server
DFLASH_SHA8=b33c0347
mkdir -p "$OUT/logs"

stop() {
  [ -f "$PIDFILE" ] || { echo "[tpd] no pidfile, nothing to stop"; return 0; }
  local pid; pid=$(cat "$PIDFILE")
  if [ ! -d "/proc/$pid" ]; then echo "[tpd] pid $pid already gone"; rm -f "$PIDFILE"; return 0; fi
  local exe; exe=$(readlink -f "/proc/$pid/exe" 2>/dev/null || true)
  case "$exe" in
    "$BIN"|"$BIN (deleted)") ;;
    *) echo "[tpd] REFUSE stop pid=$pid exe=$exe (not our binary)"; return 1 ;;
  esac
  tr '\0' '\n' < "/proc/$pid/environ" 2>/dev/null | grep -qx "MEMRA_ADDR=127.0.0.1:18400" \
    || { echo "[tpd] REFUSE stop pid=$pid (not port 18400)"; return 1; }
  echo "[tpd] SIGTERM pid=$pid exe=$exe"
  kill -TERM "$pid"
  for _ in $(seq 1 90); do kill -0 "$pid" 2>/dev/null || break; sleep 1; done
  if kill -0 "$pid" 2>/dev/null; then echo "[tpd] SIGKILL pid=$pid"; kill -KILL "$pid"; sleep 3; fi
  rm -f "$PIDFILE"
  echo "[tpd] stopped"
}
...
start() {
  ...
  for _ in $(seq 1 600); do
    ...
    if ! kill -0 "$pid" 2>/dev/null; then echo "[tpd] BOOT DIED"; tail -15 "$log"; return 1; fi
    grep -qE "panicked|FATAL" "$log" && { echo "[tpd] BOOT FAILED"; tail -15 "$log"; return 1; }
    sleep 2
  done
  echo "[tpd] NOT READY after $((SECONDS-t0))s"; tail -20 "$log"; return 1
}

gates() {
  local name="$1" pid="$2" nonce="$3"; shift 3
  local log="$OUT/logs/boot-$name.log" fail=0
  tr '\0' '\n' < "/proc/$pid/environ" | grep -qx "TPD_NONCE=$nonce" \
    || { echo "[tpd] GATE FAIL: nonce not in /proc/$pid/environ"; fail=1; }
  local nres; nres=$(grep -c "RESIDENT" "$log")
  [ "$nres" -ge 3 ] || { echo "[tpd] GATE FAIL: RESIDENT lines=$nres (<3)"; fail=1; }
  local spec_env=""
  ...
  if [ -n "$spec_env" ]; then
    grep -q "\[glm5-spec\] serve route ARMED" "$log" || { echo "[tpd] GATE FAIL: no ARMED line"; fail=1; }
    grep -q "draft source = dflash2 @ $DFLASH_SHA8" "$log" \
      || { echo "[tpd] GATE FAIL: no 'draft source = dflash2 @ $DFLASH_SHA8' line"; fail=1; }
    grep -q "native MTP head NOT loaded" "$log" \
      || { echo "[tpd] GATE FAIL: head not reported NOT loaded"; fail=1; }
  else
    [ "$nspec" -eq 0 ] || { echo "[tpd] GATE FAIL: non-spec arm has $nspec [glm5-spec] lines"; fail=1; }
  fi
  {
  ...
  [ "$fail" -eq 0 ] && echo "[tpd] GATES GREEN boot=$name" || echo "[tpd] GATES RED boot=$name"
  return "$fail"
}
...
engage() {
  ...
  chk() { # expected(1/""), pattern, label
    ...
    if [ -n "$1" ]; then
      [ "$n" -ge 1 ] || { echo "[tpd] ENGAGE FAIL: expected '$3' announce, lines=$n"; fail=1; }
    else
      [ "$n" -eq 0 ] || { echo "[tpd] ENGAGE FAIL: forbidden '$3' announce, lines=$n"; fail=1; }
    fi
  ...
  fi
  {
    ...
    chk "" "\[glm5-ep-diet\] engaged" "glm5-ep-diet(served path must never arm it)"
    chk "" "\[glm5-ep-grouped-prime\] execute" "glm5-ep-grouped-prime(served path)"
    chk "" "\[glm5-tp-preflight\]" "glm5-tp-preflight(served worker refuses TP)"
    if [ -n "$spec_env" ]; then
  ...
  [ "$fail" -eq 0 ] && echo "[tpd] ENGAGE GREEN boot=$name" || echo "[tpd] ENGAGE RED boot=$name"
  return "$fail"
}
