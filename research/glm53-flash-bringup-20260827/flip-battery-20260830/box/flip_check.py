#!/usr/bin/env python3
"""Flip-table analyzer + escalation check (owner protocol 2026-08-30, this window).

Reads /root/out-flip2/c3/<arm><round>/timed.json for arms plain/k1/k2/k3, computes per
boot: decode-pool tok/s median, deep-pool tok/s median, pool TTFT median, deep TTFT rows,
vendor row (with the 3way 128-token floor guard, excluded rows named). Aggregates per
arm as median of boot-medians, then applies the escalation rules:
  (a) within-arm relative spread of the decode-tok/s boot-medians
      ((max-min)/median) > 0.5%  -> escalate that arm (+plain)
  (b) |spec_median - plain_median| <= 2 * pooled_spread, where pooled_spread =
      max(abs spread of the spec arm, abs spread of plain) in tok/s -> too close to call
Definitions stated here ARE the receipt. Exit 0 = x3 sufficient, exit 3 = ESCALATE.
"""
import glob, json, os, re, statistics as st, sys

BASE = "/root/out-flip2/c3"
...
def main():
    ...
    for d in sorted(glob.glob(os.path.join(BASE, "*"))):
        ...
        m = re.match(r"(plain|k1-|k2-|k3-)(\d+)$", name) or re.match(r"(plain)(\d+)$", name)
