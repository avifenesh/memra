#!/usr/bin/env bash
# flip-rebattery scoped serve (window: flip-rebattery agent, cards 0/1/2, port 18400).
...
stop() {
  [ -f "$PIDFILE" ] || { echo "[fb] no pidfile, nothing to stop"; return 0; }
  local pid; pid=$(cat "$PIDFILE")
  if [ ! -d "/proc/$pid" ]; then echo "[fb] pid $pid already gone"; rm -f "$PIDFILE"; return 0; fi
  local exe; exe=$(readlink -f "/proc/$pid/exe" 2>/dev/null || true)
  case "$exe" in
    "$BIN"|"$BIN (deleted)") ;;
    *) echo "[fb] REFUSE stop pid=$pid exe=$exe (not our binary)"; return 1 ;;
  esac
  tr '\0' '\n' < "/proc/$pid/environ" 2>/dev/null | grep -qx "MEMRA_ADDR=127.0.0.1:18400" \
    || { echo "[fb] REFUSE stop pid=$pid (not port 18400)"; return 1; }
  echo "[fb] SIGTERM pid=$pid exe=$exe"
  kill -TERM "$pid"
  for _ in $(seq 1 90); do kill -0 "$pid" 2>/dev/null || break; sleep 1; done
  if kill -0 "$pid" 2>/dev/null; then echo "[fb] SIGKILL pid=$pid"; kill -KILL "$pid"; sleep 3; fi
  rm -f "$PIDFILE"
  echo "[fb] stopped"
}
...
start() {
  ...
  for _ in $(seq 1 600); do
    ...
    if ! kill -0 "$pid" 2>/dev/null; then echo "[fb] BOOT DIED"; tail -15 "$log"; return 1; fi
    grep -qE "panicked|FATAL" "$log" && { echo "[fb] BOOT FAILED"; tail -15 "$log"; return 1; }
    sleep 2
  done
  echo "[fb] NOT READY after $((SECONDS-t0))s"; tail -20 "$log"; return 1
}
...
gates() {
  ...
  tr '\0' '\n' < "/proc/$pid/environ" | grep -qx "FLIPBAT_NONCE=$nonce" \
    || { echo "[fb] GATE FAIL: nonce not in /proc/$pid/environ"; fail=1; }
  local nres; nres=$(grep -c "RESIDENT" "$log")
  [ "$nres" -ge 3 ] || { echo "[fb] GATE FAIL: RESIDENT lines=$nres (<3)"; fail=1; }
  ...
  if [ -n "$spec_env" ]; then
    grep -q "\[glm5-spec\] serve route ARMED" "$log" || { echo "[fb] GATE FAIL: no ARMED line"; fail=1; }
    if [ -n "$dfl_env" ]; then
      ...
      grep -q "draft source = dflash2 @ $DFLASH_SHA8" "$log" \
        || { echo "[fb] GATE FAIL: no 'draft source = dflash2 @ $DFLASH_SHA8' line"; fail=1; }
      if [ -z "$mtp_env" ]; then
        grep -q "native MTP head NOT loaded" "$log" \
          || { echo "[fb] GATE FAIL: MTP flag unset but head not reported NOT loaded"; fail=1; }
      fi
    else
      grep -q "\[glm5-spec\] draft source = native-mtp" "$log" \
        || { echo "[fb] GATE FAIL: no native-mtp source line"; fail=1; }
      grep -q "MTP head loaded" "$log" \
        || { echo "[fb] GATE FAIL: no 'MTP head loaded' line"; fail=1; }
    fi
  else
    [ "$nspec" -eq 0 ] || { echo "[fb] GATE FAIL: PLAIN arm has $nspec [glm5-spec] lines"; fail=1; }
  fi
  {
  ...
  [ "$fail" -eq 0 ] && echo "[fb] GATES GREEN boot=$name" || echo "[fb] GATES RED boot=$name"
  return "$fail"
}
