#!/usr/bin/env bash
# lane/glm5-extract-general standing-battery runner (rig 5090, exactness only — rig law).
# The lane is renames + seam hoists with NO behavior change: every standing suite must
# stay green untouched, every engagement counter must keep counting, and the tp-gate's
# ep-map arms must keep refusing with the exact names they assert on (the gate drives the
# ALIAS name, which is the standing-receipt path; the general name is unit-gated).
#
...
OUT=research/glm53-flash-bringup-20260827/extract-general-20260831/receipts
...
echo "=========================================================="
if [ "$fails" -eq 0 ]; then echo "extract-general battery: ALL GATES PASS"; else echo "extract-general battery: $fails GATE(S) FAILED"; fi
exit "$fails"
