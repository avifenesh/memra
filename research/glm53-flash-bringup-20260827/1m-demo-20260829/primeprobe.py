#!/usr/bin/env python3
"""One rung of the 1M-context demonstration (lane/glm53-1m-demo): stream a REAL-corpus
prompt of a target token count through the serving surface, record the prime wall-clock
(time to first content token = the monolithic prefill on this eager model), then record
per-token arrival times for the decode so the same rung yields decode tok/s at that depth.
...
The corpus file is REAL text (Project Gutenberg prose, assembled by corpus-build.sh and
banked by sha256); the probe slices the first <chars> characters. Token counts are taken
from the server's own usage.prompt_tokens, never estimated. The request carries a finite
max_tokens so the session cache is sized prompt+max+8 and MEMRA_CTX stays the admission
ceiling, and it streams so the (cell-raised) deadline bounds only first-token time.

usage: primeprobe.py <label> <corpus.txt> <chars> <max_tokens> <greedy|vendor> <out.json>
"""
import json
import sys
import time
