#!/usr/bin/env bash
# STRUCT-BATTERY CELL 6 — census re-run on the WINNER (LANE.md §7: the census IS the door-X/M
...
# Usage: c6_census.sh "<winner door flags>" "<winner K pin env or ''>"
set -uo pipefail
WINNER_DOORS="${1:?pass the winner door flags (or '' for none)}"
WINNER_K="${2:-}"
OUT=/root/out-struct/c6
...
python3 - <<'PY'
import json
d = json.load(open("/root/memra-struct/research/glm53-flash-bringup-20260827/decode-attribution-receipts/prompts.json"))
...
{
  echo "== provenance =="
  git -C /root/memra-struct log -1 --format='%H %s'
  ...
  LE=/root/memra-struct/target/release/launch-econ
  [ -x "$LE" ] && "$LE" 3200 || echo "launch-econ missing (banked box constant 2.049 us/launch applies)"
} 2>&1 | tee "$OUT/struct-census-provenance.txt"
...
echo "PP_ENV=$PP_ENV" >> "$OUT/struct-census-provenance.txt"
...
env CUDA_VISIBLE_DEVICES=0,1,2 NVIDIA_TF32_OVERRIDE=0 \
  ...
  nsys profile --trace=cuda,osrt --duration=$DUR --kill=none \
    -o "$OUT/struct-census" "$BIN" > "$OUT/census-server.log" 2>&1 &
NSYS=$!
...
curl -s "http://127.0.0.1:${PORT}/v1/chat/completions" \
  -H 'Content-Type: application/json' -d @"$OUT/census-prompt.json" > "$OUT/struct-census-response.json"
T1=$(date +%s.%N)
python3 - "$OUT/struct-census-response.json" "$T0" "$T1" <<'PY' | tee "$OUT/struct-census-wall.txt"
import json, sys
...
wait "$NSYS" || true
ls -la "$OUT"/struct-census.nsys-rep
...
nsys stats --report cuda_gpu_kern_sum --format csv "$OUT/struct-census.nsys-rep" > "$OUT/struct-census-kernsum.csv"
nsys stats --report cuda_api_sum      --format csv "$OUT/struct-census.nsys-rep" > "$OUT/struct-census-apisum.csv" || true
CT=$(grep -o 'completion_tokens=[0-9]*' "$OUT/struct-census-wall.txt" | cut -d= -f2)
python3 /root/out-struct/c8_buckets.py "$OUT/struct-census-kernsum.csv" "$OUT/struct-census-apisum.csv" "${CT:-192}" \
  | tee "$OUT/struct-census-phase-buckets.txt"

echo "=== §7 kernel-row extract (vs matvec LANE.md §1) ==="
python3 - "$OUT/struct-census-kernsum.csv" <<'PY' | tee "$OUT/struct-census-doors-extract.txt"
import csv, io, sys
