#!/bin/bash
# grouped-prefill A/B serve: PID-VERIFIED restart, never name-matched (gate-stop-pkill trap).
# R2 residency shape (cell-ppn.sh) + the mandatory pins:
#   MEMRA_PREFIX_CACHE_MB=0  (glm5 restore defect: defence in depth, and pinned by the plan)
#   NVIDIA_TF32_OVERRIDE=0, CUDA_VISIBLE_DEVICES=0,1,2 (cards 2/3 stay untouched)
# usage: serve.sh <tag> [extra env...]   -> logs to ~/gpf-ab/serve-<tag>.log
set -u
TAG=$1; shift
BIN=$HOME/memra/target/release/memra-server
PIDFILE=$HOME/gpf-ab/server.pid
LOG=$HOME/gpf-ab/serve-$TAG.log
