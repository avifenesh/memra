...
def main():
    ...
    p.add_argument('--cold-reference', action='store_true', help='Unscored greedy cold reference only')
    p.add_argument('--max-new', type=int, default=2048)
    p.add_argument('--ctx', type=int, default=49152)
    ...
    a = p.parse_args()
    if a.cold_reference and not a.gate:
        raise ValueError("cold reference requires --gate; it cannot produce a performance score")
    ...
    identity = {
        ...
        'cache_policy': 'cold-correctness-reference' if a.cold_reference else 'stable-prompt-checkpoint-reuse-required',
        'audit_reuse_sha256': digest(Path(__file__).with_name('audit_reuse.py')),
    ...
    for cycle, order in cycles:
        ...
        for arm in order:
            ...
            if a.gate:
                cmd.append('gate-cold' if a.cold_reference else 'gate')
            ...
            if (not math.isfinite(elapsed) or elapsed <= 0
                ...
                raise ValueError('request times and pooled throughput disagree')
            if a.cold_reference:
                if any(t['resumed'] != 'false' or int(t['cached_tokens']) != 0
                       or int(t['checkpoint_tokens']) != 0 for t in turns):
                    raise ValueError('cold correctness reference retained cache state')
                reuse = {'mode': 'cold-correctness-reference'}
            else:
                reuse = audit_reuse(run)
            records.append({'seed': seed, 'arm': arm, **metrics, 'reuse': reuse})
        ...
        if 'native' in order:
            audit_control_ids(a.out, seed, [arm for arm in order if arm != 'native'] if a.gate else [arm for arm in order if arm == 'measured'], turns=8)
    report = {'status': 'greedy-identity-pass', 'arms': list(cycles[0][1]), 'turns_each': 8} if a.gate else summarize(records)
