#!/usr/bin/env python3
"""Day-eight integrated offline checks; raw logs and hashes, never a GPU receipt."""
import argparse
import datetime
import gzip
import hashlib
...
commands = [
    ...
    ['python3', '-m', 'py_compile', 'research/spill-a-20260919/storage_capture.py', *sorted(str(f.relative_to(ROOT)) for f in (ROOT/'tools').glob('tier-*.py'))],
    ...
    ['bash', '-n', 'tools/kv-host-spill-identity-gate.sh', 'tools/kv-host-spill-failure-gate.sh'],
    ['git', 'apply', '--reverse', '--check', 'research/spill-d-20260919/LEGACY-EXTERNAL-LOCK.diff'],
    ['python3', 'tools/tier-battery.py', '--validate', 'research/spill-lead-20260919/rented-5090-20260919/receipts-box2'],
...
paths = [*sorted((ROOT/'tools').glob('tier-*')),
         ...
         *sorted((ROOT/'crates/memra-tier/tests/battery').glob('*.json')),
         ROOT/'research/spill-d-20260919/RIG-DAY1.md',
         ROOT/'research/spill-d-20260919/DAY8-CELLS.md',
         ROOT/'research/spill-d-20260919/CAPTURE-CONTRACT.md',
         ROOT/'research/spill-d-20260919/LEGACY-EXTERNAL-LOCK.diff',
         ROOT/'research/spill-a-20260919/storage_capture.py', Path(__file__).resolve()]
