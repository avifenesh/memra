#!/usr/bin/env python3
"""Scoped day-nine CPU checks with raw logs; no hardware qualification."""
import argparse
import datetime
import gzip
import hashlib
...
commands = [
     ...
     *sorted(str(f.relative_to(ROOT)) for f in (ROOT/'crates/memra-tier/tests/battery').glob('*.py')),
     'research/spill-d-20260919/validate-day8-archives.py', str(Path(__file__).relative_to(ROOT))],
