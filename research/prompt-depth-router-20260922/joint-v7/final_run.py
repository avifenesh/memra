...
from run_native import (
    TOP_K, input_manifests, quality, run_one, save, sha, v4,
)
...
def require_confidence_engagement(root, variant):
    if variant is None:
        return
    summary = json.loads((root / "joint-selection-summary.json").read_text())
    sessions = [
        root / row["name"]
        for row in summary["records"]
        if row["variant"] == f"joint-{variant}"
    ]
    if len(sessions) != 3 or sum(
        int(turn["confidence_decisions"])
        for session in sessions
        for turn in v4.table(session / "turns.tsv")
    ) == 0:
        raise ValueError("selected joint C model made no native confidence decisions")
...
def qualifier(args, fresh, topk, joint, router):
    ...
    if joint["variant"] is not None and any(
        ...
        raise ValueError("v7 sampled qualifier did not engage the C model")
...
def main():
    ...
    if args.phase == "heldout":
        require_confidence_engagement(args.out, joint["variant"])
    records = (
