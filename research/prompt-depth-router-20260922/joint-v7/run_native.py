"""Pinned Qwen draft-only top-k qualification and development collector."""
...
def record(root, label, k, arm, audit, schedule=None):
    observed = [
        int(row["draft_top_k"]) for row in v4.table(root / "turns.tsv")
    ]
    return {
        ...
        "draft_schedule": list(schedule) if schedule is not None else None,
        "observed_draft_top_k": observed,
...
def validate(root, entry, arm):
    ...
    for turn in range(1, 9):
        ...
        if len(confidence) != len(current):
            raise ValueError("v6 randomized confidence and depth inventories differ")
        for offered, selected in zip(confidence, current):
            ...
            if (
                ...
                raise ValueError("v6 sampled D assignment differs from actual offer")
    ...
    if path.exists():
        if json.loads(path.read_text()) != exposure:
            raise ValueError("v6 randomized D exposure changed on resume")
    else:
        save(path, exposure)
    return audit
...
def run_one(args, entry, workloads, topic, label, k, arm, cap, *,
    ...
    if schedule is not None:
        if len(schedule) != 8 or any(value not in TOP_K for value in schedule):
            raise ValueError("draft schedule needs eight allowed top-k actions")
        command.append("draft-schedule=" + ",".join(map(str, schedule)))
    ...
    options = {
        ...
        for option in extra if "=" in option
    }
    policy_hashes = {
        kind: sha(options[kind])
        for kind in ("depth-model", "confidence-model")
        if kind in options
    }
    if "joint-model-dir" in options:
        router = Path(options["topk-model"]).read_text().splitlines()
        actions = [
            int(line.split("\t")[1])
            for line in router if line.startswith("K\t")
        ]
        directory = Path(options["joint-model-dir"])
        for k_action in actions:
            for kind in ("depth", "confidence"):
                relative = f"topk{k_action}/{kind}-{options[f'{kind}-variant']}.tsv"
                policy_hashes[relative] = sha(directory / relative)
    command_record = {
        ...
        "policy_model_sha256": policy_hashes,
    ...
    if root.exists():
        ...
        if any(int(row["sampler_top_k"]) != 20 for row in turns):
            raise ValueError("target sampler K changed")
        actual = [int(row["draft_top_k"]) for row in turns]
        if (
...
def qualify(args, fresh):
    ...
    for record in records:
        ...
        functional[str(record["draft_top_k"])] = sum(outcomes)
    eligible = {
        str(record["draft_top_k"]): (
            record["format"] == 8 and record["loops"] == 0
            and functional[str(record["draft_top_k"])] == 8
        )
        for record in records
    }
    if not eligible["20"]:
...
def development(args, old):
    ...
    for index, entry in enumerate(topics):
        ...
        if assigned["training_index"] != index or assigned["workload_sha256"] != entry["sha256"]:
            raise ValueError("frozen draft schedule differs from this conversation")
        arms = [
            (f"topk{k}-d3-c0", k, "fixed:3", 3, ())
            for k in TOP_K
        ]
        ...
        if schedule_key is not None:
            records.append(run_one(
                args, entry, args.development, f"development-{index}",
                "random-draftk", 20, "fixed:3", 3,
                schedule=assigned[schedule_key],
            ))
    for k in learnable:
        counts = defaultdict(int)
        for record in records:
            if record["draft_top_k"] != k or record["arm"] != "explore-d":
                continue
