"""Seal the corrected single-head C/K/D continuation without checkpoint weights."""
...
def seal(args):
    ...
    for name in REQUIRED:
        if not (args.results / name).is_file():
            raise ValueError("v7 native study lacks required receipt " + name)
    engagement = json.loads((args.results / "engagement-result.json").read_text())
    if (
        engagement["status"] != "single-head-learned-C-engaged"
        or engagement["source_sha256"] != source_record["source_archive_sha256"]
        or engagement["binary_sha256"] != sha(args.binary)
        or engagement["confidence_decisions"] <= 0
    ):
        raise ValueError("v7 learned C engagement differs from sealed binary")
    qualification = json.loads(
        (args.results / "policy-qualification-result.json").read_text()
    )
    if qualification["confidence_decisions"].get("joint-learned", 0) <= 0:
        raise ValueError("v7 final qualifier did not engage learned C")
    named = {
    ...
    for folder, prefix in ((args.results, "native"), (args.models, "models")):
        for path in sorted(folder.rglob("*")):
            if path.is_symlink():
                raise ValueError("v7 receipt symlink cannot be sealed")
            if path.is_file():
                named[f"{prefix}/{path.relative_to(folder).as_posix()}"] = path
    files = {}
    for name, path in sorted(named.items()):
        if not path.is_file() or path.is_symlink() or path.stat().st_size == 0:
            raise ValueError("missing or empty v7 study evidence " + name)
        files[name] = {"bytes": path.stat().st_size, "sha256": sha(path)}
    manifest = {
        "schema": 1,
        "scope": "Qwen3.8 full-head fixed-target draft-only K/C/D code study with sampled single-head learned C",
