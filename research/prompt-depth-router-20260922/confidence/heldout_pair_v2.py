...
def main():
    ...
    if manifest["schema"] != 2 or manifest["binary_sha256"] != sha(args.binaries / "qwen-prefix-study"):
        raise ValueError("code-only v2 corpus has another binary or schema")
    if manifest["generator_sha256"] != sha(Path(__file__).with_name("code_only_v2_workloads.py")):
        raise ValueError("code-only v2 task generator changed")
    if manifest["original_manifest_sha256"] != sha(
        args.workloads.parent / "heldout-workloads/manifest.json"
    ):
        raise ValueError("original held-out scenarios changed before code-only v2")
    ...
    freeze = {
        ...
        "max_new": 8192,
        "ctx": 32768,
        "qualification_scope": "code-only-v2",
        "scope": "Qwen requested-code native held-out comparison; prose is diagnostic",
    }
    ...
    try:
        ...
        save(args.out / "status.json", state)
        code = [row for row in fmt["requests"] if row["kind"] == "code"]
        if len(code) != 4 or not all(
            row["format"]["requested_format_covered"] and not row["loop"] for row in code
        ):
            raise ValueError("code-only v2 format qualification failed before C selection")

        # The development report stays unopened until the disjoint prompt
        # code requests and their independent format qualification are frozen.
