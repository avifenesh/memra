"""Fit separate C/D learners at each quality-eligible MTP draft top-k."""
...
def fit(root, models, views):
    ...
    for k in (3, 10, 20):
        ...
        randomized = [
            ...
            if row["draft_top_k"] == k and row["arm"] == "explore-d"
        ]
    ...
    save(root / "model-training-summary.json", {
        "schema": 1,
        "scope": "temperature 1.0, target top-k=20, separate draft-K models",
        "top_k": result,
    })
    return result
