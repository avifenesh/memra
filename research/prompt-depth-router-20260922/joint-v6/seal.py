"""Seal draft-only top-k and C/D study receipts without checkpoint weights."""
...
def seal(args):
    ...
    for name in REQUIRED:
        if not (args.results / name).is_file():
            raise ValueError("v6 native study lacks required receipt " + name)
    named = {
    ...
    manifest = {
        "schema": 1,
        "scope": "Qwen3.8 full-head fixed-target and draft-only K/C/D code study",
