"""Measure static sampler K, draft D and after-offer C on development."""
...
def depth_grid(args, old):
    ...
    if 20 not in allowed:
        raise ValueError("recommended sampler K failed qualification")
    records = []
...
def q_values(root, k):
    ...
    for row in development["records"]:
        if row["sampler_top_k"] != k or row["arm"] != "explore-d":
            continue
...
def choose_best_depth(args, old):
    ...
    all_rows = [
        ...
        if isinstance(row["sampler_top_k"], int) and row["arm"] == "fixed:3"
    ] + depth["records"]
