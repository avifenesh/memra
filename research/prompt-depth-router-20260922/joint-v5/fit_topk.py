"""Fit a tiny first-user-token-prefix sampler K router from randomized turns."""
...
def randomized_rows(root, summary, variant, allowed):
    ...
    scheduled = [
        row for row in summary["records"] if row["variant"] == "random-topk"
    ]
...
def fit(root, models):
    ...
    for variant in FEATURES:
        ...
        lines = [f"joint-v5-topk\t1\t{variant}\t{lam:.12g}\t20"]
        for k in allowed:
            weights = (
                ridge(data[k], lam, 20)
