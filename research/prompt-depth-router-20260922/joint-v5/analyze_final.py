...
def score(root):
    ...
    for arm, sessions in by_arm.items():
        ...
        for row in all_rows:
            ...
            for turn in v4.table(session / "turns.tsv"):
                k_actions[int(turn["sampler_top_k"])] += 1
        ...
        arms[arm] = {
            ...
            "sampler_k_turns": dict(sorted(k_actions.items())),
    ...
    return {
        "schema": 1,
        "scope": "six fresh eight-turn Qwen code conversations on one nonproduction RTX 5090",
