"""Verify the sealed v5 archive and recompute fresh E2E metrics on hosted CPU."""
    ...
    with tarfile.open(archive, "r:gz") as stream:
        for member in stream:
            ...
            if (
                ...
                raise ValueError("unsafe or changed v5 archive member " + name)
            seen.add(name)
            destination = (
                target.joinpath(*logical.parts)
                if logical.parts[0] in ("native", "models") else None
            )
            ...
            if digest.hexdigest() != expected[name]["sha256"]:
                raise ValueError("v5 evidence member hash differs " + name)
    if seen != set(expected):
        raise ValueError("v5 archive omits a sealed member")
    return len(seen)
...
def replay(archive, manifest_path):
    ...
    if manifest["schema"] != 1 or sha(archive) != manifest["archive_sha256"]:
        raise ValueError("v5 archive differs from sealed manifest")
    with tempfile.TemporaryDirectory(prefix="joint-v5-replay-") as tmp:
        root = Path(tmp)
    ...
    return {"status": "all-v5-hashes-and-E2E-score-match", "members": members}
