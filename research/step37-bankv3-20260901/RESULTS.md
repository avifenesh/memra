...
# The down8 DEFAULT-ON qualification (2026-09-01)

Directive: down8 improves (+5.48% decode / +5.09% wall, x5 interleaved, no boot overlap), so it
ships default ON; this section is the deploy-grade receipt set the new-flags law requires to ride
in the same merge. Lane commits `afc681b1a` (the flip) and `c902066ec` (the mechanical overlap
test + phase scripts).

## Two findings that change the shape of the flip, before any number

Both were found by reading the call site before spending card time, and both are now receipts.

### 1. "Flip ONLY down8" ships a measured NOTHING. It is a SILENT no-op.

`down8` is armed at its call site (`tp.rs`) by

```rust
let down8 = device_routed && sel_down8_on() && shard.slot_major && (shard.local_in >> 5) <= 32;
```

and `shard.slot_major` is `ep2 || bank_slot_major_on()` — recorded once at bank build, per the
milestone-3 refactor that made layout a property of the bank rather than of the environment. So on
the TP serving path, with `MEMRA_NVFP4_BANK_SM` still default OFF, a default-ON
`MEMRA_NVFP4_SEL_DOWN8` reaches the code, is read, and then **does nothing**: no refusal, no
warning, no error, and no +5.48%.

Measured rather than argued, on the pre-flip binary `0ab9551f60989821f15b9c69a34762de` where the env
is the axis. Arm `D8ONLY`, mode `v3-d8only`: `MEMRA_NVFP4_SEL_DOWN8=1` with `MEMRA_NVFP4_BANK_SM`
**absent** from `/proc/<pid>/environ`:

```
[nvfp4-bank]  layout=block-nvfp4-v1 source=default tp=2 experts=288 in_f=4096 out_f=1280
[nvfp4-sweep] down8=false door=true device_routed=true slot_major=false nsb=20 in_class=true n_sel=8
```

Read that second line field by field, because it isolates the cause to exactly one of them:
`door=true` (the env DID reach the code — this is not a missing-receipt defect), `device_routed=true`
and `nsb=20 in_class=true` (both eligibility conditions MET), and `slot_major=false`. One field, and
the program does not run. RESULTS.md already said "the deployable unit is those two together"; the
directive's "leave the other two programs OFF" collides with that, and the collision is surfaced
rather than shipped.

**So the flip is ONE COUPLED DECISION over TWO defaults**, and `docs/FLAGS.md` records it as one
decision in two rows:

| door | before | after | why |
|---|---|---|---|
| `MEMRA_NVFP4_BANK_SM` | off | **ON** | bit-identical and measurably FREE on its own; flips solely as down8's prerequisite |
| `MEMRA_NVFP4_SEL_DOWN8` | off | **ON** | the only program that separates; +5.48% decode at x5, re-qualified below |
| `MEMRA_NVFP4_SEL_GU` | off | **off** | re-decided, not inherited: free but unmeasurable, and better-wins-by-default rewards *better*, not *free* |
| `SM_STREAM` / `GU_RPW` / `GU_WPR` | off | **off** | unpriced by design; `SM_STREAM` is now ONE env var from the serving path, so its row says so louder |

### 2. The shape this flip actually deploys was NEVER PRICED.

Milestone 4 ran **cumulative** arms — `v3-off` → `v3-sm` → `v3-sm-gu` → `v3-sm-gu-d8` — so down8's
+5.48% is the delta from `sm+gu` to `sm+gu+d8`. The flip leaves `SEL_GU` OFF, i.e. it deploys
**`sm+d8`**, and there is no row for that arm anywhere in this lane or any other. A default flip
justified by a delta measured on top of a program the flip does not enable is a bundle price by
another name — the exact error this lane exists to end. The battery below prices `sm+d8` directly,
as the DEFAULT.

## How the arms are identified, given that the flip arms carry NO door env

This is the part a pre-flip env A/B does not have to solve. In the `flip-on` arm nothing is set —
the default is the program under test — so the `/proc/<pid>/environ` census that milestone 4 trusted
is **byte-identical between `flip-on` and a pre-flip `v3-off`**. Three independent bindings replace it:

1. **binary md5** — `e7242c8e4fd012dfb2d721242463d74a`, the flipped build (`afc681b1a`, 264 s forced
   build, `build_seconds` asserted `>= 60` so a no-op build or a failed checkout cannot pass as a
   rebuild).
2. **a FLIPMARK marker test in BOTH directions.** `door_default_on()`'s WARN string exists only in a
   flipped binary. A `flip-*` mode REFUSES a pre-flip binary; every explicit-env mode REFUSES a
   flipped one. Both directions were **executed and observed to abort**, not merely added:
   ```
   ABORT: mode flip-on measures the DEFAULT, but this binary has no default-ON door (pre-flip
          build) — its 'no door env' arm would be the OFF program wearing the ON label
   ABORT: mode v3-off arms doors EXPLICITLY and asserts the others ABSENT, but this binary
          defaults them ON — an 'absent' door here would be armed anyway
   ```
   Marker-string census: pre-flip binary 0 hits, flipped binary 1 hit.
3. **the announce SOURCE**, and this is why the engine now prints it. `source=default-on` and
   `source=env=1` are different facts about the same boolean, and only the first one proves a
   DEFAULT was measured. `assert-engagement.sh` requires the exact source string per mode and
   ABORTS the cell on a miss, so no row from an unproven arm can reach an aggregate.

## Rung 4 first: exactness on the flip shape (gates before pricing)

Run before any pricing, because `TRAP:corrupt-arm-inflates-its-own-perf-price` is this lane's own
history: the 2026-08-29 door was priced at +23.7% *while serving wrong text*. A perf row from an arm
whose gates did not re-run is worthless.

Both arms on the flipped binary, spec OFF (so the tape measures the bank and not the draft/verify
scheduler), the same 18-cell corpus: 5 short margin-sensitive probes, 8 real agentic turns, and the
prefill ladder to **4,555 prompt tokens**.

| pair | what it settles | cells | verdict |
|---|---|---|---|
| `FOFF` → `FON` | the FLIP changes no byte | 18 identical, 0 differing | **PASS** |
| `GD82` → `FON` | the DEFAULT-armed program equals the pre-flip EXPLICITLY-armed program, across binary generations | 18 identical, 0 differing | **PASS** |
| `M0` → `FOFF` | the ROLLBACK SEAM really is pre-restore `main` | 18 identical, 0 differing | **PASS** |

**One tape sha for all of them, and it is the same value the five milestone-4 arms produced:**
`d6fe97d0a1928e1b6c9d43985557189d41a3f75533e3da91faaff473d7624e68`. The `GD82` → `FON` row is the
receipt that matters most and it is one md5 inequality could not give: this toolchain is **not
byte-reproducible** (measured in milestone 4: two forced builds of identical source differ), so the
only way to show the flipped binary's default program IS the program that was gated and priced
before the flip is to compare their output bytes. Content oracle green in both arms
(`all_content_ok=True`); the 25-token `17*23` probe answers `Got it, let's calculate 17 times… 391`
— the correct arm's fingerprint from the 2026-08-29 bisect, character for character.

Engagement verified per arm from the server log, 4 required facts each, including
`source=default-on` for `FON` and `source=env=0 (rollback seam)` for `FOFF`.

## Rung 1+2: the 12-boot interleaved battery, both request shapes, same boots

Six boots per arm, strictly interleaved `FOFF`/`FON` (LAW:interleaved-ab — cross-run and cross-day
comparisons are clock-drift-invalid, and this box has drifted before). Real prompts (8 agentic
turns, one measured rep each), `max_tokens` capped at 512, one binary, **12 distinct boot nonces**,
spec on at the serving policy (K=3, MTP 3), and **zero loop exclusions in all 24 boot-cells**.

Both request shapes come out of the SAME boots, always in the same order in both arms, so the two
tables are paired and carry no cross-boot drift between them.

### Vendor-default sampled — THE SERVING SHAPE (no sampling params; models.toml t=0.5/top_p=0.9 governs)

| arm | doors | decode tok/s | spread | wall tok/s | spread | TTFT | accept | loops |
|---|---|---|---|---|---|---|---|---|
| `FOFF` | rollback seam (`=0`) | **106.76** | 2.86% | **101.20** | 2.92% | 0.235 | 0.785 | 0 |
| `FON` | **the DEFAULT** (no env) | **112.57** | 2.34% | **107.19** | 1.48% | 0.239 | 0.780 | 0 |

Delta **+5.44% decode / +5.92% wall**.

### Greedy — the byte-deterministic instrument, never the product

| arm | decode tok/s | spread | wall tok/s | spread | TTFT | accept | loops |
|---|---|---|---|---|---|---|---|
| `FOFF` | **113.68** | 3.07% | **107.28** | 2.09% | 0.244 | 0.785 | 0 |
| `FON` | **121.12** | 4.05% | **114.00** | 2.25% | 0.246 | 0.779 | 0 |

Delta **+6.54% decode / +6.27% wall**. Greedy is here only because it removes sampling variance from
the comparison, so the overlap test reads the program rather than the RNG. It is never the serving
number, and the flip decision rests on the vendor-default table above.

### The decision: RULE B fires on the medians, and the NON-OVERLAP test decides

`RULE A` fires on every arm (within-arm spread > 0.5%) and `RULE B` fires on the delta in both shapes
(+5.81 tok/s against a 3.22 tok/s pooled spread; +7.44 against 4.91). **So the median table cannot
separate the arms, and saying so is part of the result** — it is the same situation milestone 4 was
in, where RULE B fired on all three programs. The per-boot ranges decide, and unlike milestone 4 the
test is now code (`aggregate.py`), validated by reproducing that cell's hand-computed verdicts
exactly before being used here.

| shape | metric | `FOFF` per-boot range | `FON` per-boot range | verdict |
|---|---|---|---|---|
| vendor | decode | `[105.58, 108.64]` | `[111.16, 113.80]` | **SEPARATED** — min(FON) 111.16 > max(FOFF) 108.64 |
| vendor | wall | `[100.80, 103.75]` | `[106.13, 107.72]` | **SEPARATED** — 106.13 > 103.75 |
| greedy | decode | `[112.25, 115.74]` | `[118.70, 123.62]` | **SEPARATED** — 118.70 > 115.74 |
| greedy | wall | `[106.61, 108.86]` | `[112.72, 115.28]` | **SEPARATED** — 112.72 > 108.86 |

Four separations out of four, on both metrics in both request shapes, with a 2.52 tok/s gap between
the closest pair of ranges. **And the per-boot sign is UNANIMOUS: 6/6 boots favour `FON` on decode
and on wall, in both shapes.**

| boot | vendor: FOFF → FON decode | delta | greedy: FOFF → FON decode | delta |
|---|---|---|---|---|
| 1 | 107.09 → 111.59 | +4.20% | 113.23 → 118.70 | +4.84% |
| 2 | 106.47 → 113.63 | +6.73% | 115.74 → 123.62 | +6.80% |
| 3 | 105.58 → 113.80 | +7.78% | 113.16 → 121.51 | +7.38% |
| 4 | 105.95 → 112.57 | +6.25% | 114.25 → 121.54 | +6.39% |
| 5 | 107.06 → 111.16 | +3.83% | 112.25 → 120.72 | +7.54% |
| 6 | 108.64 → 112.58 | +3.62% | 114.12 → 119.75 | +4.93% |

### The win is not an artifact, and that is checked rather than assumed

- **Zero loop exclusions** across 192 measured reps (96 per shape). Every rep hit the 512-token cap
  uniformly in both arms.
- **Acceptance FALLS slightly as the doors arm** (0.785 → 0.780 vendor, 0.785 → 0.779 greedy). The
  degenerate-output signature is the opposite: a looped arm repeats cheap high-accept tokens and
  inflates acceptance *and* tok/s together. This is the same check that exposed the era measurement,
  whose door-ON arm reported acceptance 0.970 against 0.935 OFF.
- **TTFT is flat** (0.235 → 0.239 vendor, 0.244 → 0.246 greedy, +1.1…+1.8%). A fused decode sweep
  should not move prefill, and it does not.
- **Spec engaged in every one of the 192 reps** (`spec_rounds > 0` from the response body, not a
  200) — the 2026-08-25 DFlash2 lesson, where the plain path answered fluently at half speed.

### Cross-checking the two cells against each other

`FOFF` here is the rollback seam on the flipped binary; milestone 4's `V3OFF` was the doors-off arm
on the pre-flip binary. Same program, different cell, different day: **106.76 vs 106.78 decode**
vendor-default. That agreement is not required by anything and is a free consistency check on the
box and the harness.

`FON` (`sm+d8`, GU off) measures 112.57 against milestone 4's `V3SMGUD8` (`sm+gu+d8`) at 113.83. The
gap is 1.1%, well inside both arms' spreads, which is what "PROGRAM 2 earns nothing measurable"
predicts — and it is the first direct evidence for that prediction, since no cell had ever run
`sm+d8` without `gu`.

## Rung 3: the 8-turn larger-prompt cache twin

`LAW:multiturn-cache-twin`. Two real 8-turn agentic conversations from the owner-blessed
`mt-agentic` packs, re-minted on the rig with `research/multiturn-cache-20260821/mint-mt-packs.py`
and verified **16/16 against the committed `packs-MANIFEST.json` sha16 list**, so the corpus is
byte-pinned without committing private session text (the pack texts and the replay transcript stay
on the box; only counters and hashes are banked here). Depth reached: **41,474** and **29,099**
prompt tokens by turn 8 — roughly 9x the single-shot ladder.

Three shaping choices, each from a banked trap rather than from taste:

- **`max_tokens` 4096, not 1024.** `QUIRK:step37:think-budget-wall-1024`: on agentic prompts this
  model's think phase alone exceeds 1024 at every depth, so a 1024 budget makes a contentless 200
  the DEFAULT outcome and fills the replayed history with nothing.
- **Fixed-history replay.** Two self-fed conversations fork after the first divergent turn and stop
  being an A/B. `MTOFF` banks its per-turn assistant CONTENT and `MTON` replays it, so every turn's
  rendered prompt is byte-identical across arms. `mt_bench` replays content only and never
  reasoning (`TRAP:reasoning-replayed-as-history-content`).
- **A cache-bust arm**, because `cached_tokens` is a counter and a counter is not evidence until
  something makes it move for a known reason.

### Cache engagement is PROVEN, not reported

The decisive probe: `--bust-turn 5` gives turns 5-8 a fresh `cache_salt` in the same arm and the
same boot. Turns 1-4 are untouched by construction and measure identically; turns 5-8 collapse.

| | turns 1-4 (not busted) | turns 5-8 (busted) |
|---|---|---|
| `cached_tokens`, normal vs bust | 18,400 vs **18,400** (identical) | 170,080 vs **0** (collapsed) |
| TTFT total, normal vs bust | unchanged | 29.74 s vs **91.80 s** — **3.09x slower** |

Single sharpest cell, `mt-agentic-01` turn 8 at 41,474 prompt tokens: TTFT **1.255 s** with 38,976
tokens cached, **16.379 s** with a fresh namespace. The counter tracks real KV reuse.

Worth recording as a serving observation, since it dwarfs anything the flip does: cache hits are
**intermittent across turns**, and the misses line up with the previous turn ending
`finish_reason=length` rather than on EOS (only EOS-terminated sessions park). `mt-agentic-01` turn 6
hit the 4096 cap, and turn 7 then re-prefilled 39,019 tokens cold at **14.25 s** TTFT while its
neighbours resumed in ~1-3 s. That is a per-turn TTFT cliff worth its own lane; it is not caused by
and not affected by this flip.

### Per-turn A/B, both arms, cache on, spec on

| conv | turn | prompt tok | cached (both arms) | TTFT OFF → ON | decode OFF → ON | delta | accept OFF / ON | content sha |
|---|---|---|---|---|---|---|---|---|
| 01 | 1 | 7,203 | 0 | 2.611 → 2.651 | 101.65 → 107.48 | **+5.74%** | 0.7488 / 0.7488 | **match** |
| 01 | 2 | 8,840 | 0 | 3.239 → 3.293 | 107.23 → 113.34 | **+5.70%** | 0.7883 / 0.7883 | **match** |
| 01 | 3 | 10,350 | 8,800 | 0.754 → 0.766 | 108.65 → 114.89 | **+5.74%** | 0.8016 / 0.8016 | **match** |
| 01 | 4 | 17,551 | 0 | 6.245 → 6.351 | 97.45 → 102.67 | **+5.36%** | 0.7524 / 0.7524 | **match** |
| 01 | 5 | 31,911 | 17,504 | 5.539 → 5.629 | 95.92 → 100.73 | **+5.01%** | 0.8297 / 0.8297 | **match** |
| 01 | 6 | 38,546 | 31,872 | 2.839 → 2.880 | 96.62 → 101.18 | **+4.72%** | 0.8346 / 0.8346 | **match** |
| 01 | 7 | 39,019 | 0 | 14.250 → 14.462 | 81.19 → 84.43 | **+3.99%** | 0.7938 / 0.7938 | **match** |
| 01 | 8 | 41,474 | 38,976 | 1.255 → 1.277 | 93.16 → 97.18 | **+4.32%** | 0.8252 / 0.8252 | **match** |
| 09 | 1 | 2,823 | 0 | 1.089 → 1.106 | 111.16 → 117.92 | **+6.08%** | 0.7978 / 0.7978 | **match** |
| 09 | 2 | 7,356 | 0 | 2.572 → 2.620 | 111.19 → 117.60 | **+5.76%** | 0.8276 / 0.8276 | **match** |
| 09 | 3 | 9,633 | 0 | 3.446 → 3.507 | 100.43 → 106.28 | **+5.82%** | 0.8037 / 0.8037 | **match** |
| 09 | 4 | 15,467 | 9,600 | 2.267 → 2.296 | 104.00 → 109.84 | **+5.62%** | 0.8522 / 0.8522 | **match** |
| 09 | 5 | 17,679 | 15,424 | 1.013 → 1.028 | 97.77 → 102.85 | **+5.20%** | 0.9073 / 0.9073 | **match** |
| 09 | 6 | 20,514 | 17,632 | 1.227 → 1.241 | 101.14 → 106.37 | **+5.17%** | 0.8678 / 0.8678 | **match** |
| 09 | 7 | 28,211 | 20,480 | 2.982 → 3.018 | 95.08 → 99.28 | **+4.42%** | 0.8665 / 0.8665 | **match** |
| 09 | 8 | 29,099 | 28,192 | 0.631 → 0.629 | 94.40 → 99.95 | **+5.88%** | 0.8134 / 0.8134 | **match** |

Four things this table settles that no single-shot cell could:

1. **The win holds at every depth, in every turn: 16/16 positive, +3.99% to +6.08%.** It does not
   decay as context grows — turn 8 at 29k tokens is +5.88%.
2. **A multi-turn exactness gate at ~9x the ladder's depth: 16/16 `content_sha16` MATCH.** The
   single-shot tape reached 4,555 prompt tokens; this reaches 41,474 and the bytes are still equal.
3. **Acceptance is IDENTICAL to four decimal places in all 16 turns.** With the history fixed and
   the program bit-identical, the draft/verify sequence is the same sequence — which is a much
   stronger statement than "acceptance did not regress".
4. **`cached_tokens` is identical between arms in every turn**, so the flip does not perturb cache
   or continuation-pool behaviour.

One honest blemish: TTFT is **systematically ~0.4-1.7% higher in the ON arm** (15 of 16 turns).
It is sub-2%, it matches the direction seen in the single-shot battery (+1.1% greedy, +1.8% vendor),
it is swamped several times over by the decode gain (net wall +5.92% measured), and the flip touches
no prefill kernel — the slot-major permutation happens once at bank build, in host memory. It is
recorded as unattributed rather than explained away; the two arms were separate boots, and a ~1%
TTFT offset is inside this box's boot-to-boot variation.

**Scope stated rather than implied:** this twin runs the `{cache on} x {spec on}` quadrant with the
door as the axis. The full `LAW:multiturn-cache-twin` matrix also asks for cache-off and plain-spec
arms; those measure the cache and the scheduler, which this flip does not touch, and they are
already banked for this model family elsewhere. The greedy request shape is used here deliberately,
because fixed-history byte identity is what makes the per-turn comparison an A/B at all — the
serving-shape numbers are the vendor-default sampled table above.

## Rung 4 (continued): the device oracle and its localizing teeth arm, on the flipped tree

Re-run from the flipped checkout rather than inherited from milestone 4's, because "the flip changes
no kernel" is a claim about a tree and this lane gates claims about trees. Forced builds throughout
(`touch build.rs`, which owns the `nvcc --fatbin` step and declares `rerun-if-changed=cu/qmatvec.cu`).

| arm | md5 | oracle exit | verdict |
|---|---|---|---|
| `clean1` (tree as committed) | `31aee4279f1b0fcad2f686d22a612e8f` | **0** | **PASS** — every cell bit-identical |
| `teethA` (`*1.5f` inside `_sel_v2` only) | `f099070fc55ad99364fa071cefe88293` | **1** | **FAIL** — the gate has teeth |
| `clean2` (reverted, rebuilt) | `94cbdc2da5c0cf72211572bfffb55cca` | **0** | **PASS** — the injection left no residue |

Clean arm green across **all four prefill tile forms** — shipping hybrid (`cross=64`),
`MEMRA_F16G_SK=128`, `MEMRA_F16G_SK=32`, `MEMRA_F16G_TAIL=0` — for 4 clean exits, every cell
reporting `nonzero_oracle = elems` and `finite = true` so no row is a comparison of zeros. All three
decode programs covered (P1 `_sel_v2`, P2 `_sel_v2_gu` both halves, P3 `_down8` vs `_sel` +
`axpy_rows_seq_md`), every oracle arm pinned to **v1 bytes and the v1 kernel**.

**The teeth arm FAILS and it LOCALIZES**, which is the property that makes it a per-program
instrument rather than a per-binary one:
...
2 cells deviating, 7 identical. The magnitudes are the injected factor to 7 significant figures:
`26.06930542 → 39.10391235` is **x1.4999982** and `-153.3289642 → -229.9934387` is **x1.4999999**.

**`clean1 != clean2` again** (`31aee42…` vs `94cbdc2…`): two forced builds of identical source still
differ on this toolchain. That reconfirms milestone 4's finding and is exactly why the teeth arm has
to be behavioural — md5 inequality proves two binaries are different BUILDS, never that they contain
different PROGRAMS.

### An instrument bug found in this rung's own gate script, and fixed

`flip-phase4.sh` piped the oracle into `grep` and then read `$?` — which is **grep's** status, not
the oracle's. Its `EXIT_teethA=0` line therefore asserted nothing, on the one arm whose entire job is
to produce a NONZERO exit. That is the banked `pipe-swallows-gate-exits` trap, hit by the very script
written to enforce a gate, and it is the third instance in this lane of a diagnostic that was itself
undiagnosed. The behavioural evidence was never in doubt (the oracle printed `FAIL` and localized in
the log), but the assertion was void, so all three arms were re-run **capture-then-gate** with the
exit code read from the command itself. Those are the exits in the table above; receipt
`raw/flip-phase4-exitcodes.log`.

## Card time, and one gap that was my instrument's fault

Phases ran chained so the cards would not idle between known-next cells. One gap happened anyway:
the chain's wait loop was `pgrep -f flip-phase2.sh`, and an orphaned `bash -c` left by a timed-out
ssh call carried the **heredoc text that defined the chain script** — text which contains the
literal string `flip-phase2.sh`. So the loop matched the process that wrote it and waited on itself.
Cost: **~9 minutes** of idle cards (phase 2 ended 22:12Z, the chain released at 22:20:50Z) plus the
diagnosis. This is the known pgrep self-match trap in a new costume: the usual form is a probe that
matches its own name, and this one was a *writer* whose command line embedded the whole script. The
fix that matters is not "kill the orphan" but the two rules underneath it: anchor or `-x` the
pattern, and never let a heredoc that defines a wait loop travel in a command line the loop can see.

## Verdict on the flip

| requirement (new-flags law) | status |
|---|---|
| default stated as an explicit decision with reasons | yes — `docs/FLAGS.md`, both rows, stated as ONE coupled decision |
| both arms measured | yes — `FOFF` (rollback seam) and `FON` (the default), 6 interleaved boots each, two request shapes |
| rollback seam named AND exercised | yes — `MEMRA_NVFP4_BANK_SM=0` / `MEMRA_NVFP4_SEL_DOWN8=0`, run as a live arm, byte-identical to pre-restore `main` |
| receipts in the same PR | yes — this section plus `raw/flip-*`, `raw/rows-flip-*`, `raw/boot-F*`, `raw/rows-mt.jsonl` |
| FLAGS.md row in the same PR | yes |
| engagement proven, not intended | yes — `source=default-on` required per mode by `assert-engagement.sh`, cell aborts on a miss |
| serving decision on sampled rows | yes — vendor-default sampled is the decision table; greedy is reported separately as the instrument |
| exactness re-run on the flip shape | yes — 18-cell tape x3 pairings, 16-turn multi-turn tape to 41k tokens, device oracle x4 tile forms with a failing localizing teeth arm |

**Recommendation: GO**, with two preconditions that are not this box's to close.

1. **The default engages only on the device-routed step TP NVFP4 path at `nsb <= 32`.** The
   qualification env carries `MEMRA_STEP_TP=0-44@0,1` and `MEMRA_STEP_NVFP4_DEV_ROUTES=1`. Before the
   flip reaches customers, the prod step37 launcher must be read (read-only) to confirm it sets both
   — `TRAP:launcher-is-restart-truth`, and this lane deliberately did not touch prod. If prod does
   not set `MEMRA_STEP_NVFP4_DEV_ROUTES=1`, the default is INERT there and the flip buys nothing
   until that changes; the `[nvfp4-sweep]` line settles it in one grep after deploy.
2. **The receipts are pinned to `afc681b1a`.** Anything that lands on `main` and touches the
   measured route needs the battery re-run, not re-argued. What the pin covers is stated in the
   merge note below.

## Merge note: `origin/main` landed under this battery, and the pin was re-verified rather than argued

The battery was measured on `afc681b1a` (binary md5 `e7242c8e4fd012dfb2d721242463d74a`). While it ran,
the owner's merge-green workflow put 14 commits of `main` ahead of the lane (glm53-flash bringup, HY3
c1-100, the degen-rerun lane). Merged in at `a063381e1` — merged, never rebased across the lane's
merge commits — clean, no conflicts, and the `MEMRA_PARALLEL_EP_Q8_SCOPE` row the coordinator flagged
is intact at its line, untouched by this lane's single FLAGS hunk 750 lines away.

**One incoming CUDA change lands on a route this recipe might take**, and it is a real functional fix,
not a comment:
...
`MEMRA_SEL_MIRROR=1` **is** in this lane's era env. The static case for harmlessness is good — the
launcher takes `threads = n <= 32 ? 32 : 128` with `grid = ceil(n/threads)`, so the two forms are
identical unless `n > 128`, and `nvfp4_routes_batched_sweeps_rank` (the function that owns the `down8`
call site) does not call `moe_sel_w_mirror` at all. **But a static case about a tree is exactly what
this lane refuses to ship.** So the merged tree was built and the tape re-run:

| pair | question | verdict |
|---|---|---|
| `POFF` → `PON` | does the flip still change no byte on the MERGED tree? | **PASS** 18/18 |
| `FON` → `PON` | **did the MERGE move a byte?** (measured tree vs merged tree, both default-ON) | **PASS** 18/18 |
| `FOFF` → `POFF` | same question on the rollback arm | **PASS** 18/18 |

All at `tape_sha256 = d6fe97d0a1928e1b6c9d43985557189d41a3f75533e3da91faaff473d7624e68` — the same
value every arm in this lane has produced since milestone 4, now across three binaries
(`0ab9551f…` pre-flip, `e7242c8e…` flipped, `b923a149…` merged) and two trees. Tests on the merged
tree: **293 passed, 0 failed, 2 ignored** (engine lib) and **2 passed** (server bank guards);
`cargo fmt --all --check` clean.

**And the win reproduces on the merged binary**, 2 interleaved boots per arm, both shapes:

| shape | `POFFP` | `PONP` | delta | per-boot ranges | verdict |
|---|---|---|---|---|---|
| vendor-default sampled | 107.42 | 113.30 | **+5.47% decode / +4.97% wall** | `[107.32, 107.52]` vs `[113.06, 113.54]` | **SEPARATED** |
| greedy | 113.77 | 121.79 | **+7.04% decode / +5.91% wall** | `[113.49, 114.06]` vs `[121.41, 122.16]` | **SEPARATED** |

Within-arm spreads on this pair came in at **0.19-1.21%**, so the vendor row clears RULE B on the
median as well as on the ranges — the only cell in this lane that has. Zero loop exclusions, one
binary, distinct nonces per boot. The +5.4% figure is stable across two trees, two binaries and 16
boots total.

So the receipts stand as measured, and what would invalidate them is now a precise statement rather
than a hedge: **any change that moves `cu/qmatvec.cu`'s `_sel_v2`/`_gu`/`_down8` kernels, the
`nvfp4_routes_batched_sweeps_rank` sweep selection, the bank-build `slot_major` decision, or the
`down8` call-site predicate.** The tape is the cheap detector; it costs two boots.

### Second merge, 439 commits, same answer — and the tape is why it is a fact

The glm53-flash bringup landed while this section was being written, putting `origin/main` **439
commits** ahead. Merged in at `c5c311998` (merged, not rebased — the lane carries merge commits and
rebasing across them is forbidden), clean, no conflicts, `MEMRA_PARALLEL_EP_Q8_SCOPE` intact.

`cu/qmatvec.cu` gained **949 lines**, which is one of the invalidators named above, so it was checked
per kernel rather than per file — every `extern "C" __global__` body extracted from both trees and
hashed:

| file | kernels measured → now | added | removed | **changed** |
|---|---|---|---|---|
| `qmatvec.cu` | 311 → 329 | 18 | 0 | **0** |
| `moe_f16_grouped.cu` | untouched | 0 | 0 | **0** |

**All 24 kernels of the measured `_sel_v2`/`_gu`/`_down8` family are byte-identical.** The 18 additions
are the new glm53 `matvec_bf16_f32acc_*_tcols` family and some q8 `moe_down8_fma_q8_rows*` variants —
different kernels that this route does not call. On the Rust side, `nvfp4_routes_batched_sweeps_rank`
(which owns the `down8` predicate) is byte-identical at 16,940 chars, `door_default_on_value` is
byte-identical, and the only change inside `upload_tensor_parallel_nvfp4` is a lint refactor of a
validation guard, `x % N != 0` → `!x.is_multiple_of(N)`, on a nonzero const divisor.

Five static checks all agreeing is still five static checks, so the tape was run again:

| pair | verdict |
|---|---|
| `QOFF` → `QON` (flip on the 439-commit tree) | **PASS** 18/18 |
| `FON` → `QON` (measured tree vs 439-commit tree, both default-ON) | **PASS** 18/18 |
| `FOFF` → `QOFF` (rollback arm) | **PASS** 18/18 |

Same `d6fe97d0…`, now across **four binaries** (`0ab9551f…`, `e7242c8e…`, `b923a149…`, `d0bc648a…`)
and **three trees**. And the win holds:

| shape | `QOFFP` | `QONP` | delta | verdict |
|---|---|---|---|---|
| vendor-default sampled | 106.54 | 113.61 | **+6.64% decode / +6.44% wall** | **SEPARATED** |
| greedy | 114.66 | 120.97 | **+5.50% decode / +5.28% wall** | **SEPARATED** |

**Five separations of five across three trees.** Vendor-default decode delta by tree: **+5.44%**
(afc681b1a, 12 boots), **+5.47%** (a063381e1, 4 boots), **+6.64%** (c5c311998, 4 boots).
