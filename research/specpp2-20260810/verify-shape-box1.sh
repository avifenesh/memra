#!/usr/bin/env bash
# N=5 interleaved c=1 K=1 default T=2 batching versus sequential-column verify.
set -euo pipefail

ROOT=${SPEC_PP2_ROOT:-/home/ubuntu/memra-cx-specpp2}
OUT=${SPEC_PP2_OUT:-/home/ubuntu/specpp2-receipts/verify-shape}
PORT=${SPEC_PP2_PORT:-8141}
ADDR=127.0.0.1:${PORT}
BASE=http://${ADDR}
...
run_arm() {
    ...
    if ! grep -q '\[spec-acc\]' "$server_log"; then
        echo "FAIL: $label emitted no spec lines"
        return 1
    fi
...
echo "VERIFY_SHAPE_LOCK_ACQUIRED $(date -u +%FT%TZ)"
echo "host=$(hostname) binary-source=7cd010c9 harness-source=lane-verify-shape-v1"
...
for arm in BATCH COL; do run_arm 1 "$arm"; done
for arm in COL BATCH; do run_arm 2 "$arm"; done
for arm in BATCH COL; do run_arm 3 "$arm"; done
for arm in COL BATCH; do run_arm 4 "$arm"; done
for arm in BATCH COL; do run_arm 5 "$arm"; done

nvidia-smi --query-compute-apps=pid,gpu_uuid,used_memory,process_name \
    --format=csv,noheader > "$OUT/gpu-processes-post.csv" 2>&1 || true
gpu_sample
echo "VERIFY_SHAPE_LOCK_RELEASED $(date -u +%FT%TZ)"
flock -u 9
echo "VERIFY_SHAPE_DONE"
