#!/usr/bin/env bash
# N=5 interleaved c=2 plain versus forced speculative K=1 on box1.
set -euo pipefail

ROOT=${SPEC_PP2_ROOT:-/home/ubuntu/memra-cx-specpp2}
OUT=${SPEC_PP2_OUT:-/home/ubuntu/specpp2-receipts/c2-k1}
PORT=${SPEC_PP2_PORT:-8140}
ADDR=127.0.0.1:${PORT}
BASE=http://${ADDR}
...
run_arm() {
    ...
    if [[ "$arm" == K1 && "$spec_lines" -eq 0 ]]; then
...
echo "C2_LOCK_ACQUIRED $(date -u +%FT%TZ)"
echo "host=$(hostname) binary-source=7cd010c9 harness-source=lane-c2-k1-v1"
...
for arm in N K1; do run_arm 1 "$arm"; done
for arm in K1 N; do run_arm 2 "$arm"; done
for arm in N K1; do run_arm 3 "$arm"; done
for arm in K1 N; do run_arm 4 "$arm"; done
for arm in N K1; do run_arm 5 "$arm"; done

nvidia-smi --query-compute-apps=pid,gpu_uuid,used_memory,process_name \
    --format=csv,noheader > "$OUT/gpu-processes-post.csv" 2>&1 || true
gpu_sample
echo "C2_LOCK_RELEASED $(date -u +%FT%TZ)"
flock -u 9
echo "C2_DONE"
