#!/usr/bin/env python3
"""Hourly public-path health and latency receipt for the two-day Q27 serve test."""
...
def run(args: argparse.Namespace) -> int:
    ...
    metrics_client = Client(base_url, metrics_token, args.timeout)
    ...
    for index in range(args.samples):
        ...
        if ok:
            ttfts.append(float(result.first_content_ms))
            latencies.append(float(result.elapsed_ms))
        else:
            errors += 1
        row = {
            "schema": "memra.cx-servetest.monitor-request.v1",
    ...
    summary = {
        "schema": "memra.cx-servetest.hourly.v1",
        "started_utc": started_utc,
        "finished_utc": utc_now(),
        "base_url": base_url,
        "model": args.model,
        "samples": args.samples,
        "successes": args.samples - errors,
        "errors": errors,
