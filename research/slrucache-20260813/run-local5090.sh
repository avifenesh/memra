#!/usr/bin/env bash
# Replay evictchurn's frozen prefix-cache contention battery on the byte-SLRU runtime.
# This is a behavior receipt. It never changes clocks and makes no throughput claim.
set -euo pipefail

ROOT=$(git rev-parse --show-toplevel)
LANE=$ROOT/research/slrucache-20260813
DRIVER=${EVICTCHURN_DRIVER:-$ROOT/research/evictchurn-20260813/evict_churn.py}
...
start_server() {
    ...
    env -u MEMRA_PP_STAGES -u MEMRA_PP_DEVICES -u MEMRA_DUAL_PP \
        ...
        -u MEMRA_PREFIX_CACHE_PROTECTED_PCT \
...
run_locked() {
    ...
    sha256sum "$MODEL" "$SERVER" "$DRIVER" \
        "$ROOT/research/prefixmoney-20260812/prefix_gate.py" >"$OUT/SHA256SUMS.input"
    {
        echo "server_source_commit=$(git rev-parse HEAD)"
        echo "base_tag=$(git describe --tags --exact-match 7cf5fd842ebc76f6e8a82910a8e6d4b864b6b42d)"
        echo "branch=$(git branch --show-current)"
        echo "evictchurn_driver=$DRIVER"
        echo "evictchurn_driver_commit=$(git -C "$(dirname "$DRIVER")" rev-parse HEAD)"
        echo "model=$MODEL"
        ...
        echo "prefix_policy=byte-SLRU protected/probation 80/20"
...
PY
    ...
    echo "SLRUCACHE_LOCAL_PASS $(date -u +%FT%TZ)"
}
