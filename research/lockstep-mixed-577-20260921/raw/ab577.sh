...
ART=/data/hy3; OUT=/root/lane/ab577; mkdir -p "$OUT"
...
run m1            MEMRA_LOCKSTEP_M=1 MEMRA_PROMPT="$P0" MEMRA_MOE_INPUT_TRACE_DIR="$OUT/trace-m1"
run m4-mixed      MEMRA_LOCKSTEP_M=4 MEMRA_PROMPTS_FILE="$OUT/prompts.txt" MEMRA_MOE_INPUT_TRACE_DIR="$OUT/trace-m4-mixed"
run m4-mixed-split   MEMRA_LOCKSTEP_M=4 MEMRA_PROMPTS_FILE="$OUT/prompts.txt" MEMRA_LOCKSTEP_SPLIT_GROUPS=1
run m4-mixed-cpurows0 MEMRA_LOCKSTEP_M=4 MEMRA_PROMPTS_FILE="$OUT/prompts.txt" MEMRA_LOCKSTEP_CPU_ROWS=0
run m4-mixed-both  MEMRA_LOCKSTEP_M=4 MEMRA_PROMPTS_FILE="$OUT/prompts.txt" MEMRA_LOCKSTEP_SPLIT_GROUPS=1 MEMRA_LOCKSTEP_CPU_ROWS=0
