#!/usr/bin/env bash
# Design W's target-card build (DAY61.md section 1), outside any hold, on the box's clone of this lane (/root/wt-a).
# From ONE clone: w (the tip's release server and its engine and server lib test binaries), base (the tip's crates
# taken to W's parent: the direct checksum), red (the tip plus red-arm.patch: the streamed copy skips each chunk's
# second vector; its engine and server lib test binaries only). Test executables are found through cargo's JSON `executable`
...
mkdir -p "$R/bins/w" "$R/bins/base" "$R/bins/red"
L=$R/build-steps.log
...
git checkout -q -B lane-a-w "$1" >> "$L" 2>&1 || { echo "rc=2 (checkout tip)" >> "$R/build.log"; exit 2; }
...
echo "== w" >> "$L"
nice -n 5 cargo build --release -p memra-server >> "$L" 2>&1 || { echo "rc=1 (w)" >> "$R/build.log"; exit 1; }
cp target/release/memra-server "$R/bins/w/memra-server"
tests "$R/bins/w" || { echo "rc=1 (w tests)" >> "$R/build.log"; exit 1; }
echo "== red" >> "$L"
cp research/spill-a-20260919/pro-single-w/red-arm.patch "$R/red-arm.patch"
...
{ echo "red marker: engine $(grep -ac 'day61 red arm' "$R/bins/red/memra-engine-tests")"
  echo "w tests marker (must be 0): $(grep -ac 'day61 red arm' "$R/bins/w/memra-engine-tests")"; } > "$R/markers.txt"
