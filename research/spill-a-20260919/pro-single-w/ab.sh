#!/usr/bin/env bash
# W's paired cell (DAY61.md section 1, (c) and (d)) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash ab.sh @COLLECTOR_LOCK_FD@ <cell> <modeA> <modeB> <prefix_mb>`.
# Two binaries (base, w) by two stall_cell.py modes: o1 = base-A w-A base-B w-B x5, o2 the reverse x5 (40 boots),
...
sha256sum "$R/bins/base/memra-server" "$R/bins/w/memra-server" > "$O/binaries.sha256"
idle=0
...
SERVER_PID=""
boot() { # $1 log $2 bin
  memra_port_guard w-ab "$PORT" MEMRA_GATE_PORT || return 1
...
for order in o1 o2; do
  seq1="base:$MA w:$MA base:$MB w:$MB"; [ $order = o2 ] && seq1="w:$MB base:$MB w:$MA base:$MA"
  ...
  for am in $arms; do
    ...
    python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode "$mode" --n 5 --server-log "$D/server.log" \
      --out "$D/$mode" --tag "w-$CELL-$arm-$mode" > "$D/$mode.log" 2>&1
