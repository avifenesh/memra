...
git checkout -q -B lane-a-l2 "$1" >> "$L" 2>&1 || { echo "rc=2 (checkout tip)" >> "$R/build.log"; exit 2; }
...
nice -n 5 cargo build --release -p memra-server >> "$L" 2>&1; grc=$?
cp target/release/memra-server "$R/bins/redgate/memra-server"
