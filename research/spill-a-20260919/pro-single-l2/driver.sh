#!/usr/bin/env bash
# Design L' target-card driver (DAY63.md section 4): waits for the build receipt, then, each under ONE
# collector hold: unit-cells.sh ((a): the native cells green and red), gates.sh ((a), 11 gates on the l binary, the
# fault gate with its changed staging-fill checks), gates-red.sh (the gate change's red arm on the redgate binary),
# ab.sh chain ((b), (d): base against l, 20 boots), ab.sh demote ((c), (d), 20 boots); then l2-reading.py. A busy collector lock: bounded retries (60 x 120 s),
# never a signal to the holder. Executed-not-qualified. No host, id or price here.
set -uo pipefail
cd /root/wt-a || exit 1
R=/root/spill-receipts/a-l2
D=research/spill-a-20260919/pro-single-l2
...
cell gates-red-cell 3600 $D/gates-red.sh @COLLECTOR_LOCK_FD@
cell ab-chain-cell 10800 $D/ab.sh @COLLECTOR_LOCK_FD@ chain
cell ab-demote-cell 10800 $D/ab.sh @COLLECTOR_LOCK_FD@ demote
python3 research/spill-a-20260919/l2-reading.py "$R" > "$R/reading-l2.log" 2>&1
echo "$(date -u +%FT%TZ) reading rc=$? $(tail -1 "$R/reading-l2.log")" | tee -a "$R/progress.log"
