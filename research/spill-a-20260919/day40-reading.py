#!/usr/bin/env python3
"""WP-A day 40 design S reader (DAY40.md section 3: clauses (c) and (d)), written before its cells run.

usage: day40-reading.py demote|promote ROOT
Input: ROOT/ab/o{1,2}/bNN-{g4,s}/server.log and .../<mode>/receipt.json (stall_cell.py), ROOT/ab/replays.log; 20 boots,
...
demote (c): per order, the steady demotes' `wall .. t0 to publication` median (the second and later `demote digests
    landed off the tick` ledger line of each boot) on s <= g4 + 8.0 ms, and the demoting intruder's e2e (`wall_ms` of
    every demote-arm run) median on s <= g4 + 1.0 ms.
promote (d): per order, PIN (the `[prefix-host] promote: .. in Y ms` lines, the second and later of each boot) median
    on s <= g4 + 1.0 ms, and the promoting intruder's e2e median on s <= g4 + 1.0 ms.
...
def main():
    ...
    for order in ("o1", "o2"):
        for arm in ARMS:
            ...
            print(f"DAY40 READING mode={mode} order={order} arm={arm} {stat('e2e', pool(order, arm, 'e2e'))} | "
    ...
    if not complete:
        print(f"DAY40 S {mode.upper()} INCOMPLETE ({'; '.join(notes)}) -> nothing is read; the cell repeats whole once")
        sys.exit(2)
    ok_all = True
    for order in ("o1", "o2"):
        if mode == "demote":
        ...
        for key, bound in terms:
            g4, s = med(pool(order, "g4", key)), med(pool(order, "s", key))
            ok &= s - g4 <= bound
            parts.append(f"{key} g4={g4:.2f} s={s:.2f} s-minus-g4={s - g4:+.2f} rule <=+{bound}")
        ok_all &= ok
        clause = "C" if mode == "demote" else "D"
        print(f"DAY40 S {clause} order={order} " + " | ".join(parts) + f" -> {'PASS' if ok else 'FAIL'}")
    print(f"DAY40 S {mode.upper()} -> {'PASS' if ok_all else 'FAIL'}")
