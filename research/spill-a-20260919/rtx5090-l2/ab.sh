#!/usr/bin/env bash
# Derived for the local RTX 5090 by rtx5090-derive.py (DAY68 section 1) from pro-single-l2/ab.sh at 21984b527: exact replacements only.
# Design L's target-card A/B (DAY63.md section 1 (b) to (d): base against l) for ONE cell in ONE collector hold:
...
# --n 5, o1 = base l x5, o2 = l base x5, door ON, the PRO environment of the S sittings, readiness bounded to 480 s;
# each boot's start temperature and SM clock in BOOT.txt, its server VmRSS and VmHWM before the stop in rss.txt; 250 ms
# telemetry; the layout l-reading.py reads (<cell>/ab/o{1,2}/bNN-{base,l}). A compute app at the hold's start:
...
O=$R/$CELL
mkdir -p "$O/ab"
python3 tools/tier-lock-proof.py --fd "$fd" --lock /tmp/memra-5090.lock --owner collector > "$O/LOCK.json"
...
sha256sum "$R/bins/base/memra-server" "$R/bins/l/memra-server" > "$O/binaries.sha256"
idle=0
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard l-ab "$PORT" MEMRA_GATE_PORT || return 1
...
for order in o1 o2; do
  arms="base l base l base l base l base l"; [ $order = o2 ] && arms="l base l base l base l base l base"
  for arm in $arms; do
    ...
    python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode "$MODE" --n 5 --server-log "$D/server.log" \
      --out "$D/$MODE" --tag "l-$CELL-$arm" > "$D/$MODE.log" 2>&1
