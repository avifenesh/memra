#!/usr/bin/env bash
# WP-A day 28, the acceptance gate's "both cards" on the local RTX 5090 Laptop GPU (9B NVFP4 MTP artifact) with the
# day-28 tree's local release binary: the hit gate OFF then ON, then the fault gate (ten cells, the two hash cells
# among them) in the default and the plain arm (C's day-23 shape on this card: MEMRA_HOSTGATE_CACHE_MB=64 so the
...
# The re-run of the one cell whose server died during boot with an empty log (the first pass, wrapped in a
# MemoryMax=28G scope): the same gate, the same arm, unwrapped, under the gate's own flock, after the same bounded wait.
cell hit-off-rerun "" bash tools/spec-on-cache-hit-gate.sh qwen "$MODEL" "$BIN"
echo "$(date -u +%FT%TZ) LOCAL-RERUN-DONE" | tee -a "$ROOT/battery.log"
