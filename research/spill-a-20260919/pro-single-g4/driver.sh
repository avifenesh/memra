#!/usr/bin/env bash
# The G4 target-card driver (DAY38 section 17, DAY39 section 5, DAY41 section 1): waits for the build receipt, then, in
# one sitting: ab.sh (G4's A/B, (c) and (d)) and hump.sh (the hump clause (f)), each under ONE collector hold; gates.sh
# under the collector on the G4 binary ((b), every fault cell, day 41's three included); hitgate.sh (its own flock, OFF
...
D=research/spill-a-20260919/pro-single-g4
