...
mkdir -p "$R/bins/g4" "$R/bins/f1" "$R/bins/hk" "$R/bins/base" "$R/bins/gpp"
L=$R/build-steps.log
...
git checkout -q -B lane-a-g4-tip "$1" >> "$L" 2>&1 || { echo "rc=2 (checkout tip)" >> "$R/build.log"; exit 2; }
...
# Markers (grep -a reads the binary as text): the copy-stream receipt line, in g4, f1 and hk (base 0: no device receipt; gpp 0: its line names the receipt stream).
for b in g4 f1 hk base gpp; do echo "$b copy-stream-receipt-line: $(grep -ac "receipts on the copy stream" "$R/bins/$b/memra-server")"; done > "$R/markers.txt"
