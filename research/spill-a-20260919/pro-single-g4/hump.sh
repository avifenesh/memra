#!/usr/bin/env bash
# G4's hump clause (f) (DAY38 section 17) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash hump.sh @COLLECTOR_LOCK_FD@`.
# Four door-ON boots xgpp xg4 xg4 xgpp (xgpp = G'', the day-38 sitting's tip, the positive control; xg4 = the G4 tip),
...
sha256sum "$R/bins/gpp/memra-server" "$R/bins/g4/memra-server" > "$D0/binaries.sha256"
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard g4-hump "$PORT" MEMRA_GATE_PORT || return 1
...
for arm in xgpp xg4 xg4 xgpp; do
  ...
  bin=$R/bins/g4/memra-server; [ $arm = xgpp ] && bin=$R/bins/gpp/memra-server
