...
sha256sum "$R/bins/hk/memra-server" "$R/bins/g4/memra-server" "$R/bins/f1/memra-server" > "$R/item3/binaries.sha256"
idle=0
...
for order in o1 o2; do
  ...
  for arm in $arms; do
    ...
    bin=$R/bins/g4/memra-server; door=MEMRA_KV_HOST_CONTRACTS=1
