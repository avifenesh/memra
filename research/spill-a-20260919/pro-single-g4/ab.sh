#!/usr/bin/env bash
# The G4 target-card A/B (DAY38 section 17: section 3's cell whole, base against G4) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash ab.sh @COLLECTOR_LOCK_FD@`.
# stall_cell.py --mode demote --n 5, base (80039a8de) against G4 (the tip), o1 = base g x5, o2 = g base x5, door ON, the PRO
...
sha256sum "$R/bins/base/memra-server" "$R/bins/g4/memra-server" > "$R/g/binaries.sha256"
idle=0
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard g4-ab "$PORT" MEMRA_GATE_PORT || return 1
...
for order in o1 o2; do
  ...
  for arm in $arms; do
    ...
    bin=$R/bins/base/memra-server; [ $arm = g ] && bin=$R/bins/g4/memra-server
