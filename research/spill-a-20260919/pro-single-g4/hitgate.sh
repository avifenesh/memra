#!/usr/bin/env bash
# The hit gate on the G4 target card (DAY38 section 17's (b)), on the G4 tip binary (no --external-lock arm): under flock on the canonical lock, door OFF then ON. The ON arm
