#!/usr/bin/env bash
# The G4 target-card unit cells on the tip tree (DAY38 section 17's (b)), under the collector's lock hold:
...
cargo test -p memra-server --lib -- hash_ hashing_ every_path_that_meets_a_hashing every_path_that_meets_a_demoting every_path_that_meets_a_promoting the_run_loop_waits_boundedly the_d2h_spans day31_ day33_ day34_ day35_ day36_ day38_ day41_ > "$R/unit/cpu-censuses.log" 2>&1
rc3=$?; grep -E '^test result' "$R/unit/cpu-censuses.log"
cargo test -p memra-engine --lib -- rules_are_as_stated native_cells_own_their_context one_side_stream_beside_the_owner day39_ > "$R/unit/engine-censuses.log" 2>&1
rc4=$?; grep -E '^test |^test result' "$R/unit/engine-censuses.log"
cargo test -p memra-tier --test contracts -- d2h_span h2d_ d2h_device_receipt d2d_ > "$R/unit/tier.log" 2>&1
