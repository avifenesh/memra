#!/usr/bin/env bash
# G''''s hump clause (f) (DAY38 section 14) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash hump.sh @COLLECTOR_LOCK_FD@`.
# Four door-ON boots xgpp xg3 xg3 xgpp (xgpp = G'', the day-38 sitting's tip, the positive control; xg3 = the G''' tip),
# each stall_cell.py --mode demote --n 8 (16 demote runs), the PRO demote environment of ab.sh, then the hump reader.
set -uo pipefail
fd=$1
...
sha256sum "$R/bins/gpp/memra-server" "$R/bins/g3/memra-server" > "$D0/binaries.sha256"
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard g3-hump "$PORT" MEMRA_GATE_PORT || return 1
...
for arm in xgpp xg3 xg3 xgpp; do
  ...
  bin=$R/bins/g3/memra-server; [ $arm = xgpp ] && bin=$R/bins/gpp/memra-server
