#!/usr/bin/env bash
# The hit gate on the G''' target card (DAY38 section 14's (b)), on the G''' tip binary (no --external-lock arm): under flock on the canonical lock, door OFF then ON. The ON arm
