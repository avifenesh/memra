#!/usr/bin/env bash
# The G''' target-card driver without item 3 (DAY38 section 16; DAY39 section 5b moves item 3 to the final tree's sitting): waits for the build receipt, then, in
# one sitting: ab.sh (G''''s A/B, (c) and (d)) and hump.sh (the hump clause (f)), each under ONE collector hold; gates.sh
# under the collector on the G''' binary ((b), every fault cell, day 41's three included); hitgate.sh (its own flock, OFF
# then ON); unit-cells.sh under the collector; item3.sh (design T's target cell) under ONE collector hold. A busy collector
...
D=research/spill-a-20260919/pro-single-g3
...
cell ab 10800 $D/ab.sh @COLLECTOR_LOCK_FD@
...
# item3.sh moves to the final tree's sitting (DAY39 section 5b)
