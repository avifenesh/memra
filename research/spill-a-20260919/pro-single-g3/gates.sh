#!/usr/bin/env bash
# The G''' target-card gate cell (DAY38 section 14's (b): the day-36 set, the fault gate with every cell, source-flip,
# copy-phase-hit on the host-side hold and day 41's three sources-* cells, on the G''' tip binary) (one RTX PRO 6000 Blackwell), run under the collector's
