#!/usr/bin/env bash
# The G''' target-card unit cells on the tip tree (DAY38 section 14's (b)), under the collector's lock hold:
...
cargo test -p memra-engine --lib -- rules_are_as_stated native_cells_own_their_context one_kernel_stream_beside_the_owner day39_ > "$R/unit/engine-censuses.log" 2>&1
