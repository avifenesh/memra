#!/usr/bin/env bash
# memra#536 Move 2 cell (i), the capture arm, WP-A day 21 (DAY20.md pre-registration): the day-16 script's boot
# with the prefix cache ON at 1024 MB (a 4096-token entry is about 278 MB on the 27B) and the host tier at 8 GiB
# (the door needs it), door OFF or ON; the harness's `restore` mode (stall_cell.py: one seeded prompt, re-posted as a
# whole-entry hit at the tenant's 24th token in every timed run; cached_tokens per run). MEMRA_SERVE_SPEC=0 so the tenant emits one
# token per tick. One collector lock hold per BOOT. usage: stall-cell.sh <lockfd> off|on
set -uo pipefail
fd=$1
which=$2
R=/root/spill-receipts/a-day21
cell=stall-restore-$which
EV=$R/$cell/ev
...
case $which in
  off|on)
    ...
    python3 $H --port "$PORT" --mode restore --server-log "$EV/server.log" --out "$EV/restore" --tag "stall-restore-$which" | tee "$EV/restore.log" || rc=1
    mark restore-done
