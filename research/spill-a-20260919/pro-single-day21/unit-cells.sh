#!/usr/bin/env bash
# The door's GPU unit cells (option_b demote unwinds, option_c promote unwinds) plus the day-20 and day-21 engine cells
# (`d2d_capture_*` and `d2d_restore_*`, memra-engine lib) on the target
...
CUDA_VISIBLE_DEVICES=0 cargo test -p memra-engine --lib -- --ignored --test-threads=1 d2d_ > "$R/unit/cargo-test-engine.log" 2>&1
rc2=$?
