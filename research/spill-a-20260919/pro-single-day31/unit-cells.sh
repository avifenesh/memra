#!/usr/bin/env bash
# The door's GPU unit cells (option_b demote unwinds with the day-30 and day-31 span cells, option_c promote unwinds), the day-20 to
# day-22 engine cells (`d2d_*`) and the day-30 span cell (`d2h_span_*`, memra-engine lib) on the target card, and the
# day-28 to day-31 CPU cells (`hash_*`, `hashing_*`, the Hashing census, the parked-only wait, the span-order census, the
...
cargo test -p memra-server --lib -- hash_ hashing_ every_path_that_meets_a_hashing every_path_that_meets_a_demoting the_run_loop_waits_boundedly the_d2h_spans day31_ > "$R/unit/cargo-test-hash.log" 2>&1
rc3=$?
