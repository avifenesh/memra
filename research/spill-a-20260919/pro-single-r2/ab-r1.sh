#!/usr/bin/env bash
# R1's corrected cell (DAY62.md section 4), on the base binary, in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash ab-mode.sh @COLLECTOR_LOCK_FD@ <cell> <modeA> <modeB> <modeC> <prefix_mb>`.
# One binary (the tip), three stall_cell.py modes as the arms: o1 = A B C x5, o2 = C B A x5 (30 boots), each boot one mode at --n 5,
...
SERVER_PID=""
boot() { # $1 log
  memra_port_guard d62-ab "$PORT" MEMRA_GATE_PORT || return 1
...
for order in o1 o2; do
  s1="$MA $MB $MC"; [ $order = o2 ] && s1="$MC $MB $MA"
  arms="$s1 $s1 $s1 $s1 $s1"
  for mode in $arms; do
    ...
    python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode "$mode" --n 5 --server-log "$D/server.log" \
      --out "$D/$mode" --tag "d62-$CELL-$mode" > "$D/$mode.log" 2>&1
