#!/usr/bin/env bash
# R2's paired cell (DAY62.md section 5, (b) and (c)) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash ab.sh @COLLECTOR_LOCK_FD@ <cell> <modeA> <modeB> <prefix_mb>`.
# Two binaries (base, r2) by two stall_cell.py modes: o1 = base-A r2-A base-B r2-B x5, o2 the reverse x5 (40 boots),
...
sha256sum "$R/bins/base/memra-server" "$R/bins/r2/memra-server" > "$O/binaries.sha256"
idle=0
...
SERVER_PID=""
boot() { # $1 log $2 bin
  memra_port_guard r2-ab "$PORT" MEMRA_GATE_PORT || return 1
...
for order in o1 o2; do
  seq1="base:$MA r2:$MA base:$MB r2:$MB"; [ $order = o2 ] && seq1="r2:$MB base:$MB r2:$MA base:$MA"
  ...
  for am in $arms; do
    ...
    python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode "$mode" --n 5 --server-log "$D/server.log" \
      --out "$D/$mode" --tag "r2-$CELL-$arm-$mode" > "$D/$mode.log" 2>&1
