#!/usr/bin/env bash
# Design R2's target-card build (DAY62.md section 5), outside any hold, on the box's clone of this lane (/root/wt-a).
# From ONE clone: r2 (the tip's release server) and base (the tip's crates taken to R2's parent). The tree is checked
# back at the tip after base. usage: build.sh <tip_sha> <base_sha>
set -uo pipefail
R=/root/spill-receipts/a-r2
export PATH=/root/.cargo/bin:/usr/local/cuda/bin:$PATH
mkdir -p "$R/bins/r2" "$R/bins/base"
L=$R/build-steps.log
...
git checkout -q -B lane-a-r2 "$1" >> "$L" 2>&1 || { echo "rc=2 (checkout tip)" >> "$R/build.log"; exit 2; }
...
echo "== r2" >> "$L"
nice -n 5 cargo build --release -p memra-server >> "$L" 2>&1 || { echo "rc=1 (r2)" >> "$R/build.log"; exit 1; }
cp target/release/memra-server "$R/bins/r2/memra-server"
echo "== base ($2)" >> "$L"
...
for b in r2 base; do echo "$b deferral wording: $(grep -ac 'retire deferred: the capture' "$R/bins/$b/memra-server") seam wording: $(grep -ac 'source retiring: ' "$R/bins/$b/memra-server")"; done > "$R/markers.txt"
