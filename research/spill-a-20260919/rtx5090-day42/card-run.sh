#!/usr/bin/env bash
# WP-A day 42 design S2 on the local RTX 5090 Laptop GPU (DAY42.md section 1), fixed before it runs. ONE bounded hold of
...
#  1. the unit cells from prebuilt test binaries of the S2 tree: the door's GPU cells (option_b_, option_c_), serial, and
#     the engine's native tier_transfer cells in parallel (the batched kernel's bitwise cell, the span cells with every
#     digest bitwise, the seal's refusals, the flip arm, the abandon and displacement reaps);
#  2. clause (c): the demote A/B, stall_cell.py --mode demote --n 5, g4 (the lane before S2's code) against s2 (the S2
#     tip), o1 = g4 s2 x5, o2 = s2 g4 x5, door ON, the day-35 5090 environment, readiness bounded to 60 s; 250 ms
#     telemetry; day42-reading.py;
#  3. clause (d): the promote A/B, the same with --mode promote --n 5, into promote/;
#  4. clause (e): the hump cell, four door-ON boots xgpp xs2 xs2 xgpp (G'' the positive control), each --mode demote
#     --n 8 (16 demote runs), each boot's start temperature and SM clock in its BOOT.txt (ruling 54's regime record);
#  5. the gates with the S2 binary, each through --external-lock 9: identity x4, failure OFF and ON, fault default and
#     plain (every cell, span-flip-landed and span-flip-resident included), hit OFF and ON.
# Executed-not-qualified. usage: card-run.sh <out_root> <model.gguf> <bin_g4> <bin_s2> <bin_gpp> <server_tests> <engine_tests>
set -uo pipefail
ROOT=$1; MODEL=$2; BG4=$3; BM=$4; BGPP=$5; ST=$6; ET=$7
...
sha256sum "$BG4" "$BM" "$BGPP" "$ST" "$ET" > "$ROOT/binaries.sha256"
...
# 2. the A/B
PORT=${MEMRA_GATE_PORT:-18145}
. tools/port-guard.sh
SERVER_PID=""
boot() { # $1 bin $2 log
    memra_port_guard day42-s2 "$PORT" MEMRA_GATE_PORT || return 1
...
trap finish EXIT
ab() { # $1 mode (demote|promote) $2 dir
    local mode=$1 AB=$2 i=0
    mkdir -p "$AB/ab"
    for order in o1 o2; do
        arms="g4 s2 g4 s2 g4 s2 g4 s2 g4 s2"; [ $order = o2 ] && arms="s2 g4 s2 g4 s2 g4 s2 g4 s2 g4"
        for arm in $arms; do
            i=$((i+1)); D=$AB/ab/$order/$(printf 'b%02d-%s' "$i" "$arm"); mkdir -p "$D"
            bin=$BG4; [ $arm = s2 ] && bin=$BM
            echo "arm=$arm order=$order mode=$mode bin=$(sha256sum "$bin" | cut -c1-16)" > "$D/BOOT.txt"
            nvidia-smi --query-gpu=temperature.gpu,clocks.sm,power.draw --format=csv,noheader >> "$D/BOOT.txt" 2>&1
            if ! boot "$bin" "$D/server.log"; then
                log "$mode $order $arm boot NOT READY within 60 s; stopped"; echo "boot-failed" >> "$D/BOOT.txt"; stop; continue
            fi
            python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode "$mode" --n 5 --server-log "$D/server.log" \
                --out "$D/$mode" --tag "stall-$mode-$arm" > "$D/$mode.log" 2>&1
            log "$mode $order $arm rc=$? $(grep -h 'STALL rule' "$D/$mode.log" | cut -c1-100)"
            stop
            { echo "== $order/$(basename "$D")"; python3 research/spill-a-20260919/stall_cell.py --replay "$D/$mode/receipt.json"; } \
                >> "$AB/ab/replays.log" 2>&1
        done
    done
    python3 research/spill-a-20260919/day42-reading.py "$mode" "$AB" > "$AB/reading-day42-$mode.log" 2>&1
    log "$mode reading $(tail -1 "$AB/reading-day42-$mode.log")"
}
ab demote "$ROOT/demote"
ab promote "$ROOT/promote"
# 4. (e) the hump cell
mkdir -p "$ROOT/hump"
j=0
for arm in xgpp xs2 xs2 xgpp; do
...
flock -u 9
log "hold released; DAY42-S2-CARD-RUN-DONE"
