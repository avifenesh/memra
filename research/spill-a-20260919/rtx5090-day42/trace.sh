#!/usr/bin/env bash
# WP-A day 42 (DAY42.md section 1, the reading without a clause) on the local RTX 5090 Laptop GPU, fixed before it runs:
# ONE bounded hold of /tmp/memra-5090.lock (60 x 120 s; then no compute app and >= 20000 MiB free, 15 x 60 s); one g4 and
# one s2 boot under Nsight Systems (--trace=cuda,osrt, no sampling), each stall_cell.py --mode demote --n 4 (8 demote
# runs), the day-35 5090 environment; each report exported to sqlite under /tmp/wt-a-d42trace and read by
# day42-trace-reading.py; the reports stay out of the tree (their sha256 banked), the scratch removed at the end.
# usage: trace.sh <out_root> <model.gguf> <bin_g4> <bin_s2>
set -uo pipefail
...
export TMPDIR=/tmp/wt-a-d42trace/tmp
mkdir -p "$TMPDIR"
...
for arm in g4 s2; do
    ...
    T=/tmp/wt-a-d42trace/nsys-$arm; mkdir -p "$T"   # the reports stay outside the tree (their sha256 banked)
    bin=$BG4; [ $arm = s2 ] && bin=$BS
    memra_port_guard day42-trace "$PORT" MEMRA_GATE_PORT || { log "port"; continue; }
...
flock -u 9
log "hold released; DAY42-TRACE-DONE"
rm -rf /tmp/wt-a-d42trace/tmp
