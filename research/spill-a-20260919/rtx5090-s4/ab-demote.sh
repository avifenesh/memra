#!/usr/bin/env bash
# Derived for the local RTX 5090 by rtx5090-derive.py (DAY68 section 1) from pro-single-s2/ab-demote.sh at a0f9968e3: exact replacements only.
# Design S2's target-card demote A/B (DAY42 section 1, clause (c): g4 against s2) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash ab-demote.sh @COLLECTOR_LOCK_FD@`.
# stall_cell.py --mode demote --n 5, g4 (the lane before S2's code) against s2 (the S2 tip), o1 = g4 s x5, o2 = s g4 x5, door ON,
# the PRO environment (DAY36 section 3's), readiness bounded to 480 s; the day42-reading.py layout
# (demote/ab/o{1,2}/bNN-{g4,s2}).
...
cd "${A_TREE:?}" || exit 1
mkdir -p "$R/demote/ab"
python3 tools/tier-lock-proof.py --fd "$fd" --lock /tmp/memra-5090.lock --owner collector > "$R/demote/LOCK.json"
log() { echo "$(date -u +%FT%TZ) $*" | tee -a "$R/demote/run.log"; }
log "model sha256: $(sha256sum "$MODEL" | cut -d' ' -f1) $(basename "$MODEL")"
sha256sum "$R/bins/g4/memra-server" "$R/bins/s2/memra-server" > "$R/demote/binaries.sha256"
idle=0
...
nvidia-smi --query-gpu=timestamp,temperature.gpu,power.draw,clocks.sm,clocks.mem,memory.used,utilization.gpu \
  --format=csv -lms 250 > "$R/demote/card-250ms.csv" 2>&1 &
SAMPLER=$!
PORT=${MEMRA_GATE_PORT:-18137}
. tools/port-guard.sh
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard s2-demote "$PORT" MEMRA_GATE_PORT || return 1
...
for order in o1 o2; do
  ...
  for arm in $arms; do
    i=$((i+1)); D=$R/demote/ab/$order/$(printf 'b%02d-%s' "$i" "$arm"); mkdir -p "$D"
    ...
    if ! boot "$bin" "$D/server.log"; then
      log "demote $order $arm boot NOT READY within 480 s; stopped"; echo "boot-failed" >> "$D/BOOT.txt"; stop; continue
    fi
    ...
    log "demote $order $arm rc=$? $(grep -h 'STALL rule' "$D/demote.log" | cut -c1-120)"
    stop
    ...
    { echo "== $order/$(basename "$D")"; python3 research/spill-a-20260919/stall_cell.py --replay "$D/demote/receipt.json"; } \
      >> "$R/demote/ab/replays.log" 2>&1
  done
done
trap - EXIT
kill "$SAMPLER" 2>/dev/null || true
nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv > "$R/demote/compute-apps.after.csv" 2>&1
python3 research/spill-a-20260919/day42-reading.py demote "$R/demote" > "$R/demote/reading-day42-demote.log" 2>&1
log "demote done; reading rc=$?"
