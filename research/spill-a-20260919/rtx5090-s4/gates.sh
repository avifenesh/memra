#!/usr/bin/env bash
# Derived for the local RTX 5090 by rtx5090-derive.py (DAY68 section 1) from pro-single-s2/gates.sh at a0f9968e3: exact replacements only.
# The design-S2 target-card gate cell (DAY42 section 1's (b): the day-36 set, the fault gate with every cell, source-flip,
# copy-phase-hit on the host-side hold and day 41's three sources-* cells, on the S2 tip binary) (one RTX PRO 6000 Blackwell), run under the collector's
