#!/usr/bin/env bash
# Derived for the local RTX 5090 by rtx5090-derive.py (DAY68 section 1) from pro-single-s2/hump.sh at a0f9968e3: exact replacements only.
# Design S2's hump clause (e) (DAY42 section 1) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash hump.sh @COLLECTOR_LOCK_FD@`.
# Four door-ON boots xgpp xs2 xs2 xgpp (xgpp = G'' 358749c9f, the positive control; xs2 = the S2 tip), each
# stall_cell.py --mode demote --n 8 (16 demote runs), the PRO demote environment of ab-demote.sh, each boot's start
# temperature and SM clock in its BOOT.txt (ruling 54's regime record), then the hump reader.
set -uo pipefail
fd=$1
R=${A_OUT:?}
...
D0=$R/hump
mkdir -p "$D0"
python3 tools/tier-lock-proof.py --fd "$fd" --lock /tmp/memra-5090.lock --owner collector > "$D0/LOCK.json"
log() { echo "$(date -u +%FT%TZ) $*" | tee -a "$D0/run.log"; }
sha256sum "$R/bins/gpp/memra-server" "$R/bins/s2/memra-server" > "$D0/binaries.sha256"
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard s2-hump "$PORT" MEMRA_GATE_PORT || return 1
...
for arm in xgpp xs2 xs2 xgpp; do
  ...
  bin=$R/bins/s2/memra-server; [ $arm = xgpp ] && bin=$R/bins/gpp/memra-server
  ...
  log "hump $arm rc=$? $(grep -h 'STALL rule' "$D/demote.log" | cut -c1-100)"
  stop
done
trap - EXIT
