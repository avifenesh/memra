#!/usr/bin/env bash
# BOX3 day-26 driver (the acceptance gate of proposal 1, ruling 36): waits for the build receipt, then, in one sitting:
# double-park.sh under the collector (one hold, twenty boots), restore-arm.sh under the collector (one hold, one ON
# boot, 100 timed restore runs), gates.sh under the collector (identity default and plain OFF/ON, failure OFF/ON, the
# fault gate all cells, twin OFF/ON), hitgate.sh (its own flock, OFF then ON), unit-cells.sh under the collector.
# Bounded retry of a busy collector lock (60 x 120 s), never signals the holder. No host, id or price here.
set -uo pipefail
cd /root/wt-a || exit 1
R=/root/spill-receipts/a-day26
D=research/spill-a-20260919/pro-single-day26
...
cell restore-arm 3600 $D/restore-arm.sh @COLLECTOR_LOCK_FD@ /root/wt-a "$R" "$MODEL" "$BIN"
echo "$(date -u +%FT%TZ) restore-arm $(cat "$R/restore-arm/ev/exit.txt" 2>/dev/null)" >> "$R/progress.log"
