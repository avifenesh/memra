#!/usr/bin/env bash
# WP-A day 26: the acceptance cell of proposal 1 (lead ruling 36 on DAY25.md): the day-25 double-park cell again,
# byte-for-byte in shape, on the tree that carries the promoted-pin refusal of the restore route. Pre-registered in
# DAY26.md before this ran. The day-16 stall script's PROMOTE arm, door ON against door OFF, ONE collector lock hold
# on the target card; the A/B interleaves across BOOTS: order 1 = ON OFF ON OFF ON OFF ON OFF ON OFF, order 2 = OFF
# ON OFF ON OFF ON OFF ON OFF ON (five boots per arm per order, twenty boots). Every boot is the day-16 script's `off`
# boot (MEMRA_PREFIX_CACHE_MB=256 MEMRA_KV_HOST_MB=8192 MEMRA_SERVE_SPEC=0 MEMRA_CTX=8192 MEMRA_MAX_SESSIONS=4) plus
# MEMRA_KV_HOST_CONTRACTS=1 on the ON boots, running lane A's harness stall_cell.py byte-for-byte, --mode promote
# --n 5 (its own idle/arm interleave in both orders inside the boot). Every receipt replayed (`STALL REPLAY`). Every
