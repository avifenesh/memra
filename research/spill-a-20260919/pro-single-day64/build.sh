...
git checkout -q -B lane-a-d64 "$1" >> "$L" 2>&1 || { echo "rc=2 (checkout tip)" >> "$R/build.log"; exit 2; }
...
echo "waiting wording: $(grep -ac 'pending on {}' "$R/bins/tip/memra-server")" > "$R/markers.txt"
