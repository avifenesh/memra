#!/usr/bin/env bash
# DAY44 section 1: DAY39 section 5's target cell (design T, item 3) on this 9950X-class host, in ONE collector hold:
...
sha256sum "$R/bins/hk/memra-server" "$R/bins/ft/memra-server" "$R/bins/f1/memra-server" > "$R/item3/binaries.sha256"
idle=0
...
boot() { # $1 bin $2 log $3 door (on|off)
  memra_port_guard g4-item3 "$PORT" MEMRA_GATE_PORT || return 1
...
for order in o1 o2; do
  ...
  for arm in $arms; do
    ...
    bin=$R/bins/ft/memra-server; door=MEMRA_KV_HOST_CONTRACTS=1
