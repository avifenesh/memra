#!/usr/bin/env bash
# DAY49 (OWED items 7 and 8) target-card build, outside any hold, on the box's clone of this lane (/root/wt-a): the tip's
# release server (the attribution binary, the split lines built in). usage: build.sh <tip_sha>
set -uo pipefail
...
git checkout -q -B lane-a-d49 "$1" >> "$L" 2>&1 || { echo "rc=2 (checkout tip)" >> "$R/build.log"; exit 2; }
...
echo "split-line wording: $(grep -ac 'demote helper split: ticket seq=' "$R/bins/tip/memra-server") $(grep -ac 'demote pre-submit split: ticket seq=' "$R/bins/tip/memra-server")" > "$R/markers.txt"
