#!/usr/bin/env bash
# BOX3 day-28 driver (option (a)'s acceptance gate, ruling 39): waits for the build receipt, then, in one sitting:
...
# under the collector (the door's GPU unit cells, the D2D engine cells, and the day-28 CPU hash cells on this host).
# Bounded retry of a busy collector lock (60 x 120 s), never signals the holder. No host, id or price here.
set -uo pipefail
cd /root/wt-a || exit 1
R=/root/spill-receipts/a-day28
D=research/spill-a-20260919/pro-single-day28
