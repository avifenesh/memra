#!/usr/bin/env bash
# Design S's hump clause (e) (DAY40 section 3) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash hump.sh @COLLECTOR_LOCK_FD@`.
# Four door-ON boots xg4 xs xs xg4 (xg4 = the G4 tip, xs = the S tip),
...
sha256sum "$R/bins/g4/memra-server" "$R/bins/s/memra-server" > "$D0/binaries.sha256"
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard s-hump "$PORT" MEMRA_GATE_PORT || return 1
...
for arm in xg4 xs xs xg4; do
  ...
  bin=$R/bins/s/memra-server; [ $arm = xg4 ] && bin=$R/bins/g4/memra-server
