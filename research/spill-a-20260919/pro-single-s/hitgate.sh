#!/usr/bin/env bash
# The hit gate on the G4 target card (DAY40 section 3's (b)), on the S tip binary (no --external-lock arm): under flock on the canonical lock, door OFF then ON. The ON arm
