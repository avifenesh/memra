#!/usr/bin/env bash
# Design S's target-card promote A/B (DAY40 section 3, clause (d): g4 against s) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash ab-promote.sh @COLLECTOR_LOCK_FD@`.
# stall_cell.py --mode promote --n 5, g4 (the G4 sitting's binary) against s (the S tip), o1 = g4 s x5, o2 = s g4 x5, door ON,
# the PRO environment (DAY36 section 3's), readiness bounded to 480 s; the day40-reading.py layout
# (promote/ab/o{1,2}/bNN-{g4,s}).
...
python3 tools/tier-lock-proof.py --fd "$fd" --lock /tmp/memra-gpu.lock --owner collector > "$R/promote/LOCK.json"
log() { echo "$(date -u +%FT%TZ) $*" | tee -a "$R/promote/run.log"; }
log "model sha256: $(sha256sum "$MODEL" | cut -d' ' -f1) $(basename "$MODEL")"
sha256sum "$R/bins/g4/memra-server" "$R/bins/s/memra-server" > "$R/promote/binaries.sha256"
idle=0
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard s-promote "$PORT" MEMRA_GATE_PORT || return 1
...
for order in o1 o2; do
  ...
  for arm in $arms; do
    ...
    bin=$R/bins/g4/memra-server; [ $arm = s ] && bin=$R/bins/s/memra-server
...
python3 research/spill-a-20260919/day40-reading.py promote "$R/promote" > "$R/promote/reading-day40-promote.log" 2>&1
