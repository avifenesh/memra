#!/usr/bin/env bash
# The hit gate on the G4 target card (DAY42 section 1's (b)), on the P2 tip binary (DAY52 section 1 (a)) (no --external-lock arm): under flock on the canonical lock, door OFF then ON. The ON arm
