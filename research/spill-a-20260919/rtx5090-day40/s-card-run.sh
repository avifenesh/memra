#!/usr/bin/env bash
# WP-A day 40 design S on the local RTX 5090 Laptop GPU (DAY40.md section 3), fixed before it runs. ONE bounded hold of
...
#  1. the unit cells from prebuilt test binaries of the S tree: the door's GPU cells (option_b_, option_c_), serial, and
#     the engine's native tier_transfer cells in parallel (the span cells with every digest bitwise and the flip arm);
#  2. clause (c): the demote A/B, stall_cell.py --mode demote --n 5, g4 (the G4 tip) against s (the S tip), o1 = g4 s x5,
#     o2 = s g4 x5, door ON, the day-35 5090 environment, readiness bounded to 60 s; 250 ms telemetry; day40-reading.py;
#  3. clause (d): the promote A/B, the same with --mode promote --n 5, into promote/;
#  4. clause (e): the hump cell, four door-ON boots xg4 xs xs xg4, each --mode demote --n 8;
#  5. the gates with the S binary, each through --external-lock 9: identity x4, failure OFF and ON, fault default and
#     plain (every cell, span-flip-landed and span-flip-resident included), hit OFF and ON.
# Executed-not-qualified. usage: s-card-run.sh <out_root> <model.gguf> <bin_g4> <bin_s> <server_tests> <engine_tests>
set -uo pipefail
ROOT=$1; MODEL=$2; BG4=$3; BM=$4; ST=$5; ET=$6
...
sha256sum "$BG4" "$BM" "$ST" "$ET" > "$ROOT/binaries.sha256"
...
SERVER_PID=""
boot() { # $1 bin $2 log
    memra_port_guard day40-s "$PORT" MEMRA_GATE_PORT || return 1
...
ab() { # $1 mode (demote|promote) $2 dir
    ...
    for order in o1 o2; do
        arms="g4 s g4 s g4 s g4 s g4 s"; [ $order = o2 ] && arms="s g4 s g4 s g4 s g4 s g4"
        for arm in $arms; do
            i=$((i+1)); D=$AB/ab/$order/$(printf 'b%02d-%s' "$i" "$arm"); mkdir -p "$D"
            bin=$BG4; [ $arm = s ] && bin=$BM
    ...
    python3 research/spill-a-20260919/day40-reading.py "$mode" "$AB" > "$AB/reading-day40-$mode.log" 2>&1
    log "$mode reading $(tail -1 "$AB/reading-day40-$mode.log")"
}
...
mkdir -p "$ROOT/hump"
j=0
for arm in xg4 xs xs xg4; do
    j=$((j+1)); D=$ROOT/hump/$(printf 'b%02d-%s' "$j" "$arm"); mkdir -p "$D"
    bin=$BM; [ $arm = xg4 ] && bin=$BG4
...
flock -u 9
log "hold released; DAY40-S-CARD-RUN-DONE"
