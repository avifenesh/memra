...
# usage: tenant-cell.sh <base|fix> <lockfd> <cell-name>
set -uo pipefail
arm=$1; fd=$2; cell=$3
...
cd /root/wt-a
git rev-parse HEAD
sha256sum "$R/bins/memra-server-$arm"
env MEMRA_GPU_LOCK=/tmp/memra-gpu.lock CUDA_VISIBLE_DEVICES=0 MEMRA_GATE_PORT=18129 \
  ...
  /root/artifacts/Qwen3.8-27B-NVFP4-Q5K-mtp.gguf "$R/bins/memra-server-$arm" "$R/$cell/ev"
