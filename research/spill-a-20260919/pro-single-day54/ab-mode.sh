#!/usr/bin/env bash
# DAY54's paired stall cell (DAY54.md section 1) in ONE collector hold:
...
SERVER_PID=""
boot() { # $1 log
  memra_port_guard d54-ab "$PORT" MEMRA_GATE_PORT || return 1
...
for order in o1 o2; do
  ...
  for mode in $arms; do
    ...
    python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode "$mode" --n 5 --server-log "$D/server.log" \
      --out "$D/$mode" --tag "d54-$CELL-$mode" > "$D/$mode.log" 2>&1
