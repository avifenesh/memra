#!/usr/bin/env bash
# DAY54 (OWED item 10) target-card build, outside any hold, on the box's clone of this lane (/root/wt-a): the tip's
...
git checkout -q -B lane-a-d54 "$1" >> "$L" 2>&1 || { echo "rc=2 (checkout tip)" >> "$R/build.log"; exit 2; }
...
echo "on-tick wording: $(grep -ac 'on-tick publish: publisher=' "$R/bins/tip/memra-server") dedup: $(grep -ac 'sibling restore(s) ' "$R/bins/tip/memra-server") park: $(grep -ac 'pause park snapshot on the tick' "$R/bins/tip/memra-server")" > "$R/markers.txt"
