#!/usr/bin/env bash
# DAY54's pause reading (DAY54.md section 1) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash pause.sh @COLLECTOR_LOCK_FD@`.
# The tip binary, three boots of stall_cell.py --mode pause --n 5 with MEMRA_KV_PAUSE_DEMOTE=1 MEMRA_KV_PAUSE_DEMOTE_MS=250
# (V's pause environment, DAY47),
...
log "model sha256: $(sha256sum "$MODEL" | cut -d' ' -f1) $(basename "$MODEL") cell=$CELL mode=pause prefix_mb=$PX"
...
SERVER_PID=""
boot() { # $1 log
  memra_port_guard d54-pause "$PORT" MEMRA_GATE_PORT || return 1
  env CUDA_VISIBLE_DEVICES=0 MEMRA_COMPAT=openai "MEMRA_MODELS=gate=$MODEL" "MEMRA_ADDR=127.0.0.1:$PORT" \
    ...
    MEMRA_KV_HOST_CONTRACTS=1 MEMRA_KV_PAUSE_DEMOTE=1 MEMRA_KV_PAUSE_DEMOTE_MS=250 "$BIN" > "$1" 2>&1 &
  SERVER_PID=$!
...
for b in 1 2 3; do
  mode=pause
  i=$((i+1)); D=$O/b$(printf '%02d' "$i"); mkdir -p "$D"
  echo "mode=$mode cell=$CELL bin=$(sha256sum "$BIN" | cut -c1-16)" > "$D/BOOT.txt"
  ...
  if ! boot "$D/server.log"; then
    log "$CELL $mode boot NOT READY within 480 s; stopped"; echo "boot-failed" >> "$D/BOOT.txt"; stop; continue
  fi
