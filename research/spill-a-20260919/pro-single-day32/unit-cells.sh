...
# H2D span cells), the engine's D2D, D2H span and day-32 H2D span cells (memra-engine lib), and the CPU cells (the hash, Hashing,
# span-order and day-31 censuses, the day-32 Filling cells and census, the memra-tier D2H and H2D span rules) on the target card
...
cargo test -p memra-server --lib -- hash_ hashing_ every_path_that_meets_a_hashing every_path_that_meets_a_demoting every_path_that_meets_a_promoting the_run_loop_waits_boundedly the_d2h_spans day31_ day32_ > "$R/unit/cargo-test-hash.log" 2>&1
rc3=$?
