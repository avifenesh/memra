#!/usr/bin/env bash
# Day-33 counts over a receipts root (DAY33.md quotes this output; nothing is counted by hand). The day-32 binary run of the
# double-park cell under <root>/pre is excluded from every section but the last two, which read it by name.
#   bash counts.sh pro-single-day33/box > pro-single-day33/box/counts.log     (run from research/spill-a-20260919)
#   bash counts.sh rtx5090-day33 > rtx5090-day33/counts.log
set -uo pipefail
R=${1:?receipts root}
...
grep -rhoE 'f32 spans filled on the copy stream \([0-9.]+MB\)' --include='*.log' --exclude=counts.log --exclude-dir=pre "$R" | sort | uniq -c
grep -rhoE 'promote submitted off the tick: [0-9]+ tokens, [0-9.]+MB, ticket seq=[0-9]+, [0-9]+ items on the contracts door.s copy stream, [0-9]+ f32 spans filled on the copy stream' --include='*.log' --exclude=counts.log --exclude-dir=pre "$R" \
  | sed -E 's/ticket seq=[0-9]+, //' | sort | uniq -c
echo "submissions without spans: $(grep -rh 'promote submitted off the tick' --include='*.log' --exclude=counts.log --exclude-dir=pre "$R" | grep -vc 'f32 spans filled on the copy stream')"
...
if [ -d "$R/pre/double-park/ev" ]; then
  echo "## the day-32 binary double-park run (the before arm): replays, receipts, compute apps"
