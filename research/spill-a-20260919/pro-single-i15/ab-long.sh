#!/usr/bin/env bash
# OWED item 15's long-entry A/B (DAY43.md section 1: g4 against g3 at 4096-token entries) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash ab-long.sh @COLLECTOR_LOCK_FD@ <mode>`, mode
# demote-long or promote-long. stall_cell.py --mode <mode> --n 5, o1 = g4 g3 x5, o2 = g3 g4 x5, door ON, the PRO
# environment of the S2 sitting with MEMRA_PREFIX_CACHE_MB=448 (one 4096-token 27B entry, about 280 MB, fits; two do
# not), readiness bounded to 480 s; each boot's start temperature and SM clock in BOOT.txt; 250 ms telemetry; the layout
# item15-reading.py reads (<mode>/ab/o{1,2}/bNN-{g4,g3}). A compute app at the hold's start: bounded 15 x 60 s, then
# NOT RUN. Executed-not-qualified.
set -uo pipefail
fd=$1; MODE=$2
...
cd /root/wt-a || exit 1
case "$MODE" in demote-long|promote-long) ;; *) echo "mode $MODE"; exit 2;; esac
O=$R/$MODE
mkdir -p "$O/ab"
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard i15-long "$PORT" MEMRA_GATE_PORT || return 1
...
for order in o1 o2; do
  arms="g4 g3 g4 g3 g4 g3 g4 g3 g4 g3"; [ $order = o2 ] && arms="g3 g4 g3 g4 g3 g4 g3 g4 g3 g4"
  for arm in $arms; do
    ...
    if ! boot "$bin" "$D/server.log"; then
      log "$MODE $order $arm boot NOT READY within 480 s; stopped"; echo "boot-failed" >> "$D/BOOT.txt"; stop; continue
    fi
    python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode "$MODE" --n 5 --server-log "$D/server.log" \
      --out "$D/$MODE" --tag "i15-$MODE-$arm" > "$D/$MODE.log" 2>&1
    log "$MODE $order $arm rc=$? $(grep -h 'STALL rule' "$D/$MODE.log" | cut -c1-120)"
    stop
