#!/usr/bin/env python3
"""WP-A day 51 (DAY51.md section 1, OWED item 17) reader: design P against base, written before its cells run.

usage: day51-reading.py R
Input (the pro-single-p sitting's layout):
  R/{demote,free,promote,chain}/ab/o{1,2}/bNN-{base,p}/{server.log,rss.txt,<mode>/receipt.json} and each
  R/<cell>/ab/replays.log (stall_cell.py; demote and free run --mode demote, promote --mode promote, chain
  --mode promote-long; --n 5; 20 boots per cell, five per arm per order);
  R/hump/reading-hump.log (day38-hump-reading.py over xgpp xp xp xgpp);
...
error (and with `chain_wall_ms` in chain), 20 `STALL REPLAY: PASS`, and in demote and free one `demote helper split`
...
The rule (P is the door's copy program on this class only if every clause holds, each in both orders):
  ...
  (e) hump: xp median HUMP <= 0.15 with the xgpp control above 0.15 (a control at or below 0.15: UNREAD)
  (f) free: p wall <= base wall + 2.0; p copy <= base copy + 1.0
  (g) chain: p chain <= base chain + 1.0; p first <= base first + 1.0
Readings: the refill lines (`payload reserve ready: N buffers, B MB in X ms (minflt +M, yielded Y time(s))`) and
VmRSS / VmHWM per arm per cell.
"""
import glob
import json
import os
...
def boot(d, cell):
    ...
    out = {k: [] for k in ("wall", "e2e", "copy", "minflt", "mb", "hit", "pin", "chain", "first", "refill_ms",
                           "refill_minflt", "refill_yields", "rss", "hwm")}
    out["ok"], out["why"] = True, []
...
def main():
    ...
    for cell in ("demote", "free", "promote", "chain"):
        ...
        for order in ("o1", "o2"):
            for arm in ARMS:
                ...
                print(f"DAY51 READING cell={cell} order={order} arm={arm} {terms} | refill N={len(rf)} "
        ...
        if not complete:
            print(f"DAY51 CELL {cell} INCOMPLETE ({'; '.join(notes)}) -> nothing is read; the cell repeats whole once")
            unread = True
            pools[cell] = None
    rules = {
        "b": ("demote", lambda P, B, o: [
            ("minflt", med(P(o, "p", "minflt")), 0.25 * med(P(o, "p", "mb")) * 1e6 / 4096, "<=", "p-median (0.25 x pages)"),
            ("copy", med(P(o, "p", "copy")) - med(P(o, "base", "copy")), -8.0, "<=", "p-minus-base"),
            ("hits", sum(P(o, "p", "hit")) / max(1, len(P(o, "p", "hit"))), 0.90, ">=", "fraction of steady demotes"),
        ]),
        "c": ("demote", lambda P, B, o: [
            ("wall", med(P(o, "p", "wall")) - med(P(o, "base", "wall")), -8.0, "<=", "p-minus-base"),
            ("e2e", med(P(o, "p", "e2e")) - med(P(o, "base", "e2e")), 1.0, "<=", "p-minus-base"),
        ]),
        "d": ("promote", lambda P, B, o: [
            ("pin", med(P(o, "p", "pin")) - med(P(o, "base", "pin")), 1.0, "<=", "p-minus-base"),
            ("e2e", med(P(o, "p", "e2e")) - med(P(o, "base", "e2e")), 1.0, "<=", "p-minus-base"),
        ]),
        "f": ("free", lambda P, B, o: [
            ("wall", med(P(o, "p", "wall")) - med(P(o, "base", "wall")), 2.0, "<=", "p-minus-base"),
            ("copy", med(P(o, "p", "copy")) - med(P(o, "base", "copy")), 1.0, "<=", "p-minus-base"),
        ]),
        "g": ("chain", lambda P, B, o: [
            ("chain", med(P(o, "p", "chain")) - med(P(o, "base", "chain")), 1.0, "<=", "p-minus-base"),
            ("first", med(P(o, "p", "first")) - med(P(o, "base", "first")), 1.0, "<=", "p-minus-base"),
        ]),
    }
    for clause, (cell, terms) in rules.items():
        if pools[cell] is None:
            print(f"DAY51 P ({clause}) cell={cell} -> UNREAD (incomplete)")
            continue
        for order in ("o1", "o2"):
            parts, ok = [], True
            ...
            failed |= not ok
            print(f"DAY51 P ({clause}) cell={cell} order={order} " + " | ".join(parts) + f" -> {'PASS' if ok else 'FAIL'}")
    ...
    if "xp" not in arms or "xgpp" not in arms or arms["xp"][0] != 2 or arms["xgpp"][0] != 2:
        print(f"DAY51 P (e) hump -> UNREAD (arms {arms})")
        unread = True
    elif arms["xgpp"][1] <= 0.15:
        print(f"DAY51 P (e) hump xp={arms['xp'][1]:+.3f} control xgpp={arms['xgpp'][1]:+.3f} -> UNREAD (the control "
              f"did not hump; the cell repeats once)")
        unread = True
    else:
        ok = arms["xp"][1] <= 0.15
        failed |= not ok
        print(f"DAY51 P (e) hump xp={arms['xp'][1]:+.3f} rule <=0.15 control xgpp={arms['xgpp'][1]:+.3f} (humps) -> "
    ...
    print(f"DAY51 P (a) gates {' '.join(f'{k}={v}' for k, v in exits.items())} | unit [{unit_line}] -> "
          f"{'PASS' if gates_ok and unit_ok else 'FAIL'}")
    print(f"DAY51 P -> {'FAIL' if failed else ('INCOMPLETE' if unread else 'PASS')}")
