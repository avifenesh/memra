#!/usr/bin/env bash
# WP-A day 35 design M on the local RTX 5090 Laptop GPU (DAY35.md section 2), fixed before it runs. ONE bounded hold of
...
#  1. the unit cells from prebuilt test binaries: the door's GPU cells (option_b_, option_c_) and the engine's H2D and
#     D2H cells (h2d_, d2h_), serial;
#  2. the pre-registered A/B: stall_cell.py --mode demote --n 5 (byte for byte), the base binary (the lane after F was
#     settled) against the M binary, o1 = base m x5, o2 = m base x5, door ON, the section-1 boot environment, each
...
#  3. the gates with the M binary, each through --external-lock 9: identity x4, failure ON (acceptance (a)'s served-path
#     cell), fault default and plain, hit OFF and ON (the binary is named memra-server: the hit gate's stop keys on it).
# Executed-not-qualified. usage: m-card-run.sh <out_root> <model.gguf> <bin_base> <bin_m> <server_tests> <engine_tests>
set -uo pipefail
...
(cd crates/memra-engine && "$ET" --ignored --test-threads=1 --nocapture h2d_ d2h_ > "$ROOT/unit/engine-transfer-cells.log" 2>&1); log "unit engine rc=$? $(grep -h '^test result' "$ROOT/unit/engine-transfer-cells.log")"
# 2. the A/B
PORT=${MEMRA_GATE_PORT:-18137}
. tools/port-guard.sh
SERVER_PID=""
boot() { # $1 bin $2 log
    memra_port_guard day35-m "$PORT" MEMRA_GATE_PORT || return 1
...
flock -u 9
log "hold released; DAY35-M-CARD-RUN-DONE"
