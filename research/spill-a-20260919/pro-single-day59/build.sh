#!/usr/bin/env bash
# DAY59 (OWED item 10, the fanout attribution) target-card build, outside any hold, on the box's clone of this lane (/root/wt-a): the tip's
...
git checkout -q -B lane-a-d59 "$1" >> "$L" 2>&1 || { echo "rc=2 (checkout tip)" >> "$R/build.log"; exit 2; }
...
echo "split wording: $(grep -ac 'on-tick split: snapshot alloc' "$R/bins/tip/memra-server")" > "$R/markers.txt"
