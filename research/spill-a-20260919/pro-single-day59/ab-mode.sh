#!/usr/bin/env bash
# DAY59's attribution cell (DAY59.md section 1; DAY54's paired cell shape) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash ab-mode.sh @COLLECTOR_LOCK_FD@ <cell> <modeA> <modeB> <prefix_mb>`.
# One binary (the tip), two stall_cell.py modes as the arms: o1 = A B x5, o2 = B A x5, each boot one mode at --n 5,
...
SERVER_PID=""
boot() { # $1 log
  memra_port_guard d59-ab "$PORT" MEMRA_GATE_PORT || return 1
  env CUDA_VISIBLE_DEVICES=0 MEMRA_COMPAT=openai "MEMRA_MODELS=gate=$MODEL" "MEMRA_ADDR=127.0.0.1:$PORT" \
    MEMRA_CTX=8192 MEMRA_MAX_SESSIONS=8 MEMRA_SERVE_SPEC=0 "MEMRA_PREFIX_CACHE_MB=$PX" MEMRA_KV_HOST_MB=8192 \
...
for order in o1 o2; do
  ...
  for mode in $arms; do
    ...
    python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode "$mode" --n 5 --server-log "$D/server.log" \
      --out "$D/$mode" --tag "d59-$CELL-$mode" > "$D/$mode.log" 2>&1
