...
# mode, 20 boots, the prefix cache at 256 MB) under ONE collector hold, then day64-split-reading.py. A busy collector
# lock: bounded retries (60 x 120 s). Executed-not-qualified. No host, id or price here.
set -uo pipefail
cd /root/wt-a || exit 1
R=/root/spill-receipts/a-d64b
D=research/spill-a-20260919/pro-single-day64b
...
python3 research/spill-a-20260919/day64-split-reading.py "$R" > "$R/reading-day64b.log" 2>&1
echo "$(date -u +%FT%TZ) reading rc=$? $(tail -1 "$R/reading-day64b.log")" | tee -a "$R/progress.log"
