#!/usr/bin/env bash
# DAY64's placing cell (DAY64.md section 1 step 2) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash ab-mode.sh @COLLECTOR_LOCK_FD@ promote promote promote 256`.
# One binary (the tip), stall_cell.py --mode promote at --n 5, 20 boots (o1 ten, o2 ten), door ON, the PRO environment of
# the S sittings (MEMRA_KV_HOST_MB=8192, MEMRA_SERVE_SPEC=0, MEMRA_MAX_SESSIONS=4), the prefix cache at <prefix_mb>;
# each boot's start temperature and SM clock in BOOT.txt; 250 ms telemetry. A compute app at the hold's start: bounded
# 15 x 60 s, then NOT RUN. Executed-not-qualified.
set -uo pipefail
fd=$1; CELL=$2; MA=$3; MB=$4; PX=$5
...
log "model sha256: $(sha256sum "$MODEL" | cut -d' ' -f1) $(basename "$MODEL") cell=$CELL modes=$MA,$MB prefix_mb=$PX"
...
SERVER_PID=""
boot() { # $1 log
  memra_port_guard d64-ab "$PORT" MEMRA_GATE_PORT || return 1
...
for order in o1 o2; do
  arms="$MA $MB $MA $MB $MA $MB $MA $MB $MA $MB"; [ $order = o2 ] && arms="$MB $MA $MB $MA $MB $MA $MB $MA $MB $MA"
  for mode in $arms; do
    i=$((i+1)); D=$O/ab/$order/$(printf 'b%02d-%s' "$i" "$mode"); mkdir -p "$D"
    echo "mode=$mode order=$order cell=$CELL bin=$(sha256sum "$BIN" | cut -c1-16)" > "$D/BOOT.txt"
    ...
    if ! boot "$D/server.log"; then
      log "$CELL $order $mode boot NOT READY within 480 s; stopped"; echo "boot-failed" >> "$D/BOOT.txt"; stop; continue
    fi
    python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode "$mode" --n 5 --server-log "$D/server.log" \
      --out "$D/$mode" --tag "d64-$CELL-$mode" > "$D/$mode.log" 2>&1
    log "$CELL $order $mode rc=$? $(grep -h 'STALL rule' "$D/$mode.log" | cut -c1-120)"
    stop
