#!/usr/bin/env bash
# WP-A day 38 design G''' on the local RTX 5090 Laptop GPU (DAY38.md section 14), fixed before it runs. ONE bounded hold of
...
#  1. the unit cells from prebuilt test binaries of the G''' tree: the door's GPU cells (option_b_, option_c_), serial, and
#     the engine's native tier_transfer cells in parallel (each owns a pool context);
#  2. section 3's A/B: stall_cell.py --mode demote --n 5 (byte for byte), base (80039a8de) against G''' (the tip), o1 =
#     base g x5, o2 = g base x5, door ON, the day-35 5090 environment, readiness bounded to 60 s; 250 ms telemetry;
#  3. (f) the hump cell: four door-ON boots xgpp xg3 xg3 xgpp (G'' the pre-G''' tip as the control, G''' the tip), each
#     stall_cell.py --mode demote --n 8 (16 demote runs), read by day38-hump-reading.py;
#  4. the gates with the G''' binary, each through --external-lock 9: identity x4, failure OFF and ON, fault default and
#     plain (every cell, day 41's three included), hit OFF and ON (binary named memra-server).
# Executed-not-qualified. usage: g3-card-run.sh <out_root> <model.gguf> <bin_base> <bin_g3> <bin_gpp> <server_tests> <engine_tests>
set -uo pipefail
...
SERVER_PID=""
boot() { # $1 bin $2 log
    memra_port_guard day38-g3 "$PORT" MEMRA_GATE_PORT || return 1
...
mkdir -p "$ROOT/hump"
j=0
for arm in xgpp xg3 xg3 xgpp; do
...
flock -u 9
log "hold released; DAY38-G3-CARD-RUN-DONE"
