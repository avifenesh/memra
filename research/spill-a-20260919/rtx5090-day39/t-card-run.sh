#!/usr/bin/env bash
# WP-A day 39 design T on the local RTX 5090 Laptop GPU (DAY39.md section 5), fixed before it runs. ONE bounded hold of
...
#  1. the unit cells from prebuilt test binaries of the FT tree: the door's GPU cells (option_b_, option_c_), serial, and
#     the engine's native tier_transfer cells in parallel (each owns a pool context), the filled-batch cell on three threads;
#  2. DAY35's cell with the arms FT F1 OFF: stall_cell.py --mode promote --n 5 (byte for byte), o1 = ft f1 off x5, o2 = off
#     f1 ft x5, 30 boots, the day-35 5090 environment (door ON on ft and f1; off is the FT binary with the door off), each
#     server's readiness bounded to 60 s (not ready: stopped, recorded; the reader then reads the cell INCOMPLETE); 250 ms
#     card telemetry across the hold;
#  3. the gates with the FT binary, each through --external-lock 9: identity x4, failure OFF and ON, fault default and
#     plain, hit OFF and ON (binary named memra-server).
# Executed-not-qualified. usage: t-card-run.sh <out_root> <model.gguf> <bin_ft> <bin_f1> <server_tests> <engine_tests>
set -uo pipefail
ROOT=$1; MODEL=$2; BM=$3; BF1=$4; ST=$5; ET=$6
...
sha256sum "$BM" "$BF1" "$ST" "$ET" > "$ROOT/binaries.sha256"
...
# 2. the A/B
PORT=${MEMRA_GATE_PORT:-18139}
. tools/port-guard.sh
SERVER_PID=""
boot() { # $1 bin $2 log $3 door (on|off)
    memra_port_guard day39-t "$PORT" MEMRA_GATE_PORT || return 1
    local door=""
...
for order in o1 o2; do
    arms="ft f1 off ft f1 off ft f1 off ft f1 off ft f1 off"
    [ $order = o2 ] && arms="off f1 ft off f1 ft off f1 ft off f1 ft off f1 ft"
    for arm in $arms; do
...
flock -u 9
log "hold released; DAY39-T-CARD-RUN-DONE"
