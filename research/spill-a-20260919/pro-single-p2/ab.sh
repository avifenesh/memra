...
sha256sum "$R/bins/base/memra-server" "$R/bins/p2/memra-server" "$R/bins/p/memra-server" > "$O/binaries.sha256"
idle=0
...
for order in o1 o2; do
  ...
  if [ "$CELL" = chain ]; then
    arms="base p p2 base p p2 base p p2 base p p2 base p p2"; [ $order = o2 ] && arms="p2 p base p2 p base p2 p base p2 p base p2 p base"
  fi
  for arm in $arms; do
