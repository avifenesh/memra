#!/usr/bin/env bash
# Design P2's target-card build (DAY52.md section 1), outside any hold, on the box's clone of this lane (/root/wt-a,
...
mkdir -p "$R/bins/p2" "$R/bins/base" "$R/bins/p" "$R/bins/gpp"
L=$R/build-steps.log
GPP=${3:-358749c9f}
...
git checkout -q -B lane-a-p2-tip "$1" >> "$L" 2>&1 || { echo "rc=2 (checkout tip)" >> "$R/build.log"; exit 2; }
...
PATCH=$(pwd)/research/spill-a-20260919/pro-single-p2/p-arm.patch
cp "$PATCH" "$R/p-arm.patch"
for arm in base p gpp; do
  ...
  # p: base's crates plus P (day 51) re-applied onto the split lines (the patch is kept beside the receipts).
  if [ $arm = p ]; then git apply --index "$R/p-arm.patch" >> "$L" 2>&1 || { echo "rc=2 (p patch)" >> "$R/build.log"; exit 2; }; fi
...
for b in p2 base p gpp; do echo "$b reserve-ready wording: $(grep -ac 'payload reserve ready: ' "$R/bins/$b/memra-server") arming wording: $(grep -ac ' fresh pages of ' "$R/bins/$b/memra-server") publication-split wording: $(grep -ac 'demote publication split: ticket seq=' "$R/bins/$b/memra-server")"; done > "$R/markers.txt"
