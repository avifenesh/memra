...
# L''s half (section 1), item 16 (section 3, DAY45's cell from its registration tree 7b849a817), then S4's and V's
# halves (section 4, each in its two holds). Run as a copy:
...
bash "$S/s4/scripts/rtx5090-half-sv.sh" card s4 > "$S/s4/card.out" 2>&1; log "s4 card rc=$? $(tail -1 "$S/s4/receipts/run.log")"
bash "$S/v/scripts/rtx5090-half-sv.sh" card v > "$S/v/card.out" 2>&1; log "v card rc=$? $(tail -1 "$S/v/receipts/run.log")"
log "chain done"
