#!/usr/bin/env bash
# WP-A day 29 RE-RUN of the two fault arms only (the first pass died at the gate's first cell: `grep -c` under the gate's
...
cell fault-default-rerun "MEMRA_HOSTGATE_CACHE_MB=64" bash tools/kv-host-contract-fault-gate.sh "$MODEL" "$BIN"
cell fault-plain-rerun "MEMRA_HOSTGATE_CACHE_MB=64 MEMRA_SERVE_SPEC=0" bash tools/kv-host-contract-fault-gate.sh "$MODEL" "$BIN"
echo "$(date -u +%FT%TZ) LOCAL-FAULT-RERUN-DONE" | tee -a "$ROOT/battery.log"
