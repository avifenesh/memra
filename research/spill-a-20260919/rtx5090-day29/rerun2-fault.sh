#!/usr/bin/env bash
# WP-A day 29 second RE-RUN of the two fault arms (the first re-run passed its hand-off wait with 0 hand-offs and still failed `the next demote publishes`; the wait is now the check`s own condition, a publication after the refusal). First: the first pass died at the gate`s first cell: `grep -c` under the gate`s
# `set -e`, fixed in the tree; the first-pass dirs stay as the record). Same protocol as battery-5090.sh.
# WP-A day 29, the acceptance gate's "both cards" on the local RTX 5090 Laptop GPU (9B NVFP4 MTP artifact) with the
# day-29 tree's local release binary: the identity gate default ON (the arm day 28 read red), the fault gate (ten
...
cell fault-default-rerun2 "MEMRA_HOSTGATE_CACHE_MB=64" bash tools/kv-host-contract-fault-gate.sh "$MODEL" "$BIN"
cell fault-plain-rerun2 "MEMRA_HOSTGATE_CACHE_MB=64 MEMRA_SERVE_SPEC=0" bash tools/kv-host-contract-fault-gate.sh "$MODEL" "$BIN"
echo "$(date -u +%FT%TZ) LOCAL-FAULT-RERUN2-DONE" | tee -a "$ROOT/battery.log"
