...
git checkout -q -B lane-a-r1 "$1" >> "$L" 2>&1 || { echo "rc=2 (checkout tip)" >> "$R/build.log"; exit 2; }
...
echo "== r1" >> "$L"
nice -n 5 cargo build --release -p memra-server >> "$L" 2>&1 || { echo "rc=1 (r1)" >> "$R/build.log"; exit 1; }
cp target/release/memra-server "$R/bins/r1/memra-server"
echo "== base ($2)" >> "$L"
