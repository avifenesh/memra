#!/usr/bin/env bash
# Design F's hump reading for (d) (DAY64 section 5; S's cell, DAY42 section 1) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash hump.sh @COLLECTOR_LOCK_FD@`.
# Four door-ON boots xbase xf xf xbase (xbase = F's parent, xf = F's tip), each
...
sha256sum "$R/bins/base/memra-server" "$R/bins/f/memra-server" > "$D0/binaries.sha256"
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard f-hump "$PORT" MEMRA_GATE_PORT || return 1
...
trap finish EXIT
i=0
for arm in xbase xf xf xbase; do
  i=$((i+1)); D=$D0/$(printf 'b%02d-%s' "$i" "$arm"); mkdir -p "$D"
  bin=$R/bins/f/memra-server; [ $arm = xbase ] && bin=$R/bins/base/memra-server
