...
for pass in 1 2; do
  for which in off on; do
    ...
    bash $D/stall-cell.sh "$fd" $which > "$R/stall-capture-$which.pass$pass.log" 2>&1 || rc=1
    # the second pass writes into the same ev dir: move the receipt aside first
    mv "$R/stall-capture-$which/ev" "$R/stall-capture-$which/ev.pass$pass" 2>/dev/null || true
    echo "$(date -u +%FT%TZ) stall-capture-$which pass=$pass rc=$rc"
  done
done
