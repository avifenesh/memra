...
MODEL=${MEMRA_DAY36_MODEL:-/root/artifacts/Qwen3.8-27B-NVFP4-Q5K-mtp.gguf}
export PATH=/root/.cargo/bin:/usr/local/cuda/bin:$PATH
cd /root/wt-a || exit 1
mkdir -p "$R/price" "$R/m2/ab"
...
nvidia-smi --query-gpu=timestamp,temperature.gpu,power.draw,clocks.sm,clocks.mem,memory.used,utilization.gpu \
  --format=csv -lms 250 > "$R/m2/card-250ms.csv" 2>&1 &
SAMPLER=$!
# 1. the price cell
bash research/spill-a-20260919/pro-single-day26/restore-arm.sh "$fd" /root/wt-a "$R/price" "$MODEL" "$R/bins/tip/memra-server" > "$R/price/restore-arm.log" 2>&1
log "price cell rc=$? $(tail -1 "$R/price/restore-arm/ev/replays.log" 2>/dev/null)"
# 2. the A/B
PORT=${MEMRA_GATE_PORT:-18137}
. tools/port-guard.sh
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard day36-m2 "$PORT" MEMRA_GATE_PORT || return 1
...
for order in o1 o2; do
  ...
  for arm in $arms; do
    ...
    { echo "== $order/$(basename "$D")"; python3 research/spill-a-20260919/stall_cell.py --replay "$D/demote/receipt.json"; } \
      >> "$R/m2/ab/replays.log" 2>&1
  done
done
trap - EXIT
kill "$SAMPLER" 2>/dev/null || true
nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv > "$R/m2/compute-apps.after.csv" 2>&1
log "price-and-ab done"
