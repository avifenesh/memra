#!/usr/bin/env bash
# The hit gate on the day-36 target card (DAY36 section 3), on the tip binary (no --external-lock arm): under flock on the canonical lock, door OFF then ON. The ON arm
