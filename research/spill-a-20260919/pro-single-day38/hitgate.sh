#!/usr/bin/env bash
# The hit gate on the day-38 target card (DAY38 section 4), on the tip binary (no --external-lock arm): under flock on the canonical lock, door OFF then ON. The ON arm
