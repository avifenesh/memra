#!/usr/bin/env bash
# Day-38 target-card driver (DAY38.md section 4): waits for the build receipt, then, in one sitting: ab.sh under
# ONE collector hold (design G's A/B), gates.sh under the collector on the tip binary, hitgate.sh (its own
# flock, OFF then ON), unit-cells.sh and DAY39's fill-survey.sh under the collector. A busy collector lock: bounded retries (60 x 120 s), never a
# signal to the holder. Every cell executed-not-qualified. No host, id or price here.
set -uo pipefail
cd /root/wt-a || exit 1
R=/root/spill-receipts/a-day38
D=research/spill-a-20260919/pro-single-day38
...
cell fill-survey 1200 $D/fill-survey.sh @COLLECTOR_LOCK_FD@
