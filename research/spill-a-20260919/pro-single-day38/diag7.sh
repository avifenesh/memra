#!/usr/bin/env bash
# DAY38 section 13i's bisection in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash diag7.sh @COLLECTOR_LOCK_FD@`.
# Six door-ON boots, x1 x25 x2 x2 x25 x1 (x1 = the tip binary; x2 = bins/x2; x25 = the diag7 patch: the demote stream,
# the receipt and then the device-receipt batch's own copies and spans on the receipt stream), each stall_cell.py
...
sha256sum "$R"/bins/{tip,x2,x25}/memra-server > "$D0/binaries.sha256"
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard day38-diag7 "$PORT" MEMRA_GATE_PORT || return 1
...
trap finish EXIT
i=0
for arm in x1 x25 x2 x2 x25 x1; do
  ...
  python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode demote --n 8 --server-log "$D/server.log" \
    --out "$D/demote" --tag "diag7-$arm" > "$D/demote.log" 2>&1
