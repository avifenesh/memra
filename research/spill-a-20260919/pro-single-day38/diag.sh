#!/usr/bin/env bash
# DAY38 section 12's diagnosis in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash diag.sh @COLLECTOR_LOCK_FD@`.
# Four long door-ON boots, X1 X2 X1 X2 (X1 = the tip binary, G''; X2 = bins/x2, the receipt on the copy stream), each
# stall_cell.py --mode demote --n 15 (30 demote runs), the PRO demote environment of ab.sh, then the growth reader.
set -uo pipefail
fd=$1
...
sha256sum "$R/bins/tip/memra-server" "$R/bins/x2/memra-server" > "$D0/binaries.sha256"
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard day38-diag "$PORT" MEMRA_GATE_PORT || return 1
...
for arm in x1 x2 x1 x2; do
  ...
  python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode demote --n 15 --server-log "$D/server.log" \
    --out "$D/demote" --tag "diag-$arm" > "$D/demote.log" 2>&1
  log "diag $arm rc=$? $(grep -h 'STALL rule' "$D/demote.log" | cut -c1-100)"
  stop
done
trap - EXIT
kill "$SAMPLER" 2>/dev/null || true
python3 research/spill-a-20260919/day38-growth-reading.py "$D0" > "$D0/reading-growth.log" 2>&1
