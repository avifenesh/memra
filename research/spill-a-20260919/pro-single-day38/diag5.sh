#!/usr/bin/env bash
# DAY38 section 13g's bisection in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash diag3.sh @COLLECTOR_LOCK_FD@`.
# Six door-ON boots, x1 x17 x2 x2 x17 x1 (x1 = the tip binary; x2 = bins/x2; x17 = the diag5 patch: the copy stream waits
# on the receipt kernel before its copies, the receipt still on its own stream), each stall_cell.py
...
sha256sum "$R"/bins/{tip,x2,x17}/memra-server > "$D0/binaries.sha256"
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard day38-diag5 "$PORT" MEMRA_GATE_PORT || return 1
...
trap finish EXIT
i=0
for arm in x1 x17 x2 x2 x17 x1; do
  ...
  python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode demote --n 8 --server-log "$D/server.log" \
    --out "$D/demote" --tag "diag5-$arm" > "$D/demote.log" 2>&1
