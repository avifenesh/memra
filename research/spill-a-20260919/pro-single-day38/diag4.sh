#!/usr/bin/env bash
# DAY38 section 13f's bisection in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash diag3.sh @COLLECTOR_LOCK_FD@`.
# Six door-ON boots, x1 x11 x2 x2 x11 x1 (x1 = the tip binary; x2 = bins/x2; x11 = the diag4 patch: the receipt kernel
# writes its digests into the pinned twin through its host pointer, no D2H on the receipt stream), each stall_cell.py
...
sha256sum "$R"/bins/{tip,x2,x11}/memra-server > "$D0/binaries.sha256"
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard day38-diag4 "$PORT" MEMRA_GATE_PORT || return 1
...
trap finish EXIT
i=0
for arm in x1 x11 x2 x2 x11 x1; do
  ...
  python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode demote --n 8 --server-log "$D/server.log" \
    --out "$D/demote" --tag "diag4-$arm" > "$D/demote.log" 2>&1
