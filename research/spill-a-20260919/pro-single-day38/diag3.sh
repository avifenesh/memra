#!/usr/bin/env bash
# DAY38 section 13e's bisection in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash diag3.sh @COLLECTOR_LOCK_FD@`.
# Twelve door-ON boots, x1 x2 x3 x4 x6 x8 then the reverse (x1 = the tip binary; x2 = bins/x2; x3, x4, x6, x8 = the
# diag3 patches), each stall_cell.py --mode demote --n 8 (16 demote runs), the PRO demote environment of ab.sh, then the
# hump reader.
set -uo pipefail
fd=$1
...
sha256sum "$R"/bins/{tip,x2,x3,x4,x6,x8}/memra-server > "$D0/binaries.sha256"
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard day38-diag3 "$PORT" MEMRA_GATE_PORT || return 1
...
trap finish EXIT
i=0
for arm in x1 x2 x3 x4 x6 x8 x8 x6 x4 x3 x2 x1; do
  ...
  python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode demote --n 8 --server-log "$D/server.log" \
    --out "$D/demote" --tag "diag3-$arm" > "$D/demote.log" 2>&1
