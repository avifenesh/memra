#!/usr/bin/env bash
# DAY38 section 13h's bisection in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash diag3.sh @COLLECTOR_LOCK_FD@`.
# Eight door-ON boots, x1 x1c x21 x2 x2 x21 x1c x1 (x1 = the tip binary; x1c = the same binary booted with the CUDA
# driver's CUDA_DEVICE_MAX_CONNECTIONS=32; x2 = bins/x2; x21 = the diag6 patch: the receipt stream at the highest stream
# priority), each stall_cell.py
...
sha256sum "$R"/bins/{tip,x2,x21}/memra-server > "$D0/binaries.sha256"
...
. tools/port-guard.sh
SERVER_PID=""
boot() { # $1 bin $2 log $3 extra env (or empty)
  memra_port_guard day38-diag6 "$PORT" MEMRA_GATE_PORT || return 1
  env CUDA_VISIBLE_DEVICES=0 MEMRA_COMPAT=openai "MEMRA_MODELS=gate=$MODEL" "MEMRA_ADDR=127.0.0.1:$PORT" \
    ...
    MEMRA_KV_HOST_CONTRACTS=1 $3 "$1" > "$2" 2>&1 &
  SERVER_PID=$!
...
trap finish EXIT
i=0
for arm in x1 x1c x21 x2 x2 x21 x1c x1; do
  i=$((i+1)); D=$D0/$(printf 'b%02d-%s' "$i" "$arm"); mkdir -p "$D"
  bin=$R/bins/$arm/memra-server; extra=""; [ $arm = x1 ] && bin=$R/bins/tip/memra-server
  [ $arm = x1c ] && { bin=$R/bins/tip/memra-server; extra="CUDA_DEVICE_MAX_CONNECTIONS=32"; }
  echo "arm=$arm bin=$(sha256sum "$bin" | cut -c1-16) extra=$extra" > "$D/BOOT.txt"
  if ! boot "$bin" "$D/server.log" "$extra"; then log "diag $arm boot NOT READY"; echo boot-failed >> "$D/BOOT.txt"; stop; continue; fi
  python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode demote --n 8 --server-log "$D/server.log" \
    --out "$D/demote" --tag "diag6-$arm" > "$D/demote.log" 2>&1
