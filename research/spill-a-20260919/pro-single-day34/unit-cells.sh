...
# H2D span cells), the engine's D2D, D2H span and day-32 and day-33 H2D span cells, the day-34 deferred-checksum and owner-hold cells (memra-engine lib), and the CPU cells (the hash, Hashing,
# span-order and day-31 censuses, the day-33 and day-34 censuses, the memra-tier D2H span, H2D span and deferred-checksum rules) on the target card
...
cargo test -p memra-server --lib -- hash_ hashing_ every_path_that_meets_a_hashing every_path_that_meets_a_demoting every_path_that_meets_a_promoting the_run_loop_waits_boundedly the_d2h_spans day31_ day33_ day34_ > "$R/unit/cargo-test-hash.log" 2>&1
rc3=$?
