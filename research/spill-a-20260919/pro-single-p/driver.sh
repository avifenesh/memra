#!/usr/bin/env bash
# The design-P target-card driver (DAY51.md section 1): waits for the build receipt, then, in one sitting, each under
# ONE collector hold: ab.sh demote ((b), (c)), ab.sh free ((f)), ab.sh promote ((d)), ab.sh chain ((g)), hump.sh ((e));
# gates.sh on the P binary ((a): the day-36 set, the fault gate default and plain, twin OFF and ON, the pause gate);
# hitgate.sh (its own flock, OFF then ON); unit-cells.sh ((a)); then day51-reading.py over the receipts. A busy
...
D=research/spill-a-20260919/pro-single-p
...
python3 research/spill-a-20260919/day51-reading.py "$R" > "$R/reading-day51.log" 2>&1
echo "$(date -u +%FT%TZ) reading rc=$? $(tail -1 "$R/reading-day51.log")" | tee -a "$R/progress.log"
