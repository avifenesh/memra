#!/usr/bin/env bash
# Design P's target-card A/B (DAY51.md section 1: base against p) for ONE cell in ONE collector hold:
...
# --n 5, o1 = base p x5, o2 = p base x5, door ON, the PRO environment of the S sittings, readiness bounded to 480 s;
# each boot's start temperature and SM clock in BOOT.txt, its server VmRSS and VmHWM before the stop in rss.txt; 250 ms
# telemetry; the layout day51-reading.py reads (<cell>/ab/o{1,2}/bNN-{base,p}). A compute app at the hold's start:
...
sha256sum "$R/bins/base/memra-server" "$R/bins/p/memra-server" > "$O/binaries.sha256"
idle=0
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard p-ab "$PORT" MEMRA_GATE_PORT || return 1
...
for order in o1 o2; do
  arms="base p base p base p base p base p"; [ $order = o2 ] && arms="p base p base p base p base p base"
  for arm in $arms; do
    ...
    python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode "$MODE" --n 5 --server-log "$D/server.log" \
      --out "$D/$MODE" --tag "p-$CELL-$arm" > "$D/$MODE.log" 2>&1
