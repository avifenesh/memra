#!/usr/bin/env bash
# Design T-H's hump reading for (d) (DAY65 section 1; S's cell, DAY42 section 1) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash hump.sh @COLLECTOR_LOCK_FD@`.
# Four door-ON boots xbase xth xth xbase (xbase = T-H's parent, xth = T-H's tip), each
...
sha256sum "$R/bins/base/memra-server" "$R/bins/th/memra-server" > "$D0/binaries.sha256"
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard th-hump "$PORT" MEMRA_GATE_PORT || return 1
...
trap finish EXIT
i=0
for arm in xbase xth xth xbase; do
  i=$((i+1)); D=$D0/$(printf 'b%02d-%s' "$i" "$arm"); mkdir -p "$D"
  bin=$R/bins/th/memra-server; [ $arm = xbase ] && bin=$R/bins/base/memra-server
