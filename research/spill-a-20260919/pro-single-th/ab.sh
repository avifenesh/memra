#!/usr/bin/env bash
# Design T-H's target-card A/B (DAY65.md section 1 (b) to (d): base against th) for ONE cell in ONE collector hold:
...
# --n 5, o1 = base th x5, o2 = th base x5, door ON, the PRO environment of the S sittings, readiness bounded to 480 s;
# each boot's start temperature and SM clock in BOOT.txt, its server VmRSS and VmHWM before the stop in rss.txt; 250 ms
# telemetry; the layout th-reading.py reads (<cell>/ab/o{1,2}/bNN-{base,th}). A compute app at the hold's start:
...
sha256sum "$R/bins/base/memra-server" "$R/bins/th/memra-server" > "$O/binaries.sha256"
idle=0
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard th-ab "$PORT" MEMRA_GATE_PORT || return 1
...
for order in o1 o2; do
  arms="base th base th base th base th base th"; [ $order = o2 ] && arms="th base th base th base th base th base"
  for arm in $arms; do
    ...
    python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode "$MODE" --n 5 --server-log "$D/server.log" \
      --out "$D/$MODE" --tag "th-$CELL-$arm" > "$D/$MODE.log" 2>&1
