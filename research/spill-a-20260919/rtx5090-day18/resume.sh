...
# usage: driver.sh <model.gguf> <server_bin> <evidence_dir>   (resume.sh: the same, re-trying lock-busy cells only)
set -uo pipefail
MODEL=$1; BIN=$2; EV=$3
cd "$(dirname "$0")/../../.."
mkdir -p "$EV"
...
run() { # name, then the command (env assignments allowed as leading words via env)
    ...
    # A lock-busy exit 2 (15 retries exhausted while another lane held the card) is NOT a run: the
    # resume tries the cell again; every other exit is kept.
    if [[ -f "$EV/$name.exit" ]] && [[ "$(cat "$EV/$name.exit")" != 2 ]]; then
        echo "$(date -u +%FT%TZ) $name already ran (rc=$(cat "$EV/$name.exit")), kept"; return 0
    fi
    if [[ -f "$EV/$name.exit" ]]; then
        echo "$(date -u +%FT%TZ) $name re-tried (earlier exit 2: lock busy or a refusal)"; rm -f "$EV/$name.exit"
    fi
