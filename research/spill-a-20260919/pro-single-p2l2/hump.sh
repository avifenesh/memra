#!/usr/bin/env bash
# Design P2's hump clause (e) (DAY52 section 1; S's cell, DAY42 section 1) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash hump.sh @COLLECTOR_LOCK_FD@`.
# Four door-ON boots xgpp xp2 xp2 xgpp (xgpp = G'' 358749c9f, the positive control; xp2 = P2's tip), each
...
sha256sum "$R/bins/gpp/memra-server" "$R/bins/p2/memra-server" > "$D0/binaries.sha256"
...
SERVER_PID=""
boot() { # $1 bin $2 log
  memra_port_guard p2-hump "$PORT" MEMRA_GATE_PORT || return 1
...
trap finish EXIT
i=0
for arm in xgpp xp2 xp2 xgpp; do
  i=$((i+1)); D=$D0/$(printf 'b%02d-%s' "$i" "$arm"); mkdir -p "$D"
  bin=$R/bins/p2/memra-server; [ $arm = xgpp ] && bin=$R/bins/gpp/memra-server
