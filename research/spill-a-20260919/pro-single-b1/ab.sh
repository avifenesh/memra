#!/usr/bin/env bash
# B1's paired cell (DAY59.md section 7, (b) to (d)) in ONE collector hold:
# `tier-battery.py --rig pro-single --external-lock --execute bash ab.sh @COLLECTOR_LOCK_FD@ <cell> <modeA> <modeB> <prefix_mb>`.
# Two binaries (base, b1) by two stall_cell.py modes: o1 = base-A b1-A base-B b1-B x5, o2 the reverse x5 (40 boots),
...
sha256sum "$R/bins/base/memra-server" "$R/bins/b1/memra-server" > "$O/binaries.sha256"
idle=0
...
SERVER_PID=""
boot() { # $1 log $2 bin
  memra_port_guard b1-ab "$PORT" MEMRA_GATE_PORT || return 1
...
for order in o1 o2; do
  seq1="base:$MA b1:$MA base:$MB b1:$MB"; [ $order = o2 ] && seq1="b1:$MB base:$MB b1:$MA base:$MA"
  ...
  for am in $arms; do
    ...
    python3 research/spill-a-20260919/stall_cell.py --port "$PORT" --mode "$mode" --n 5 --server-log "$D/server.log" \
      --out "$D/$mode" --tag "b1-$CELL-$arm-$mode" > "$D/$mode.log" 2>&1
