...
log() { echo "[$(date -u +%H:%M:%SZ)] $*" | tee -a "$R/logs/first-result-driver.log"; }
...
for ARM in on off; do
  ...
  curl -s $BASE/v1/chat/completions -H "Content-Type: application/json" -d \
    ...
    > "$R/logs/first-B128-$ARM.bg.json" &
  BGPID=$!
  sleep 2  # let the background request get past prefill into its burst loop
  OUT=$(python3 "$PROBE" --base $BASE --model q --label "nv-K3-B128-contended-yield$YIELD" \
        --out "$R/logs/points-first-result.jsonl" --n 3 --max-tokens 128)
