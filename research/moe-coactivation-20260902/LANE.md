...
1. `MEMRA_MOE_SEL_DUMP=<path>` (default OFF, `docs/FLAGS.md` §4 row; module
   ...
   and the `vrows_dev` / T=1 decode-graph device-table arm, read back with one DtoH per layer-call).
   ...
   device-routed TP walks refuse an armed dump by name; `MEMRA_GLM5_DECODE_GRAPH` declines capture
   (its MoE arm still engages eagerly and is recorded).
