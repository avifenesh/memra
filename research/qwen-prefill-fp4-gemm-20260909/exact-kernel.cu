// Research-only exact equal-scale fold candidate. No engine dispatch.
...
#include "../../crates/memra-engine/cu/mmq_common.cuh"
...
#pragma unroll
                for (int l = 0; l < tile_C::ne; ++l) {
                    const float s0 = dA[n][l / 2][k01 / 4 + 0];
                    const float s1 = dA[n][l / 2][k01 / 4 + 1];
                    // Positive equal E4M3 scales only: each INT32 K16 dot and the
                    // sum fit exactly in FP32 after multiplication by that scale.
                    // Preserve zero-sign behavior and every unequal-scale fold.
                    if (s0 > 0.0f && s0 == s1) {
                        const int dot = C[0].x[l] + C[1].x[l];
                        sum[(j0 / tile_C::J + n) * tile_C::ne + l] += dB[l % 2] * (dot * s0);
                    } else {
