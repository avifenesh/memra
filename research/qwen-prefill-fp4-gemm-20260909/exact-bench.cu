...
extern "C" int exact_gemm(const void*,const float*,float*,int,int,int,void*,void*,float,int);
...
Gemm funcs[]={base_gemm,exact_gemm};
int main(){
    void* test;CK(cudaMalloc(&test,4));CK(cudaMemset(test,0,4));CK(cudaDeviceSynchronize());CK(cudaFree(test));
    for(int shape=0;shape<2;++shape)for(int distribution=0;distribution<2;++distribution){
        ...
        for(size_t i=0;i<blocks;++i){
            ...
            for(int j=0;j<4;++j)w[blocks*32+i*4+j]=uint8_t(distribution ? 52 : 40+rng()%25);
        }
        ...
        for(int arm=0;arm<2;++arm){
            ...
            printf("{\"kind\":\"identity\",\"m\":%d,\"k\":%d,\"n\":%d,\"arm\":%d,\"distribution\":%d,\"elements\":%zu,\"bit_mismatch\":%zu,\"nonfinite\":%zu,\"max_abs\":%.9g,\"relative_l2\":%.9g}\n",m,k,n,arm,distribution,ref.size(),mismatch,bad,ma,sqrt(err2/ref2));
            if(bad)return 4;
        }
        for(int arm=0;arm<2;++arm) for(int rep=0;rep<3;++rep)run(arm);
        CK(cudaDeviceSynchronize());
        cudaEvent_t a,b;CK(cudaEventCreate(&a));CK(cudaEventCreate(&b));
        for(int round=0;round<6;++round)for(int pos=0;pos<2;++pos){
            int arm=round%2?1-pos:pos;
            ...
            printf("{\"kind\":\"timing\",\"m\":%d,\"k\":%d,\"n\":%d,\"arm\":%d,\"distribution\":%d,\"round\":%d,\"calls\":20,\"ms\":%.9g,\"input_tok_s\":%.9g,\"tops\":%.9g}\n",m,k,n,arm,distribution,round,ms,1024000./ms,2.*m*k*n/(ms*1e9));fflush(stdout);
        }
