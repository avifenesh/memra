...
O35B=/data/ai-ml/hf-models/kat-coder-v25-dev-gguf/Kwaipilot_KAT-Coder-V2.5-Dev-IQ4_XS.gguf
...
row() { # arm metric value rep
  printf '{"ts":"%s","git":"%s","cell":"kat-barcheck-kq","arm":"%s","metric":"%s","value":%s,"rep":%s,"profile":"%s","temp_c":%s}\n' \
...
memra_board() { # rep
  local rep=$1 log="$R/kbar-memra-board-rep$1.log"
  wait_idle
  ...
  local log2="$R/kbar-memra-gen512-rep$1.log"
  wait_idle
...
if m:
    for r in json.loads(m.group(0)):
        if r['n_prompt'] > 0 and r['n_gen'] == 0:
            ...
            continue
        print(json.dumps({"cell":"kat-barcheck-kq","arm":"llama","metric":met,"value":r['avg_ts'],"rep":int(rep),"temp_c":int(temp)}))
EOF
  tail -3 "$OUT" | sed 's/^/  [llama] /'
}

memra_class() { # class rep
  local cls=$1 rep=$2 log="$R/kbar-memra-$1-rep$2.log"
  wait_idle
...
if m:
    for r in json.loads(m.group(0)):
        if r['n_prompt'] > 0 and r['n_gen'] == 0:
            ...
            continue
        print(json.dumps({"cell":"kat-barcheck-kq","arm":"llama-class","metric":met,"value":r['avg_ts'],"rep":int(rep),"temp_c":int(temp)}))
EOF
  tail -4 "$OUT" | sed 's/^/  [llama-class] /'
}

echo "=== KAT BAR CHECK (iq4xs-mmq) x$N $TS git=$GIT_SHA profile=$PROFILE ===" | tee -a "$R/sweep-console.log"
{
