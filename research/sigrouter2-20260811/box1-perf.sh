#!/usr/bin/env bash
# One-lock, interleaved x5 comparison against increment 1 plus CUDA transfer receipts.
set -euo pipefail

REPO=${SIGROUTER2_REPO:-/home/ubuntu/memra-cx-sigrouter2}
OUT=${SIGROUTER2_PERF_OUT:-$REPO/research/sigrouter2-20260811/raw/box1-perf}
MODEL_ROOT=${SIGROUTER2_MODEL_ROOT:-/home/ubuntu/step37/models/step-3.7-flash}
MODEL=${SIGROUTER2_MODEL:-$MODEL_ROOT/IQ4_XS/Step-3.7-flash-IQ4_XS-00001-of-00003.gguf}
...
run_point() {
    ...
    python3 "$BENCH" \
        --base "$BASE" --model "$MODEL_NAME" --shape decode --label "$label" \
        --concurrency "$concurrency" --max-tokens 512 --require-length \
...
run_arm() {
    ...
    local -a policy=()
    [[ $arm == inc1 ]] && policy=(MEMRA_MOE_DEV=0)
    echo "arm=$label start=$(date -u +%FT%TZ)"
    {
        echo "arm=$arm"
        echo "MEMRA_SIG_ROUTER=<unset>"
        [[ $arm == default ]] && echo "MEMRA_MOE_DEV=<unset>" || echo "MEMRA_MOE_DEV=0"
        echo "MEMRA_SERVE_SPEC=0"
        echo "MEMRA_PP_STAGES=2"
        echo "MEMRA_PP_DEVICES=0,1"
        echo "MEMRA_PREFIX_CACHE_MB=0"
    } >"$OUT/${label}-env.txt"
    env -u MEMRA_SIG_ROUTER -u MEMRA_MOE_DEV -u MEMRA_SERVE_BATCH -u MEMRA_SPEC_K \
        -u MEMRA_BG_JOB -u MEMRA_SERVE_B1FAST "${policy[@]}" \
        CUDA_VISIBLE_DEVICES=0,1 MEMRA_MODELS="$MODEL_NAME=$MODEL" MEMRA_COMPAT=openai \
        MEMRA_ADDR="127.0.0.1:$PORT" MEMRA_SERVE_SPEC=0 MEMRA_PP_STAGES=2 \
        MEMRA_PP_DEVICES=0,1 MEMRA_CTX=262144 MEMRA_MOE_GROUPED=1 \
        MEMRA_PREFILL_TICK=2048 MEMRA_PREFIX_CACHE_MB=0 MEMRA_TAG="sigrouter2-$label" \
        "$SERVER" >"$log" 2>&1 &
    server_pid=$!
    wait_ready "$log"
    python3 "$BENCH" --base "$BASE" --model "$MODEL_NAME" --shape warmup \
...
run_trace() {
    ...
    local -a policy=()
    [[ $arm == inc1 ]] && policy=(MEMRA_MOE_DEV=0)
    local prefix="$OUT/nsys-$arm"
    local repfile="$OUT/nsys-$arm.nsys-rep"
    echo "trace=$arm start=$(date -u +%FT%TZ)"
    env -u MEMRA_SIG_ROUTER -u MEMRA_MOE_DEV "${policy[@]}" \
        ...
        MEMRA_NGEN=1 MEMRA_PROMPT_FILE="$PROMPT" \
        nsys profile --force-overwrite=true --trace=cuda,nvtx --sample=none \
            --cpuctxsw=none --output="$prefix" "$RUN_GEN" "$MODEL" \
