#!/usr/bin/env bash
# integ68 (lane F's items 17 and 18, MEMRA_MOE_COLD_BYPASS and MEMRA_KV_HOST_HANDOFF_IO, both default off, and the MoE cache dispatch through dispatch_source_once, payload and consumed; lane C days 75 to 77, I16 reverted, I17 flat; on main 6581fecfc), run on a rented RTX PRO 6000 Workstation box with the 35B at the rig path so serve-smoke's Q35 cold-prefill arm runs; the lock is the pair lock /tmp/memra-gpu.lock, the model the same 9B the 5090 battery uses
...
# promote-span-refusal cell), all under ONE collector hold of /tmp/memra-gpu.lock (75 x `flock -w 120`: sized to outlast lane B day 34's first order, then take the card between its orders), then an idle
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG68-PRO-DONE" | tee -a "$ROOT/battery.log"
