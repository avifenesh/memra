#!/usr/bin/env bash
# integ61 (C days 59 to 62: MEMRA_MOE_PREFETCH=1 decided on the target card, the door's gap attributed, I11 and I12; on main 7bf457384), run on a rented RTX PRO 6000 Workstation box because the local RTX 5090 needs a reset (Xid 119 and 154 at 01:25Z); the lock is the pair lock /tmp/memra-gpu.lock, the model the same 9B the 5090 battery uses
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG61-PRO-DONE" | tee -a "$ROOT/battery.log"
