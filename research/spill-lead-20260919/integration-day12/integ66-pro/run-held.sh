#!/usr/bin/env bash
# integ66 (lane F's M1 build-out through BOX27, ef5640af8; lane C days 63 to 74, d36857771: the MoE slot cache door's I13 to I15 inside the door's banked path, the run-gen CPU probes, the legacy prefetch unchanged; on main a233f6fe5), run on a rented RTX PRO 6000 Workstation box; the lock is the pair lock /tmp/memra-gpu.lock, the model the same 9B the 5090 battery uses
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG66-PRO-DONE" | tee -a "$ROOT/battery.log"
