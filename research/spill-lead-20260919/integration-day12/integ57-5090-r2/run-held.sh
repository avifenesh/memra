#!/usr/bin/env bash
# integ57 (A day 35: F kept, M refuted and reverted, M' the bind re-hash on the helper; on main e20a4c5bb, plus the lease-guard seq fix from review) on the
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG57-5090-R2-DONE" | tee -a "$ROOT/battery.log"
