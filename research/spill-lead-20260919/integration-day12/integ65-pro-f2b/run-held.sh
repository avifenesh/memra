#!/usr/bin/env bash
# integ65 (A days 56 to 58: item 23 the isolated shed tests, item 24 the stop-mode wiring test, item 25 the route book's end ordering in route_telemetry.rs; on main 4b24740f4), run on a rented RTX PRO 6000 Workstation box so the local RTX 5090 stays with the lanes' timed queues; the lock is the pair lock /tmp/memra-gpu.lock, the model the same 9B the 5090 battery uses Rerun on the tree with lane A's F2b (the admission counters pair), same cells.
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG65-F2B-PRO-DONE" | tee -a "$ROOT/battery.log"
