#!/usr/bin/env bash
# integ47 (A day 30 + C day 38 on main 9717e8d57) on the local RTX 5090 Laptop GPU: serve-smoke, the engine
# d2d_* and d2h_span native cells, the worker option_b_span* native cells, then the five gates of A's
# rerun-held.sh (research/spill-a-20260919/rtx5090-day30-merge/), all under ONE collector hold of
# /tmp/memra-5090.lock (15 x `flock -w 120`), then an idle check under the hold (no compute app, >= 20000 MiB
# free, 15 x 60 s). Either bound running out records every cell NOT RUN. Gates take the held FD via
# `--external-lock 9`. No gate, cell or threshold changed from A's script. Executed-not-qualified.
...
run worker-span-cells "" cargo test -p memra-server --lib --offline -- --ignored --test-threads=1 option_b_span
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG47-5090-DONE" | tee -a "$ROOT/battery.log"
