#!/usr/bin/env bash
# integ64 (A days 52 to 55: item 22 closed, T-a the single clock read in health.rs, T-b, T-c and T-e the starvation-proof tests; F1 the ordered admission test locks; the publication split and on-tick lines, log only; on main 968c0fa68), run on a rented RTX PRO 6000 Workstation box so the local RTX 5090 stays with the lanes' timed queues; the lock is the pair lock /tmp/memra-gpu.lock, the model the same 9B the 5090 battery uses
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG64-PRO-DONE" | tee -a "$ROOT/battery.log"
