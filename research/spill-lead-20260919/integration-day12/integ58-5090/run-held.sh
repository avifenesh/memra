#!/usr/bin/env bash
# integ58 (A day 36: the D2D restore price cell closes, M' on the target card, the log-only restore instrument; on main 1d0cf13bc) on the
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG58-5090-DONE" | tee -a "$ROOT/battery.log"
