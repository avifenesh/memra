#!/usr/bin/env bash
# integ53 (B day 33, the memra#680 fix under MEMRA_ADMIT_BY_MEMORY, on main 0afd88e1d at 9f335ac48) on the local
# RTX 5090 Laptop GPU: integ52's cells unchanged, plus tools/admit-mem-burst-gate.sh (the #680 gate) and
# tools/spec-ctx-edge-gate.sh (the #668 gate, since this change sits beside the door's budget) after them; serve-smoke, the engine d2d_*, d2h_span* and h2d_span* native cells, the worker option_b_* and option_c_*
...
run engine-gpu-cells "" cargo test -p memra-engine --lib --offline -- --ignored --test-threads=1 d2d_ d2h_span h2d_span
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG53-5090-DONE" | tee -a "$ROOT/battery.log"
