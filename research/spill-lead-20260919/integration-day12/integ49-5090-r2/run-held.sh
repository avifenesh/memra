#!/usr/bin/env bash
# integ49 r2 (A day 31 merged with main f69119ae0 at d1760ac21) on the local RTX 5090 Laptop GPU: the first run's
# cells unchanged (serve-smoke, the engine d2d_* and d2h_span native cells, the worker option_b_* and option_c_*
# native cells, the five gates), all under ONE collector hold of /tmp/memra-5090.lock (15 x `flock -w 120`), then
# an idle check under the hold (no memra-server compute app, >= 20000 MiB free, 15 x 60 s; a foreign process is
# recorded, not waited out: every cell here is a correctness verdict, none reads timing). Either bound running
# out records every cell NOT RUN. Gates take the held FD via `--external-lock 9`. Two changes from the first run,
# both in the harness: the binary is hashed again after serve-smoke's unconditional build, and the idle check
# above. No gate, cell or threshold changed. Executed-not-qualified.
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG49-5090-R2-DONE" | tee -a "$ROOT/battery.log"
