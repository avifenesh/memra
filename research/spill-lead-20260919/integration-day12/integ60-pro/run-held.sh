#!/usr/bin/env bash
# integ60 (C days 40 to 58: the MoE slot cache door tuned (I1 to I10, I8f, I5f), verify digest v3, the DFlash tail class, two partial-restore fixes; on main 5d653e851), run on a rented RTX PRO 6000 Workstation box because the local RTX 5090 needs a reset (Xid 119 and 154 at 01:25Z); the lock is the pair lock /tmp/memra-gpu.lock, the model the same 9B the 5090 battery uses
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG60-PRO-DONE" | tee -a "$ROOT/battery.log"
