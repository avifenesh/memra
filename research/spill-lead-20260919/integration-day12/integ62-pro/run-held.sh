#!/usr/bin/env bash
# integ62 (A days 42 to 48: S4 the span receipts, V the pause sweep's demotes off the tick, items 3 and 15 closed; on main d6515742f), run on a rented RTX PRO 6000 Workstation box because the local RTX 5090 needs a reset (Xid 119 and 154 at 01:25Z); the lock is the pair lock /tmp/memra-gpu.lock, the model the same 9B the 5090 battery uses
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG62-PRO-DONE" | tee -a "$ROOT/battery.log"
