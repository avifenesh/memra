...
cd /root/wt-integ62
{ cargo build --release -p memra-server; echo build_rc=$?; } > /root/integ62-pro/build.log 2>&1
grep -q build_rc=0 /root/integ62-pro/build.log || { echo BUILD-FAILED >> /root/integ62-pro/build.log; exit 1; }
bash research/spill-lead-20260919/integration-day12/integ62-pro/run-held.sh /root/integ62-pro /root/models/Qwen3.5-9B-NVFP4-MTP-GGUF.gguf /root/wt-integ62/target/release/memra-server > /root/integ62-pro/driver.out 2>&1
# The pause-demote gate (A day 47, design V) needs a tool-calling model: the 27B, as lane A ran it.
P=/root/integ62-pro/pause-demote; mkdir -p $P/ev
...
flock -u 9
echo DRIVE-DONE >> /root/integ62-pro/driver.out
