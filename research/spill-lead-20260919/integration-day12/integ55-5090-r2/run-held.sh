#!/usr/bin/env bash
# integ55 (B day 35, the prefix seed booked under MEMRA_ADMIT_BY_MEMORY, on main 55679db26 at 34a4b7f23, the seed cap from review) on the
# local RTX 5090 Laptop GPU: integ54's cells unchanged (the admit-mem-burst gate is this change's gate); serve-smoke, the engine d2d_*, d2h_span* and h2d_span* native cells, the worker option_b_* and option_c_*
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG55-5090-R2-DONE" | tee -a "$ROOT/battery.log"
