#!/usr/bin/env bash
# integ67 (lane A days 59 and 66 records with design B1, the prefix snapshot's and restore's per-plane copies as one launch, adopted on the target card, plus its gridDim.y guard; on main 649fe632a), run on a rented RTX PRO 6000 Workstation box; the lock is the pair lock /tmp/memra-gpu.lock, the model the same 9B the 5090 battery uses Rerun on the tree with main 8f93ccd99 (#752) merged, same cells.
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG67-MAIN752-PRO-DONE" | tee -a "$ROOT/battery.log"
