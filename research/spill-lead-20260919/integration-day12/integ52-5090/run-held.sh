#!/usr/bin/env bash
# integ52 (A day 32, the H2D half, merged with main 649d96210 as a fast-forward at e009df6d3) on the local RTX 5090
# Laptop GPU: serve-smoke, the engine d2d_*, d2h_span* and h2d_span* native cells, the worker option_b_* and option_c_*
# native cells (serial: A's day-32 finding 3), then the five gates (the fault gate now carries day 32's
# promote-span-refusal cell), all under ONE collector hold of /tmp/memra-5090.lock (15 x `flock -w 120`), then an idle
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG52-5090-DONE" | tee -a "$ROOT/battery.log"
