#!/usr/bin/env bash
# integ63 (A days 49 to 55 records with P reverted, F's OWED 14 collector label bound to the NVMe proof; on main 5228ff0cd), run on a rented RTX PRO 6000 Workstation box already staged for lane C's sitting, so the local RTX 5090 (back after its reboot) stays free for the lanes' queued cells; the lock is the pair lock /tmp/memra-gpu.lock, the model the same 9B the 5090 battery uses
...
flock -u 9
echo "$(date -u +%FT%TZ) collector hold released; INTEG63-PRO-DONE" | tee -a "$ROOT/battery.log"
