...
1. ~~A greedy tape on the real artifact for `=2`~~ — **DONE 2026-09-03, GREEN.** The 128-token
   greedy digits tape on the real artifact is **sha16 `9437b599f6b9d2a9` for door off, `=1` and
   `=2` alike**, and the same value every other posture produces today; the `=2` run's server log
   shows `attn_gathered t=1 arm=warp-online chunks=16 class=dsa-warp-online-f32` at 43.0 tok/s.
   Two independent admission bars now hold for the named class: the argmax gate says the kernel
   does not move a latent-row argmax at t_q=1, and the tape says the class does not move a TOKEN
   on the real checkpoint. Neither substitutes for the other - the argmax gate covers shapes the
   tape never visits, and the tape covers the model the gate's synthetic shapes only approximate.
