...
mode=${1:-}
if [[ $mode != control && $mode != prequeue ]]; then
    print -u2 -r -- "usage: $0 control|prequeue"
    exit 2
fi

evidence_dir=/home/avifenesh/.codex/worktrees/e25d/bw24/research/per-expert-quant/evidence/local-5090-sota-20260719
log_path=$evidence_dir/hy3-mtp-k1-capped-${mode}-ram20-warm32-ngen16-run1.log
scope_name=bw24-k1-${mode}-$$
scope_unit=${scope_name}.scope
typeset -a candidate_env
candidate_env=()
if [[ $mode == prequeue ]]; then
    candidate_env+=(BW24_CPU_EXPERT_VERIFY_PREQUEUE=1)
fi
