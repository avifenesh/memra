...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (dir, inp, n, out) = (
        a.next()
            .expect("usage: tok-trim <hf_dir> <in.txt> <n_tokens> <out.txt>"),
    ...
    assert!(
        total >= n,
        "input encodes to {total} < requested {n} tokens"
    );
    ...
    while lo + 1 < hi {
        let mid = (lo + hi) / 2;
        if tok.encode(&text[..idx[mid]], true).len() <= n {
            lo = mid
        } else {
            hi = mid
        }
    ...
    let mut cut = lo;
    while cut > 0 && tok.encode(&text[..idx[cut]], true).len() > n {
        cut -= 1;
    }
    let got = tok.encode(&text[..idx[cut]], true).len();
    assert_eq!(
        got, n,
        "could not hit exactly {n} tokens (closest prefix = {got})"
    );
