...
pub struct Cache {
    ...
    /// glm5 TP-2 (`MEMRA_GLM5_TP`) per-layer, per-rank KDA state planes: `[root, peer]`
    /// shard-geometry conv ring + ssm ping-pong, lazily hydrated by the engine's TP walk on
    /// first touch (the kpool-plane precedent). The canonical `recur[il]` planes stay
    /// allocated untouched (full-width; never read by the TP walk). `None` everywhere the
    /// seam is off. Rollback/checkpoint seams REFUSE while any slot is live — spec is
    /// co-refused with the TP door, so nothing reaches them in v1.
    pub glm5_tp_recur: Vec<Option<[RecurLayer; 2]>>,
    /// glm5 TP-2 PEER replica of the MLA latent+indexer plane (replicated deterministic
    /// compute: both ranks append identical bytes in the same calls). The canonical
    /// `latent[il]` IS the root replica. Lazily hydrated like the field above.
    pub glm5_tp_latent_peer: Vec<Option<LatentKvLayer>>,
    pub pos: usize,
    pub max_ctx: usize,
