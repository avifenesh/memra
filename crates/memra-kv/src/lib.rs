...
/// Rows of slack the DSA indexer TAIL RING carries on top of the nominal prime chunk.
///
/// The nominal chunk is not the true per-call `t`: `fixed_prime_chunk_ranges` merges a runt tail
/// into its predecessor (up to `PRIME_MIN_T` = 16 extra tokens) and `align_prime_ranges_to_gdn`
/// moves internal boundaries DOWN, growing the final range by up to `gdn_chunk_size() - 1` = 31.
/// 1024 rows covers both with three orders of magnitude of margin and costs 1 MiB per layer
/// against the 1.07 GiB per layer this ring exists to delete — headroom is free at this ratio,
/// and `mla_kpool_indices` still asserts the real bound per call rather than trusting it.
pub const INDEX_RING_HEADROOM_ROWS: usize = 1024;

/// Upper bound on the tokens ONE forward call can append, given a parsed `MEMRA_PRIME_CHUNK`.
///
/// `Some(0)` is monolithic prime — one call carries the whole prompt, so the bound is the whole
/// context. An explicit smaller chunk cannot RAISE the bound above the built-in cap, because
/// decode/spec/MTP widths and the tail-merge rule are bounded by the cap either way; an explicit
/// LARGER chunk is authoritative when the SWA ring door is shut (`prime_chunk_tokens` returns it
/// unclamped), so it becomes the bound.
pub fn prime_chunk_bound(parsed: Option<usize>, max_ctx: usize) -> usize {
    match parsed {
        Some(0) => max_ctx,
        Some(n) => n.max(PRIME_CHUNK_MAX_TOKENS),
        None => PRIME_CHUNK_MAX_TOKENS,
    }
}
...
/// row budget (gates use a tiny one to reach the wrap in a micro fixture), `None` derives it from
/// the per-call token bound. A ring that is not SHORTER than the flat plane is pointless, so
...
pub fn index_ring_rows_for(
    ...
    let rows = match explicit {
        Some(0) => return None,
        Some(n) => n,
        None => chunk_bound.checked_add(INDEX_RING_HEADROOM_ROWS)?,
    };
    (rows > 0 && rows < max_ctx).then_some(rows)
}

/// DSA k-pool indexer TAIL RING sizing (`MEMRA_DSA_INDEX_RING`, default ON — see docs/FLAGS.md).
/// Unparseable values are treated as unset, exactly as `prime_chunk_tokens` treats an
/// unparseable `MEMRA_PRIME_CHUNK`.
pub fn index_ring_rows(max_ctx: usize) -> Option<usize> {
    let explicit = std::env::var("MEMRA_DSA_INDEX_RING")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok());
    let parsed = std::env::var("MEMRA_PRIME_CHUNK")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok());
    index_ring_rows_for(explicit, prime_chunk_bound(parsed, max_ctx), max_ctx)
}
...
pub struct LatentKvLayer {
    ...
    ///     lapped row. The plane only has to hold the incomplete tail plus the current call's
    ///     tokens, so a ring of `R` rows with `R` a multiple of `pool` (which keeps each pool
    ///     contiguous mod `R`) and `R >= pool - 1 + max_chunk_t` replaces 12 GiB with 60 MiB,
    ///     EXACTLY — same rows, same kernel, different addresses, zero numeric cost, gated by
    ...
    ///
    /// TWO inputs the ring needs that the residency change lacked, and only one of them was the
    /// documented one: a per-call `t` bound (supplied by `prime_chunk_bound`, i.e. the same
    /// `PRIME_CHUNK_MAX_TOKENS` the SWA ring already sizes against) AND the indexer's `pool`,
    /// which the state plan does NOT carry — the same gap that makes `index_pool_keys` a lazy
    /// allocation below. `pool` is therefore NOT used to size the ring: the allocator books
    /// `index_ring_rows` PHYSICAL rows and the engine rounds that DOWN to a multiple of `pool` on
    /// first use, so the effective ring is always `>= rows - pool + 1`.
...
mod tp_transaction_tests {
    use super::{
        Cache, INDEX_RING_HEADROOM_ROWS, KvRingAppend, PRIME_CHUNK_MAX_TOKENS, ResidentTpKvCache,
        TpKvTransactionState, index_ring_rows_for, prime_chunk_bound, tp_kv_rank_allocation_shape,
    };
    ...
    #[test]
    fn index_ring_sizing_is_pure_and_bounded_by_the_real_per_call_t() {
        // Default derivation: the built-in prime cap plus headroom, engaged only when it is
        // actually SHORTER than the flat plane it replaces.
        let rows = PRIME_CHUNK_MAX_TOKENS + INDEX_RING_HEADROOM_ROWS;
        assert_eq!(prime_chunk_bound(None, 1 << 20), PRIME_CHUNK_MAX_TOKENS);
        assert_eq!(
            index_ring_rows_for(None, prime_chunk_bound(None, 1 << 20), 1 << 20),
            Some(rows)
        );
        ...
        assert_eq!(
            index_ring_rows_for(None, PRIME_CHUNK_MAX_TOKENS, 4096),
            None
        );
        assert_eq!(
            index_ring_rows_for(None, PRIME_CHUNK_MAX_TOKENS, rows),
            None
        );
        assert_eq!(
            index_ring_rows_for(None, PRIME_CHUNK_MAX_TOKENS, rows + 1),
            Some(rows)
        );

        // MEMRA_PRIME_CHUNK=0 is MONOLITHIC prime — one call carries the whole prompt, so the
        // bound is the context and the ring disengages instead of failing at run time.
        assert_eq!(prime_chunk_bound(Some(0), 1 << 20), 1 << 20);
        assert_eq!(index_ring_rows_for(None, 1 << 20, 1 << 20), None);
        // A SMALLER explicit chunk cannot lower the bound (decode/spec/MTP widths and the
        // tail-merge rule live under the cap either way); a LARGER one raises it, because
        // `prime_chunk_tokens` returns it unclamped with the SWA-ring door shut.
        assert_eq!(
            prime_chunk_bound(Some(256), 1 << 20),
            PRIME_CHUNK_MAX_TOKENS
        );
        assert_eq!(prime_chunk_bound(Some(65_536), 1 << 20), 65_536);
        assert_eq!(
            index_ring_rows_for(None, prime_chunk_bound(Some(65_536), 1 << 20), 1 << 20),
            Some(65_536 + INDEX_RING_HEADROOM_ROWS)
        );

        // The knob: 0 is the rollback seam, n pins the row budget (how the wraparound gate
        // reaches a wrap in a micro fixture).
        assert_eq!(
            index_ring_rows_for(Some(0), PRIME_CHUNK_MAX_TOKENS, 1 << 20),
            None
        );
        assert_eq!(
            index_ring_rows_for(Some(16), PRIME_CHUNK_MAX_TOKENS, 64),
            Some(16)
        );
        assert_eq!(
            index_ring_rows_for(Some(64), PRIME_CHUNK_MAX_TOKENS, 64),
            None
        );
    }
