...
impl CandidateProbe {
    pub fn new(
        ...
        if present.iter().filter(|&&x| !x).count() < 32 {
            return Err("bounded probe needs 32 outside rows".into());
        }
        let mut probe = Self {
    ...
    pub fn observe(&mut self, committed: &[u32]) {
        for &id in committed {
            ...
            if let Some(i) = self.recent.iter().position(|&x| x == id) {
    ...
    pub fn prepare(
        ...
        while ids.len() < 32 {
            ...
            if !self.present[id as usize] && !ids.contains(&id) {
                ids.push(id);
            }
        }
        let mut bytes = Vec::with_capacity(32 * stride);
        for &id in &ids {
            bytes.extend_from_slice(&source[id as usize * stride..(id as usize + 1) * stride]);
        }
        let head = GpuTensor::Quant {
            bytes: e.htod_bytes(&bytes)?,
            qtype: crate::QT_Q8_0,
            row_bytes: stride,
            ne: vec![width as u64, 32],
            scale: 1.0,
            rp: false,
            #[cfg(memra_cutlass)]
            cutlass: None,
    ...
    pub fn record(
        ...
        writeln!(
            self.file,
            "{{\"schema\":1,\"kind\":\"bounded_candidates\",\"round\":{round},\"position\":{position},\"context\":{context},\"vocab\":{vocab},\"history_rows\":{history_rows},\"proposal\":{proposal},\"target\":{target},\"candidates\":[{rows}]}}"
        )?;
        self.states += 1;
        Ok(())
    }
