...
impl Dsv4Gpu {
    ...
    pub fn load(
        ...
        let paired_issue = match std::env::var("MEMRA_DSV4_PAIRED_ISSUE").as_deref() {
            Err(std::env::VarError::NotPresent) | Ok("0") => false,
            Ok("1") => true,
            _ => return Err("MEMRA_DSV4_PAIRED_ISSUE requires 0 or 1".into()),
        };
        if paired_issue && (!topology.is_tp_ep() || attention_tp.is_none()) {
            return Err("paired issue requires all-layer TP/EP with attention TP2".into());
        }
        let mut me = Dsv4Gpu {
            topology,
            attention_tp,
            ...
            small_kernel_diet,
            paired_issue,
            paired_issue_stages: AtomicU64::new(0),
    ...
    fn decode_step_tp_ep(
        ...
        let mut result = (|| -> Res<(Option<Vec<f32>>, u32)> {
            ...
            for (il, rank1_cache) in rank1_caches.iter_mut().enumerate() {
                for issue_stage in 0..if self.paired_issue { 5 } else { 1 } {
                    for rank in 0..2usize {
                        let st = &self.stages[rank];
                        let layer = st
                            .layers
                        ...
                        st.gpu
                            .ctx
                            .bind_to_thread()
                            .map_err(e("TP/EP attention stage bind"))?;
                        if self.attention_tp.is_some() {
                            self.attention_verify_stage_dev(
                                st,
                                layer,
                                cache,
                                lck,
                                ...
                                1,
                                false,
                                self.paired_issue.then_some(issue_stage),
                            )?;
                            if !self.paired_issue || issue_stage == 4 {
                        ...
                        if self.paired_issue {
                            self.paired_issue_stages.fetch_add(1, Ordering::Relaxed);
                        }
                        if !self.paired_issue || issue_stage == 4 {
                ...
                if self.attention_tp.is_some() {
                    ...
                    for issue_stage in 0..if self.paired_issue { 4 } else { 1 } {
                        for (rank, workspace) in [(0usize, &mut *owner_ws), (1usize, &mut *peer_ws)]
                        {
                            ...
                            st.gpu
                                .ctx
                                .bind_to_thread()
                                .map_err(e("TP/EP MoE stage bind"))?;
                            self.post_attention_moe_verify_stage_dev(
                                st,
                                layer,
                                workspace,
                                false,
...
impl Dsv4Gpu {
    pub fn paired_issue_enabled(&self) -> bool {
        self.paired_issue
    }

    pub fn paired_issue_stages(&self) -> u64 {
        self.paired_issue_stages.load(Ordering::Relaxed)
    }

    /// Exclusive borrow keeps the gate arm fixed for the complete request walk.
    pub fn set_paired_issue_for_gate(&mut self, enabled: bool) -> Res<()> {
        if !self.topology.is_tp_ep() || self.attention_tp.is_none() {
            return Err("paired issue requires all-layer TP/EP with attention TP2".into());
        }
        self.paired_issue = enabled;
        Ok(())
    }
...
impl Dsv4Gpu {
    ...
    fn attention_verify_dev(
        ...
        self.attention_verify_stage_dev(
            st, layer, cache, lck, vws, input_rx, pos0, t, host_math, None,
        )
    }

    // The five independent rank-local stages preserve the original stream order.
    #[allow(clippy::too_many_arguments)]
    fn attention_verify_stage_dev(
        &self,
        st: &Stage,
        layer: &LayerDev,
        cache: &mut LayerCache,
        ...
        if issue_stage.is_none_or(|stage| stage == 3) {
                                ...
                                ck(
                                    ...
                                    self.indexer_score_arm(
                                        (vws.qi.device_ptr(&stream).0 as usize
                                            ...
                                            as *const f32,
                                        dpf!(ikvc.as_ref().expect("ikvc"), &stream),
                                        (vws.wproj.device_ptr(&stream).0 as usize
                                            + i * ix.heads * 4)
                                            as *const f32,
                                        wscale,
        ...
        if issue_stage.is_none_or(|stage| stage == 4) {
    ...
    #[allow(clippy::too_many_arguments)]
    fn post_attention_moe_verify_stage_dev(
        &self,
        st: &Stage,
        layer: &LayerDev,
        vws: &mut VerifyWs,
        ...
        if issue_stage != Some(0) {
            self.moe_verify_stage_dev(
                st,
                layer,
                vws,
                t,
        ...
        if let Some(ep) = &layer.ep {
            ...
            if self.matrix_moe {
                ...
                if self.topology.is_tp_ep() {
                    ...
                    let calls = crate::dsv4_ep::execute_matrix_local_stage(
                        &st.gpu,
                        ep,
                        layer
                            .experts_modelopt_table
