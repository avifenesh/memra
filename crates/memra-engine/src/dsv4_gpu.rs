...
pub fn dense_tile_counts_for_gate() -> [u64; 2] {
    unsafe extern "C" {
        fn memra_dsv4_dense_tile_counts_for_gate(
            decompositions: *mut u64,
            launches: *mut u64,
        ) -> i32;
    }
...
impl LayerDev {
    ...
    fn expert_ptrs(
        ...
        let (weights, scales, local) = match &self.ep {
            Some(ep) if ep.local_only => {
                if !(ep.local_first..ep.local_first + ep.count).contains(&expert) {
                    return Err("local-only TP/EP bank cannot address a peer expert".into());
                }
                (&self.experts_w, &self.experts_sc, expert - ep.local_first)
            }
...
/// Cached pinned host words for one async D2H readback. Portable, so every stage context
...
unsafe impl Send for PinnedWords {}
unsafe impl Sync for PinnedWords {}

impl PinnedWords {
    fn new(len: usize) -> Res<Self> {
        let bytes = len.max(1) * std::mem::size_of::<i32>();
        ...
        let ptr = unsafe { cudarc::driver::result::malloc_host(bytes, 1) }
            ...
            .cast::<i32>();
        unsafe { ptr.write_bytes(0, len.max(1)) };
        Ok(Self { ptr, len })
    }

    /// # Safety
    /// `src` must stay alive and unwritten until an event recorded on `stream` after this
    /// call has completed, and nothing may read these words before that.
    unsafe fn enqueue_from(
        ...
        if n == 0 || n > self.len || n > src.len() {
            return Err(format!(
                "pinned readback of {n} words outside {} / {}",
                self.len,
                src.len()
            ));
        }
...
impl Dsv4Gpu {
            ...
            && (self.prefill_grouped
                ...
                || self.stages.iter().flat_map(|s| &s.layers).any(|l| {
                    l.expert_kind != ExpertKind::Nvfp4
                        ...
                        || (self.ep_enabled
                            && l.ep.as_ref().is_none_or(|ep| {
                                ep.peer_table.is_none()
                                    || (self.topology.is_tp_ep() && !ep.local_only)
                            }))
                }))
        {
    ...
    fn load_layer(&mut self, stage: usize, il: u32, prefix: &str) -> Res<LayerDev> {
        self.load_layer_partitioned(stage, il, prefix, None)
    }

    /// Load a layer while retaining only one expert-ID partition when the
    /// all-layer TP/EP topology is active.  Attention and routing metadata are
    /// still replicated; the large quantized expert bytes are not.
    fn load_layer_partitioned(
        &mut self,
        stage: usize,
        il: u32,
        prefix: &str,
        expert_partition: Option<(usize, usize)>,
    ) -> Res<LayerDev> {
        ...
        let hidden = self.model.mc.n_embd as usize;
        let (expert_first, resident_experts) = expert_partition.unwrap_or((0, ne));
        if resident_experts == 0 || expert_first >= ne || resident_experts > ne - expert_first {
            return Err(format!(
                "{p}: invalid resident expert partition {expert_first}+{resident_experts} of {ne}"
            ));
        }
        ...
        let mut experts_w = stream
            .alloc_zeros::<u8>(resident_experts * 3 * wbytes)
            .map_err(e("alloc expert slab"))?;
        let mut experts_sc = stream
            .alloc_zeros::<u8>(resident_experts * 3 * sbytes)
        ...
        for ex in 0..ne {
            for (pi, pname) in ["w1", "w2", "w3"].iter().enumerate() {
                ...
                if (expert_first..expert_first + resident_experts).contains(&ex) {
                    let local_ex = ex - expert_first;
                    let off = (local_ex * 3 + pi) * wbytes;
                    let mut view = experts_w.slice_mut(off..off + wbytes);
                    stream
                        .memcpy_htod(wb, &mut view)
                        .map_err(e("htod expert w"))?;
                    let soff = (local_ex * 3 + pi) * sbytes;
        ...
        self.stages[stage].loaded_bytes +=
            (resident_experts * 3 * (wbytes + sbytes)) as u64 + (ne * 3 * 4) as u64;
    ...
    fn enable_tp_ep_local_banks_for_gate(&mut self) -> Res<()> {
        if self.ep_enabled {
            return Ok(());
        }
        if !self.topology.is_tp_ep()
        ...
        {
            return Err(
                "TP/EP local banks require replicated matrix layers, native device math, and device routing with reused storage".into(),
            );
        }
        let moe = self.model.mc.moe.as_ref().expect("moe");
        let ne = moe.expert_count as usize;
        if ne == 0 || !ne.is_multiple_of(2) {
            return Err("TP/EP local banks require an even expert count".into());
        }
        let count = ne / 2;
        let hidden = self.model.mc.n_embd as usize;
        let inter = moe.expert_ff_length as usize;
        ...
        for owner_index in 0..2usize {
            let stage = &mut self.stages[owner_index];
            stage
                .gpu
                .ctx
                .bind_to_thread()
                .map_err(e("bind TP/EP local bank"))?;
            let stream = stage.gpu.stream();
            let mut table_bytes = 0u64;
            for layer in &mut stage.layers {
                if layer.experts_w.len() != count * 3 * wbytes
                    || layer.experts_sc.len() != count * 3 * sbytes
                    || layer.experts_modelopt_table.is_none()
                {
                    return Err(format!(
                        "TP/EP layer {} was not loaded as one local expert partition",
                        layer.il
                    ));
                }
                let peer_table = crate::dsv4_grouped::modelopt_table(
                    &stream,
                    &layer.experts_w,
                    &layer.experts_sc,
                    count,
                ...
                table_bytes += (peer_table.len() * 8) as u64;
                layer.ep = Some(crate::dsv4_ep::EpLayer {
                    peer_stage: 1 - owner_index,
                    local_first: owner_index * count,
                    count,
                    peer_first: (1 - owner_index) * count,
                    peer_w: stream
                        .alloc_zeros::<u8>(0)
                        .map_err(e("TP/EP empty peer weights"))?,
                    peer_sc: stream
                        .alloc_zeros::<u8>(0)
                        .map_err(e("TP/EP empty peer scales"))?,
                    peer_s2: stream
                        .alloc_zeros::<f32>(0)
                        .map_err(e("TP/EP empty peer scales2"))?,
                    peer_table: Some(peer_table),
                    local_only: true,
                });
            }
            stage.loaded_bytes += table_bytes;
        }
        for stage in &self.stages {
            stage
                .gpu
    ...
    fn arm_full_token_replay_inner(
        ...
        if work.failed || work.verify.open.is_some() || work.replay.is_some() {
            return Err("replay requires a closed, unarmed request".into());
        }
        for (rank, ws) in work.verify.ws.iter_mut().enumerate() {
            if self.stages[rank].gpu.ctx.is_event_tracking()
                || ws.is_prefill
                || work.verify.tmax != 1
            {
                return Err(
                    "replay requires t=1 decode workspace allocated without event tracking".into(),
                );
            }
        }
        let pair = crate::dsv4_graph::ReplayPair::new(
    ...
    pub fn full_token_replay_counts_for_gate(&self, state: &DecodeState) -> Res<[[u64; 2]; 2]> {
        state
            .matrix_step
            .as_ref()
    ...
    pub fn full_token_replay_variant_census_for_gate(
        &self,
        state: &DecodeState,
    ) -> Res<[[[u64; 7]; 4]; 2]> {
        Ok(state
    ...
    pub fn full_token_ar_epochs_for_gate(&self) -> Res<[Vec<u32>; 2]> {
        ...
        self.tp_ep_ar
            ...
            .as_ref()
            .ok_or("TP/EP AR missing")?
            .epochs_for_gate(&self.stages[0].gpu, &self.stages[1].gpu)
    }

    pub fn full_token_replay_census_for_gate(
        &self,
        state: &DecodeState,
    ) -> Res<[[[u64; 7]; 2]; 2]> {
        Ok(state
    ...
    pub fn full_token_replay_captures_for_gate(&self, state: &DecodeState) -> Res<[u64; 2]> {
        Ok(state
            .matrix_step
            .as_ref()
            .and_then(|w| w.replay.as_ref())
            .ok_or("replay is not armed")?
            .captures)
    }
    pub fn dump_full_token_replay_for_gate(
        &self,
        state: &DecodeState,
        directory: &Path,
    ) -> Res<()> {
    ...
    /// Gate-only same-load row reset. Copy the immutable primed state back into
    /// existing allocations; retained graph/input/sampler addresses never change.
    /// A failed request remains quarantined and cannot use this as a retry door.
    pub fn restore_full_token_prefix_for_gate(
        &self,
        dst: &mut DecodeState,
        src: &DecodeState,
    ) -> Res<()> {
        self.validate_full_token_program()?;
        if src.pos == 0 || src.pos >= 512 || src.capacity != dst.capacity {
            return Err("replay prefix position/capacity mismatch".into());
        }
        let _walk_guard = self
            .tp_ep_walk_lock
            .lock()
        ...
        let sw = src
            .matrix_step
            .as_ref()
            .ok_or("source matrix workspace missing")?;
        let dw = dst
            .matrix_step
            .as_mut()
            .ok_or("destination matrix workspace missing")?;
        if sw.failed || dw.failed || sw.verify.open.is_some() || dw.verify.open.is_some() {
            return Err("cannot reset an open or quarantined request".into());
        }
        let source_peer = src
            .tp_ep_caches
            .as_ref()
            .ok_or("source peer cache missing")?;
        let destination_peer = dst
            .tp_ep_caches
            .as_mut()
            .ok_or("destination peer cache missing")?;
        fn same(a: &Option<CudaSlice<f32>>, b: &Option<CudaSlice<f32>>) -> bool {
            match (a, b) {
                (None, None) => true,
        ...
        for (from, to) in [
            (&src.caches, &dst.caches),
            (source_peer, &*destination_peer),
        ] {
            if from.len() != 43 || to.len() != 43 {
                return Err("prefix cache layer count mismatch".into());
            }
            for (a, b) in from.iter().zip(to) {
                if a.c4_host.is_some()
                    || b.c4_host.is_some()
                    || a.kvc.len() != b.kvc.len()
                    || !same(&a.pend_kv, &b.pend_kv)
                    || !same(&a.pend_score, &b.pend_score)
                    || !same(&a.ikvc, &b.ikvc)
                    || !same(&a.ipend_kv, &b.ipend_kv)
                    || !same(&a.ipend_score, &b.ipend_score)
                {
                    return Err("prefix plane layout mismatch".into());
                }
            }
        }
        if sw.verify.tmax != 1
            ...
            || dw.verify.ws.len() != 2
        {
            return Err("prefix workspace shape mismatch".into());
        }
        for rank in 0..2 {
            if sw.verify.ws[rank].h_a.len() != dw.verify.ws[rank].h_a.len()
                || sw.verify.ws[rank].logits.len() != dw.verify.ws[rank].logits.len()
            {
                return Err("prefix workspace allocation mismatch".into());
            }
        }
        let mut result = (|| -> Res<()> {
            for rank in 0..2 {
                let st = &self.stages[rank];
                st.gpu.ctx.bind_to_thread().map_err(e("prefix copy bind"))?;
                let stream = st.gpu.stream();
                let (from, to) = if rank == 0 {
                    (&src.caches, &mut dst.caches)
                } else {
                    (source_peer, &mut *destination_peer)
                };
                for (a, b) in from.iter().zip(to) {
                    stream
                        .memcpy_dtod(&a.kvc, &mut b.kvc)
                        .map_err(e("prefix ring/store copy"))?;
                    for (source, target) in [
                        (&a.pend_kv, &mut b.pend_kv),
                        (&a.pend_score, &mut b.pend_score),
                        (&a.ikvc, &mut b.ikvc),
                        (&a.ipend_kv, &mut b.ipend_kv),
                        (&a.ipend_score, &mut b.ipend_score),
                    ] {
                        if let (Some(source), Some(target)) = (source, target) {
                            stream
                                .memcpy_dtod(source, target)
                                .map_err(e("prefix pending copy"))?;
                        }
                    }
                    b.n_blocks = a.n_blocks;
                    b.i_blocks = a.i_blocks;
                }
                stream
                    .memcpy_dtod(&sw.verify.ws[rank].h_a, &mut dw.verify.ws[rank].h_a)
                    .map_err(e("prefix hidden copy"))?;
                stream
                    .memcpy_dtod(&sw.verify.ws[rank].logits, &mut dw.verify.ws[rank].logits)
                    .map_err(e("prefix logits copy"))?;
            }
            Ok(())
        })();
        for (rank, st) in self.stages.iter().enumerate() {
            if let Err(error) = st
                .gpu
                .ctx
                .bind_to_thread()
                .and_then(|()| st.gpu.stream().synchronize())
            {
                let message = format!("prefix copy drain rank {rank}: {error}");
                if let Err(previous) = &mut result {
                    previous.push_str(&format!("; {message}"));
                } else {
                    result = Err(message);
                }
            }
    ...
    /// First executable all-layer TP/EP vertical slice: t=1 only, replicated
    /// attention/cache state on both ranks, local expert partitions, then the
    /// explicit owner-first rank reduction and common FFN tail on both ranks.
    ...
    fn decode_step_tp_ep(
        ...
        let host_marks = work.replay.as_ref().map(|_| {
            (
                state
                    .caches
                    .iter()
                    .map(|c| (c.n_blocks, c.i_blocks))
                    .collect::<Vec<_>>(),
                rank1_caches
                    .iter()
                    .map(|c| (c.n_blocks, c.i_blocks))
        ...
        let mut result = (|| -> Res<(Option<Vec<f32>>, u32)> {
            ...
            if let Some(pair) = &mut work.replay {
                let injection = self
                    .attention_tp_refusal_injection
                    .lock()
                    .map_err(|_| "refusal injection mutex poisoned")?
                    .take();
                let fault = injection.map_or(0, |i| {
                    (u64::from(i.code as u32) << 32) | ((i.rank as u64) << 16) | i.layer as u64
                });
            ...
            let variants: &[Option<ReplayCadence>] =
                if capture && work.replay.as_ref().is_some_and(|p| p.cadence) {
                    &[
                        Some(ReplayCadence::Ordinary),
                        Some(ReplayCadence::C4),
                        Some(ReplayCadence::C128),
                    ]
                } else {
                    &[None]
                };
            // Capture every forward variant before any execution. Each pass must
            // restore host metadata; device snapshots execute only on later replay.
            for &cadence in variants {
                let forward_slot = cadence.map_or(0, ReplayCadence::slot);
                if capture {
                    for ws in &mut work.verify.ws {
                        ws.replay_cadence = cadence;
                    }
                    work.replay.as_mut().expect("replay").begin(forward_slot)?;
                }
                if !replaying || capture {
                    let pos_i32 = [state.pos as i32];
                    let tok_i32 = [tok as i32];
                    for rank in 0..2usize {
                        let st = &self.stages[rank];
                        let vws = &mut work.verify.ws[rank];
                        st.gpu.ctx.bind_to_thread().map_err(e("TP/EP bind rank"))?;
                        let stream = st.gpu.stream();
                        if let Some(pair) = &mut work.replay {
                            unsafe {
                                ck(
                                    "replay token controls",
                                    k::memra_dsv4_replay_input(
                                        pair.input_ptr(rank),
                                        ...
                                        self.model.cfg().sliding_window as i32,
                                        pair.counter_ptr(rank, forward_slot),
                                        sp(&stream),
                                    ),
                                )?;
                            }
                            ...
                            stream
                                .memcpy_htod(&tok_i32, &mut vws.tok)
                                .map_err(e("TP/EP token upload"))?;
                            stream
                                .memcpy_htod(&pos_i32, &mut vws.pos_dev)
                                .map_err(e("TP/EP position upload"))?;
                        }
                        unsafe {
                            ck(
                                "TP/EP embed",
                ...
                if capture {
                    ...
                    // Rust ran while CUDA only recorded: restore every host high-water
                    // mark before the first execution. GPU snapshots execute on replay.
                    for (c, ckpt) in state.caches.iter_mut().zip(&work.verify.layers) {
                        if let Some(ck) = &ckpt.cmp {
                            c.n_blocks = ck.n_blocks0;
                        }
                        if let Some(ck) = &ckpt.idx {
                            c.i_blocks = ck.n_blocks0;
                        }
                    }
                    for (c, ckpt) in rank1_caches
                        .iter_mut()
                        .zip(work.verify.tp_ep_layers.as_ref().expect("rank1"))
                    {
                        if let Some(ck) = &ckpt.cmp {
                            c.n_blocks = ck.n_blocks0;
                        }
            ...
            if replaying {
                // Refresh host checkpoint metadata once/token, not once/node. All
                // device append/store/selector/attention addresses come from pos_dev.
                for rank in 0..2 {
                    let (cs, checkpoints) = if rank == 0 {
                        (&mut state.caches, &mut work.verify.layers)
                    } else {
                        (
                            &mut rank1_caches,
                            work.verify.tp_ep_layers.as_mut().expect("rank1"),
                        )
                    };
                    for (cache, checkpoint) in cs.iter_mut().zip(checkpoints) {
                        if let Some(ck) = &mut checkpoint.cmp {
                            ck.n_blocks0 = cache.n_blocks;
                            cache.n_blocks = (state.pos + 1) / ck.ratio;
                        }
                        if let Some(ck) = &mut checkpoint.idx {
                            ck.n_blocks0 = cache.i_blocks;
                            cache.i_blocks = (state.pos + 1) / ck.ratio;
                        }
                    }
                }
                forward_started = true;
                let pair = work.replay.as_ref().expect("replay");
                let slot = if pair.cadence {
                    ReplayCadence::for_position(state.pos).slot()
                } else {
                    0
                };
                pair.launch(slot)?;
            ...
            if replaying {
                if capture {
                    work.replay.as_mut().expect("replay").begin(1)?;
                }
                if capture {
                    ...
                    for rank in 0..2 {
                        ...
                        st.gpu
                            .ctx
                            .bind_to_thread()
                            .map_err(e("replay commit tick bind"))?;
                        unsafe {
                            ck(
                                "replay commit counter",
                                k::memra_dsv4_replay_tick(
                                    pair.counter_ptr(rank, 1),
                                    sp(&st.gpu.stream()),
                                ),
                            )?;
                        }
    ...
    fn decode_step_fast_tap(
        ...
        if self.topology.is_tp_ep() {
            return self.decode_step_tp_ep(tok, state, want_logits, device_logits, taps, false);
        }
        self.ensure_walk_topology_ready()?;
        crate::dsv4_grouped::ensure_program(self.matrix_moe, state.matrix_moe)?;
        if self.matrix_moe {
    ...
    pub fn decode_step_greedy_complete(&self, state: &mut DecodeState) -> Res<u32> {
        ...
        let result = (|| {
            ...
            let rows = self.complete_step_landing(&mut work.verify)?;
            if rows.len() != 1 {
                return Err(format!("pipelined greedy step landed {} rows", rows.len()));
            }
            self.commit_verify_dev(state, &mut work.verify, 1)?;
            Ok(rows[0])
        })();
...
pub struct VerifyWs {
    ...
    graph_scalars: bool,
    /// Strict retained full-token capture; separate from the older guarded probes.
            ...
            for il in 0..n_trunk {
                ...
                let mk = |cmp: &CmpDev| -> Res<CmpCkptDev> {
                    ...
                    Ok(CmpCkptDev {
                        kv_snap: stream
                            .alloc_zeros::<f32>(slots * cmp.latent)
                            .map_err(e("tp ckpt kv snap"))?,
                        sc_snap: stream
                            .alloc_zeros::<f32>(slots * cmp.latent)
                            .map_err(e("tp ckpt sc snap"))?,
                        rows_kv: stream
                            .alloc_zeros::<f32>(tmax * cmp.latent)
                            .map_err(e("tp ckpt rows kv"))?,
                        rows_sc: stream
                            .alloc_zeros::<f32>(tmax * cmp.latent)
                            .map_err(e("tp ckpt rows sc"))?,
                        latent: cmp.latent,
                        ratio: cmp.ratio,
                        overlap: cmp.overlap,
                        n_blocks0: 0,
                ...
                let idx = layer.idx.as_ref().map(|ix| mk(&ix.cmp)).transpose()?;
                for c in cmp.iter().chain(idx.iter()) {
                    let slots = if c.overlap { 2 * c.ratio } else { c.ratio };
                    bytes[rank] += ((2 * slots * c.latent + 2 * tmax * c.latent) * 4) as u64;
                }
                rank_layers.push(LayerCkptDev {
                    cmp,
                    idx,
                    trans_base: d.sliding_window as usize + cap_blocks,
                });
...
impl Dsv4Gpu {
    ...
    fn cmp_decode_batch_dev(
        ...
        {
            return Err("full-token compressor requires t=1 device C4/C128".into());
        }
        let stream = st.gpu.stream();
        let (ratio, d, latent) = (cmp.ratio, cmp.d, cmp.latent);
        ...
        for i in 0..t {
            ...
            if let Some(pos_dev) = replay_pos {
                unsafe {
                    for (src, dst) in [
                        (&ck_dev.rows_kv, &mut *pend_kv),
                        (&ck_dev.rows_sc, &mut *pend_score),
                    ] {
                        ck(
                            "replay compressor append",
                            k::memra_dsv4_replay_copy_row(
                                dpf!(*src, &stream),
                                dpm!(*dst, &stream),
                                pos_dev,
                                latent as i32,
                    ...
                    if replay_cadence.is_none_or(|cadence| cadence.emits(ratio)) {
                        ck(
                            "replay compressor emission",
                            k::memra_dsv4_replay_compressor_emit(
                                dpm!(*pend_kv, &stream),
...
impl Dsv4Gpu {
    ...
    pub fn moe_components_for_gate(
        ...
        self.moe_verify_dev(st, layer, ws, t, tokens, false, false, false, None, false)?;
        ...
        if let Some(ep) = &layer.ep {
            ...
            if self.matrix_moe {
                ...
                if self.topology.is_tp_ep() {
                    if !defer_tp_ep_tail {
                        return Err(
                            "DSV4 TP/EP rank-local expert execution requires a paired rank-order join"
                                .into(),
                        );
                    }
...
impl Dsv4Gpu {
    ...
    fn verify_head_output(
        ...
        if let Some(lg) = &logits {
            ...
            if output == VerifyOutput::ArgmaxDeferred {
                self.enqueue_step_landing(vstate, t)?;
                vstate.open = Some((pos0, t));
                return Ok((None, Vec::new()));
            }
    ...
    fn enqueue_step_landing(&self, vstate: &mut VerifyState, t: usize) -> Res<()> {
        ...
        for si in 0..n_stages {
            ...
            if si == last {
                // SAFETY: as above, for this request's argmax rows.
                unsafe { landing.argmax.enqueue_from(&vws.argmax, t, &stream)? };
                queued = true;
            }
    ...
    fn complete_step_landing(&self, vstate: &mut VerifyState) -> Res<Vec<u32>> {
        let landing = vstate
            .landing
            .as_mut()
    ...
    pub fn commit_verify_dev(
        ...
        // Deferred MoE faults refuse the round before either plane commits.
        self.take_moe_faults(&mut vstate.ws)?;
        if self.topology.is_tp_ep() {
    ...
    fn commit_verify_dev_plane(
        ...
        let graph_commit = n_commit == 1
            && t == 1
            && stage_override.is_some()
            && stage_override
                .and_then(|s| ws.get(s))
                .is_some_and(|w| w.full_token_replay);
