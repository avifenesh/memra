    ...
    fn enable_tp_ep_intermediate_banks_for_gate(&mut self) -> Res<()> {
        ...
        for rank in 0..2usize {
            ...
            for layer in &mut stage.layers {
                ...
                resident_delta += new_bytes as i64 - old_bytes as i64;
            }
            if resident_delta >= 0 {
                stage.loaded_bytes = stage
                    .loaded_bytes
                    .checked_add(resident_delta as u64)
                    .ok_or("intermediate TP resident-byte counter overflow")?;
            } else {
                stage.loaded_bytes = stage
                    .loaded_bytes
                    .checked_sub((-resident_delta) as u64)
                    .ok_or("intermediate TP resident-byte counter underflow")?;
            }
        }
        for stage in &self.stages {
            stage
