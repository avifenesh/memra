...
static GROUPED_GRAPH_GATE: AtomicBool = AtomicBool::new(false);
static GROUPED_GRAPH_EPOCH: AtomicU64 = AtomicU64::new(1);
static GROUPED_GRAPH_CAPTURES: AtomicU64 = AtomicU64::new(0);
static GROUPED_GRAPH_REPLAYS: AtomicU64 = AtomicU64::new(0);
static GROUPED_GRAPH_FALLBACKS: AtomicU64 = AtomicU64::new(0);
static GROUPED_GRAPH_EAGER_PREPARES: AtomicU64 = AtomicU64::new(0);

#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct Dsv4GroupedGraphKey {
    pub epoch: u64,
    pub shape: [usize; 4],
    pub partition: [usize; 3],
    pub flags: u64,
    pub pointers: Vec<u64>,
}

pub(crate) struct Dsv4GroupedGraph {
    pub(crate) capture: Dsv4LayerCapture,
    pub(crate) key: Dsv4GroupedGraphKey,
    pub(crate) stream: Arc<CudaStream>,
}

impl Dsv4GroupedGraph {
    pub(crate) fn node_count(&self) -> usize {
        self.capture.nodes.values().sum()
    }

    pub(crate) fn kernel_count(&self) -> usize {
        self.capture.kernels.len()
    }

    pub(crate) fn replay(&self, key: &Dsv4GroupedGraphKey) -> Result<(), String> {
        if &self.key != key {
            GROUPED_GRAPH_FALLBACKS.fetch_add(1, Ordering::Relaxed);
            return Err("grouped graph key changed; refusing stale replay".into());
        }
        self.capture.replay()?;
        GROUPED_GRAPH_REPLAYS.fetch_add(1, Ordering::Relaxed);
        Ok(())
    }
}

impl Drop for Dsv4GroupedGraph {
    fn drop(&mut self) {
        let _ = self.stream.synchronize();
    }
}

pub(crate) fn set_grouped_graph_gate(enabled: bool) -> bool {
    GROUPED_GRAPH_EPOCH.fetch_add(1, Ordering::SeqCst);
    GROUPED_GRAPH_GATE.swap(enabled, Ordering::SeqCst)
}

pub(crate) fn grouped_graph_gate() -> bool {
    GROUPED_GRAPH_GATE.load(Ordering::Acquire)
}

pub(crate) fn grouped_graph_counters() -> (u64, u64, u64) {
    (
        GROUPED_GRAPH_CAPTURES.load(Ordering::Relaxed),
        GROUPED_GRAPH_REPLAYS.load(Ordering::Relaxed),
        GROUPED_GRAPH_FALLBACKS.load(Ordering::Relaxed),
    )
}

pub(crate) fn grouped_graph_epoch() -> u64 {
    GROUPED_GRAPH_EPOCH.load(Ordering::Acquire)
}

pub(crate) fn grouped_graph_fallback_recorded() {
    GROUPED_GRAPH_FALLBACKS.fetch_add(1, Ordering::Relaxed);
}

pub(crate) fn grouped_graph_eager_prepare_recorded() {
    GROUPED_GRAPH_EAGER_PREPARES.fetch_add(1, Ordering::Relaxed);
}

pub(crate) fn grouped_graph_eager_prepare_count() -> u64 {
    GROUPED_GRAPH_EAGER_PREPARES.load(Ordering::Relaxed)
}

pub(crate) fn grouped_graph_capture_recorded() {
    GROUPED_GRAPH_CAPTURES.fetch_add(1, Ordering::Relaxed);
}
