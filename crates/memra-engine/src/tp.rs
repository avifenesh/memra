...
/// MEMRA_FENCE_RANK1=1: the peer rank rings a doorbell in ROOT memory with a kernel store
/// (`memra_ring_flag`) and the model engine waits it with a SAME-DEVICE stream memop, instead
/// of waiting a cross-device event. Completes the half the memops receipt left open (peer
/// memops are rejected; peer kernel stores are the direct-join mechanism). Ordering only —
/// values are untouched. Requires MEMRA_FENCE_MEMOPS=1 (it owns the flag allocation).
pub(crate) fn fence_rank1_on() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| std::env::var("MEMRA_FENCE_RANK1").as_deref() == Ok("1"))
}
...
impl TpE4m3HostBounce {
        ...
        for (rank_index, engine) in self.ranks.iter().enumerate() {
            ...
            for expert in 0..gate.expert_count {
                if ep2 {
                    ...
                    owned += 1;
                    gate_host
                        .extend_from_slice(&nvfp4_repack_bank_matrix(gate.expert(expert)?, true));
                    up_host.extend_from_slice(&nvfp4_repack_bank_matrix(up.expert(expert)?, true));
                } else {
                    let gate_shard = nvfp4_column_shard(gate.expert(expert)?, tp, rank_index)?;
                    gate_host.extend_from_slice(&nvfp4_repack_bank_matrix(gate_shard, false));
                    let up_shard = nvfp4_column_shard(up.expert(expert)?, tp, rank_index)?;
                    up_host.extend_from_slice(&nvfp4_repack_bank_matrix(up_shard, false));
                }
            }
        ...
        for shard_index in 0..NVFP4_CANONICAL_ROW_SHARDS {
            ...
            for expert in 0..down.expert_count {
                ...
                if ep2 {
                    ...
                    owned += 1;
                    down_host.extend_from_slice(&nvfp4_repack_bank_matrix(down_matrix, true));
                } else {
    ...
    fn run_full_bank_expert_nvfp4(
        ...
        // EP2 banks are ALWAYS slot-major (see nvfp4_repack_bank_matrix), so the oracle
        // must be the slot-major reader.
        let output = engine.qmatvec_nvfp4_fast_v2(
            &bank.expert(slot),
            &activations,
            1,
            bank.in_features,
    ...
    fn run_full_down_expert_nvfp4(
        ...
        let _main = engine.gpu.enter_main()?;
        let activations = engine.htod(input)?;
        // EP2 down banks are ALWAYS slot-major (see nvfp4_repack_bank_matrix).
        let output = engine.qmatvec_nvfp4_fast_v2(
            &shard.expert(slot),
            &activations,
            1,
    ...
    fn nvfp4_routes_batched_sweeps_rank(
    ...
    ) -> Result<(), Box<dyn std::error::Error>> {
        {
            ...
            // fold the down macro in-kernel from the device selection.
            {
                let Nvfp4DeviceRoutesWorkspace {
                    partial,
                    combine_w,
    ...
    pub fn run_tensor_parallel_routes_nvfp4_prime_grouped(
        ...
        // The dequant must read the layout the bank was BUILT in (feeding slot-major bytes
        // to the v1 kernel was a garbage-output bug this line exists for). The layout is a
        // property of the bank now, never of the environment: EP2 banks are always
        // slot-major, TP shard banks always block_nvfp4 v1 (nvfp4_repack_bank_matrix).
        let bank_qt = if experts.ep2 {
            crate::QT_NVFP4_V2
        } else {
            crate::QT_NVFP4
        };
    ...
    pub fn run_tensor_parallel_routes_nvfp4_device_routed_prejoin_add3(
        ...
        if memops {
            ...
            // rank1's fence: a peer stream MEMOP is rejected over PCIe P2P
            // (CUDA_ERROR_INVALID_VALUE, receipted 2026-08-23), but a peer KERNEL STORE into
            // root memory is legal — the direct join already relies on it. Under
            // MEMRA_FENCE_RANK1 rank1 rings flag[0] that way and e waits it same-device,
            // replacing the cross-device event wait below.
            if fence_rank1_on() {
                let peer = &self.ranks[1];
                let _pmain = peer.gpu.enter_main()?;
                peer.ring_flag_raw(base, ticket)?;
            }
            {
                let root = &self.ranks[0];
                let _main = root.gpu.enter_main()?;
        ...
        if moe_direct_on() && self.ranks.len() == 2 {
            ...
            if memops {
                ...
                if fence_rank1_on() {
                    // Same-device wait on the flag rank1 rang over P2P.
                    let r = unsafe {
                        sys::cuStreamWaitValue32_v2(
                            e.stream().cu_stream() as sys::CUstream,
                    ...
                    if r != sys::CUresult::CUDA_SUCCESS {
                        return Err(format!("fence wait rank1: {r:?}").into());
                    }
                } else {
                    for ev in workspace.ev_rank.iter().skip(1) {
                        e.stream().wait(ev)?;
