...
impl CudaTransfers {
    ...
    /// caller's wait on the zero-fill). Each span's value is `d2d_receipt_digest`'s.
    fn span_digests_on(
        &self,
        stream: &Arc<CudaStream>,
        spans: &[(u64, u64)],
        ...
        for (c, chunk) in spans.chunks(SPAN_ITEMS).enumerate() {
            ...
            let blocks = longest.div_ceil(8).div_ceil(2048).clamp(1, 2048) as u32;
            let cfg = LaunchConfig {
    ...
    pub fn submit_d2h_spans(
        ...
        if let Some(scratch) = scratch.as_mut() {
            let digested = (|| -> Result<()> {
                ...
                self.span_digests_on(&copy, &sources, lanes, 0, 64)
            })();
    ...
    pub fn seal_d2h_span_receipt(
        ...
        let sealed = (|| -> Result<()> {
            let lanes = scratch.lanes.device_ptr(&copy).0;
            self.span_digests_on(&copy, &landed, lanes, 32, 64)?;
            cuda(copy.memcpy_dtoh(&scratch.lanes, &mut scratch.pinned))?;
            scratch.event = Some(cuda(copy.record_event(None))?);
            Ok(())
        })();
        ...
        if !failed && let Some(scratch) = scratch.as_mut() {
            let sealed = (|| -> Result<()> {
                ...
                let destinations: Vec<(u64, u64)> = slots
                    ...
                    .collect();
                let lanes = scratch.lanes.device_ptr(&copy).0;
                self.span_digests_on(&copy, &destinations, lanes, 0, 32)?;
                cuda(copy.memcpy_dtoh(&scratch.lanes, &mut scratch.pinned))?;
                scratch.event = Some(cuda(copy.record_event(None))?);
                Ok(())
            })();
...
mod tests {
    ...
    fn span_receipt_rules_are_as_stated() {
        ...
        let source = at(d2h, "self.span_digests_on(&copy, &sources, lanes, 0, 64)");
    ...
    fn span_receipt_digests_are_the_program_per_span() {
        ...
        let bases = [
            ("device", device.device_ptr(&copy).0),
            ("pinned host", host.device_address().unwrap()),
        ];
        for (what, base) in bases {
            ...
            t.span_digests_on(&copy, &items, at, 0, 32).unwrap();
