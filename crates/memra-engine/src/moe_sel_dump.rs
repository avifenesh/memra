...
//!     two device-routed single-device arms (`vrows_dev` door D / the T=1 decode-graph
//!     twin, and the Step-3.7 resident `moe_ffn_sigmoid_dev`) reads the freshly selected
//!     `[sel, w]` back with one extra DtoH per layer-call, DIAGNOSTIC ONLY, never in a timed
//!     arm;
...
///
/// Refuses while a glm5 decode-graph capture is open: a capture region admits no DtoH and
/// no sync. `glm5_decode_graph_refusal` already declines capture when the door is armed,
/// so this is the belt to that suspender, never the expected path.
pub(crate) fn record_device(
    ...
    if crate::glm5_graph_capture_open() {
        return Err(format!(
            "{ENV} cannot read the selection back inside an open decode-graph capture; the \
             decode-graph door declines capture while this dump is armed, so reaching here is \
             a wiring bug, refused rather than corrupting the capture"
        )
        .into());
    }
    let n = t * n_used;
