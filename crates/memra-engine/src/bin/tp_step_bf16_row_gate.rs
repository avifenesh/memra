//! Official Step-3.7 BF16 O-projection gate for native tensor-parallel collectives.
//!
//! TP1, host-staged TP, and native TP all execute the same eight TP8-sized K blocks and reduce them
//! on the root in global checkpoint-column order. The input and final output remain host-canonical;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let model = std::env::args().nth(1).expect(
        "usage: tp-step-bf16-row-gate <official-step-safetensors-dir> \
    ...
    let plan = contract.plan(TopologyRequest {
        ...
        // TP4/TP8 pair legal attention projections with expert ownership; this gate exercises
        // only the O row projection and never labels local attention as tensor parallelism.
        expert_parallel: devices.len() > 2,
        available_devices: devices.len(),
        hardware: HardwareTarget::RtxPro6000Blackwell,
    })?;

    let name = format!("blk.{layer}.attn_output.weight");
    let tensor = source
        .find(&name)
    ...
    matrix.validate()?;
    let expected_in = contract.query_heads[layer] * contract.head_dim;
    if matrix.in_features != expected_in || matrix.out_features != contract.hidden_size {
        return Err(format!(
            "{name} shape {}x{} != registered {}x{expected_in}",
    ...
    let oracle = runtime.step_bf16_row_parallel_resident(&resident, &input, tokens)?;
    let canonical_runtime = TpE4m3HostBounce::new_single_rank_oracle(devices[0])?;
    let canonical_resident = canonical_runtime.upload_step_bf16_row_parallel(matrix)?;
    let canonical =
        canonical_runtime.step_bf16_row_parallel_resident(&canonical_resident, &input, tokens)?;
    ...
    let run_native = || -> Result<Vec<f32>, Box<dyn std::error::Error>> {
        let _ambient = ambient_engine
            ...
            .transpose()?;
        runtime.step_bf16_row_parallel_resident_native(&resident, &input, tokens)
    };
    let native_first = run_native()?;
    let native_second = run_native()?;
    compare_exact("host_oracle_vs_native", &oracle, &native_first)?;
    compare_exact("native_repeat", &native_first, &native_second)?;
    compare_exact("canonical_tp1_vs_host_tp", &canonical, &oracle)?;
    compare_exact("canonical_tp1_vs_native", &canonical, &native_first)?;

    if f32_mirror {
        println!(
            "STEP_TP_BF16_ROW_GATE_PASS layer={layer} projection=o tokens={tokens} \
             ...
             transport={} native_p2p={} activation=host-canonical \
             reduction=global-tp8-block-order output=root-readback \
             attention_local=true kv_local=true performance_claim=false",
            plan.request.tensor,
