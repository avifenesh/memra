...
fn census(gpu: &Dsv4Gpu, state: &DecodeState, dir: &Path) -> [[String; 4]; 2] {
    ...
    for (rank, rank_hashes) in hashes.iter_mut().enumerate() {
        for (segment, hash) in rank_hashes.iter_mut().enumerate() {
            ...
            .unwrap();
            let partial = count_kernel(&dot, "moe_m1_splitk_fast_partial_kernel");
            let reduce = count_kernel(&dot, "moe_m1_splitk_fast_reduce_kernel");
...
fn qualify_arm(gpu: &Dsv4Gpu, prompt: &[u32], output: &Path, cfg: Dsv4SampleCfg) {
    let on = fast_on();
    select_arm(gpu, false); // Explicit OFF prefix and eager oracle.
        ...
        .unwrap();
    {
        ...
        for step in 0..OUTPUT {
            ...
            println!(
                "STEP_IDENTITY step={step} token={carry} next={next} identity={:?} ar_epochs={:?}",
                identity(gpu, &graph),
                gpu.full_token_ar_epochs_for_gate().unwrap()
            );
            carry = next;
        }
        ...
        println!(
            "QUALIFIED {{\"splitk_fast\":{},\"generated_sha256\":\"{}\",\"final_logits_sha256\":\"{}\",\"final_cache_digest\":{:?},\"final_hidden_digest\":{:?},\"positions\":{OUTPUT},\"within_class\":true}}",
            fast_on(),
            sha_tokens(&inputs),
            ident.0,
            ident.1,
...
fn select_arm(gpu: &Dsv4Gpu, on: bool) {
    ...
    unsafe {
        memra_moe_m1_splitk_fast_set_for_gate(i32::from(on));
    }
}
...
fn run_abba(
    ...
    // Each arm starts with its own fresh prefix and uncaptured scored state.
...
fn scored_row(
    ...
    println!(
        "MEASURE {{\"row\":{row},\"arm_row\":{},\"reverse\":{reverse},\"splitk_fast\":{on},\"generated_tokens\":{OUTPUT},\"decode_wall_ns\":{ns},\"decode_tok_s\":{},\"eligible\":{},\"looped\":{looped},\"generated_sha256\":\"{hash}\",\"final_logits_sha256\":\"{}\",\"final_cache_digest\":{:?},\"final_hidden_digest\":{:?},\"first_capture_inside_timing\":{},\"captures_before\":{captures_before:?},\"captures\":{captures:?},\"device_replays\":{after:?}}}",
        arm.rows,
