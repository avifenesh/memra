...
#[path = "rewrite_identity_gate/gemma_hidden.rs"]
mod gemma_hidden;
...
fn output(label: &str, prompt: usize, logits: &[f32], vocab: usize, bundle: &Path) -> Result<()> {
    ...
    persist_output(label, prompt, logits, bundle)
}
...
fn output_for_model(
    ...
    if !model.uses_gemma_program() {
        return output(label, prompt, logits, vocab, bundle);
    }
    // Preserve original F32 bytes, including a failed observation, before adjudication.
    persist_output(label, prompt, logits, bundle)?;
    require(logits.len() == vocab, "GEMMA_OUTPUT_SHAPE")?;
    memra_engine::plan_backend::PlanLogits::from_plan(&model.plan)?.validate(logits)?;
    Ok(())
}

fn verify_model_logits(
    model: &HybridModel,
...
) -> Result<memra_engine::plan_backend::RewriteParityReceipt> {
    if model.uses_gemma_program() {
        Ok(
        ...
        Ok(rewrite.verify_logits(implementation, reference, candidate, policy)?)
    }
}

fn persist_output(label: &str, prompt: usize, logits: &[f32], bundle: &Path) -> Result<()> {
...
fn run() -> Result<()> {
    ...
    require(
        args.len() == 3,
        "USAGE: rewrite_identity_gate <capture|check|fresh-control|library-drift|retained-capture|retained-{step,prof-apply,prof-launch,prof-read,prime-run}-{library,environment}|retained-eg-capture|retained-eg-{step,prof-apply,prof-launch,prof-read}-{library,environment}|retained-eg-{prime-api-refusal,prime-production-refusal,prime-fallback}> <model-path> <bundle-dir>",
    )?;
    let mode = args[0].to_str().ok_or("MODE_NOT_UTF8")?;
    if mode == "gemma-hpost" {
        return gemma_hidden::run(Path::new(&args[1]), Path::new(&args[2]));
    }
    ...
    if mode == "check" {
        ...
        for (i, prompt) in PROMPTS.into_iter().enumerate() {
            ...
            output_for_model(
                &model,
                "pre-install-tokenwise",
                i,
                &candidate,
                vocab,
                bundle,
            )?;
            let parity = verify_model_logits(
                &model,
                &rewrite,
        ...
        let receipt = verify_model_logits(
            &model,
            &rewrite,
            &identity.implementation_sha256,
            &references,
        ...
        for (i, prompt) in PROMPTS.into_iter().enumerate() {
            ...
            output_for_model(&model, "installed-eager", i, &actual, vocab, bundle)?;
            bitwise(
        ...
        output_for_model(&model, "restored-eager", 0, &restored, vocab, bundle)?;
