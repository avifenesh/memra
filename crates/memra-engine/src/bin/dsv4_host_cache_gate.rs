...
fn main() {
    ...
    if std::env::var("MEMRA_DSV4_PREFILL_EXPERT_SORT_GATE").as_deref() == Ok("1") {
        let sort_prompt = fixture
            .tokens_160
            .as_ref()
            .expect("fixture tokens_160 required");
        let capacity = sort_prompt.len() + 1;
        gpu.prefill_expert_sort = false;
        let mut reference_state = gpu
            .alloc_decode_state_for_transient(capacity, 64)
            .expect("expert-sort reference state");
        let reference_logits = gpu
            .prefill_with_cache_chunked(sort_prompt, &mut reference_state, 64)
            .expect("expert-sort reference prefill");
        gpu.prefill_expert_sort = true;
        let mut sorted_state = gpu
            .alloc_decode_state_for_transient(capacity, 64)
            .expect("expert-sort state");
        let sorted_logits = gpu
            .prefill_with_cache_chunked(sort_prompt, &mut sorted_state, 64)
            .expect("expert-sort prefill");
        assert!(
            bits_equal(&reference_logits, &sorted_logits),
            "expert-sort logits differ"
        );
        assert!(
            classes_equal(
                &gpu.cache_classes(&reference_state)
                    .expect("expert-sort reference classes"),
                &gpu.cache_classes(&sorted_state)
                    .expect("expert-sort classes"),
            ),
            "expert-sort cache classes differ"
        );

        let mut reference_wall = Vec::new();
        let mut sorted_wall = Vec::new();
        for _ in 0..3 {
            gpu.prefill_expert_sort = false;
            let mut state = gpu
                .alloc_decode_state_for_transient(capacity, 64)
                .expect("expert-sort timed reference state");
            let started = std::time::Instant::now();
            let _ = gpu
                .prefill_with_cache_chunked(sort_prompt, &mut state, 64)
                .expect("expert-sort timed reference");
            reference_wall.push(started.elapsed().as_secs_f64());

            gpu.prefill_expert_sort = true;
            let mut state = gpu
                .alloc_decode_state_for_transient(capacity, 64)
                .expect("expert-sort timed state");
            let started = std::time::Instant::now();
            let _ = gpu
                .prefill_with_cache_chunked(sort_prompt, &mut state, 64)
                .expect("expert-sort timed prefill");
            sorted_wall.push(started.elapsed().as_secs_f64());
        }
        reference_wall.sort_by(f64::total_cmp);
        sorted_wall.sort_by(f64::total_cmp);
        println!(
            "[dsv4-prefill-expert-sort-gate] PASS logits_bits=true cache_bits=true \
             tokens={} x3_interleaved reference_median_s={:.6} sorted_median_s={:.6} \
             speedup={:.3}x",
            sort_prompt.len(),
            reference_wall[1],
            sorted_wall[1],
            reference_wall[1] / sorted_wall[1],
        );
    }
