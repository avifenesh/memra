//! Dense batching gate on the default cadence + exact-tail program.
//! Adapted from the composed gate; shared graph and profile harnesses are unchanged.
...
const FP8_NODES: &str = "dsv4_dense_batch_fp8_kernel";
const DOT_NODES: &str = "dsv4_dense_batch_dots_kernel";
...
unsafe extern "C" {
    fn memra_dsv4_dense_batch_set_for_gate(enabled: c_int) -> c_int;
    fn memra_dsv4_dense_batch_counts_for_gate(fp8: *mut u64, dots: *mut u64) -> c_int;
}
fn enqueues() -> [u64; 2] {
    let mut c = [0u64; 2];
    let rc = unsafe { memra_dsv4_dense_batch_counts_for_gate(&mut c[0], &mut c[1]) };
...
fn set_dense(on: bool) {
    assert_eq!(
        unsafe { memra_dsv4_dense_batch_set_for_gate(i32::from(on)) },
        0
    );
}
...
fn captures_once(gpu: &Dsv4Gpu, state: &DecodeState, on: bool) {
    ...
    for rank in gpu
        ...
        .unwrap()
    {
        let saved = if on { 169 } else { 0 };
        let slots: &[(usize, u64)] = &[(0, 2741 - saved), (2, 3140 - saved), (3, 3240 - saved)];
...
fn dense_census(gpu: &Dsv4Gpu, state: &DecodeState, on: bool, dir: &Path) -> GraphHashes {
    ...
    for (rank, row) in hashes.iter_mut().enumerate() {
        for (segment, hash) in row.iter_mut().enumerate() {
            ...
            assert_eq!(
                candidate,
                if on && segment != 1 { [64, 62] } else { [0, 0] },
...
fn main() {
    ...
    assert!(
        ...
        "usage: dsv4_dense_batch_gate <model-dir> <source.txt> <new-output-dir> [--reverse|--capture]"
    );
    ...
    if args.get(4).is_some_and(|arg| arg == "--capture") {
        unsafe extern "C" {
            fn memra_dsv4_dense_batch_capture_for_gate(dir: *const std::ffi::c_char) -> c_int;
        }
        let dir = std::ffi::CString::new(output.to_str().unwrap()).unwrap();
        assert_eq!(
            unsafe { memra_dsv4_dense_batch_capture_for_gate(dir.as_ptr()) },
            0
        );
        select(&gpu, true);
        let mut prefix = state(&gpu);
        ...
        drain(&gpu);
        assert_eq!(
            unsafe { memra_dsv4_dense_batch_capture_for_gate(std::ptr::null()) },
            0
        );
        select(&gpu, false);
        let count = std::fs::read_dir(&output)
            ...
            .count();
        assert_eq!(count, 252, "all 126 layer-pairs on both ranks");
        println!("CAPTURE_PASS pairs={count} prefix_tokens=8 candidate_kernels_executed=0");
    } else {
        run(
            &gpu,
            &prompt[..PRIME],
...
mod tests {
    use super::*;
    #[test]
    fn sampled_orders_are_balanced_and_opposite() {
        assert_eq!(block_arms(false), [1, 0, 0, 1]);
        assert_eq!(block_arms(true), [0, 1, 1, 0]);
    }
    #[test]
    fn both_arms_cover_cadence_boundaries() {
        assert_eq!(expected_variants(false, 256, 512, true), [192, 256, 62, 2]);
        assert_eq!(expected_variants(true, 256, 512, true), [192, 256, 62, 2]);
    }
    #[test]
    fn census_counts_function_records_only() {
        let dot = "label=dsv4_dense_batch_fp8_kernel\n| {ID | 1} dsv4_dense_batch_fp8_kernel\ndsv4_dense_batch_fp8_kernel -> head\n";
        assert_eq!(count_kernel(dot, FP8_NODES), 1);
    }
    #[test]
    fn loop_exclusion_detects_long_repetition() {
        assert!(looped(&[7; 256]));
