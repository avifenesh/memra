...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &t in &[1usize, 4, 8] {
        ...
        // ---- v4z: (v4 + the served norm launch) vs v4z, back-to-back x300 ----
        // Arm A is what the walk does per hc site under MEMRA_HC_PRE_V4 + the fused-norm door
        // off: the v4 kernel, then `Engine::rms_norm_zq8_f32` (kernels.cu v2 at rms_block(),
        // allocating its q8 pair as serving does). Arm B is one v4z launch into fixed buffers.
        if t == 1 {
            const NB: usize = 300;
            let eng = memra_engine::Engine::new(dev)?;
            let xe = eng.htod(&s.x[..HC * D])?;
            let mixes_e = eng.htod(&s.mixes)?;
            let scale_e = eng.htod(&s.scale)?;
            let base_e = eng.htod(&s.base)?;
            let nw_e = eng.htod(&vecf(D, 0x77).iter().map(|v| 1.0 + v).collect::<Vec<_>>())?;
            let mut pre_e = eng.uninit(HC)?;
            let mut post_e = eng.uninit(HC)?;
            let mut comb_e = eng.uninit(HC * HC)?;
            let mut y_e = eng.uninit(D)?;
            let mut z_e = eng.uninit(D)?;
            let mut q_e = eng.uninit_i8(D)?;
            let mut d_e = eng.uninit(D / 32)?;
            let es = eng.stream();
            let esp = es.cu_stream() as *mut c_void;
            let mut res: Vec<(&str, f64)> = Vec::new();
            for round in 0..2 {
                for arm in ["v4+norm", "v4z"] {
                    let mut launch = || -> Res<()> {
                        unsafe {
                            if arm == "v4+norm" {
                                let rc = k::memra_dsv4_hc_pre_v4(
                                    xe.device_ptr(&es).0 as *const f32,
                                    mixes_e.device_ptr(&es).0 as *const f32,
                                    scale_e.device_ptr(&es).0 as *const f32,
                                    base_e.device_ptr(&es).0 as *const f32,
                                    pre_e.device_ptr_mut(&es).0 as *mut f32,
                                    post_e.device_ptr_mut(&es).0 as *mut f32,
                                    comb_e.device_ptr_mut(&es).0 as *mut f32,
                                    y_e.device_ptr_mut(&es).0 as *mut f32,
                                    1,
                                    HC as i32,
                                    D as i32,
                                    iters(),
                                ...
                                let _ = eng.rms_norm_zq8_f32(&y_e, &nw_e, &mut z_e, D, 1, 1e-5)?;
                            } else {
                                let rc = k::memra_dsv4_hc_pre_v4z(
                                    ...
                                    EPS,
                                    std::ptr::null_mut(),
                                    nw_e.device_ptr(&es).0 as *const f32,
                                    z_e.device_ptr_mut(&es).0 as *mut f32,
                                    q_e.device_ptr_mut(&es).0 as *mut i8,
                                    d_e.device_ptr_mut(&es).0 as *mut f32,
                                    1e-5,
                                    256,
                                    esp,
                                );
            ...
            println!(
                "[v4z-timing] t=1 back-to-back x{NB}: v4 + rms_norm_zq8 launch {:.2} us, v4z {:.2} us ({:+.1}%)",
                res[0].1,
                res[1].1,
                100.0 * (res[1].1 / res[0].1 - 1.0)
            );
