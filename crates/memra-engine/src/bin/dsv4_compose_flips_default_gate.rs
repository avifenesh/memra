...
/// Both doors move together in this instrument. A process where they disagree
/// is not a composition arm and must never produce a scored row.
fn composed_on(gpu: &Dsv4Gpu) -> bool {
    let fast = fast_on();
    let norm2 = norm2_on(gpu);
    assert_eq!(fast, norm2, "composed arm requires both doors on one side");
    fast
}
    ...
    for (rank, rank_hashes) in hashes.iter_mut().enumerate() {
        for (segment, hash) in rank_hashes.iter_mut().enumerate() {
            ...
            assert_eq!(
                ...
                "norm2 pack rank={rank} segment={segment}"
            );
            ...
            *hash = format!("{:x}", Sha256::digest(dot.as_bytes()));
            println!(
                "GRAPH_CENSUS on={on} rank={rank} segment={segment} splitk_fast={fast_partial}/{fast_reduce} splitk_base={base_partial}/{base_reduce} pack={pack} swiglu={swiglu} quant={quant} sha256={hash}"
            );
        }
    }
    hashes
...
/// Both doors move as one arm. Split-K fast has no per-load restore hook, so the
/// caller reads its policy once before the first override and passes it back.
...
fn qualify_arm(
    ...
    println!(
        "QUALIFIED {{\"splitk_fast\":{},\"norm_fuse2\":{},\"generated_sha256\":\"{}\",\"final_logits_sha256\":\"{}\",\"final_cache_digest\":{:?},\"final_hidden_digest\":{:?},\"positions\":{OUTPUT},\"within_class\":true}}",
        fast_on(),
        norm2_on(gpu),
        sha_tokens(&inputs),
        ident.0,
...
fn scored_row(
    ...
    println!(
        "MEASURE {{\"row\":{row},\"arm_row\":{},\"reverse\":{reverse},\"composed_on\":{on},\"splitk_fast\":{on},\"norm_fuse2\":{on},\"generated_tokens\":{OUTPUT},\"decode_wall_ns\":{ns},\"decode_tok_s\":{},\"eligible\":{},\"looped\":{looped},\"generated_sha256\":\"{hash}\",\"final_logits_sha256\":\"{}\",\"final_cache_digest\":{:?},\"final_hidden_digest\":{:?},\"first_capture_inside_timing\":{},\"captures_before\":{captures_before:?},\"captures\":{captures:?},\"device_replays\":{after:?}}}",
        arm.rows,
...
fn main() {
    ...
    assert!(
        ...
        "usage: dsv4_compose_flips_default_gate <model-dir> <source.txt> <new-output-dir> [--qualify|--reverse|--defaults]"
    );
    assert!(
        !dsv4_prof_on(),
    ...
    // This bin observes both flipped defaults. A mixed pin is not a composition
    // arm, so refuse it before a model exists rather than scoring half of one.
    let fast_env = std::env::var(SPLITK_FAST_ENV).ok();
    let norm2_env = std::env::var(NORM_FUSE2_ENV).ok();
    assert!(
        (fast_env.is_none() && norm2_env.is_none())
            || (fast_env.as_deref() == Some("0") && norm2_env.as_deref() == Some("0")),
        "composition requires both doors unset or both explicit zero, got {fast_env:?}/{norm2_env:?}"
    );
    ...
    println!(
        "COMPOSE_POLICY {SPLITK_FAST_ENV}={} {NORM_FUSE2_ENV}={} splitk_fast={fast_initial} graph_splitk={}",
    ...
    // Loaded norm2 policy, before any gate override, must agree with both the
    // environment and the split-K door read above.
    ...
    if args.get(4).is_some_and(|s| s == "--defaults") {
        ...
        println!(
            "DEFAULT_ENGAGEMENT composed_on={norm2_initial} splitk_fast_env={} norm_fuse2_env={} scored_rows=0",
            door_env(SPLITK_FAST_ENV),
            door_env(NORM_FUSE2_ENV)
        );
...
mod tests {
    ...
    fn mixed_door_pins_are_refused() {
        fn accepted(fast: Option<&str>, norm2: Option<&str>) -> bool {
            (fast.is_none() && norm2.is_none()) || (fast == Some("0") && norm2 == Some("0"))
        }
        assert!(accepted(None, None));
        assert!(accepted(Some("0"), Some("0")));
        for (fast, norm2) in [
            (None, Some("0")),
            (Some("0"), None),
            (Some("1"), Some("0")),
