...
/// Merge this token's ledger rows from BOTH arms of the ledger into one per-layer view.
///
/// This is where box take 13's `step 0: ledger row count 21 (eager) != 6 (graph)` came from, and
/// it was the instrument, not the door. The eager arm produces only HOST rows (21 on the head
/// stage: 24 layers minus the 3 dense ones). The graph arm produces BOTH: the captured KDA runs
/// route through the device-table arm and land in the persistent device slots, while the eager
/// gap layers (the MLA/DSA ones, which are never captured) still read their selection back on the
/// host. The old code said `if !host.is_empty() { step_rows = host; }`, which threw the 15 device
/// rows away the moment a single gap layer pushed a host row, leaving 6 against 21.
///
/// Merging by `(dev, layer)` is the honest comparison: exactly one arm writes any given layer in
/// a given token, so there is no precedence question to get wrong. A device slot that was
/// pre-armed but never written keeps its `-1` sentinel and is dropped here rather than compared,
/// so a wiring miss still shows up as a MISSING row instead of an accidental match.
