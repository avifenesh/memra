...
fn qualified(model: &HybridModel) -> Result<()> {
    require(model.rewrite_is_qualified(), "RETAINED_NOT_QUALIFIED")?;
    for rewrite in execution_rewrites(&model.plan) {
        require(
            model.rewrite_allowed(rewrite.surface) == SURFACES.contains(&rewrite.surface),
...
fn seal(identity: &RewriteIdentity, index: &[u8], lock: &[u8]) -> String {
    format!(
...
pub(super) fn run(mode: &str, source: &Path, bundle: &Path) -> Result<()> {
    let capture_mode = mode == "retained-capture";
    let case = if capture_mode {
        None
    } else {
        let (api, drift) = mode
    ...
    let result = (|| -> Result<()> {
        ...
        if capture_mode {
            return capture::run(&engine, &mut model, bundle, &lock, &identity, &mut evidence);
        }
        qualified(&model)?;
        let index = std::fs::read(bundle.join("rewrite-receipts.tsv"))?;
        require(
            std::fs::read_to_string(bundle.join("retained-capture/PASS"))?
                == seal(&identity, &index, &lock),
        ...
        evidence.event(format!(
            "RETAINED_CALLER_PASS mode={mode} api={} receipt_emitted=false support_promotion=false",
            api.name()
        ))
    })();
