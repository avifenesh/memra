...
pub(super) fn prime_rows(
    ...
    let (mut ordinary, ordinary_seed, _) = model.prime_cache(e, prompt, &mut ordinary_cache, 0)?;
...
pub(super) fn run(
    ...
    let installed = (|| -> Result<()> {
        ...
        qualified(model)?;
        evidence.bytes("PASS", seal(identity, index.as_bytes(), lock).as_bytes())?;
        evidence.event("RETAINED_CAPTURE_PASS surfaces=decode-eager,decode-graph,carried-prime support_promotion=false performance=not-measured")
    })();
    if installed.is_err() {
