...
use memra_engine::dsv4_gpu::{
    Dsv4Fp4Reduce, Dsv4Gpu, Dsv4SampleCfg, Dsv4Vt, SpecRunGpu, dsv4_sample_row,
};
...
fn main() {
    ...
    assert_eq!(
        args.len(),
        3,
        "usage: dsv4_fp4_reduce_gate <model-dir> <0731-fixtures.json>"
    );
    ...
    for prompt in [&fixture.tokens_32, long] {
        for width in [1, 32, 64] {
            gpu.set_fp4_reduce_for_gate(Dsv4Fp4Reduce::Block)
                .expect("block");
            let reference = plain(&gpu, prompt, width, &sample);
            gpu.set_fp4_reduce_for_gate(Dsv4Fp4Reduce::Warp)
                .expect("warp");
            assert_eq!(
                reference,
            ...
            // Return to block, proving same-process arm reversal as well as A->B.
            gpu.set_fp4_reduce_for_gate(Dsv4Fp4Reduce::Block)
            ...
            println!(
                "EXACT plain prompt={} width={width} block/warp/block",
                prompt.len()
            );
        }
    ...
    println!("PASS FP4 rewrite one-load exactness; serving/performance not claimed");
}
