...
fn main() {
    ...
    assert_eq!(
        args.len(),
        5,
        "usage: dsv4_quality_twin <model> <prompts-dir> <new-output-dir> <A|B|C>"
    );
    ...
    println!(
        "PROTOCOL arm={arm} graph_splitk={} hc_slices={slices} prompts={} max_new={LIMIT} capacity={capacity} sampler=argmax ties=lowest_id dspark=off stop=eos",
        arm != "A",
        prompts.len()
    );
