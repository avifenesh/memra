...
fn main() {
    ...
    assert_eq!(
        args.len(),
        5,
        "usage: dsv4_quality_twin <model> <prompts-dir> <new-output-dir> <A|B|C|D>"
    );
    let arm = args[4].as_str();
    assert!(matches!(arm, "A" | "B" | "C" | "D"));
    for (name, value) in [
    ...
    if arm == "A" {
        assert_eq!(
            std::env::var("MEMRA_DSV4_MOE_M1_SPLITK").as_deref(),
            Ok("0")
        );
    } else {
    ...
    let fp8_slices = if arm == "D" {
        let s: i32 = std::env::var("MEMRA_DSV4_FP8_KSPLIT")
            .expect("D requires selected S")
            .parse()
            .unwrap();
    ...
    assert_eq!(unsafe { memra_dsv4_fp8_ksplit_set_for_gate(fp8_slices) }, 0);
    let slices = if matches!(arm, "C" | "D") { 16 } else { 0 };
    assert_eq!(
    ...
    println!(
        "PROTOCOL arm={arm} graph_splitk={} hc_slices={slices} fp8_slices={fp8_slices} prompts={} max_new={LIMIT} capacity={capacity} sampler=argmax ties=lowest_id dspark=off stop=eos",
        arm != "A",
        prompts.len()
    );
    ...
    .unwrap();
    assert!(
        gpu.topology().is_tp_ep() && gpu.topology().layers == 43 && gpu.small_kernel_diet_enabled()
    );
    ...
    for (i, tokens) in prompts.iter().enumerate() {
        ...
        assert_eq!(
            unsafe { memra_dsv4_fp8_ksplit_slices_for_gate() },
            fp8_slices
        );
    ...
    println!(
        "SUMMARY_TASK_EVIDENCE_PASS arm={arm} items={}",
        prompts.len()
    );
}
#[cfg(test)]
mod tests {
    #[test]
    fn argmax_lowest_id_on_ties() {
        assert_eq!(super::argmax(&[1.0, 3.0, 3.0, -1.0]), 1);
    }
    #[test]
    #[should_panic]
    fn nonfinite_logits_refused() {
