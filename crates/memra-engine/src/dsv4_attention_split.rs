...
/// The current existing-GEMV exact component gate consumes these two row-partitioned planes.
/// The eventual launcher must use the existing `memra_dsv4_gemv_fp8_m` arithmetic unchanged;
/// this module only supplies the rank-local storage and shape contract.
...
/// Named numeric class for the future `wo_b` input-column partials. The existing full GEMV and
/// this rank-ordered sum are intentionally different programs; no tolerance is hidden here.
...
/// Fixed rank-order FP32 join for the future `wo_b` partial program. This is a deterministic
/// oracle for that named class, not a claim of equality with the full 8192-column GEMV.
pub fn wo_b_rank_order_sum_f32(rank0: &[f32], rank1: &[f32], out: &mut [f32]) -> Res<()> {
    if rank0.len() != 4096 || rank1.len() != 4096 || out.len() != 4096 {
        return Err(format!(
            "{WO_B_PARTIAL_SUM_NUMERIC_CLASS} requires three 4096-element planes: rank0={} rank1={} out={}",
            rank0.len(),
            rank1.len(),
            out.len()
        ));
    ...
    fn cuda_attention_split_existing_fp8_gemv_component_gate() {
        ...
        for (rows, cols, seed) in [(32768usize, 1024usize, 0x10u64), (8192, 4096, 0x20)] {
            ...
            for rank in 0..2usize {
                ...
                assert_eq!(
                    local_output,
                    full[rank * half..(rank + 1) * half],
                    "existing GEMV exact row half rank={rank} rows={rows}"
                );
        ...
        for rank in 0..2usize {
            ...
            assert_eq!(device_output, host_output, "wo_b packed half rank={rank}");
            assert_plane_unchanged(&gpu, &full_plane, &full_codes, &full_scales);
        ...
        println!(
            "PASS existing GEMV Q_b/wo_a exact row halves; wo_b host/device halves exact; class={WO_B_PARTIAL_SUM_NUMERIC_CLASS}; full_width_bit_equal_diagnostic={full_equal}"
        );
    }
