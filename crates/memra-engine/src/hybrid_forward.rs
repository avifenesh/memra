...
impl HybridModel {
    ...
    pub(crate) fn ffn_act_lim(
        ...
        if let Some(m3) = cfg.m3.as_ref() {
            debug_assert!(
                limit.is_none(),
                "m3 swigluoai and step35 clamp are different archs"
            );
    ...
    fn moe_ffn_grouped_add_shared_inner(
            ...
            (&m.gate_shexp, &m.up_shexp, &m.down_shexp)
        {
            ...
            let sh = if let Some((wg, wu)) = bf16_dual {
                ...
                if cfg.m3.is_none() {
                    // FUSION #2b: dual matvec + SwiGLU act in one launch — exact dual
                    // per-row program + exact silu/clamped expression, bit-identical.
                    e.matvec_bf16_dual_silu_into(wg, wu, z, act, n_embd, n_ff_sh, lim)?;
                    let _ = (&gate, &up);
                } else {
