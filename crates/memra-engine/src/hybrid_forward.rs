...
impl HybridModel {
    ...
    fn forward_hyper(
        ...
        // The M2 pipeline door shards trunk weights across devices and hands the activation
        // over at each stage boundary; this walk drives every layer from the primary engine.
        // Refuse rather than dereference another device's pointers.
        if crate::pp::pp_cuts(self.layers.len()).is_some() {
            return Err(
                "MEMRA_PP_SHARD is set, but the hyper-connection trunk walk is \
                        single-engine: the sharded stage handoff is unwired for this residual \
                        topology"
                    .into(),
            );
        }
