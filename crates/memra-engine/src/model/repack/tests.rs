...
#[test]
fn bound_canonical_consumer_uses_real_operands_and_propagates_failures() {
    use memra_gguf::{
    ...
    let raw = r#"{"model_type":"qwen3_moe","num_hidden_layers":1,"hidden_size":64,"num_attention_heads":2,"num_key_value_heads":1,"head_dim":32,"intermediate_size":64,"vocab_size":32,"max_position_embeddings":128,"num_experts":2,"num_experts_per_tok":1,"moe_intermediate_size":64}"#;
    std::fs::write(fixture.root.join("config.json"), raw).unwrap();
    ...
    let mut append = |name: &str, dtype: &str, shape: &[u64], bytes: &[u8]| {
        let start = data.len();
        data.extend(bytes);
        header.push(format!(
            "{name:?}:{{\"dtype\":{dtype:?},\"shape\":{shape:?},\"data_offsets\":[{start},{}]}}",
    ...
    for r in contract.requirements.iter().filter(|r| r.required) {
        ...
        for (index, name) in names.iter().enumerate() {
            if r.id == id {
                let codes = vec![0x22 + index as u8; 64 * 32];
                let scales = vec![0x38; 64 * 4];
                append(name, "U8", &[64, 32], &codes);
                ...
                expected.extend(memra_gguf::nvfp4_repack::repack_modelopt_to_gguf(
                    &codes, &scales, 64, 64,
                ));
            } else {
                append(
    ...
    let json = format!("{{{}}}", header.join(","));
    let mut file = std::fs::File::create(fixture.root.join("model.safetensors")).unwrap();
    file.write_all(&(json.len() as u64).to_le_bytes()).unwrap();
    file.write_all(json.as_bytes()).unwrap();
    file.write_all(&data).unwrap();
    drop(file);
    let source = SafetensorsSource::open(&fixture.root).unwrap();
    let bound = BoundTensorSource::compile(&source).unwrap();
    let runtime = bound.runtime().unwrap();
    let name = "blk.0.ffn_gate_exps.weight";
    assert!(load_bound(&source, name, expected.len()).unwrap().is_none());
    let view = load_bound(&runtime, name, expected.len()).unwrap().unwrap();
    assert_eq!(view.bytes(), expected);
    assert!(load_bound(&runtime, name, expected.len() + 1).is_err());
    assert!(load_bound(&runtime, "unrecognized.weight", expected.len()).is_err());
    assert!(load_bound(&runtime, "blk.0.ffn_up_exps.weight", expected.len()).is_err());
    drop(runtime);
    drop(bound);
    drop(source);
    ...
    let tail = view.subrange(64 * 36..expected.len()).unwrap();
    assert_eq!(tail.bytes(), &expected[64 * 36..]);
    assert!(!memra_gguf::bound_disk::ExpertDiskView::Bound(tail).populate_whole_slab("partial"));
    std::fs::remove_dir_all(fixture.root.join(".memra-repack")).unwrap();
    std::os::unix::fs::symlink(std::env::temp_dir(), fixture.root.join(".memra-repack")).unwrap();
    let source = SafetensorsSource::open(&fixture.root).unwrap();
    let bound = BoundTensorSource::compile(&source).unwrap();
    let runtime = bound.runtime().unwrap();
    assert!(load_bound(&runtime, name, expected.len()).is_err());
}
