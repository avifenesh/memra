...
impl HybridModel {
    ...
    fn glm5_verify_range(
        ...
        for il in lo..hi {
            ...
            if [2, 4, 7].contains(&t)
                && matches!(layer.mixer, Mixer::Kda(_))
                && let Ok(setting) = std::env::var("MEMRA_GLM5_VERIFY_TALLY")
                && let Some(dir) = setting.strip_prefix("capture:")
            {
                use std::io::Write;
                let dir = std::path::Path::new(dir).join(format!("t{t}"));
                std::fs::create_dir_all(&dir)?;
                let path = dir.join(format!("layer-{il:03}.f32"));
                if !path.exists() {
                    let input = e.dtoh(&h)?;
                    ...
                    file.sync_all()?;
                    std::fs::write(
                        dir.join(format!("layer-{il:03}.meta")),
                        format!("t={t} layer={il} ctx={}\n", pos.pos0),
                    )?;
                }
            }
