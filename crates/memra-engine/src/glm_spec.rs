...
/// For sampled DFlash2, selected-token truncation biases the target distribution.
/// `MEMRA_GLM5_SPEC_CAUSAL_PMIN=1` instead tests the slot's max(q), independent
/// of its selected token; see the exact counterexample in `conf_keep_tests`.
    ...
    fn causal_pmin_preserves_target_distribution_after_an_accepted_prefix() {
        ...
        for token in 0..2 {
            for (out, keep) in [
                (
                    &mut legacy,
                    glm5_conf_keep(&[1.0, q[token] as f32], 0.7, false),
                ),
                (
                    &mut causal,
                    selector_confidence_keep(&rows, 2, 2, 0.7, false),
                ),
            ] {
                if keep == 1 {
                    for i in 0..2 {
