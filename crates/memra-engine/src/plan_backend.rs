...
/// Resolve the actual driver's JIT implementation before freezing library identity.
/// Linking (rather than loading cached PTX) forces compiler initialization even when a
/// prior process populated the CUDA disk cache. The resulting no-op entry is never
/// launched and participates in no model computation. Its module keeps the context alive.
pub(crate) fn initialize_rewrite_jit(
    engine: &crate::Engine,
) -> Result<std::sync::Arc<cudarc::driver::CudaModule>, Box<dyn std::error::Error>> {
    use cudarc::driver::sys;
    engine.ctx().bind_to_thread()?;
    struct Link(sys::CUlinkState);
    impl Drop for Link {
        fn drop(&mut self) {
            if !self.0.is_null() {
                // SAFETY: this local guard owns the link state; its context remains bound
                // throughout this synchronous function, including failure cleanup.
                unsafe {
    ...
    // SAFETY: output pointer is valid; zero options require no option arrays.
    unsafe {
        sys::cuLinkCreate_v2(0, std::ptr::null_mut(), std::ptr::null_mut(), &mut state).result()?;
    }
    let mut state = Link(state);
    // sm_80 is only a PTX minimum for an initialization-only ret entry. The linker
    // targets the active context (all supported Memra targets are newer); model kernels
    // retain their compiled architecture and numerical program without substitution.
    let mut ptx = b".version 7.0\n.target sm_80\n.address_size 64\n.visible .entry memra_identity_jit_init() { ret; }\n\0".to_vec();
    let mut image = std::ptr::null_mut();
    let mut bytes = 0;
    // SAFETY: owned nul-terminated input lives through linking, output pointers are
    // valid, and the produced image is copied before the owning link state is destroyed.
    let image = unsafe {
        sys::cuLinkAddData_v2(
            ...
            ptx.as_mut_ptr().cast(),
            ptx.len(),
            c"memra-identity-init".as_ptr(),
            0,
            std::ptr::null_mut(),
            std::ptr::null_mut(),
        )
        .result()?;
        sys::cuLinkComplete(state.0, &mut image, &mut bytes).result()?;
        if image.is_null() || bytes == 0 {
            return Err("CUDA JIT identity initializer produced no image".into());
        }
        std::slice::from_raw_parts(image.cast::<u8>(), bytes).to_vec()
    };
    let module = engine
        .ctx()
        .load_module(cudarc::nvrtc::Ptx::from_binary(image))?;
    module.load_function("memra_identity_jit_init")?;
    // SAFETY: the image/module is independent of this still-owned link state.
    let owned = std::mem::replace(&mut state.0, std::ptr::null_mut());
    unsafe {
        sys::cuLinkDestroy(owned).result()?;
    }
    Ok(module)
}
