...
// (qwen models + gemma globals-hd256 elsewhere). Opt-in MEMRA_FA_P1=1 until the qwen battery
...
//  partial layout -> same combine. Dispatch: t_kv >= MEMRA_FA_SMEM_TKV   //
...
//  (row, token, split); host dispatches above MEMRA_FA_SMEM_TKV.          //
// ===================================================================== //
...
//  differ in FP order from fa_decode_vec_q => MEMRA_FA_V2=1 is its own    //
...
// the graph-vs-eager identity the graph_decode_gate pins. Without this twin, MEMRA_FA_V2=1 would
...
//  => MEMRA_FA_V3=1 is its OWN numeric config (default OFF, own argmax    //
...
// verify and decode share the numeric config when MEMRA_FA_V4=1 flips both (dispatch parity).
...
// ================= T=2 VERIFY WALK (MEMRA_SPEC_FA2, the DSpark lesson applied) ==============
// The spec verify pays fa PER COLUMN: two full KV walks for two query rows that differ only in
// their causal bound (row r attends [0, pos0+r+1)). Upstream DSpark's production verify flattens
// all rows into one kernel; our dspark lane banked the read-equivalence law ("appending all T
// rows before any attend is read-equivalent", fa-execupdate, +4.5% q38 agentic). This twin walks
// the KV stream ONCE with TWO query rows: per tile it stages V once, loads each K pack once and
// dp4a's it against BOTH queries, and keeps per-row (m, l, acc) with the row's own key bound.
// PER-ROW FP ORDER IS THE t=1 WALK'S VERBATIM: same tile boundaries (t_lo/t_hi from the LONGER
// row's T_kv; the shorter row skips keys past its bound, which only truncates its chain at the
// same place the per-column launch would have), same score expression, same B2 bookkeeping, same
// B3 ascending-t accumulation -> each row's partials are BIT-IDENTICAL to its own per-column
// launch at the same (bucket, split_keys). Rows-1-key-apart caveat: row0's ns_eff can differ
// from a t=1 launch's when T_kv0 crosses a split boundary — the caller keys the split grid on
// T_kv1 (the longer row) for BOTH rows, so row0's partials land in the same grid; the combine
// consumes ns_eff(T_kv0) splits of it. That is the one-partition-law shape the dcw kernel
// already relies on (empty trailing splits are skipped by the combine).
static __device__ __forceinline__ void fa_dec_v3_walk2(
        const uint8_t* __restrict__ K, const uint8_t* __restrict__ V,
        __nv_bfloat16* sV, int bt, int bsz,
        const int* qq0, float dQ0, const int* qq1, float dQ1,
        int dpl, int lane, int head_dim,
        int t_lo, int t_hi, int lo0, int bound0, int lo1, int bound1, int kblk0,
        long k_tok_bytes, long v_tok_bytes,
        ...
        float& m1, float& l1, float* acc1)
{
    ...
    for (int t0 = t_lo; t0 < t_hi; t0 += FA_DEC_TILE) {
        const int nt = min(FA_DEC_TILE, t_hi - t0);
        // Phase A: stage V once for both rows (v3 recipe verbatim).
        for (int b = bt; b < nt * blocks_per_key; b += bsz) {
            const int j     = b / blocks_per_key;
            const int blk_i = b - j * blocks_per_key;
            const uint8_t* blk = V + (size_t)(t0 + j) * v_tok_bytes
                                   + (size_t)(kblk0 + blk_i) * 24;
            uint32_t wdm; memcpy(&wdm, blk, 4);
            const float d = __half2float(__ushort_as_half((unsigned short)(wdm & 0xFFFFu)));
        ...
        // Phase B1: one K pack load per key, dp4a'd against BOTH queries.
        float my0 = NEG_INF, my1 = NEG_INF;
        float tm0 = m0, tm1 = m1;
        #pragma unroll 4
        for (int j = 0; j < nt; ++j) {
            const int t = t0 + j;
            const uint8_t* blk = K + (size_t)t * k_tok_bytes
                                   + (size_t)(kblk0 + bK) * 34;
            const float dK = __half2float(*(const half*)blk);
            const uint8_t* qsp = blk + 2 + koff;
            ...
            const int ki0 = (int)__funnelshift_r(w0, w1, sh8);
            const int ki1 = (dpl > 4) ? (int)__funnelshift_r(w1, w2, sh8) : 0;
            if (t >= lo0 && t < bound0) {
                int sumi = __dp4a(ki0, qq0[0], 0);
                if (dpl > 4) sumi = __dp4a(ki1, qq0[1], sumi);
                const float part = __fmul_rn(__fmul_rn(dK, dQ0), (float)sumi);
                const float score = warp_reduce_sum(part);
                if (lane == j) my0 = score;
                tm0 = fmaxf(tm0, score);
            }
            if (t >= lo1 && t < bound1) {
                int sumi = __dp4a(ki0, qq1[0], 0);
                if (dpl > 4) sumi = __dp4a(ki1, qq1[1], sumi);
                const float part = __fmul_rn(__fmul_rn(dK, dQ1), (float)sumi);
                const float score = warp_reduce_sum(part);
                if (lane == j) my1 = score;
                tm1 = fmaxf(tm1, score);
            }
        }
        // Phase B2 per row (v3 verbatim).
        const float mn0 = tm0;
        const float a0f = (m0 == NEG_INF) ? 0.0f : exp2f((m0 - mn0) * LOG2E);
        const float p0  = (lane < nt && (t0 + lane) >= lo0 && (t0 + lane) < bound0)
            ? exp2f((my0 - mn0) * LOG2E) : 0.0f;
        l0 = l0 * a0f + warp_reduce_sum(p0);
        #pragma unroll
        for (int i = 0; i < FA_DEC_MAX_DPL; ++i) if (i < dpl) acc0[i] *= a0f;
        m0 = mn0;
        const float mn1 = tm1;
        const float a1f = (m1 == NEG_INF) ? 0.0f : exp2f((m1 - mn1) * LOG2E);
        const float p1  = (lane < nt && (t0 + lane) >= lo1 && (t0 + lane) < bound1)
            ? exp2f((my1 - mn1) * LOG2E) : 0.0f;
        l1 = l1 * a1f + warp_reduce_sum(p1);
        #pragma unroll
        for (int i = 0; i < FA_DEC_MAX_DPL; ++i) if (i < dpl) acc1[i] *= a1f;
        m1 = mn1;

        __syncthreads();   // sV staged before any warp reads it

        // Phase B3 per row (v3 paired loads, ascending t).
        #pragma unroll 2
        for (int j = 0; j < nt; ++j) {
            const int t = t0 + j;
            const __nv_bfloat162* vj2 = (const __nv_bfloat162*)(sV + (size_t)j * head_dim);
            if (t >= lo0 && t < bound0) {
                const float p = __shfl_sync(0xffffffffu, p0, j);
                #pragma unroll
                for (int i2 = 0; i2 < FA_DEC_MAX_DPL / 2; ++i2) {
                    if (2 * i2 < dpl) {
                        const __nv_bfloat162 vv = vj2[lane + (i2 << 5)];
                        acc0[2 * i2]     += p * __bfloat162float(vv.x);
                        acc0[2 * i2 + 1] += p * __bfloat162float(vv.y);
                    }
                }
            }
            if (t >= lo1 && t < bound1) {
                const float p = __shfl_sync(0xffffffffu, p1, j);
                #pragma unroll
                for (int i2 = 0; i2 < FA_DEC_MAX_DPL / 2; ++i2) {
                    if (2 * i2 < dpl) {
                        const __nv_bfloat162 vv = vj2[lane + (i2 << 5)];
                        acc1[2 * i2]     += p * __bfloat162float(vv.x);
                        acc1[2 * i2 + 1] += p * __bfloat162float(vv.y);
                    }
                }
            }
        }
...
// T=2 dcw twin (MEMRA_SPEC_FA2): BOTH verify columns' rows already appended (read-equivalence
// law: row r's bound excludes row r+1's entry, so appending all rows before any attend is
// read-equivalent to the per-column order). len_dev is read AFTER both appends; row bounds are
// len-1 and len. The caller guards the equal-partition condition (ns_eff and per identical for
// both bounds) — boundary rounds fall back to the per-column kernel, keeping this launch
// BIT-IDENTICAL per row to that row's own per-column launch.
// Q2 = [2, n_head, head_dim] (row-major rows); partials laid out row-major: row r's head h
// lands at ((r * n_head + h) * n_splits + split). The per-row combine consumes its half.
extern "C" __global__ void fa_decode_vec_q_v3_dcw2(
        ...
        long k_tok_bytes, long v_tok_bytes)
{
    const int len   = len_dev[0];              // AFTER both appends
    // View based at ROW 0's window start (one key earlier than row1's on capped SWA), so
    // both rows' key sets are subranges of one view: row r attends [lo_r, hi_r). On capped
    // SWA the two rows' tile grids cannot both align to their own per-column launches, so
    // this kernel is a REDUCTION-CLASS door there (argmax gate + battery, the QKV_FUSED
    // acceptance); on global layers (lstart identical) it is bit-identical per row under
    // the caller's equal-partition guard.
    const int len0  = len - 1;
    const int lstart = (window > 0 && len0 > window) ? (len0 - window) : 0;
    ...
    const int hi1 = len - lstart;
    const int hi0 = hi1 - 1;
    const int lo0 = 0;
    const int lo1 = (window > 0 && len > window) ? ((len - window) - lstart) : 0;
    const int T1 = hi1;
    ...
    // Equal-partition grid (caller-guarded): derive from T1; T0 shares ns_eff and per.
    const int ns_eff = max(1, (T1 + split_keys - 1) / split_keys);
    const int per  = (T1 + ns_eff - 1) / ns_eff;
    const int t_lo = split * per;
    const int t_hi = (split < ns_eff) ? min(T1, t_lo + per) : t_lo;

    int qq0[8]; float dQ0;
    int qq1[8]; float dQ1;
    fa_dec_v3_qquant(Q2, (size_t)head * head_dim, scale, dpl, lane, qq0, dQ0);
    fa_dec_v3_qquant(Q2, ((size_t)n_head + head) * head_dim, scale, dpl, lane, qq1, dQ1);

    float m0 = NEG_INF, l0 = 0.0f, m1 = NEG_INF, l1 = 0.0f;
    float acc0[FA_DEC_MAX_DPL], acc1[FA_DEC_MAX_DPL];
    #pragma unroll
    for (int i = 0; i < FA_DEC_MAX_DPL; ++i) { acc0[i] = 0.0f; acc1[i] = 0.0f; }

    extern __shared__ __nv_bfloat16 ssh_v3_dcw2[];
    __nv_bfloat16* sV = ssh_v3_dcw2;
    const int bt  = wy * WARP_SZ + lane;
    const int bsz = WARP_SZ * gqa;
    const int kblk0 = (kv_head * head_dim) >> 5;
    fa_dec_v3_walk2(Kv, Vv, sV, bt, bsz, qq0, dQ0, qq1, dQ1, dpl, lane, head_dim,
                    t_lo, t_hi, lo0, hi0, lo1, hi1, kblk0, k_tok_bytes, v_tok_bytes,
                    m0, l0, acc0, m1, l1, acc1);

    #pragma unroll
    for (int i = 0; i < FA_DEC_MAX_DPL; ++i) {
        if (i < dpl) {
            int d = (lane << 1) + ((i >> 1) << 6) + (i & 1);
            partO[((size_t)head * n_splits + split) * head_dim + d] = acc0[i];
            partO[(((size_t)n_head + head) * n_splits + split) * head_dim + d] = acc1[i];
        }
    }
    if (lane == 0) {
        partM[head * n_splits + split] = m0;
        partL[head * n_splits + split] = l0;
        partM[(n_head + head) * n_splits + split] = m1;
        partL[(n_head + head) * n_splits + split] = l1;
    }
}
...
extern "C" __global__ void fa_decode_vec_q_v3_dcw_h(
        const float* __restrict__ Q,
        const uint8_t* __restrict__ K,
        const uint8_t* __restrict__ V,
        float* __restrict__ partO,
        ...
        long k_tok_bytes, long v_tok_bytes)
{
    ...
    extern __shared__ __nv_bfloat16 ssh_v3_dcw_h[];
    __nv_bfloat16* sV = ssh_v3_dcw_h;
    const int bt  = wy * WARP_SZ + lane;
    const int bsz = WARP_SZ * gqa;
    const int kblk0 = (kv_head * head_dim) >> 5;
    fa_dec_v3_walk_u<4, true>(Kv, Vv, sV, bt, bsz, qq, dQ, dpl, lane, head_dim,
                   t_lo, t_hi, kblk0, k_tok_bytes, v_tok_bytes, m_i, l_i, acc);
...
extern "C" __global__ void fa_decode_vec_q_v3_dcw_hc(
        const float* __restrict__ Q,
        const uint8_t* __restrict__ K,
        const uint8_t* __restrict__ V,
        float* __restrict__ partO,
        ...
        long k_tok_bytes, long v_tok_bytes)
{
    ...
    extern __shared__ __nv_bfloat16 ssh_v3_dcw_hc[];
    __nv_bfloat16* sV = ssh_v3_dcw_hc;
    const int bt  = wy * WARP_SZ + lane;
    const int bsz = WARP_SZ * gqa;
    const int kblk0 = (kv_head * head_dim) >> 5;
    fa_dec_v3_walk_u<4, true, true>(Kv, Vv, sV, bt, bsz, qq, dQ, dpl, lane, head_dim,
                   t_lo, t_hi, kblk0, k_tok_bytes, v_tok_bytes, m_i, l_i, acc);
