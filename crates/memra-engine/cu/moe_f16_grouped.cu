...
// The M=1 split-K entries (memra #392 base, #425 paired-fetch) lived here
// until 2026-09-11. Every one of them was reachable only behind the fused
// gate/up launch, whose arm is a gate-only function called after load, so no
// serving process could ever dispatch one. When the matrix expert program
// became the loaded default (memra #461) the doors had to serve or go, and
// they went: partial/reduce kernels for both families, the component and
// bandwidth instruments, the merged-wrapper control and the entry policy.
// Verdict and receipts: docs/FLAGS.md removed-doors ledger.
