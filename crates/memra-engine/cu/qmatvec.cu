...
// ---- TWO-EXPERTS-IN-FLIGHT down twin (lane/moe-down-ilp2-20260905) ----
//
// `moe_down_rows_ilp_warp` walks its n_used experts ONE AT A TIME: a 4-deep hoisted load round,
// a warp reduction, one __fmaf_rn into the slot-ordered chain, then the next expert. A warp never
// has more than one row's loads outstanding, and the census on the B200 pair prices the down
// kernel at ~25% of the measured wall (24.9 us for 25 MB). Here experts j and j+1 are walked
// TOGETHER: both rows' groups are loaded before either is consumed (8 groups in flight per lane),
// each expert keeps its OWN accumulator and its OWN g-order (character for character the `_ilp`
// body's sequence), each is reduced with the same warp_reduce_sum, and the chain takes j then
// j+1. No floating-point op moved or reassociated, so the output is bit-identical to the `_ilp`
// twin by construction (tests/moe_down_ilp2_gpu.rs: bitwise at t=1..4, even and odd n_used, red
// arm). An odd n_used runs its last expert on the `_ilp` body verbatim.
template <bool V2>
__device__ __forceinline__ void moe_down_nvfp4_rows_ilp2_body(
        ...
        const float* __restrict__ da, const float* __restrict__ db, int nsb, int lane,
        float& acca, float& accb) {
    int g = lane;
    for (; g + 96 < nsb; g += 128) {
        nvfp4_grp_regs a0 = nvfp4_load_g<V2>(wa, g, nsb);
        nvfp4_grp_regs a1 = nvfp4_load_g<V2>(wa, g + 32, nsb);
        nvfp4_grp_regs a2 = nvfp4_load_g<V2>(wa, g + 64, nsb);
        nvfp4_grp_regs a3 = nvfp4_load_g<V2>(wa, g + 96, nsb);
        nvfp4_grp_regs b0 = nvfp4_load_g<V2>(wb, g, nsb);
        nvfp4_grp_regs b1 = nvfp4_load_g<V2>(wb, g + 32, nsb);
        nvfp4_grp_regs b2 = nvfp4_load_g<V2>(wb, g + 64, nsb);
        nvfp4_grp_regs b3 = nvfp4_load_g<V2>(wb, g + 96, nsb);
        acca += nvfp4_regs_dot(a0, (const int*)(aa + (size_t)g * 32), da[g]);
        acca += nvfp4_regs_dot(a1, (const int*)(aa + (size_t)(g + 32) * 32), da[g + 32]);
        acca += nvfp4_regs_dot(a2, (const int*)(aa + (size_t)(g + 64) * 32), da[g + 64]);
        acca += nvfp4_regs_dot(a3, (const int*)(aa + (size_t)(g + 96) * 32), da[g + 96]);
        accb += nvfp4_regs_dot(b0, (const int*)(ab + (size_t)g * 32), db[g]);
        accb += nvfp4_regs_dot(b1, (const int*)(ab + (size_t)(g + 32) * 32), db[g + 32]);
        accb += nvfp4_regs_dot(b2, (const int*)(ab + (size_t)(g + 64) * 32), db[g + 64]);
        accb += nvfp4_regs_dot(b3, (const int*)(ab + (size_t)(g + 96) * 32), db[g + 96]);
    }
    for (; g + 32 < nsb; g += 64) {
        nvfp4_grp_regs a0 = nvfp4_load_g<V2>(wa, g, nsb);
        nvfp4_grp_regs a1 = nvfp4_load_g<V2>(wa, g + 32, nsb);
    ...
    for (; g < nsb; g += 32) {
        acca += nvfp4_tail_dot<V2>(wa, g, nsb, aa + (size_t)g * 32, da[g]);
        accb += nvfp4_tail_dot<V2>(wb, g, nsb, ab + (size_t)g * 32, db[g]);
    }
}
__device__ __forceinline__ void moe_down_rows_ilp2_warp(
        const unsigned long long* __restrict__ ptrs, const float* __restrict__ scl,
        const signed char* __restrict__ aq2, const float* __restrict__ ad2,
        float* __restrict__ dst, int in_f, int out_f, int n_used, int n_pairs, int qt, long rb,
        int o, int tok, int lane) {
    ...
    for (; j + 1 < n_used; j += 2) {
        int pa = tok * n_used + j;
        int pb = pa + 1;
        const unsigned char* wa = (const unsigned char*)ptrs[2 * n_pairs + pa] + (long)o * rb;
        const unsigned char* wb = (const unsigned char*)ptrs[2 * n_pairs + pb] + (long)o * rb;
        const signed char* aa = aq2 + (size_t)pa * in_f;
        const signed char* ab = aq2 + (size_t)pb * in_f;
        const float* da = ad2 + (size_t)pa * nsb;
        const float* db = ad2 + (size_t)pb * nsb;
        float acca = 0.0f, accb = 0.0f;
        if (qt == QT_NVFP4) {
            moe_down_nvfp4_rows_ilp2_body<false>(wa, wb, aa, ab, da, db, nsb, lane, acca, accb);
        } else if (qt == QT_NVFP4_V2) {
            moe_down_nvfp4_rows_ilp2_body<true>(wa, wb, aa, ab, da, db, nsb, lane, acca, accb);
        } else {
            acca = accb = __int_as_float(0x7fc00000);  // wiring error: poison, never a silent zero
        }
        acca = warp_reduce_sum(acca);
        accb = warp_reduce_sum(accb);
        if (lane == 0) {
            chain = __fmaf_rn(scl[2 * n_pairs + pa], acca, chain);
            chain = __fmaf_rn(scl[2 * n_pairs + pb], accb, chain);
        }
    }
    for (; j < n_used; j++) {  // odd tail: the `_ilp` body verbatim
        int pr = tok * n_used + j;
        const unsigned char* wrow = (const unsigned char*)ptrs[2 * n_pairs + pr] + (long)o * rb;
        const signed char* arow = aq2 + (size_t)pr * in_f;
        const float* adrow = ad2 + (size_t)pr * nsb;
...
extern "C" __global__ void moe_down8_fma_q8_rows_ilp2(
        const unsigned long long* __restrict__ ptrs, const float* __restrict__ scl,
        const signed char* __restrict__ aq2, const float* __restrict__ ad2,
        float* __restrict__ dst, int in_f, int out_f, int n_used, int n_pairs, int qt, long rb) {
    int o = blockIdx.x;
    int tok = blockIdx.y;
    if (o >= out_f) return;
    moe_down_rows_ilp2_warp(ptrs, scl, aq2, ad2, dst, in_f, out_f, n_used, n_pairs, qt, rb, o, tok,
                            threadIdx.x);
}
extern "C" __global__ void moe_down8_fma_q8_rows_w4_ilp2(
        const unsigned long long* __restrict__ ptrs, const float* __restrict__ scl,
        const signed char* __restrict__ aq2, const float* __restrict__ ad2,
        float* __restrict__ dst, int in_f, int out_f, int n_used, int n_pairs, int qt, long rb) {
    int o = blockIdx.x * MEMRA_MMVQ_ROWS + threadIdx.y;
