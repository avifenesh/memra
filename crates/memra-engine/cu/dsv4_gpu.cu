...
// Four 64-thread groups reproduce the original per-128 quantizer tree.
// Codes/scales stay in shared memory until the original 256-thread gather tree
// consumes them. No expert projection or reduction is changed.
extern "C" __global__ void dsv4_norm2_quant_half_kernel(
        const float* x, __half* out, float* row_scale, int* row_status) {
    constexpr int cols = 2048, groups = cols/128;
    const int row = blockIdx.x, tid = threadIdx.x, lane = tid % 64, team = tid/64;
    __shared__ float red[256], scales[groups];
    __shared__ uint8_t codes[cols];
    __shared__ int bad;
    const float* xr = x + (long)row*cols;
    for (int base=0; base<groups; base+=4) {
        int group=base+team;
        float a=0.0f;
        for (int i=lane;i<128;i+=64) a=fmaxf(a,fabsf(xr[group*128+i]));
        red[tid]=a;
        __syncthreads();
        for (int stride=32;stride>0;stride>>=1) {
            if (lane<stride) red[tid]=fmaxf(red[tid],red[tid+stride]);
            __syncthreads();
        }
        float amax=fmaxf(red[team*64],1e-4f);
        const float inv=(float)(1.0/448.0);
        float scale=dsv4_pow2_ceil(amax*inv);
        if (lane==0) scales[group]=scale;
        for (int i=lane;i<128;i+=64) {
            float q=fminf(fmaxf(xr[group*128+i]/scale,-448.0f),448.0f);
            uint8_t c=(uint8_t)__nv_cvt_float_to_fp8(q,__NV_SATFINITE,__NV_E4M3);
            if ((c & 0x7F)==0) c=0;
            codes[group*128+i]=c;
        }
    ...
    if (tid==0) bad=0;
    float local=0.0f;
    for (int g=tid;g<groups;g+=256) local=fmaxf(local,scales[g]);
    float max_scale=dsv4_block_max(local,red);
    float rs=ldexpf(max_scale,-7);
    if (!(rs>0.0f) || !isfinite(rs)) atomicOr(&bad,1);
    for (int i=tid;i<cols;i+=256) {
        uint8_t code=codes[i];
        float v=dsv4_e4m3(code)*scales[i/128];
        __half h=__float2half_rn(v/rs);
        float back=__half2float(h)*rs;
        if ((code & 127)==127 || !isfinite(v) || __float_as_uint(back)!=__float_as_uint(v))
            atomicOr(&bad,1);
        out[(long)row*cols+i]=h;
    }
    __syncthreads();
    if (tid==0) { row_scale[row]=rs; row_status[row]=bad; }
}
extern "C" int memra_dsv4_norm2_quant_half(const float* x, void* out,
        float* row_scale, int* row_status, int rows, int cols, void* stream_v) {
    if (!x || !out || !row_scale || !row_status || rows<1 || rows>6 || cols!=2048)
        return 40004;
    dsv4_norm2_quant_half_kernel<<<rows,256,0,(cudaStream_t)stream_v>>>(
        x,(__half*)out,row_scale,row_status);
    DSV4_ERR();
    return 0;
}
...
// Shared expert SwiGLU -> BF16. Keep the f32 intermediate rounding and the
...
extern "C" __global__ void dsv4_norm2_pack_f32_fixed_order_kernel(const float* __restrict__ x,
                                                      const float* __restrict__ w,
    ...
    for (int i = threadIdx.x; i < ncols; i += blockDim.x)
    {
        float v = (w ? w[i] : 1.0f) * (xr[i] * rsq);
        dr[i] = v;
        packed[(long)row * ncols + i] = __float2bfloat16_rn(v);
    }
}
