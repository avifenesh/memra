...
#include <cstdlib>   // getenv, for the decode-twin rollback door
#include <cuda_fp16.h>
#include <cuda_bf16.h>
#include <cuda_fp8.h>
...
// ======================= DECODE TWIN: 128 weight rows x 8 tokens =======================
// WHY (lane/glm5-fp4-tc-decode-20260904). The GEMM above puts TOKENS on the M axis (128 per CTA)
// and OUTPUT ROWS on N. At decode there is ONE token, so 127 of 128 M-slots are padding and the
// kernel does 128x the math for one useful row of output: measured 0.885x the shipped
// `qmatvec_nvfp4_mmvq` matvec at n_tokens = 1 on the sm_120a twin (b200_matvec_bench family 10,
// 2026-09-04), i.e. the prefill shape LOSES at decode. This twin swaps the roles: WEIGHT rows on
// M (128 per CTA, and a glm5 expert plane has 2048 of them) and TOKENS on N, the narrowest legal
// N for `scale_vec::4X` (8). One token then wastes 8x of the N axis instead of 128x of M, while
// the weights — which are what decode is actually bound by — stream exactly once.
//
// SAME NUMERIC PROGRAM: the same `tcgen05.mma.kind::mxf4nvf4.block_scale.scale_vec::4X` on the
// same `block_nvfp4` weights and `block_fp4_mmq` activations, the same UE4M3 scale vectors, the
// same per-token epilogue scale. Only the tiling and the operand roles move, so a disagreement
// with the GEMM above at the same shape is a bug in this twin, and `nvfp4_mmq_check`'s host
// oracle is the arbiter.
// `MEMRA_MMQ_NVFP4_DEC=0` forces the GEMM tile even at narrow token counts (the rollback seam);
// unset or any other value takes the decode twin. Read once, like every other kernel-side door.
static bool memra_mmq_nvfp4_dec_enabled() {
    static const bool on = [] {
        const char * v = getenv("MEMRA_MMQ_NVFP4_DEC");
        return !(v && v[0] == '0' && v[1] == '\0');
    }();
    return on;
}

constexpr int SM100_NVFP4_DEC_M = 128;   // weight rows per CTA
constexpr int SM100_NVFP4_DEC_N = 8;     // tokens per CTA (1 real at decode)
constexpr int SM100_NVFP4_DEC_TMEM_COLS = 64;

static __device__ __forceinline__ uint32_t sm100_tcgen05_nvf4_idesc_dec() {
    uint32_t desc = 0;
    desc |= 1u << 7;  // A = e2m1
    desc |= 1u << 10; // B = e2m1
    desc |= ((uint32_t) (SM100_NVFP4_DEC_N >> 3) & 0x3Fu) << 17;
    // bit 23 clear: UE4M3 scale factors (NVFP4).
    desc |= ((uint32_t) (SM100_NVFP4_DEC_M >> 7) & 0x3u) << 27;
    return desc;
}

__launch_bounds__(SM100_NVFP4_DEC_M, 1)
static __global__ void mul_mat_q_nvfp4_sm100_dec(
        const block_nvfp4 * __restrict__ weights,
        const block_fp4_mmq * __restrict__ acts,
        float * __restrict__ dst,
        const float * __restrict__ act_scale,
        int in_f, int out_f, int n_tokens, float out_scale) {
    __shared__ __align__(128) uint8_t s_a[SM100_NVFP4_DEC_M * SM100_NVFP4_PACKED_K];
    __shared__ __align__(128) uint8_t s_b[SM100_NVFP4_DEC_N * SM100_NVFP4_PACKED_K];
    __shared__ __align__(128) uint8_t s_sfa[SM100_NVFP4_DEC_M * 4];
    __shared__ __align__(128) uint8_t s_sfb[SM100_NVFP4_DEC_N * 4];
    __shared__ __align__(16) uint64_t mma_barrier[1];
    __shared__ __align__(16) uint32_t tmem_base_slot[1];

    const int tid = threadIdx.x;
    const int out_row = (int) blockIdx.x * SM100_NVFP4_DEC_M + tid;   // A rows = weight rows
    const int blocks_per_weight_row = in_f / QK_NVFP4;
    const int act_blocks_per_k_plane = n_tokens;
    const uint32_t barrier_addr = (uint32_t) __cvta_generic_to_shared(mma_barrier);
    ...
    if (tid < WARP_SIZE) {
        asm volatile(
            "tcgen05.alloc.cta_group::1.sync.aligned.shared::cta.b32 [%0], %1;"
            :: "r"((uint32_t) __cvta_generic_to_shared(tmem_base_slot)),
               "r"(SM100_NVFP4_DEC_TMEM_COLS));
    }
    __syncthreads();

    const uint32_t tmem_base = tmem_base_slot[0];
    const uint32_t d_tmem = tmem_base;
    const uint32_t sfa_tmem = tmem_base + SM100_NVFP4_DEC_N;
    const uint32_t sfb_tmem = sfa_tmem + 4;
...
#pragma unroll
            for (int c = 0; c < SM100_NVFP4_PACKED_K; ++c) {
                const int core_off =
                    memra_sm100::core_row_outer_offset(tid, c, SM100_NVFP4_PACKED_K);
                s_a[core_off] = b_block->qs[c];
            }
#pragma unroll
            for (int j = 0; j < 4; ++j) {
                s_sfa[memra_sm100::sf4x_offset(tid, j)] = b_block->d[j];
            }
        } else {
#pragma unroll
            for (int c = 0; c < SM100_NVFP4_PACKED_K; ++c) {
...
#pragma unroll
        ...
        // B = ACTIVATIONS (the first SM100_NVFP4_DEC_N threads stage one token each; the padding
        // tokens are zeroed so their accumulators are defined and simply discarded).
        if (tid < SM100_NVFP4_DEC_N) {
            const int token = tid;
            if (token < n_tokens) {
                const block_fp4_mmq * a_block =
                    acts + (int64_t) act_k_block * act_blocks_per_k_plane + token;
...
#pragma unroll
                for (int c = 0; c < SM100_NVFP4_PACKED_K; ++c) {
                    const int core_off =
                        memra_sm100::core_row_outer_offset(token, c, SM100_NVFP4_PACKED_K);
                    s_b[core_off] = a_codes[c];
                }
#pragma unroll
                for (int j = 0; j < 4; ++j) {
                    s_sfb[memra_sm100::sf4x_offset(token, j)] = a_scales[j];
                }
            } else {
#pragma unroll
                for (int c = 0; c < SM100_NVFP4_PACKED_K; ++c) {
...
#pragma unroll
                for (int j = 0; j < 4; ++j) {
                    s_sfb[memra_sm100::sf4x_offset(token, j)] = 0;
                }
            }
        }
        ...
        if (tid == 0) {
            ...
            const uint64_t b_desc = sm100_tcgen05_smem_desc(
                (uint32_t) __cvta_generic_to_shared(s_b), 128, 256);
            const uint32_t i_desc = sm100_tcgen05_nvf4_idesc_dec();
            asm volatile(
                "{.reg .pred p; setp.ne.u32 p, %6, 0;\n\t"
                "tcgen05.mma.cta_group::1.kind::mxf4nvf4.block_scale.scale_vec::4X "
                "[%0], %1, %2, %3, [%4], [%5], p;}"
            ...
            asm volatile(
                "{.reg .pred p;\n\t"
                "WAIT_NVFP4_DEC: mbarrier.try_wait.parity.shared::cta.b64 p, [%0], %1;\n\t"
                "@!p bra WAIT_NVFP4_DEC;}"
                :: "r"(barrier_addr), "r"((uint32_t) (k_slice & 1)) : "memory");
        }
        __syncthreads();
    }
    ...
    // Epilogue: TMEM row = A row = weight row, column = token. Each thread owns its own row.
    const uint32_t row_tmem = d_tmem + ((uint32_t) tid << 16);
    uint32_t accum[32];
    cuda::ptx::tcgen05_ld_32x32b(accum, row_tmem);
    cuda::ptx::tcgen05_wait_ld();
    if (out_row < out_f) {
#pragma unroll
        for (int t = 0; t < SM100_NVFP4_DEC_N; ++t) {
            if (t < n_tokens) {
                const float scale = act_scale ? act_scale[t] : 1.0f;
                dst[(int64_t) t * out_f + out_row] =
                    __uint_as_float(accum[t]) * out_scale * scale;
            }
        }
    }
    __syncthreads();
    if (tid < WARP_SIZE) {
        asm volatile(
            "tcgen05.dealloc.cta_group::1.sync.aligned.b32 %0, %1;"
            :: "r"(tmem_base), "r"(SM100_NVFP4_DEC_TMEM_COLS));
        asm volatile("tcgen05.relinquish_alloc_permit.cta_group::1.sync.aligned;");
    }
}
#endif // MEMRA_SM100_TCGEN05
...
#if defined(MEMRA_SM100_TCGEN05)
    // DECODE TWIN (lane/glm5-fp4-tc-decode-20260904): at narrow token counts the GEMM tile above
    // pads the M (token) axis 128-wide for one real row, and the sm_120a measurement of that shape
    // is 0.885x the shipped matvec. `mul_mat_q_nvfp4_sm100_dec` puts the WEIGHT rows on M and the
    // tokens on the narrowest legal N (8), so the padding is 8x on N instead of 128x on M. Same
    // MMA, same operands, same epilogue scale. MEMRA_MMQ_NVFP4_DEC=0 forces the GEMM shape back
    // (the rollback seam); unset takes the twin at n_tokens <= 8.
    if (n_tokens <= SM100_NVFP4_DEC_N && memra_mmq_nvfp4_dec_enabled()) {
        const dim3 grid((unsigned) ((out_f + SM100_NVFP4_DEC_M - 1) / SM100_NVFP4_DEC_M), 1, 1);
        mul_mat_q_nvfp4_sm100_dec<<<grid, SM100_NVFP4_DEC_M, 0, st>>>(
