...
/// `MEMRA_MOE_ROWS_SHACT` (lane/moe-rows-ceiling-20260906): the 4-warp gate/up block that stages
/// the activation in shared must produce the SHIPPED pair's bytes exactly, and it must actually
/// engage (the counter delta), so a silent fallback to `_ilp` cannot read as a pass. The red arm
/// perturbs the activation and requires the output to move -- with the clamp opened, because at
/// the served limit the pre-clamped SwiGLU saturates on these operands and NOTHING moves, which
/// would make the arm vacuous; the shipped pair's own sensitivity is asserted first.
#[test]
#[ignore]
fn gpu_moe_shact_matches_shipped_pair_bitwise() {
    let _gpu = gpu_guard();
    force_true_f32();
    let e = Engine::new(0).expect("CUDA engine on device 0");
    let (n_expert, n_used) = (16usize, 8usize);
    ...
    // in_f 4096 is the served width and the only one that exercises the ILP main loop; 128 also
    // runs so the shared prologue's tail (in_f / 16 chunks over 128 threads) is covered.
    for &(in_f, n_ff) in &[(128usize, 64usize), (4096, 2048)] {
        let slabs = nvfp4_slab(&e, n_expert, n_ff, in_f, 0x3D0 + in_f as u64);
        let slabs_d = nvfp4_slab(&e, n_expert, in_f, n_ff, 0xD3D + n_ff as u64);
        for t in [1usize, 3] {
            let z = varied(t * in_f, 0x71 + t as u64 + in_f as u64, 2.0);
            let (sel, w) = mk_sel(t);
            let want = without_flag("MEMRA_MOE_ROWS_SHACT", || {
                with_flag("MEMRA_MOE_VROWS_ILP", || {
                    moe_pairs_program(
                        &e,
                        &slabs,
                        &slabs_d,
            ...
            let d0 = memra_engine::moe_rows_shact_dispatches();
            let got = with_flag("MEMRA_MOE_ROWS_SHACT", || {
                with_flag("MEMRA_MOE_VROWS_ILP", || {
                    moe_pairs_program(
                        &e,
                        &slabs,
            ...
            assert!(
                memra_engine::moe_rows_shact_dispatches() > d0,
                "in_f={in_f} t={t}: the shact door did not engage"
            );
            let nan = got.iter().filter(|v| v.is_nan()).count();
            assert_eq!(
                nan, 0,
                "in_f={in_f} t={t}: the shact kernel poisoned {nan} outputs"
            );
            let diffs = bit_diffs(&got, &want);
            assert_eq!(
                diffs, 0,
                "in_f={in_f} n_ff={n_ff} t={t}: shact diverged from the shipped pair in {diffs} outputs"
            );
            // big enough that the pre-clamp never binds at either width: a 4096-term dot of these
            // operands reaches the thousands, and at limit 64 (or the served 0.75) every output
            // saturates and the arm cannot bite.
            let red_limit = 1.0e6f32;
            let z2: Vec<f32> = z
                .iter()
                .enumerate()
                .map(|(i, v)| if i % 3 == 0 { -v * 1.5 } else { *v })
                .collect();
            let red_ref = without_flag("MEMRA_MOE_ROWS_SHACT", || {
                with_flag("MEMRA_MOE_VROWS_ILP", || {
                    moe_pairs_program(
                        &e,
                        &slabs,
            ...
            let red_ref2 = without_flag("MEMRA_MOE_ROWS_SHACT", || {
                with_flag("MEMRA_MOE_VROWS_ILP", || {
                    moe_pairs_program(
                        &e,
                        &slabs,
            ...
            assert!(
                bit_diffs(&red_ref2, &red_ref) > 0,
                "in_f={in_f} t={t}: this red arm is VACUOUS -- the shipped pair itself does not \
                 move when the activation changes, so it cannot bite the shact kernel"
            );
            let got2 = with_flag("MEMRA_MOE_ROWS_SHACT", || {
                with_flag("MEMRA_MOE_VROWS_ILP", || {
                    moe_pairs_program(
                        &e,
                        &slabs,
            ...
            assert_eq!(
                bit_diffs(&got2, &red_ref2),
                0,
                "in_f={in_f} t={t}: shact diverged from the shipped pair on the perturbed activation"
            );
            assert!(
                bit_diffs(&got2, &red_ref) > 0,
                "in_f={in_f} t={t}: a different activation left the shact output unchanged"
            );
            println!(
                "shact PASS in_f={in_f} n_ff={n_ff} t={t}: {} outputs bit-identical",
                got.len()
            );
        }
    }
