...
// ---------------------------------------------------------------------------------------------
// Gates 15-18 — THE SPEC EXCLUSIONS LANE (lane/spec-exclusions-20260902): the two doors that
// admit request classes the route used to serve plain. Every gate here is exactness only.
//
// 15. PENALTY ARM, greedy: a penalized GREEDY request's served tape is byte-identical to the
...
// 16. DEVICE PENALTIES == HOST SAMPLER, bit for bit: `penalize_logits_rows_inc` (the round's
...
//     every coefficient class and for a window that slides. This is the property gate 15's
//     identity rests on; a 1-ulp drift is an argmax flip on a near-tie.
// 17. PENALTY ARM, sampled: pinned-seed determinism, burst-split invariance, seed
...
//     claim is distributional and is carried by gate 16 + the rejection walk's exactness.
// 18. WARM, cold drafter: a restored session whose drafter re-arms EMPTY at the restored
...
fn gpu_penalized_greedy_spec_tape_matches_the_plain_penalized_sampler() {
    ...
    assert!(
        ...
        "gate 15 pins the DARK posture first; the arm is thrown below"
    );
    let dark = h
        .model
    ...
    for k in 1..=K {
        ...
        println!(
            "gate 15 PASS K={k}: penalized greedy spec == plain penalized sampler over \
...
fn gpu_device_penalties_are_bit_identical_to_the_host_sampler() {
    ...
    for (last_n, rep, freq, present) in classes {
        ...
        println!(
            "gate 16 PASS last_n={last_n} rep={rep} freq={freq} present={present}: \
...
fn gpu_penalized_sampled_twin_is_deterministic_split_invariant_and_engaged() {
    ...
    println!(
        "gate 17 PASS: penalized sampled twin deterministic, split-invariant, seed-sensitive, \
...
fn gpu_cold_drafter_restore_bytes_match_plain_decode_and_republishes_a_floor_tail() {
    ...
    println!(
        "gate 18 PASS: cold-drafter restore == plain bytes ({accepted1}/{drafted1} accepted), \
