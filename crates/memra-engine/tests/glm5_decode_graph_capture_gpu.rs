...
/// overlap counter advanced.
#[test]
#[ignore = "needs a CUDA device — run under flock /tmp/memra-5090.lock"]
fn moe_shexp_overlap_matches_serial_bitwise() {
    let _gpu = gpu_guard();
    let h = Harness::new();
    let ids = tokens(40, 0x5EED);
    let (prompt, steps) = (8usize, 16usize);
    // The overlap lives in the verify-rows branch, the served t=1 MoE arm; the fixture's eager
    // walk takes it only when forced (the same env the serving posture sets).
    unsafe {
        std::env::set_var("MEMRA_HTOD_DIET", "1");
        std::env::set_var("MEMRA_GLM5_VROWS_T1_DEV", "1");
        std::env::set_var("MEMRA_GLM5_DECODE_GRAPH", "0");
        std::env::set_var("MEMRA_MOE_SHEXP_OVERLAP", "0");
    }
    let (serial, _) = h.decode_bits(&ids, prompt, steps);
    let c0 = memra_engine::MOE_SHEXP_OVERLAP_DISPATCHES.load(Ordering::Relaxed);
    unsafe {
        std::env::set_var("MEMRA_MOE_SHEXP_OVERLAP", "3");
    }
    let (probe3, _) = h.decode_bits(&ids, prompt, steps);
    let bad3 = serial.iter().zip(&probe3).filter(|(a, b)| a != b).count();
    println!(
        "main-stream probe (=3, fork program, no override): {bad3}/{steps} steps diverge from serial"
    );
    unsafe {
        std::env::set_var("MEMRA_MOE_SHEXP_OVERLAP", "2");
    }
    let (probe, _) = h.decode_bits(&ids, prompt, steps);
    let bad_probe = serial.iter().zip(&probe).filter(|(a, b)| a != b).count();
    println!("side-stream serial probe (=2): {bad_probe}/{steps} steps diverge from serial");
    unsafe {
        std::env::set_var("MEMRA_MOE_SHEXP_OVERLAP", "1");
    }
    let (overlapped, _) = h.decode_bits(&ids, prompt, steps);
    let c1 = memra_engine::MOE_SHEXP_OVERLAP_DISPATCHES.load(Ordering::Relaxed);
    assert!(
        c1 > c0,
        "VACUOUS: the overlap never engaged (counter {c0} -> {c1})"
    );
    let bad = serial
        .iter()
        .zip(&overlapped)
    ...
    assert_eq!(
        bad, 0,
        "{bad}/{steps} steps diverge between the serial and the overlapped shared expert (eager)"
    );
    unsafe {
        std::env::set_var("MEMRA_GLM5_DECODE_GRAPH", "1");
    }
    ...
    assert!(
        replays > 0,
        "VACUOUS: the door never replayed with the overlap on"
    );
    let bad = serial.iter().zip(&graphed).filter(|(a, b)| a != b).count();
    assert_eq!(
        bad, 0,
        "{bad}/{steps} steps diverge between eager-serial and graph+overlap ({replays} replays)"
    );
    println!(
