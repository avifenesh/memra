...
/// A typed partial return at an internal chunk boundary, not whole-call success.
#[derive(Clone, Copy, Debug)]
struct BoundaryCancellation {
    chunk: usize,
    rows_done: usize,
    rows_total: usize,
}
...
struct Quantum {
    ...
    cancelled_at_boundary: Option<BoundaryCancellation>,
}
...
/// Lifecycle v2 adds a typed partial quantum return; v1 readers must refuse it.
...
impl Trace {
    ...
    /// v2 partial-return observation. Only prime_observation's exact PrimeCancelled
    /// downcast calls this in production. The call did not complete and its remaining
    /// chunk count is unknown; the typed boundary counts are not a GPU fence or proof
    /// of client cancellation (EventSender can close because of queue overflow).
    /// No new work may follow this terminal prime state. HTTP/receiver/after-release
    /// retirement must still be recorded and independently validated by the consumer.
    pub(crate) fn mark_prime_quantum_cancelled(
        &self,
        chunk: usize,
        rows_done: usize,
        rows_total: usize,
    ) {
        self.record("prime_quantum_cancelled", None, |state, at| {
            state.require_worker();
            let Some(mut quantum) = state.active_quantum.take() else {
                state.invalidate("quantum_end_without_start");
                return;
            ...
            if rows_done == 0
                ...
                || rows_total != quantum.rows
                || chunk >= rows_done
            {
                state.invalidate("invalid_prime_cancellation_boundary");
            }
            quantum.end_ns = Some(at);
            quantum.completed = Some(false);
            quantum.remaining_chunks = None;
            quantum.cancelled_at_boundary = Some(BoundaryCancellation {
                chunk,
                rows_done,
                rows_total,
            });
            state.last_quantum = Some(quantum);
            state.phase = "prime_cancelled";
        });
    }
    ...
    fn record(
        ...
        let quantum_json = quantum.map_or_else(
            || "null".to_string(),
            |q| {
                let cancellation = q.cancelled_at_boundary.map_or_else(
                    || "null".to_string(),
                    |c| format!(
                        "{{\"chunk\":{},\"rows_done\":{},\"rows_total\":{}}}",
                        c.chunk, c.rows_done, c.rows_total
                    ),
                );
                format!(
                    "{{\"id\":{},\"route\":{},\"rows\":{},\"start_ns\":{},\"end_ns\":{},\"completed\":{},\"remaining_chunks\":{},\"cancelled_at_boundary\":{}}}",
                    q.id, json_string(q.route), q.rows, q.start_ns, json_option(q.end_ns),
                    json_option(q.completed), json_option(q.remaining_chunks), cancellation
                )
            },
        );
        let line = format!(
            "{{\"schema\":\"memra-request-lifecycle-v2\",\"pid\":{},\"trace_id\":{},\"seq\":{},\"ordinary_event_limit\":{},\"clock\":\"trace_elapsed_ns\",\"at_ns\":{},\"event\":{},\"client_trace_key\":{},\"request_id\":{},\"model\":{},\"http_route\":{},\"worker_generation\":{},\"worker_route\":{},\"phase\":{},\"quantum_active\":{},\"quantum\":{},\"http_body\":{},\"receiver_close_cause\":{},\"observed_close_cause\":{},\"retirement\":{},\"retirement_site\":{},\"bindings_complete\":{},\"sequence_valid\":{},\"first_error\":{},\"suppressed_events\":{},\"evidence_only\":true}}",
            std::process::id(),
            self.trace_id,
            state.seq,
