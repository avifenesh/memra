...
const ADMISSION_FORMAT: &str = "memra.tenant-admission.v1";
...
/// Generic per-tenant admission policy layered over prepaid accounting.
///
/// `Metered` records exact cost and usage without debiting prepaid balance;
/// provisioning remains responsible for applying a bounded policy above it.
/// `Blocked` rejects before worker admission. `PrepaidLocked` behaves like
/// prepaid and is irreversible, so a stale provisioning retry cannot move a
/// terminal prepaid entitlement back onto sponsored traffic.
...
impl TenantBudgets {
    fn open(source_path: &Path, request_ledger_path: &Path) -> Result<Self, String> {
        ...
        let admission_path = sidecar_path(request_ledger_path, ".tenant-admission.toml");
        for (tenant, mode) in load_admission_modes(&admission_path)? {
            let account = state.accounts.get_mut(&tenant).ok_or_else(|| {
                format!(
                    "tenant admission state {} names unenrolled tenant {tenant:?}",
                    admission_path.display(),
                )
            })?;
            account.admission_mode = mode;
        }
        let budgets = Self {
    ...
    pub(crate) fn admit(
        ...
        let Some(account) = state.accounts.get(tenant) else {
            return Err(BudgetAdmissionError::Unenrolled);
        };
        match account.admission_mode {
            AdmissionMode::Metered => return Ok(None),
            AdmissionMode::Blocked => return Err(BudgetAdmissionError::Blocked),
            AdmissionMode::Prepaid | AdmissionMode::PrepaidLocked => {}
        }
    ...
    pub(crate) fn balance(&self, tenant: &str) -> Result<Option<BalanceView>, String> {
        ...
        Ok(state.accounts.get(tenant).map(|account| BalanceView {
            ...
            admission_mode: account.admission_mode,
        }))
    }

    /// Compare-and-set one enrolled tenant's admission mode and persist it in a
    /// sidecar separate from the prepaid journal. Keeping the formats separate
    /// is the format-compatibility seam: an older binary ignores the sidecar and
    /// returns every tenant to prepaid admission instead of failing to replay a
    /// new row kind. It does not preserve metered or blocked behavior.
    pub(crate) fn set_admission_mode(
        &self,
        tenant: &str,
        expected: AdmissionMode,
        next: AdmissionMode,
        ...
        self.maybe_reload().map_err(AdmissionError::Unavailable)?;
        let mut state = self.lock_state().map_err(AdmissionError::Unavailable)?;
        let current = state
            .accounts
            .get(tenant)
            .ok_or_else(|| {
                AdmissionError::Unenrolled(format!("tenant {tenant:?} is not enrolled"))
            })?
            .admission_mode;
        if current == next {
            return Ok(AdmissionView {
                tenant: tenant.into(),
                admission_mode: current,
            });
        }
        if current == AdmissionMode::PrepaidLocked {
            return Err(AdmissionError::Conflict(
                "prepaid_locked admission is irreversible".into(),
            ));
        }
        if current != expected {
            return Err(AdmissionError::Conflict(format!(
                "tenant admission mode is {current:?}, expected {expected:?}",
            )));
        }
        ...
        state
            ...
            .expect("account existence was checked under the same lock")
            .admission_mode = next;
        if let Err(err) = write_admission_modes(&self.inner.admission_path, &state) {
            state
                .accounts
                .get_mut(tenant)
                .expect("account existence was checked under the same lock")
                .admission_mode = current;
            return Err(AdmissionError::Unavailable(err));
        }
        Ok(AdmissionView {
            tenant: tenant.into(),
            admission_mode: next,
    ...
    pub(crate) fn credit(
        ...
        let account = state
            ...
            .or_insert_with(|| BudgetAccount {
                currency: "USD".into(),
                balance_micro: 0,
                admission_mode: AdmissionMode::Prepaid,
            });
        account.balance_micro = balance_after_micro;
...
fn load_admission_modes(path: &Path) -> Result<HashMap<String, AdmissionMode>, String> {
    if !path.exists() {
        return Ok(HashMap::new());
    }
    let (text, _) = read_secure_text(path, "tenant admission state")?;
    let file: AdmissionFile = toml::from_str(&text)
        .map_err(|err| format!("{}: tenant admission TOML: {err}", path.display()))?;
    if file.format != ADMISSION_FORMAT {
        return Err(format!(
            "{}: unknown tenant admission format {:?}",
            path.display(),
            file.format,
        ));
    }
    let mut modes = HashMap::new();
    for (index, row) in file.tenants.into_iter().enumerate() {
        ...
        if modes.insert(row.tenant.clone(), row.mode).is_some() {
            return Err(format!(
                "{} row {}: duplicate tenant {:?}",
                path.display(),
                index + 1,
                row.tenant,
            ));
...
fn write_admission_modes(path: &Path, state: &BudgetState) -> Result<(), String> {
    let mut tenants: Vec<AdmissionRow> = state
        .accounts
        .iter()
        .filter(|(_, account)| account.admission_mode != AdmissionMode::Prepaid)
        .map(|(tenant, account)| AdmissionRow {
            tenant: tenant.clone(),
            mode: account.admission_mode,
        })
        .collect();
    tenants.sort_by(|left, right| left.tenant.cmp(&right.tenant));
    let payload = toml::to_string(&AdmissionFile {
        format: ADMISSION_FORMAT.into(),
        tenants,
    })
    .map_err(|err| format!("serialize tenant admission state: {err}"))?;
    atomic_write(path, payload.as_bytes(), "tenant admission state")
}
...
mod tests {
    ...
    #[test]
    fn admission_modes_are_cas_guarded_durable_and_separate_from_the_budget_journal() {
        let dir = test_dir("admission-modes");
        let request_path = dir.join("requests.jsonl");
        let budget_path = dir.join("budgets.toml");
        budget_source(&budget_path, "tenant-a", 1);

        {
            ...
            let budgets = ledger.budgets().unwrap();
            assert_eq!(
                budgets.balance("tenant-a").unwrap().unwrap().admission_mode,
                AdmissionMode::Prepaid,
            );

            let metered = budgets
                .set_admission_mode("tenant-a", AdmissionMode::Prepaid, AdmissionMode::Metered)
                .unwrap();
            assert_eq!(metered.admission_mode, AdmissionMode::Metered);
            assert!(
                ledger
                    .reserve_budget("tenant-a", "q27", 100, 5)
                    .unwrap()
                    .is_none(),
                "metered traffic records cost but does not require prepaid balance",
            );
            ...
            assert_eq!(
                budgets.balance("tenant-a").unwrap().unwrap().balance_micro,
                1,
                "included traffic must not consume purchased credit",
            );
            let request: serde_json::Value =
                serde_json::from_str(std::fs::read_to_string(&request_path).unwrap().trim())
                    .unwrap();
            assert!(request["budget"].is_null());
            assert_eq!(request["cost_usd"]["total"], "0.0000174910");

            budgets
                .credit("tenant-a", 9, "admission-mode-credit")
                .unwrap();
            let after_credit = budgets.balance("tenant-a").unwrap().unwrap();
            assert_eq!(after_credit.balance_micro, 10);
            assert_eq!(
                after_credit.admission_mode,
                AdmissionMode::Metered,
                "credit must not erase the provisioning policy before it is locked",
            );

            budgets
                .set_admission_mode("tenant-a", AdmissionMode::Metered, AdmissionMode::Blocked)
                .unwrap();
            assert_eq!(
                budgets.admit("tenant-a", 1).unwrap_err(),
                BudgetAdmissionError::Blocked,
            );
            ...
            budgets
                .set_admission_mode(
                    ...
                    AdmissionMode::PrepaidLocked,
                )
                .unwrap();
            assert_eq!(
                budgets.admit("tenant-a", 11).unwrap_err(),
                BudgetAdmissionError::Insufficient,
            );
            assert!(matches!(
                budgets.set_admission_mode(
                    "tenant-a",
                    AdmissionMode::PrepaidLocked,
                    AdmissionMode::Metered,
                ),
            ...
            let journal = read_jsonl::<BudgetJournalRow>(budgets.journal_path()).unwrap();
            assert_eq!(journal.len(), 1);
            assert_eq!(
                journal[0].kind, "credit",
                "older binaries must never encounter admission rows in the budget journal",
            );
            #[cfg(unix)]
            {
                use std::os::unix::fs::PermissionsExt as _;
                let mode = std::fs::metadata(&budgets.inner.admission_path)
                    .unwrap()
                    .permissions()
                    .mode()
                    & 0o777;
                assert_eq!(mode, 0o640, "admission policy is private operator state");
            }
        }
        ...
        assert_eq!(
            ledger
                ...
                .admission_mode,
            AdmissionMode::PrepaidLocked,
            "the paid floor must survive restart",
        );
        std::fs::remove_dir_all(dir).unwrap();
    }

    #[test]
    fn admission_state_refuses_an_unenrolled_tenant_on_restart() {
        let dir = test_dir("admission-unenrolled");
        let request_path = dir.join("requests.jsonl");
        let budget_path = dir.join("budgets.toml");
        budget_source(&budget_path, "tenant-a", 1);
        let admission_path = sidecar_path(&request_path, ".tenant-admission.toml");
        write_private(
            &admission_path,
            "format = \"memra.tenant-admission.v1\"\n\n[[tenants]]\ntenant = \"ghost\"\nmode = \"metered\"\n",
        );
        ...
        .err()
        .expect("admission state must not enroll a tenant by itself");
        assert!(err.contains("unenrolled tenant \"ghost\""), "{err}");
        std::fs::remove_dir_all(dir).unwrap();
    }
    ...
    fn budgeted_completion_debits_exact_cached_price_once() {
        ...
        let budgets = ledger.budgets().unwrap();
        assert_eq!(
            budgets.admit("unlisted", 100).unwrap_err(),
            BudgetAdmissionError::Unenrolled
        );
    ...
    #[test]
    fn jsonl_budget_source_adds_integer_rows_and_rejects_absent_tenants() {
        ...
        assert_eq!(
            budgets.admit("unlisted", 1).unwrap_err(),
            BudgetAdmissionError::Unenrolled
        );
        std::fs::remove_dir_all(dir).unwrap();
    }
