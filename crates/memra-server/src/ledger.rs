...
// one of these outcomes, and ONLY the two in `BILLABLE_OUTCOMES` may carry a debit:
//
...
pub(crate) const BILLABLE_OUTCOMES: [&str; 2] = ["completed", "abandoned"];
...
impl PendingReceipt {
    ...
    fn finalize(&mut self, mut row: serde_json::Value) -> Result<(), String> {
        ...
        // FAULT-ATTRIBUTION INVARIANT (owner ruling 2026-08-23): only `completed` and
        // `abandoned` may debit > 0. This is the last write point every terminal row
        ...
        if !outcome_may_debit(&outcome) && !row["cost_usd"].is_null() {
            eprintln!(
                "[ledger] ERROR: request {} outcome {outcome:?} carried accounting; \
                 stripped it (only completed/abandoned may bill)",
                self.request_id
            );
...
mod tests {
    ...
    //
    // Only `completed` and `abandoned` may debit > 0; every other outcome debits ZERO.
