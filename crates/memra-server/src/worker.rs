...
/// Is this interactive session a genuinely COLD prefill, i.e. is nothing primed yet?
///
/// Extracted as a pure function 2026-08-28 (cold-TTFT lane) because the inline form was
/// untestable and had silently disabled [`interactive_prefill_budget`]'s solo widening for
/// EVERY cold interactive request that armed a plain-affinity checkpoint.
///
/// `ckpt_at` is NOT a freshness term. It is a pre-generation capture BOUNDARY in `fed`-length,
/// armed at admit for a COLD plain session whose prompt carries turn markers (see `Session`),
/// and `prefill_tick_take`'s `bound_rem` already stops the prime exactly on it whatever the
/// budget is. So an armed checkpoint bounds the TAKE; it carries no primed state and cannot
/// make a wider budget unsafe. Worse, arming it is a COLD-only signal: a resumed session
/// never re-arms one, so the term excluded precisely the sessions the widening exists for.
/// Measured on a 1480-token cold request (`[primeseg]` receipt, 2026-08-28): budget stayed at
/// `PREFILL_TICK_T`=1024, so the prompt primed as 1024 + 416 + 40 instead of 1440 + 40, and
/// only the FIRST of those calls got `hybrid_forward`'s batched GEMM prime, which is gated on
/// `cache.pos == 0`. The 456-token remainder went through the slow chunked continuation prime
/// at roughly 4.8 ms/token: 2.2 s of a 4.3 s cold TTFT.
///
/// `snapshot_at` (the prefix-cache LCP split) is the same boundary class and keeps its
/// ORIGINAL veto from the widening commit: widening it is plausible but has no receipt, and
/// a cold single-request box never arms one, so nothing here could measure it.
fn solo_widen_fresh(
    fed_len: usize,
    ...
    let _ = ckpt_at; // deliberate: a capture boundary bounds the take, it is not primed state
    fed_len == 0 && cache_pos == Some(0) && snapshot_at.is_none()
}

/// `MEMRA_STEP_GEMM_PRIME=0` on a step37-class (SlidingGatedMoe) model is a dead rollback in
/// serving: the whole-prompt walk primes at ~57.5 ms/token and every fresh prompt past ~1500
/// tokens hits the 90 s first-token deadline (walk-only attribution arm, 2026-08-28, every row
/// timed out). The server refuses to BOOT in that state -- loudly, at load, naming the
/// measurement -- instead of coming up as a box that answers /health and times out real work.
fn dead_prime_kill_switch_refusal(
    is_sliding_gated_moe: bool,
    step_gemm_prime_env: Option<&str>,
) -> Option<String> {
    if is_sliding_gated_moe && step_gemm_prime_env == Some("0") {
        return Some(
            "MEMRA_STEP_GEMM_PRIME=0 is refused in serving for step37-class models: the \
             fallback walk primes at ~57.5 ms/token and every fresh prompt past ~1500 tokens \
             times out on the 90 s first-token deadline (measured 2026-08-28, walk-only arm, \
             0 valid rows). Unset it or use =1; the flag remains usable in offline diagnostics"
                .to_string(),
        );
    }
    None
...
impl AdsdDetector {
    fn observe(
        ...
        if let Some((baseline_accepted, baseline_drafted)) = baseline
            && state.window.samples.len() == ADSD_TENANT_WINDOW
...
struct Session {
    ...
    #[allow(dead_code)]
    // allow: RAII: holds the host patch-memory permit for the session lifetime; only Drop reads it
...
pub fn run(
    ...
    for (name, path, draft) in &models {
        ...
        // DEAD-KILL-SWITCH REFUSAL (owner order 2026-08-28). MEMRA_STEP_GEMM_PRIME=0 walks
        // the prompt at ~57.5 ms/token, so any fresh prompt past ~1500 tokens dies on the 90 s
        // first-token deadline: measured on the walk-only attribution arm, where EVERY row of a
        // 1500+-token battery timed out. A rollback seam that cannot serve is not a rollback,
        // it is an outage with a flag name, so the SERVER refuses to boot a step37-class model
        // with it set rather than come up half-dead. Offline diagnostics (run-gen, the engine
        // bins) are unaffected -- this guard lives in the server load path only.
        if let Some(msg) = dead_prime_kill_switch_refusal(
            model.uses_sliding_gated_moe_program(),
            std::env::var("MEMRA_STEP_GEMM_PRIME").ok().as_deref(),
        ) {
    ...
    loop {
        ...
        if !batching {
            ...
            if automatic_demote || demote_at.is_some() {
                ...
                if n_live >= spec_gate_high() || forced {
                    ...
                    for i in 0..active.len() {
                        ...
                        let lm = &loaded[&s.model];
                        if sess.has_pending()
                            && let Err(err) = lm.model.spec_flush_pending(&engine, &mut sess, None)
                        {
            ...
            for i in 0..active.len() {
                ...
                let fresh = solo_widen_fresh(
                    s.fed.len(),
                    s.cache.as_ref().map(|c| c.pos),
                    s.snapshot_at,
                    s.ckpt_at,
                );
                ...
                // which term is false, and one false term was silently costing 76% of cold
                // TTFT: a lone fresh 1480-token request primed only PREFILL_TICK_T=1024 and
                // took the remaining 456 PROMPT tokens through the slow chunked continuation
                // prime (the batched GEMM prime is gated on cache.pos == 0, so tick 1 shut
                // that door), and the only reason was an armed plain-affinity ckpt_at -- a
                // capture BOUNDARY, not primed state. Fixed 2026-08-28 in `solo_widen_fresh`.
                // The probe stays, so the next false term is one env var away, not a lane.
                // NOTE the field polarity: `ckpt=false` means a checkpoint IS armed, which
                // since the fix is a PASSING shape (`fresh=true`), not a blocked one.
                // `rule=` is a BUILD FINGERPRINT: a deployed binary carrying the fix answers
                // `strings memra-server | grep -c ckpt-bounds-take-v2` with 1, so a measurement
                // can prove which predicate it measured instead of trusting a build log.
                if std::env::var("MEMRA_PREFILL_WIDEN_PROBE").as_deref() == Ok("1") {
                    eprintln!(
                        "[widen-probe] rule=ckpt-bounds-take-v2 configured={} explicit={} \
                         sole_unfinished={} fresh={} (fed_empty={} cache_pos={:?} snapshot={} \
                         ckpt={}) queued={} -> budget={}",
                        budgets[0],
                        prefill_tick_explicit,
                        sole_unfinished,
                ...
                if let Some(cache) = s.cache {
                    ...
                    if !last_logits.is_empty() {
                        ...
                        if let Some(id) = affinity.as_deref()
                            && let Some(sp) = spec_reuse.get_mut(&pool_key)
                        {
                            let before = sp.len();
...
fn admit(
        ...
        let resumed = if constraint.is_some() {
            ...
            if let Some(pool) = spec_reuse.get(&pool_key) {
                if let Some(index) = pool.iter().rposition(|e| {
                    ...
                    && let Some(index) = pool.iter().rposition(|e| {
                        e.sess.cache_max_ctx() >= ctx_cap
    ...
    if dspark_draft_ready
        ...
        && !vision_req
        && !capture_req
        && let Some(pool) = dspark_reuse.get_mut(&pool_key)
    {
        ...
        match hit {
            ...
            Some((entry, suffix)) if suffix.len() < memra_engine::hybrid_forward::PRIME_MIN_T => {
                eprintln!(
    ...
    Ok(Session {
        ...
        prefill_queue: if let Some(sfx) = dspark_resume_suffix {
            ...
            prompt[spec_resumed..].iter().copied().collect()
        } else {
...
mod tests {
    ...
    fn solo_prefill_widening_preserves_operator_and_fairness_caps() {
        ...
        // resumed session never re-arms it), and it is a pre-generation capture BOUNDARY
        // that `prefill_tick_take`'s `bound_rem` already stops the take exactly on -- it
        // carries no primed state, so it cannot make the widening unsafe. Vetoing on it
        // made the widening unreachable for exactly the case it exists for: measured on a
        // 1480-token cold request, budget stayed 1024, the prompt primed as 1024 + 416 + 40
        // instead of 1440 + 40, and only the first call got the batched GEMM prime (gated on
        // `cache.pos == 0`), which cost 2.2 s of a 4.3 s cold TTFT.
        assert!(
            solo_widen_fresh(0, Some(0), None, Some(1440)),
            "an armed affinity checkpoint bounds the take, it does not carry primed state"
        );
        assert_eq!(
            interactive_prefill_budget(
                1024,
                false,
                true,
                solo_widen_fresh(0, Some(0), None, Some(1440)),
        ...
        // The prefix-cache LCP split keeps its ORIGINAL veto (c09afe4c33): it is the same
        // boundary class, but widening it has no receipt, so this locks the scope of the
        // 2026-08-28 fix instead of letting it drift.
        assert!(!solo_widen_fresh(0, Some(0), Some(300), None));
        // Anything already primed is not fresh, from either half of the carried state.
        assert!(!solo_widen_fresh(7, Some(7), None, None));
