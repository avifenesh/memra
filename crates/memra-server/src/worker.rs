...
fn should_probe_prefix_cache(
    ...
    prefix_on && !has_prior_reuse && !dspark_prefers_cold
}
...
pub fn run(
    ...
    for (name, path, draft) in &models {
        ...
        let mut model = {
            ...
            if let Some(dpath) = draft {
                ...
                match memra_engine::hybrid::MtpHead::load_draft(&engine, &dg, &model.cfg) {
                    Ok(head) => {
                        let draft_cfg = memra_gguf::config::ModelConfig::from_gguf(&dg);
        ...
        eprintln!(
            "[worker]   loaded {name:?}: {} layers, eos={eos_id}",
            model.cfg.n_layer
        );
        let tok = Arc::new(tok);
        ...
        .map(|(n, lm)| {
            ...
            eprintln!(
                "[worker] {n}: template caps tools={} think={} think_switch={} chat_ok={} \
                   effort_levels={} qwen_effort={} gemma_think={} dsv4={} ctx={} tok={:?} \
                   instruct={:?} chat_defaults={:?}/{:?}",
                caps.tools_branch,
                caps.qwen_think,
                caps.think_switch,
                caps.chat_ok,
    ...
    for n in &eager_only {
        if eager_decode.contains(n) {
            eprintln!(
                "[worker] {n}: EAGER-ONLY serving (gemma4 class — no batched decode arm): \
    ...
    loop {
            ...
            if pending_constraints.is_empty() {
                ...
                match rx.try_recv() {
                    Ok(cmd) => {
                        ...
                        handle_cmd(cmd, &loaded, &dsv4_routes, &order, &mut queue);
                    }
                ...
                match rx.recv_timeout(wait) {
                    Ok(cmd) => handle_cmd(cmd, &loaded, &dsv4_routes, &order, &mut queue),
        ...
        if !batching {
            ...
            for s in active.iter_mut() {
                ...
                // DFlash captures only full prompt-end trunk state: there is no restorable
                // draft plane, hidden anchor, LCP split, or message-boundary arm. Low-load
                // DFlash ignores this trunk-only entry; a later plain-shed request consumes it.
                ...
                if !defer_interactive_prefill && pb_max >= 2 && !confidence_trace_enabled() {
                    for i in 0..active.len() {
                        ...
                        if s.spec.is_none() && !s.prefill_done && s.graph.is_none()
                            ...
                            // concat prime has no overlay seam)
                            && s.vision.is_none()
                            && s.lane == crate::lanes::Lane::Interactive
...
fn admit(
    ...
    let dspark_prefers_cold = dspark_prefers_cold_over_prefix(DsparkColdPrefixAdmission {
    ...
    if should_probe_prefix_cache(prefix_on, reused.is_some(), dspark_prefers_cold) {
...
fn prefill_tick(
    ...
    let budget = if s.vision.is_some() {
        ...
        && bound_rem.is_none_or(|r| r >= memra_engine::hybrid_forward::PRIME_MIN_T)
    {
        ...
        let (l, _h, _x) = lm.model.prime_cache_overlaid(
            engine,
            &chunk,
            s.cache.as_mut().unwrap(),
            s.prefill_queue.len(),
...
mod tests {
    ...
    use super::{
        ...
        ParkedCandidate, ParkedPool, ReuseMetrics, SPEC_SHRINK_RESERVE, admission_required,
        admission_reserve, alloc_with_single_reclaim_retry, context_cache_bytes,
        dual_pp_boundary_slot_bytes, dual_pp_device_requirements, dual_pp_stage_admission,
        enforce_prompt_limit, is_cuda_oom, oldest_parked_candidate, parallel_device_requirements,
        parked_entry_count, prepare_park, request_ctx_cap,
    };
    ...
    fn dspark_prefix_lookup_bypass_tracks_the_exact_load_verdict() {
        ...
        let route = |a| {
            ...
            (
                prefers,
                super::should_probe_prefix_cache(true, false, prefers),
            )
        };
    ...
    fn dspark_prefix_preference_preserves_refusals_and_capture_scope() {
        ...
        assert!(!super::should_probe_prefix_cache(false, false, false));
