...
pub(crate) mod auth;
/// Default-off per-tenant request capture (`MEMRA_CAPTURE_DIR`): settled prompt/completion
/// pairs for tenants explicitly marked via the admin surface. Fail-open for serving.
mod capture;
...
struct AppState {
    ...
    /// Optional per-tenant prompt/completion retention (MEMRA_CAPTURE_DIR). Rows are
    /// enqueued fail-open at settle; which tenants are captured is admin-surface state.
    capture: Option<capture::CaptureStore>,
    /// Optional prepaid enforcement. Absence preserves the existing unlimited behavior;
    /// presence still limits only tenants explicitly enrolled by a source or credit row.
...
pub(crate) fn parse_timeout_ms(v: Option<&serde_json::Value>) -> Result<u64, String> {
    let Some(v) = v.filter(|v| !v.is_null()) else {
        return Ok(TIMEOUT_MS_DEFAULT);
    };
    let Some(ms) = v.as_u64() else {
        return Err(format!(
            "timeout_ms must be an integer number of milliseconds in \
             {TIMEOUT_MS_MIN}..={TIMEOUT_MS_MAX}, got {v}; for work longer than \
             {TIMEOUT_MS_MAX} ms use \"stream\": true — the deadline then bounds only the \
             time to first token and the stream may run as long as it needs"
        ));
    };
    if !(TIMEOUT_MS_MIN..=TIMEOUT_MS_MAX).contains(&ms) {
        return Err(format!(
            "timeout_ms {ms} is outside the accepted range \
             {TIMEOUT_MS_MIN}..={TIMEOUT_MS_MAX} (milliseconds). {TIMEOUT_MS_MAX} is a \
             ...
             lie. For work longer than {TIMEOUT_MS_MAX} ms use \"stream\": true — the \
...
/// Absolute interactive-queue bound (the backpressure backstop): `MEMRA_MAX_QUEUE_DEPTH`,
/// default 4x the session cap. At the bound, new interactive requests shed with a 429
/// (`shed_queue`, never billed) instead of queueing — a saturated box answers honestly
/// instead of just getting slower. Read once.
...
pub(crate) fn admission_backpressure(
    st: &AppState,
    lane: lanes::Lane,
    rl: &RateLimit,
    deadline: RequestDeadline,
) -> Result<(), (Response, &'static str)> {
    if lane != lanes::Lane::Interactive || rl.remaining > 0 {
        return Ok(());
    }
    let m = st.metrics.lock().map(|m| m.clone()).unwrap_or_default();
    let backlog = m.queued_requests as usize
    ...
    if backlog >= bound {
        let msg = format!(
            "interactive queue is at its bound ({backlog} queued, bound {bound}); this \
    ...
    if est_wait_s.saturating_mul(1_000) > remaining_ms {
        let msg = format!(
...
/// budget admission. Video groups arrive `Ready`: the GIF decoder header-admits its own
/// budget as decode proceeds (`GIF_MAX_TOTAL_PIXELS`) and the pad runs need the sampled
/// frame count + delays, so its bounded decode stays in the plan phase.
enum PendingVisionUnit {
    Ready(memra_engine::vision_pre::VisionUnit),
    Still {
        bytes: Vec<u8>,
        gh: usize,
        gw: usize,
    ...
    for p in parts {
        match p.get("type").and_then(|t| t.as_str()) {
            ...
            Some("video_url") => {
                ...
                // v1 container: animated GIF (decoded in-process; no ffmpeg dependency).
                let bytes = memra_engine::vision_pre::decode_data_uri(url)?;
                ...
                *next_video += 1;
                ...
                for (g, ts) in vid.timestamps.iter().enumerate() {
                    let prep = &vid.groups[g];
                    out.push_str(&format!("<{ts:.1} seconds>"));
                    out.push_str("<|vision_start|>");
                    for _ in 0..prep.n_tokens() {
                        out.push_str("<|video_pad|>");
                    }
                    out.push_str("<|vision_end|>");
                }
                for prep in vid.groups {
                    // GIF video decode stays in the plan phase: its budget is already
                    // header-admitted as decode proceeds (GIF_MAX_TOTAL_PIXELS), and the
                    // per-group pad runs need the sampled frame count + delays.
                    images.push(PendingVisionUnit::Ready(
                        memra_engine::vision_pre::VisionUnit {
                            prep,
                            video: Some(vidx),
                        },
                    ));
...
fn parse_think(
    ...
    let effort_arm = match effort.as_deref() {
        None => None,
        Some(raw) => {
            let level = canonical_effort_for(raw, dsv4).ok_or_else(|| {
                format!(
...
pub(crate) fn canonical_effort_for(value: &str, dsv4_max: bool) -> Option<&'static str> {
    match value {
        "none" => Some("none"),
        ...
        "medium" => Some("medium"),
        "high" => Some("high"),
        "xhigh" | "max" | "ultra" => Some(if dsv4_max { "max" } else { "high" }),
        _ => None,
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bind_loopback = match validate_bind_security(&addr, api_auth.configured(), allow_open_bind)
    {
    ...
    let capture_store = match capture::CaptureStore::from_env(request_ledger.is_some()) {
        Ok(store) => store,
        Err(err) => {
            eprintln!("[server] FATAL: MEMRA_CAPTURE_DIR: {err}");
            std::process::exit(1);
        }
    };
    ...
    let admin_config = match admin::Config::from_env(
        request_ledger.as_ref(),
        auth::global(),
        capture_store.as_ref(),
    ) {
        Ok(config) => config,
        Err(err) => {
    ...
    // Bind the hard-gated admin socket before the expensive GPU/model load. The listener is
    // held but not served until the worker is ready, so a bad address fails fast without
    // exposing a half-initialized provisioning surface.
...
fn model_entry_v1(
    ...
    json!({
        ...
        "type": "chat",
        "context_length": ctx,
        "max_output_tokens": metadata.and_then(|m| m.max_output_length),
        "endpoints": ["chat/completions"],
        "input_modalities": input_modalities,
        "output_modalities": ["text"],
        "capabilities": {
            "streaming": true,
            "tools": caps.is_some_and(|c| c.tools_branch),
            "structured_output": !is_dsv4,
            "reasoning": thinking,
            "prompt_caching": !is_dsv4,
        },
        "pricing": {
            "currency": "USD",
            "unit": "per_1m_tokens",
...
fn build_chat_request_with_trace(
    ...
    let level_template = caps
        .map(|c| c.effort_levels || c.dsv4 || c.qwen_effort)
        .unwrap_or(false);
    ...
    if think_client_explicit && think == ThinkMode::NoThink {
        if let Some(c) = caps {
            if c.qwen_think && !c.think_switch && !c.dsv4 {
                return Err(format!(
        ...
        if let Some(c) = caps {
            if c.qwen_think && think != ThinkMode::NoThink {
                if c.think_switch {
                    ...
                    return Err(
                        "response_format requires disabling the model's think tail, \
                                but this chat template has no enable_thinking switch"
                            .into(),
                    );
                }
            }
    ...
    let parser = if is_dsv4 && (dsv4_tools || dsv4_think_open) {
...
fn decode_pending_vision(plan: &mut ChatPlan) -> Result<(), String> {
    for (i, unit) in plan.pending_images.drain(..).enumerate() {
        match unit {
            PendingVisionUnit::Ready(v) => plan.request.images.push(v),
...
/// traffic) pay only a read-lock membership check. The settle-time re-check in the
/// receipt remains the authoritative capture decision.
fn arm_capture(
    ...
    if let (Some(receipt), Some(store)) = (receipt.as_mut(), st.capture.as_ref())
        && store.is_armed(&tenant.tenant)
    {
        receipt.arm_capture(store.clone(), prompt());
    }
    receipt
}
...
async fn completions(
    ...
    let receipt = arm_capture(receipt, &st, &tenant, || json!({ "prompt": req.prompt }));
    ...
    if let Err((resp, outcome)) = admission_backpressure(&st, lane, &rl, deadline) {
    ...
    let resp = if stream {
        ...
        // Non-streaming: the deadline covers the COMPLETE response. The collect future
        // owns rx, so a miss drops it (cancelling generation at the worker's next tick)
        // while the receipt stays out here to be settled deadline_exceeded, debit ZERO.
        ...
        .await;
        drop(guard); // response complete or cancelled — free the slot before headers
        match collected {
            Ok(resp) => resp.into_response(),
            Err(_) => {
                return rl.attach(ledger_unbilled(
                    receipt,
                    deadline_exceeded_response(deadline.ms, false),
                    "deadline_exceeded",
                    "deadline_exceeded",
                    &env.id,
                ));
...
async fn chat_completions(
    ...
    let receipt = if let Some(prompt) = capture_prompt {
        arm_capture(receipt, &st, &tenant, move || prompt)
    } else {
        receipt
    };
    // Vision phase 2 (hermes decode-bomb finding, fixed 2026-08-23): the canvases expand
    // only HERE — after budget admission priced the header-planned pad runs.
    if let Err(err) = decode_pending_vision(&mut plan) {
        return ledger_rejected(
            receipt,
            bad_request(&err, Some("messages")),
            "invalid_request_error",
            &env.id,
        );
    }
    ...
    let resp = if stream {
        ...
        // Non-streaming: the deadline covers the COMPLETE response — see `completions`.
        let mut receipt = receipt;
        let collected = tokio::time::timeout_at(
            deadline.at,
            blocking_response_with_receipt(
...
async fn blocking_response(
    ...
    blocking_response_with_receipt(rx, model, chat, stop_strings, parser, env, &mut None).await
}

/// `receipt` is BORROWED (lane/deadline-billing): the handler wraps this future in
/// `timeout_at`, and a deadline miss drops the future (cancelling generation via the
/// owned rx) while the receipt survives at the caller to be settled `deadline_exceeded`,
/// debit zero — never `abandoned`, which bills partial usage and means the CLIENT left.
...
mod tests {
    ...
    /// The serve-shape capture gate: a request driven through the REAL blocking response
    /// path (the same consumer the HTTP handler awaits) lands one JSONL row whose prompt
    /// and completion are byte-exact — and only for a marked tenant. A trial mark with no
    /// budget account writes nothing (fail-closed toward under-capture).
    #[tokio::test]
    async fn served_completion_capture_is_byte_exact_and_marked_tenants_only() {
        let dir = std::env::temp_dir().join(format!(
            "memra-capture-servegate-{}-{}",
            std::process::id(),
        ...
        let ledger = ledger::Ledger::for_test(&dir.join("requests.jsonl"), "m", &metadata);
        let capture_dir = dir.join("capture");
        let store = capture::CaptureStore::open(&capture_dir).unwrap();
        store
            .set_mode("marked", Some(capture::CaptureMode::Consent))
            .unwrap();
        store
            .set_mode("trial-unbudgeted", Some(capture::CaptureMode::Trial))
            .unwrap();
        let prompt = json!([{ "role": "user", "content": "capture me — exactly" }]);

        let drive = |receipt: Option<ledger::PendingReceipt>| async {
            let (tx, rx) = tokio::sync::mpsc::unbounded_channel::<Event>();
        ...
        // Unmarked tenant: the handler's arm gate leaves the receipt bare — no row.
        let receipt = ledger.start(
            "cap-unmarked",
            "unmarked",
            "m",
            "/v1/chat/completions",
        ...
        // Trial mark WITHOUT a budget account: armed, but flush cannot prove the
        // allowance, so it must under-capture.
        let mut receipt = ledger.start(
            "cap-trial-unbudgeted",
            "trial-unbudgeted",
            "m",
            "/v1/chat/completions",
            "interactive",
        ...
        // Marked tenant: exactly one row, byte-exact prompt and completion.
        let mut receipt = ledger.start(
            "cap-marked",
            "marked",
            "m",
        ...
        let rows: Vec<serde_json::Value> = (0..400)
            ...
            .expect("the marked tenant's capture row never arrived");
        assert_eq!(
            rows.len(),
            1,
            "only the marked tenant may be retained: {rows:?}"
        );
        assert_eq!(rows[0]["request_id"], "cap-marked");
        assert_eq!(rows[0]["tenant"], "marked");
        assert_eq!(rows[0]["posture"], "opt_in");
        assert_eq!(
            rows[0]["prompt"], prompt,
            "prompt must round-trip byte-exact"
        );
        assert_eq!(rows[0]["completion"], "Hello");
        assert_eq!(rows[0]["usage"]["prompt_tokens"], 7);
        assert_eq!(rows[0]["usage"]["completion_tokens"], 2);
    ...
    fn dsv4_tool_result_long_runs_render_tokenize_roundtrip() {
        ...
        for (name, blob) in &cases {
            ...
            if *name == "ascii-letter-131k" {
                if let Ok(dir) = std::env::var("DSV4_LONGRUN_DUMP_DIR") {
...
<parameter=city>\nParis\n</parameter>\n</function>\n</tool_call>"
    ...
    fn cache_salt_validation_accepts_normal_value() {
        ...
        assert_eq!(
            validate_cache_namespace(&salt, false).unwrap(),
            salt.unwrap()
        );
    ...
    /// NON-STREAMING deadline: the response is 408 with our standard error object
    /// (`type: "timeout"`, `code: "deadline_exceeded"`), the message names the effective
    /// deadline, generation is CANCELLED (the worker's channel is closed — observed via
    /// the receiver the fake worker holds), the ledger row is `deadline_exceeded`, and the
    /// debit is ZERO even though a prompt and a token were already recorded.
    #[tokio::test]
    async fn a_missed_non_stream_deadline_is_408_zero_billed_and_cancels_generation() {
        let _l = DRAIN_LOCK.lock().unwrap();
        let dir = std::env::temp_dir().join(format!("memra-deadline-nonstream-{}", gen_hex128()));
        ...
        // 408 must stay retryable: no x-should-retry:false, and no invented Retry-After.
        ...
        assert!(
            message.contains("1000") && message.contains("not billed"),
            "the 408 must name the effective deadline and the billing promise: {message}"
        );
        ...
        // LEDGER: named outcome, and ZERO debit despite recorded partial usage.
        let row: serde_json::Value =
            serde_json::from_str(std::fs::read_to_string(&path).unwrap().trim()).unwrap();
        assert_eq!(row["outcome"], "deadline_exceeded");
        assert_eq!(row["http_status"], 408);
        assert!(
            row["usage"].is_null() && row["cost_usd"].is_null() && row["budget"].is_null(),
            "a deadline miss must not bill: {row}"
        );
        std::fs::remove_dir_all(dir).unwrap();
    }
    ...
    async fn a_stream_that_misses_ttft_is_a_preheader_408_and_not_billed() {
        let _l = DRAIN_LOCK.lock().unwrap();
        let dir = std::env::temp_dir().join(format!("memra-deadline-ttft-{}", gen_hex128()));
        std::fs::create_dir(&dir).unwrap();
        let path = dir.join("requests.jsonl");
        let metadata = OpenRouterModelMetadata {
            pricing: OpenRouterPricing {
        ...
        let row: serde_json::Value =
            serde_json::from_str(std::fs::read_to_string(&path).unwrap().trim()).unwrap();
        assert_eq!(row["outcome"], "deadline_exceeded");
        assert!(row["cost_usd"].is_null(), "TTFT miss must not bill: {row}");
        std::fs::remove_dir_all(dir).unwrap();
    }
    ...
    async fn a_stream_is_immune_to_the_deadline_after_its_first_token() {
        let _l = DRAIN_LOCK.lock().unwrap();
        let dir = std::env::temp_dir().join(format!("memra-deadline-immune-{}", gen_hex128()));
        std::fs::create_dir(&dir).unwrap();
        let path = dir.join("requests.jsonl");
        let metadata = OpenRouterModelMetadata {
            pricing: OpenRouterPricing {
        ...
        assert_eq!(
            ...
            "a stream past its deadline after first token still completes: {row}"
        );
        assert_eq!(row["usage"]["completion_tokens"], 4);
        std::fs::remove_dir_all(dir).unwrap();
    }
    ...
    fn the_queue_bound_sheds_with_429_retry_after_and_the_ratelimit_trio() {
        let st = fake_worker_state();
        // Saturated lane (remaining 0) with a backlog past 4x the cap.
        let cap = lane_cap(lanes::Lane::Interactive);
        {
            let mut m = st.metrics.lock().unwrap();
            m.completed = 10;
            m.tokens_out = 1_000; // 100 tokens/request
            m.step_p50_ms = 10.0; // => ~1s mean service time
            m.queued_requests = (cap * 4 + 1) as u64;
        }
        let rl = RateLimit {
            limit: cap,
            remaining: 0,
        ...
        let (resp, outcome) = admission_backpressure(
            &st,
            lanes::Lane::Interactive,
            &rl,
            RequestDeadline::starting_now(TIMEOUT_MS_MAX),
        )
        .expect_err("a backlog past the bound must shed");
    ...
    /// BACKPRESSURE, deadline test: the SAME saturated box admits a request whose deadline
    ...
    fn admission_sheds_only_when_the_estimated_wait_cannot_fit_the_deadline() {
        ...
        let cap = lane_cap(lanes::Lane::Interactive);
        {
            ...
            m.queued_requests = cap as u64; // one wave ahead => ~2s estimate
        }
        let rl = RateLimit {
            limit: cap,
            remaining: 0,
    ...
    /// Free capacity never sheds, and neither do the dark lanes (they shed at cap inside
    /// the worker — a second gate here would double-refuse them).
    #[test]
    fn admission_backpressure_is_interactive_only_and_silent_with_free_slots() {
        let st = fake_worker_state();
        let cap = lane_cap(lanes::Lane::Interactive);
        {
            let mut m = st.metrics.lock().unwrap();
            ...
            m.step_p50_ms = 100.0;
            m.queued_requests = (cap * 100) as u64;
        }
        // ...but a free slot means no wait to estimate.
        let free = RateLimit {
            limit: cap,
            remaining: 1,
            reset_s: 0,
        ...
        // Saturated, but a judge-lane request: the worker's own lane gate owns this.
        let full = RateLimit {
            limit: cap,
            remaining: 0,
            reset_s: 5,
        ...
        for lane in [lanes::Lane::Judge, lanes::Lane::Harvest] {
            assert!(
                ...
                .is_ok(),
                "{lane:?} must not be shed by the interactive gate"
            );
        }
    }
...
"#,
    ...
    async fn completion_admission_returns_402_at_zero_then_debits_a_credited_balance() {
        let _l = DRAIN_LOCK.lock().unwrap();
        let dir = std::env::temp_dir().join(format!("memra-handler-budget-{}", gen_hex128(),));
        std::fs::create_dir(&dir).unwrap();
        ...
        #[cfg(unix)]
        {
            ...
            std::fs::set_permissions(&budget_path, std::fs::Permissions::from_mode(0o640)).unwrap();
        }
        let metadata = OpenRouterModelMetadata {
            pricing: OpenRouterPricing {
                prompt: Some("0.000000289".into()),
        ...
        // Worker truth is one prompt + one completion token: $0.000002289 -> 3 micro-USD.
        assert_eq!(
    ...
    async fn streaming_client_disconnect_records_partial_usage_and_cost() {
        let _l = DRAIN_LOCK.lock().unwrap();
        // Drop-classified outcome (abandoned): serialized against the drain-kill latch test.
        let _serial = ledger::DROP_CLASS_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir =
            std::env::temp_dir().join(format!("memra-handler-ledger-disconnect-{}", gen_hex128(),));
        std::fs::create_dir(&dir).unwrap();
        let path = dir.join("requests.jsonl");
        let metadata = OpenRouterModelMetadata {
            pricing: OpenRouterPricing {
