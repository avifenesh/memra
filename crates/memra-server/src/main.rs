...
fn body_requires_admission(req: &AxumRequest) -> bool {
    ...
    declared_body_length(req).map_or(true, |length| length > BODY_ADMISSION_BYPASS_BYTES)
}
...
struct AppState {
    ...
    /// presence requires every authenticated tenant to have an explicit budget row.
...
/// `admission_backpressure` check remains useful for diagnostics/tests, but a
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !bind_loopback && !api_auth.configured() && !allow_open_bind {
        let message = format!(
            "refusing unauthenticated non-loopback bind {addr:?}; configure MEMRA_API_KEY or MEMRA_API_KEYS, or set MEMRA_ALLOW_OPEN_BIND=1 for an explicit development override"
        );
    ...
    // /admin/trim needs the worker channel; AppState takes cmd_tx by move below.
    let admin_cmd_tx = cmd_tx.clone();
    ...
    let admin_task = match (admin_config, admin_listener) {
        (Some(config), Some(listener)) => {
            ...
            let admin_app = config.router(admin_cmd_tx.clone());
    ...
    // /admin/trim's sender clone (admin_cmd_tx) must not outlive this point: the
    // worker's exit condition is "all senders dropped", and a live clone here made
    // worker_thread.join() below hang forever on graceful SIGTERM — caught by
    // serve-smoke's stop_server wait in the v0.116.0 release battery (38-minute
    // hang; the AdminState clone inside the admin app died with the admin task
    // above, but this main-scope binding survived).
    drop(admin_cmd_tx);
...
fn openrouter_supported_parameters(
    ...
    if caps.is_some_and(|c| !c.dsv4 && !(c.qwen_think && !c.think_switch)) {
...
fn admit_tenant_budget(
    ...
    match ledger.reserve_budget(
        ...
        Err(ledger::BudgetAdmissionError::Blocked) => Err(BudgetRejection::Insufficient),
        Err(ledger::BudgetAdmissionError::Unenrolled) => Err(BudgetRejection::Unenrolled),
        Err(ledger::BudgetAdmissionError::Unavailable(err)) => {
...
mod tests {
    ...
    fn the_feasibility_gate_is_wired_on_every_surface_not_just_the_two_i_remembered() {
        ...
        let main_src = strip(include_str!("main.rs"));
    ...
    fn glm5_model_row_does_not_claim_structured_output() {
        ...
        .err()
        .expect("response_format must be refused on a switchless think template");
        assert!(err.contains("no enable_thinking switch"), "{err}");
        ...
        let glm_params = openrouter_supported_parameters(Some(&caps), None);
        assert!(
        ...
        let qwen_params = openrouter_supported_parameters(Some(&tool_caps()), None);
        assert!(
...
<parameter=city>\nParis\n</parameter>\n</function>\n</tool_call>"
    ...
    async fn a_missed_non_stream_deadline_delivers_the_partial_bills_it_and_cancels_generation() {
        ...
        // LEDGER: the delivered tokens ARE billed, under an outcome that stays
        // DISTINGUISHABLE from a full answer. Writing `completed` here (the first version of
        // this lane) lost the deadline everywhere except an ephemeral log line — a review
        // caught it — so `deadline_partial` is its own billable census row.
        let row: serde_json::Value =
            serde_json::from_str(std::fs::read_to_string(&path).unwrap().trim()).unwrap();
        assert_eq!(row["outcome"], "deadline_partial");
        assert_eq!(row["error_code"], "deadline_exceeded");
        assert_eq!(row["http_status"], 200);
        assert_eq!(row["usage"]["completion_tokens"], 1);
        assert!(
            !row["cost_usd"].is_null(),
            "a delivered partial must carry its cost: {row}"
        );
        std::fs::remove_dir_all(dir).unwrap();
    }
    ...
    async fn a_deadline_missed_before_any_token_is_still_408_and_unbilled() {
        let _l = DRAIN_LOCK.lock().unwrap();
        let dir = std::env::temp_dir().join(format!("memra-deadline-zero-{}", gen_hex128()));
        std::fs::create_dir(&dir).unwrap();
        let path = dir.join("requests.jsonl");
        let metadata = OpenRouterModelMetadata {
            pricing: OpenRouterPricing {
        ...
        assert_eq!(
            ...
            "the named census outcome, not the generic `rejected` — every sibling deadline \
             path writes this one"
        );
        assert!(
            row["usage"].is_null() && row["cost_usd"].is_null() && row["budget"].is_null(),
            "nothing delivered must bill nothing: {row}"
        );
        std::fs::remove_dir_all(dir).unwrap();
    }
...
"#,
    ...
    async fn completion_admission_supports_metered_blocked_and_paid_transitions() {
        ...
        budgets
            .set_admission_mode(
                "default",
                ledger::AdmissionMode::Prepaid,
                ledger::AdmissionMode::Metered,
            )
            .unwrap();
        ...
        assert_eq!(
            ...
            0,
            "metered traffic records cost without debiting prepaid balance",
        );
        ...
        budgets
            .set_admission_mode(
                "default",
                ledger::AdmissionMode::Blocked,
                ledger::AdmissionMode::PrepaidLocked,
            )
            .unwrap();
        ...
        assert!(rows[1]["budget"].is_null());
        assert_eq!(rows[2]["http_status"], 402);
        assert!(rows[2]["budget"].is_null());
        assert_eq!(rows[3]["http_status"], 200);
        assert_eq!(rows[3]["cost_usd"]["total"], "0.0000022890");
        assert_eq!(rows[3]["budget"]["debit_micro"], 3);
        std::fs::remove_dir_all(dir).unwrap();
    }
