...
fn refusal(
    ...
    for event in events(rx) {
        match event {
            Event::Error(error) => errors.push(error.message),
            other => return Err(format!("DRIFT_PUBLISHED_SUCCESS_EVENT: {other:?}").into()),
        }
    }
    require(
        !errors.is_empty() && errors.iter().all(|e| e.contains(expected)),
...
fn native_worker_boundary() -> ProbeResult<()> {
    ...
    let attempt = caller.call(&engine, &loaded, &mut s, &mut px, &mut hpx, token);
    ...
    let refused = refusal(attempt, &mut rx, expected);
    if drift == "env" {
        engine.ctx().synchronize()?;
    ...
    let restored = caller.call(&engine, &loaded, &mut s, &mut px, &mut hpx, token);
    let reason = refusal(restored, &mut rx, "snapshot was revoked")?;
