...
| `q4e_push_f32` | UVA store of a small f32 vector into a PEER device buffer (raw address; P2P + pool access from `tp2_enable_p2p`) — the qwen4_exp TP2 direct-join push: each card stores its [hidden] partial into the other's staging, values verbatim | f32 | — | qwen4exp_gpu TP2 join (`decode_step_tp2`) | fatbin/by-name |
...
### cu/qmatvec.cu — 321 symbols (fatbin `MEMRA_QMATVEC_FATBIN`)

Count basis, stated because BOTH merge parents moved it and neither number survives the union:
321 is a MEASURED `grep -c 'extern "C" __global__'` on this file after the merge — 314 at the
shared base, + 4 from lane/glm5-door-r, + 3 from lane/glm5-dedup. Each side was arithmetically
right for its own tree (door-r wrote 318, dedup wrote 317) and both are wrong for the merged one,
which is why this row is re-measured rather than resolved by picking a side. The long-dead 323 was
on an unreproducible basis and was already drifting. This file is now measured; every other file's
count in this document still is not (the vrest lane's named inventory-recount follow-up).
...
| `qmatvec_nvfp4_{bf16,q8}_ep_*` (`dual_slots`, `dual_pairs`, `quad_pairs`, `down_{fma,pairs,slots}`) | automatic whole-expert EP gate/up/down over global fixed route slots; exact W4A16 and optional internal W4A8/mixed-Q8 scopes write canonical slot rows at every qualified batch width, while adjacent-row exact tiles preserve the one-row t=1 wave-fill route | NVFP4 W4A16 exact; optional internal W4A8 or mixed W4A8/W4A16 | — | MEMRA_PARALLEL_EP_DEVICE_ROUTER; MEMRA_PARALLEL_EP_GRAPH; MEMRA_PARALLEL_EP_PAIR_DOWN; MEMRA_PARALLEL_EP_Q8_ACT; MEMRA_PARALLEL_EP_Q8_SCOPE | lib.rs by-name wrappers |
