...
| moe_f16_grouped.cu | `moe_m1_splitk_fast_partial_kernel<1/2>`, `moe_m1_splitk_fast_reduce_kernel<1/2>` | Same `moe_m1_graph_splitk_f32_fixed_order` class, unchanged slice assignment and arithmetic order. Instrument + default-OFF door scaffold, initially using baseline bodies. Report modeled effective weight GB/s (unique codes + scales / partial time), with 1,792 GB/s a nominal peak. Live 0..6 cases are prefixes derived from one observed six-live CSR per rank/projection, plus an independently observed low-live case when seen in the bounded capture. | M=1, 4096/2048, 6 routes, device CSR live 0..6 | `MEMRA_DSV4_SPLITK_FAST=1`, default OFF, decide-by: 2026-09-23 | `dsv4_splitk_fast_gate`: real-route replay components, raw partial/output bits, canaries, warm/cold byte accounting; qualification pending. |
