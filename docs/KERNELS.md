...
| dsv4_gpu.cu | `memra_dsv4_dots_f32acc_mrow` (the eager TP/EP decode vocab head) | NO NEW KERNEL. Door `MEMRA_DSV4_TP_HEAD_SPLIT` (memra #433, default OFF, decide-by 2026-09-24) changes only WHICH OUTPUT ROWS each rank asks this kernel for. OFF: one launch on rank 1, N=129280, reading the whole 1,059,061,760-byte bf16 slab at 646.792 us and 1638 GB/s, 91.4% of the 1,792 GB/s nominal (measured in the private Darklanes split-K replay map, lane `dsv4f-replay-map-splitk-20260908`). ON: one launch per rank at N=64640 over a byte-offset weight pointer and a byte-offset output pointer, so each rank reads 529,530,880 B, plus one 258,560 B consumer-issued `cuMemcpyPeerAsync` pull of rank 0's half into rank 1's logits ordered by one event. Same numeric class by construction: the partition is over N, so row j is still `sum_k W[j,k]*x[k]` on one rank with the same kernel, the same 128-lane tree and the same rounding points; there is no cross-rank arithmetic, the pull is pure movement. Requires t=1 (the row split is a pointer offset into a `[t, vocab]` output) and refuses under armed full-token replay, so no retained graph node set changes. | sm_120a, `-fmad=false`, unchanged | `MEMRA_DSV4_TP_HEAD_SPLIT`; unset and `0` keep the single-rank head, which is the ROLLBACK SEAM; an explicit `1` outside the two-rank all-layer TP/EP f32x device program refuses at load | `dsv4-tp-head-split-gate`: `--component` (ABBA re-head on a shared committed hidden state, raw-bit equality over all 129280 rows at 32 different positions, refusing on the first differing bit, with a one-row weight shift and a disarmed-door refusal as red arms and per-rank dispatch/pull counters for non-vacuity), `--identity` (256 sampled positions, both arms interleaved in one process, token stream, cache/hidden digests, AR epochs and refusal words), `--replay-refusal`, `--defaults`. Receipts: PENDING, no GPU cell has run. |
