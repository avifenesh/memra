...
| moe_f16_batched.cu | `memra_moe_{pad_act_f16,unpad_scale_f32,bgemm_f16_strided}` | MEMRA_B200_PRIME_V2 arm 3: dequant ONCE into a persistent f16 slab (reusing `memra_moe_f16g_dequant`, no new dequant), pad the expert CSR to a uniform rows-per-expert, ONE cuBLASLt strided-batched f16 matmul per projection on the caller's stream, unpad with the per-pair amax scale folded. Exists because the shipped direct-from-quant kernel's `mma.sync.m16n8k16` warp-MMA class cannot reach sm_100a's tcgen05 rate (measured 75 TFLOP/s = 3.4% of peak, ~30x from both the compute and byte floors); cuBLASLt can. Strided and not grouped so the GEMM is stream-ordered — `cublasGemmGroupedBatchedEx`'s internal streams are the race that killed the worker in boot D. |
