...
| moe_f16_grouped.cu | `memra_moe_m1_graph_splitk`; `moe_m1_graph_splitk_partial_kernel<1/2>`, `moe_m1_graph_splitk_reduce_kernel<1/2>` | Numeric class `moe_m1_graph_splitk_f32_fixed_order`: fixed max-slice grid, device CSR live count and slice assignment, original half operands, ascending f32 partial reduction. Reducer writes +0 to inactive output rows `[CSR live, 6)`. GU rowwise packing stays within each row and the downstream projection excludes that tail through the CSR; down scatter ignores its `pairs=-1` entries. Component gates assert the actual CSR/scatter mask and poison inactive activation rows to verify consumer exclusion. Deterministic for routing, NOT token-identical to non-split-K because the reduction tree differs. Eager adaptive drift instrument: 5/160 top-1 changes, mean KL 0.002488327, max KL 0.066065030; report-only, not admission. | M=1, 4096/2048, up to 6 local routes, 16 slices | `MEMRA_DSV4_MOE_M1_SPLITK=graph`, default OFF, decide-by: 2026-09-22 | Graph gates pending; `graph-splitk-<sha>-rN` |
