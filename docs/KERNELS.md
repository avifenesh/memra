...
| Symbol | Purpose | Types | Architecture | Door | Binding |
| --- | --- | --- | --- | --- | --- |
| `d2d_receipt_digest` | Four wrapping u64 lane sums of `mix64(w_j + (j + 1) * C_l)` over a byte span read as LE words (order-independent, so the block and atomic order cannot move the value); the host folds the byte count. CPU oracle `memra_tier::conformance::receipt_digest`. Issued on the receipt stream (the engine's one kernel stream beside the owner's since design G''', DAY38 section 14; the copy stream before) for the source (behind the producer fence) and the destination (after the copy) of every D2D capture and restore item | u8 span, u64 lanes | any (no arch-specific instruction) | `MEMRA_KV_HOST_CONTRACTS=1` (the door's D2D classes; no flag of its own) | `CudaTransfers::digest_on`, read by `progress` and `d2d_receipt` |
| `tier_delay_spin` | One thread spinning on `%globaltimer` for `ns` nanoseconds; the `MEMRA_KV_HOST_FAULT=d2d-delay-capture` / `d2d-delay-restore` fault's delay ahead of the copy so the early reader deterministically sees the fresh destination, on the receipt stream with the D2D classes (the `d2h-delay` fault's spin of design G' is a host-side hold since G''') | u64 scalar | any | fault values only (diagnostics; `docs/FLAGS.md` `MEMRA_KV_HOST_FAULT`) | `CudaTransfers::delay_on`, armed by `inject_d2d_early_reader` and `inject_d2h_delay` |
| `d2h_receipt_sha256` | WP-A day 38 (`DAY38.md` design G): the D2H receipt program `memra_tier::contracts::checksum` on the device, byte for byte: SHA-256 over the frame `memra-tier\0v1\0`, le64(11), `valid-bytes`, le64(len), then the payload; one thread per item (a sequential chain per item, the items in SIMT lockstep), up to 64 items per launch passed by value, 32 bytes out per item. Issued on the receipt stream (a third stream beside the copy stream, design G'; since G''' the engine's one kernel stream beside the owner's, with the D2D classes: two non-owner streams running kernels slowed every later owner kernel boundary, DAY38 sections 13e to 13k) over every accepted D2H item's DEVICE source behind its producer fence, concurrently with the item's copy; the host's re-hash of the landed bytes at the bind is the witness. Bitwise against the CPU program in the native cells and the day-38 survey (56 sizes and offsets) | u8 sources, 32-byte digests | any | `MEMRA_KV_HOST_CONTRACTS=1` (the door's D2H class on the copy stream; no flag of its own) | `CudaTransfers::seal_d2h_device_receipt`, read by `progress` |
| `tier_flip_byte` | One thread XORing one byte with 0x40; the `MEMRA_KV_HOST_FAULT=d2h-source-flip` fault, queued on the receipt stream after the digest, the batch's copies waiting on it | u8 pointer | any | fault values only (diagnostics) | `CudaTransfers::seal_d2h_device_receipt`, armed by `inject_d2h_source_flip` |
...
The matrix whole-request program is the LOADED DEFAULT since 2026-09-11 (memra
#461; it was `MEMRA_DSV4_MOE_PROGRAM=matrix`, a default-OFF door, until the
owner flipped it and the door was deleted): the existing grouped CUDA kernels run through one native batched
