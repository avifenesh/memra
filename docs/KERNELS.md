...
| `dspark_prefill_prime_chunked` / `prefill_with_cache_chunked` -> `verify_batch_dev_output` per width-N transaction | Bounded-memory prefill: teacher-force the prompt through width-N device transactions instead of one monolithic transaction, keeping activation and indexer scratch O(N) instead of O(prompt). NUMERIC CLASS, measured 2026-09-10 and NOT what FLAGS.md previously implied: bit-invariant to the configured width whenever every transaction carries >= 2 rows; a 1-row transaction takes the m=1 decode-shaped kernels and is a DIFFERENT class, so two widths that both leave a 1-row tail agree with each other and disagree with the batched class (widths 31 and 33 on a 1024-row suffix), while widths 32 and 63 are bit-identical to 64 there. Refuses width 0, width > MEMRA_CTX, and width > the 64-row serving ceiling. Under TP/EP the topology refuses a batched prime outright (`dsv4_gpu.rs:5885-5895`), so this path is reachable only on the PP-2 reference/matrix programs | `MEMRA_DSV4_PREFILL_CHUNK=N`, default 0 (OFF) | `dsv4_prefill_chunk_door_gate`: two-sided width-class law (predicted-exact must be exact, predicted-differ must differ), fixed-width determinism twin, one-row-vs-two-row decisive probe, comparator red arm, cache-class census via `cache_class_census`, prefill-head census via `prefill_head_census`, width-0 and over-wide refusal arms, chunked-vs-monolithic drift rows and sampled cache transparency |
