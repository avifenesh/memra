...
2. **`release/claim-v0.106.0` is what enforces it here.** Version claims stay on origin
   after publication, including incomplete publication. The claim remains, and
...
The `release/claim-*` namespace records reserved versions, including released and incomplete
versions. **Lane cleanup must preserve every claim ref.** Check the tag and publication
state to distinguish a completed release from one still in flight.
...
fails both tag workflows when the claim branch is missing. Keep the claim after the release
lands: it reserves the number and lets later publish retries pass the same guard. Publication
and lane cleanup must not delete claim refs.
...
| Minor release | `v0.129.0` | memra#301 (memra#249 lever 13): the dspark draft KV is a live ring, `MEMRA_DFLASH_KV_RING` default ON with `0` as the rollback seam. Two windows plus a block (4,120 rows, 166 MB) stay resident and the trailing window compacts to the front once per window of ingest or decode; a pending prefix capture pins its rows until the tail is exported. Receipt on the 5090 lane box: byte-identical to the cold oracle on 3 boots x the 4-turn restore chain (24/24) and on a 22.9k-prompt greedy decode with 13 compactions; resident VRAM after the chain -1,152 MiB, peak at a 30k prompt -640 MiB. No published number moves. Follow-up memra#302 (admission still charges the draft KV per token). |
| Minor release | `v0.130.0` | memra#310: greedy requests with penalties take the dspark route (`MEMRA_DSPARK_GREEDY_PENALTY`, default ON, `0` = rollback seam): the verify rows get the request's penalties applied incrementally with the sampled arm's kernel, the plain argmax-prefix walk accepts over them, and the cold / restored / resumed boundary tokens take the same penalised argmax. Before it every temperature-0 request on qwen's non-thinking profile (vendor presence_penalty 1.5) was refused by the route and served plain. Receipt on the 5090 lane box: 15 of 15 turns byte-identical to the host sampler over three boots of a five-turn restore chain, decode 75 -> 107 tok/s at 400-700 tokens. Also memra#305 (the `[dspark] declined: greedy-penalised` receipt for the door-0 path) and memra#312 (the refusal-set test pins the door shut). No published number moves. |
| Minor release | `v0.132.0` | GLM-5.3-Flash TP-2 on the 2x B200 pair: the split grouped prime's host diet and the TP prefix cache. memra#322: `MEMRA_GLM5_TP_SPLIT_PRIME_HOSTDIET` (default OFF) removes the host round trip after the all-reduce push (CSR once, shexp early, pinned staging): 1M prime 310 -> 251 s, 256k 74.3 -> 59.9 s, tapes and ids identical. memra#324: `MEMRA_GLM5_TP_PREFIX` (default OFF) makes prefix entries carry every rank's shards (layout version 5): a re-sent 128k prompt hits 130,433 of 130,433 cached tokens with identical ids. Battery on box 2 (2x B200 sm_100a); nothing on the rig. |
| Minor release | `v0.138.0` | 133 commits since v0.137.0 (the release commit itself and #494, a docs-only `docs/SPEECH.md` correction, landed after the battery base `d71656c96` and are inside the tag); the fleet's GLM pair and the sm_120a boxes both move onto it. GLM: #395 restricts the TP pool-split indexer to PRIME and DELETES the decode split dispatch and its workspace handling, on a negative-128k / flat-1M decode pair receipt; the retained door is RENAMED `MEMRA_GLM5_TP_INDEXER_SPLIT_PRIME`, default ON, `=0` restores the replicated prime, decode (`t=1`) ignores it. The bare `MEMRA_GLM5_TP_INDEXER_SPLIT` is gone from the server and survives only in `crates/memra-engine/tests/glm5_tp_indexer_split.rs`: an operator launcher that asserts the OLD name with `grep -F` still passes by substring, so that capability check is vacuous until it is repointed. #455/#444: the served context window is resolved from the model's own config (`MEMRA_CTX` unset = the checkpoint's declared context through `max_position_embeddings`/`cfg.context_length`), never a literal 8192, and the three checkpoint cases are told apart with the two that are not a window refused. #431: memory-shaped admission, `MEMRA_ADMIT_BY_MEMORY` default OFF, decide-by 2026-09-23, with `MEMRA_ADMIT_OPEN_OUTPUT_TOKENS` (8192) and `MEMRA_ADMIT_DEFER_BUDGET_MS` (8000) read only when it is armed; #477 makes the boot line name the arm it is in and a test read it. DSV4F: #482 takes the matrix expert program as the default and three doors leave with the flip; #467 records it as a NEW numeric class with its drift rows; #490 ports, measures on the SERVED program and DELETES the three norm-fusion doors; #471 makes the dense wide-prefill tile width the code; #488 parks an eos-terminated session at the boundary that includes the EOS it swallowed; #447 and #481/#486 delete the split vocab head and short-first prefill doors on their own first receipts. Qwen: #429 flips `MEMRA_SERVE_DEVPENALTY` ON for Qwen3.5 on sm_120; #421 adds opt-in GQA-packed FA2-class quantized-KV prefill (`MEMRA_PRIME_ATTN_FA2`, default OFF, decide-by 2026-09-23); #417 carries the prime graph with live state tables; #424 records the calibrated FP4-activation prefill program as a NEGATIVE verdict with its instruments. Gemma: sampled-spec admission defaults ON (`MEMRA_GEMMA_SPEC_SAMPLED`, landed OFF 2026-09-09, flipped 2026-09-10), the Gemma-4 12B Unified checkpoint loads on the dense pack, and a DIRECTORY model may take its tokenizer from a verified GGUF (`MEMRA_TOKENIZER_GGUF`). SPEECH is new as a program, not a serving claim: the capability plan and gate set (#440), the served ASR decode contract G7 (`docs/SPEECH.md` section 5), native Whisper and streaming RNNT execution with pinned oracles, an audio surface that sheds honestly with a re-runnable c4 collapse (#489, `MEMRA_AUDIO_*`), the TTS concurrency leg measured at 0.53x the incumbent (#492), and #493 striking implementation cost as a selection criterion. RELEASE BATTERY: rc=0, 140 s, on the dev B200 pair vast 50431646 at `MEMRA_CUDA_ARCH=100a` under `flock /tmp/memra-glm5b200.lock` + `flock /tmp/memra-gpu.lock`; kernel-check ALL GREEN 95 cells 22 skipped; ornith-1.5-35b-a3b argmax flips=1 bad=0, qwen3.8-27b flips=0 bad=0, both run-spec K=1..8 identical to plain; no missing-model waiver, both roster GGUFs staged from R2 with sha256 matching `ops/serving/artifact-registry.tsv`. No gate, bench or CI ran on the local rig (owner block, 2026-09-06); the engine push used `MEMRA_SKIP_PERF_CI=1`. No published number moves in this repo: the GLM served throughput claim is re-measured at the box vantage on the DEPLOYED pin in darklanes, and it moves there or not at all. Publicity: skipped, maintenance release. |
| Minor release | `v0.137.0` | Owner-approved release. #325 preserves model-owned GLM/TP host images through demotion and promotion and reserves the complete pinned arena at startup; `MEMRA_GLM5_TP_KV_HOST` stays OFF, decide-by 2026-09-21. Host budget remains 0 and tenant share 50 by default. #377 admits validated DFlash retained-prefix restores and refuses failed carrier restoration. Inherited #374 defaults `MEMRA_DSV4_REPLAY_CADENCE` and `MEMRA_DSV4_DENSE_EXACT_TAIL` ON within their admitted paths, with exact `0` rollback and seam review on 2026-09-22; #376/#382 profiling/gate changes carry no GLM claim. #379 adds the owned speculative-prime fairness walker, `MEMRA_PRIME_YIELD` OFF (unset/`0`), `1` enables bounded peer service; decide-by 2026-09-22. Receipts: `research/prefill-fairness-20260908/{RESULTS,COMPOSITION,CHUNKS,SEAM}.md`. GLM5 adapter qualification remains separate. #392 changes `MEMRA_DSV4_MOE_M1_SPLITK` to ON (unset = `graph`); explicit `0` restores sktail in a fresh process with fresh graphs, seam review 2026-09-22. Owner-accepted numeric drift and performance receipts: [Darklanes #520](https://github.com/avifenesh/darklanes/pull/520); actual unset/0 engagement: [Memra #392](https://github.com/avifenesh/memra/pull/392), namespace `splitk-default-on-b589df9-gate-r1`, binary `ae91d10f4404759b6bd0b0c102eaa074aec27aa5b86a8b5c0a772c96de5201ee`. This supplies no GLM performance claim. Rebased full roster PASS on `72d424979` on a non-serving B200: 95 kernel cells, 22 skips; Ornith flips=1 bad=0, Qwen flips=0 bad=0, both K=1..8 identical. Current release-head and squash battery receipts are attached to #391; see `research/release-v0.137.0-20260909/GO-QUALIFICATION.md`. Released-composition pair gates remain part of the separate deployment. No published number moves. Publicity: skipped, maintenance release. GLM TP-2 gains opt-in GPU sampling (#378, merge `ff0937dd09b808cd97dde174de1b6aedff63a0b8`, `MEMRA_GLM5_TP_DEVICE_SAMPLE` default OFF, decide-by 2026-09-22). Five p32k OFF/ON pairs across two windows measured median wall throughput 81.32 -> 91.25 tok/s and server decode 11.89 -> 10.61 ms/token; 160-token greedy/top-k-one twins were byte-identical. Unset/0 with a restart restores host sampling. [Sampler receipts](../research/glm5-tp2-gpu-sampler-20260908/RESULTS.md). |
| Minor release | `v0.136.0` | DFlash serving prefill taps stream in 256-row batches on cold and resumed prime (#370, 15f4bf96f). The 128k cold tap transient of 13.42 GB is removed; a resumed suffix sink falls from 926.4 MB to 104.9 MB plus 26.2 MB carry. Four oracle sets and greedy cold/restore tapes are byte-identical; see `research/dflash-tap-storage-20260908/qualification.json`. Historical combined-candidate 2k/8k/32k TTFT deltas were +0.05 to +0.37%, three boots per arm; the qualification marks that comparison as history, not isolated timing of the final tap-only source. No new flag or broad performance/capacity claim; warm admission remains #372. Non-production RTX 5090 sm_120a battery on versioned candidate 12790ac96728: 95 kernel cells PASS, 21 skipped; Ornith margins flips=1 bad=0, Qwen flips=0 bad=0, both run-spec K=1..8 identical to plain. Vendor-default Qwen HTTP cell: 512 output tokens, dspark 170 rounds, drafted 546, accepted 345; server log cum=345/546=0.632. The initial sampled harness refused a deployment-only ledger setting before GPU startup, then a wrong readiness URL was corrected; both setup logs are retained. Corrected cell and release guard PASS. No local-rig build or gate; roster paths used unchanged through the box symlinks. Receipts: `research/release-v0.136.0-20260908/RESULTS.md`; final-commit battery receipt belongs to the annotated tag. Publicity: skipped, maintenance release. |
| Minor release | `v0.135.0` | Eligible single-device text restore admission (#350, #362) keeps the matching donor leased and replaces only cold workspace with the suffix charge. External DFlash and TP restores keep their existing charges; full KV, draft state, fixed residual and reserve remain paid. Shared native/OpenAI cache qualification rejects invalid identity rows (#360); #363 adds a test-only HTTP extractor regression for raw duplicate and escaped model keys before handler work. GLM plain TP2 exact prefix-radix ordering is limited to temperature 1, top-p 0.95, no top-k/min-p and neutral penalties (#359); shared callers and overrides retain comparison ordering. DSV4 diagnostics: `dsv4-full-token-control-gate`, `Dsv4Gpu::arm_full_token_replay_for_gate`, `--full-token-replay`, `--full-token-replay-baab`, and `--full-token-replay-profile` all remain OFF, decide-by 2026-09-22. `MEMRA_TEST_REPLAY_DRAIN_FAILURE_CHILD` is unset by default and cfg(test)-only; `error:0`, `error:1`, `drop:0`, `drop:1` inject failure. The profile selector requires existing `MEMRA_DSV4_NVTX=1` and refuses `MEMRA_DSV4_ROUND_PROFILE=1`; neither env switch is new. The paired-copy switch is absent (#355). Versioned candidate 17f36a4e passed the full release battery: 95 kernel cells, 21 skipped; calibrated margins Ornith flips=1 bad=0 and Qwen flips=0 bad=0; both K=1..8 identical to plain. #363 composition changes only cfg(test) source, with production byte-identity binding; combined CPU suite: 634 passed, 0 failed, 1 ignored. Battery log SHA256 `e7ec0538e10e4dfb5fe4e35590cb92892bcb6aee0f50f1324ed56160fac54609`; source/binary archive `831f2699131c0dead2a1e981f764ba1df82dac527b6bac40676b61e83dfa344a`, private `research/qwen-ornith-5090-capacity-20260908/release-135-candidate.json`. Publication retains the claim; the previous workflow deleted it automatically. Publicity: skipped, maintenance release. No new fleet/profile or broad performance claim. |
| Minor release | `v0.134.0` | New default-OFF mechanism plus two robustness fixes. #356 (`2d0271fb3`): standalone report-only teacher-forced DSV4 split-K drift instrument and its CI tests; build, three unit tests, and targeted clippy PASS on the release checkout, no library/runtime/CUDA change, quality certification, or serving-default promotion. #352 (`4fe96ccfc`): records the DSV4 issue-interleave no-go and removed gate in documentation only, with no runtime diff. #339 (`f80553700`): DSV4 small-kernel diet, `MEMRA_DSV4_SMALL_KERNEL_DIET` default OFF; `0` retains the existing kernels, `1` selects fused HC preparation and Q normalization/packing. No serving-default promotion or new performance claim. #353 (`dd8cc9c74`): startup GPU canary retries up to six consecutive hangs under the existing deadline before latching a fault; a successful probe ends startup retries, and steady-state behavior is unchanged. #354 (`df1273928`): a non-resident or host-routed MoE MTP head refuses MTP verify-pool creation and reuse, names eager verification, and avoids the capture error. New environment-door inventory: `MEMRA_DSV4_SMALL_KERNEL_DIET` only, default OFF; no new flag from #353 or #354. Qualification receipt: `research/release-v0.134.0-20260908/RESULTS.md`: versioned candidate `9d669417e` on a non-serving RTX 5090, CUDA 13.0, full battery PASS (95 kernel cells, 21 skips; both models K=1..8 identical), 10 GiB MTP squeeze PASS, engine/server suites and clippy PASS. Release validation also repairs one CPU-reference test-only Gemma golden: a sparse hand-derived oracle checks all logits bitwise and discriminates scaling, K-as-V, and softcap; no runtime arithmetic or tolerance changes. See the baseline/tanh control receipts in `NUMERICS.md`. No published performance number moves. Publicity: skipped (maintenance release). |
| Minor release | `v0.133.0` | memra#341 (the SGLang 0.5.19 lesson adopted): the batched-class linear verify ran its three GDN state kernels once per draft row per layer; the packed twins (`ssm_conv1d_fused_decode_tloop_f32`, `gdn_prep_decode_b` at b_n=T, `gdn_scan_s128_tsnap`) run each once per layer with the state resident and write the checkpoint snapshots in-kernel, bitwise vs the per-row program (24-cell GPU gate). `MEMRA_SPEC_GDN_PACKED` default ON, `0` = rollback seam. 5090 receipts: bin oracle ALL EXACT at vt 3/5/8 and the ladder (verify 23.1 -> 19.5 ms/round at vt 8), served ladder 140.6 -> 153.2 tok/s sampled / 147.4 -> 160.7 greedy, served chain 15 of 15 turns byte-identical over three boots. memra#348 (the graph fix, same lane): the first cut captured raw state handles into the verify CUDA graph and skipped the post-row snapshots for the captured body, so the Ornith battery (`qwen35moe`, num_k 16, num_v 32) produced a NaN verify trunk at round 0 while qwen passed by luck of allocation; both kernels now resolve `[conv, canonical ssm, alt ssm]` through the per-round table at replay and the GPU gate covers both geometries plus a re-binding replay (42 + 84 cells bit-identical each). Also carries the dsv4 lane's #329/#333/#334/#335/#337 (gate-only), `MEMRA_DSV4_SAMPLER` (host; `device` OFF, memra#338), and 60f40f3ca (plain prefix-hit leases end before deeper snapshots). No published number moves. 5090 BATTERY (rig gates suspended by the owner, 2026-09-06): rc=0 on `2f12d82fb` (this commit's parent, the #348 merge), 2026-09-08: kernel-check 95 cells ALL GREEN (21 skipped), ornith-1.5-35b-a3b argmax-margin flips=1 bad=0 and run-spec K=1..8 identical to plain, qwen3.8-27b flips=0 bad=0 and K=1..8 identical. Publicity: OWNER CALL, +9% served decode on the qwen route is a measured customer-visible number. |
| Minor release | `v0.131.0` | GLM-5.3-Flash TP-2 serving on the 2x B200 pair, the DSA indexer split, the tensor-core scorer; 1M prime 782 -> 310 s composed. memra#313: memra-server executes requests on the glm5 TP-2 symmetric walk behind `MEMRA_GLM5_TP` (plain route only, `MEMRA_SERVE_SPEC=0` literal, spec programs and the batched hc walk `MEMRA_HYPER_BATCH=1` refused by name, sessions decode eagerly through the per-rank graphs); smoke on the pair at `MEMRA_CTX=262144`: READY 90 s, 30k sampled request, streaming, two concurrent sessions, a 128k prompt, no engine errors. memra#314: `MEMRA_GLM5_TP_INDEXER_SPLIT` (default OFF, decide-by 2026-09-21) scores and selects half the chunk's query rows per rank and exchanges the idx rows over the transport, byte-identical to the replicated selection (CHECK door, tapes, ids): 256k prime 105.7 -> 89.6 s, 1M 782 -> 534 s alone. memra#309: `MEMRA_DSA_SCORE_TC` (default OFF, decide-by 2026-09-21) runs the prefill scorer on bf16 mma.sync with f32 accumulation (`=1` bf16 operands, `=2` hi/lo split, three MMAs); admitted like the TC prefill attention on the first-token bar, finer than the vendor's FP8 indexer; 1M prime 782 -> 340 s alone, 310 s composed with the split. memra#315: `MEMRA_B200_DSA_DECODE` defaults to level 2 on sm_100a (served since 2026-09-03). memra#307/#308: the DSA decode scorer's rows-per-thread default (RP=1, bit-identical) and the multi-CTA selector's block-wide descent (exact, -8% at 1M). Also #318 (dsv4 plain and host-cache execution work) and #319 (a one-shot correctness gate on the PRO 6000 pair). No published number moves; the fleet pin stays on v0.129.0 until the darklanes rollout moves it. Battery on box 2 (2x B200 sm_100a, vast 50146119), never the rig: receipt in the release PR. |
...
- The guard's third refusal ("no claim branch release/claim-vX.Y.Z on origin") fired although the
  ...
  The release workflow deleted its own claim automatically after successful publication.
  That step caused claim loss before later publish retries and is now removed. Preserve claims.
