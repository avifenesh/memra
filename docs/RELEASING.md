...
| Minor release | `v0.135.0` | Retained prefix restore admission (#350, #362) keeps the matching donor leased and replaces only cold prefill workspace with the suffix charge; full KV, draft state, fixed residual and reserve remain paid, and failed restoration requires cold admission. Shared native/OpenAI cache qualification rejects invalid identity rows (#360). GLM plain TP2 uses restricted exact prefix-radix nucleus ordering for its measured vendor-default shape (#359); shared callers keep comparison ordering. DSV4 full-token replay selectors remain diagnostic and OFF (#358), including the component executable, request-local arm, BAAB/profile selectors and test-only drain-failure child. The paired-copy switch is absent (#355). No new fleet/profile or published performance claim is made by this row. Candidate battery, exact source/binary binding and release checks are recorded in the release PR. |
