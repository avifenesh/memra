...
| Minor release | `v0.134.0` | New default-OFF mechanism plus two robustness fixes. #339 (`f80553700`): DSV4 small-kernel diet, `MEMRA_DSV4_SMALL_KERNEL_DIET` default OFF; `0` retains the existing kernels, `1` selects fused HC preparation and Q normalization/packing. No serving-default promotion or new performance claim. #353 (`dd8cc9c74`): startup GPU canary retries up to six consecutive hangs under the existing deadline before latching a fault; a successful probe ends startup retries, and steady-state behavior is unchanged. #354 (`df1273928`): a non-resident or host-routed MoE MTP head refuses MTP verify-pool creation and reuse, names eager verification, and avoids the capture error. New environment-door inventory: `MEMRA_DSV4_SMALL_KERNEL_DIET` only, default OFF; no new flag from #353 or #354. Qualification receipt: `research/release-v0.134.0-20260908/RESULTS.md` (candidate validation pending). No published performance number moves. Publicity: skipped (maintenance release). |
