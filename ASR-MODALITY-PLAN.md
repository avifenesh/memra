...
| End-to-end clip parity vs CT2 beam-1 | **Gate missed, landed on a diagnosis.** 36 of 71 clips swept (stopped clean; sweep closed 2026-09-10, partial-sweep verdict stands): 213/259 windows token-exact, 27/36 clips text-exact, `d1` 0.0739 pt and `whatsapp` 0.2237 pt WER vs CT2, both over the 0.05 pt per-domain limit. Of the 46 divergent windows, 31 are measured CT2 fp16 precision effects, 2 are individually diagnosed against a matched-program FP32 reference, and 13 are seek-shifted cascades each traced to one of those 33; no window is a native defect against a same-program reference. The 0.21-0.34 pt inter-backend band is a scale reference measured on the 8 FP32 clips, not an envelope containing these 36-clip deltas: see `research/asr-modality-20260909/TRANSCRIBE-PARITY.md` |
...
`NativeReference` needs the tokenizer's encode direction and vocabulary binding inside the
engine (the detokenize direction is done and agrees 36/36), and the speech plans wired into
the reference executor and `model inspect`. The sweep stays closed at 36 of 71 clips (owner
decision, 2026-09-10): its verdict stands on the partial sweep, and the remaining 35 clips
are optional new evidence, not a requirement. Everything above them
